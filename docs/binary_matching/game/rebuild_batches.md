...
| 8 | animation | `animations_selector`, `*animation_controller*`, `animation_space_*`, `animations_search_service`, `base_animation_controller`, `fingers_to_weapon_corrector` (+nested enum), `animated_model_instance{,_cook}` - PRIORITY: re-instantiates the stripped `mixing_n_ary_tree_*` COMDATs | |
