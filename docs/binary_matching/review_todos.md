...
- **Live set** (source of truth): `grep -rn "sushi@TODO:" sources/` — the markers travel with
  the code, so this is always current even if the tables below drift.
- **These tables** are the curated index: one row per marker, with the function and the PR it
  came from, grouped by lifecycle so a sweep agent knows what it may act on:
  - **Blocked — needs sushi's sign-off**: a disputed verdict only the human can close. Do NOT
    work these; surface them when sushi is reviewing.
  - **Actionable**: open questions / investigations a matcher or sweep agent can pick up.
    The orchestrator's queue-building step sweeps this section.
  - **Done**: resolved — kept for the record (drop the in-source marker when resolving).
- Append a new row under **Actionable** whenever you add a `sushi@TODO:` to the source; MOVE a
  row between sections when its status changes; never delete a row (the ledger is union-merged
  across branches, like `unanswered_questions.md`).

## Blocked — needs sushi's sign-off
...
| coverage-gap (fixed) | `delayed_packets_predicate` (class + ctor) | `network_core/sources/udp_network_flow_emulator.cpp:16` | #269 | sushi: size verifier **missed this class** (and its ctor) - it swept only headers, so .cpp-local classes are skipped. FIXED: added `STATIC_SIZE_ASSERT(.., 0x8)`. Confirmed it's the ONLY .cpp-defined class in network_core (no others). ctor/operator() emit no standalone symbol until tick's remove_if instantiates them. |
