...
| investigate | `get_ip_address` | `network_core/sources/network_core_entry_point.cpp:49` | #269 (network_core carcass) | 99.01%, **one extra basic block in target** (earlier "ICF naming" verdict was WRONG). Target routes the if-false branch through its own L148 block (`0x216 jmp short` -> `0x218 jmp .1`/++iter); base folds it straight to the back-edge (`0x216 jmp .1`). The 2-byte `jmp short` shifts every later offset by 2 = the whole residual. Confirmed brace/whitespace changes can't add a block (cosmetic). **Hypothesis:** a trailing loop-body statement (after the `if`, before the for `}`) that compiled out under MASTER_GOLD — an `ASSERT`/unreferenced-eater leaving an empty block (cf. debug-machinery-vanishes pattern). Needs a targeted experiment (each try = full relink ~10min) or evidence of the original statement before chasing. |
