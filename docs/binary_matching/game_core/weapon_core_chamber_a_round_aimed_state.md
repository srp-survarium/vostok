...
- ctor `??0...@@IAE` (protected, non-const)        -> 100.0% (DONE)
- `new_object` cook `@@AAE` (private)              -> 100.0% (DONE)
- `get_weapon_lexeme_pair` `@@ABE` (private const) -> 100.0% (DONE)
- `weapon_and_hands_expression` `@@EBE` (private virtual const) -> 83.52% PARTIAL (wall)
- `get_user_hands_expression` `@@ABE` (private const)          -> 74.02% PARTIAL (wall)

Unit measure (report.json): fuzzy 90.63%, 3/5 matched - identical to the sibling.
(Per-fn `fuzzy_match_percent` from report.json: ctor/new_object/get_weapon_lexeme_pair
all clean 100.0; the two walls 83.52% / 74.02%, matching the non-aimed sibling exactly.)
...
## ctor (target 0x799ed0 / base 0x450390) - 100.0% DONE
...
is byte-identical to target; report.json scores a clean 100.0.

## get_weapon_lexeme_pair (target 0x79a1a0 / base 0x450670) - 100.0% DONE
...
trailing ASSERT (dummy::nonnull). Full mnemonic stream is IDENTICAL to the target;
report.json scores a clean 100.0.

## new_object (cook COMDAT) - 100.0% DONE
`return new (buffer.c_ptr()) weapon_core_chamber_a_round_aimed_state(params->weapon, 1.0f,
animations, animations_count);` After the access-specifier fix + relink, report.json scores
this a clean 100.0 (no residual) - the earlier 99.84% / `computed_*`-LTCG note was a stale
pre-relink reading. byte-identical to the target.

## weapon_and_hands_expression (target 0x79a240) - 83.52% PARTIAL - WALL
...
separate cross-cutting unit, out of this unit's scope. Same wall as the sibling (83.52%).

## get_user_hands_expression (target 0x79a0a0) - 74.02% PARTIAL - WALL
