...
the only residual is the `empty_stub` inlined/stripped-call prologue
(`mov byte[ebp-1],0; lea eax,[ebp-1]; call empty_stub`) + the frame-size/[ebp-N]
slot shift it forces - an LTCG cross-module artifact not steerable from this
function's source, identical to get_target_koef's cap.
