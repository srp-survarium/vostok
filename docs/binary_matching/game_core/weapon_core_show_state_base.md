...
`finalize_impl`) is a COMPILED-OUT ASSERT, NOT an LTCG-folded/stripped empty call.
Master Gold drops the condition but keeps the `call` (~+0x0c). Recovered:
- initialize: `ASSERT( UNKNOWN_EXPRESSION );` placed BETWEEN the base initialize()
  call and instant_toggle_start() (target offset 0x11, body line 28).
- on_animation_end_impl: `ASSERT( UNKNOWN_EXPRESSION );` as the FIRST statement
  (target offset 0x09, body line 42), before the two bool stores.
The empty_stub bytes are now emitted from source; residual (if any) is the
inline-vs-call frame shape. Regressions caused: none.
