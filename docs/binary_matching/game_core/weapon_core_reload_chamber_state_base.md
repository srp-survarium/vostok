...
if ( !deserializing() && m_weapon.chamber_a_round_on_reload() && m_weapon.round_is_chambered() )
    m_weapon.unload_chambered_round();
```
RESIDUAL = LTCG inline-vs-call of the trivial accessor `round_is_chambered()`:
- target keeps it standalone (`pdb_rich_query target ...round_is_chambered` -> 0x09b360) and emits `call`.
- our /GL inlines it whole-program (`pdb_rich_query base ...round_is_chambered` -> nothing) -> `mov cl,[eax+48Eh]` (reads m_is_round_chambered @0x48E).
- `chamber_a_round_on_reload()` is inlined on BOTH sides (no standalone in either), so it matches.
This is the documented unsteerable trivial-accessor inline-vs-call class (is_aimed()/get_user());
the 0x4-byte frame/slot shift cascades from the single inline. No source change steers it. Mark PARTIAL.
