...
`user_anim_length = ...animations[4]...`, `time_scale = weapon/user`,
`new ( buffer.c_ptr() ) weapon_core_show_state( params->weapon, time_scale, animations, animations_count, params->shown )`.

REVIEW FIX (was 99.98% PARTIAL): the sole penalizing instruction was at line 90 -
target `mov ecx,[ebp+14h]; add ecx, 10h` vs base `add ecx, 4`. `managed_resource_ptr`
is 4 bytes, so offset 0x10 == `animations[4]`; the source had `animations[1]` (= +4).
The ctor loads weapon anims into indices 0-3 and user anims into 4-7, so the first user
animation is `animations[4]` (sibling pistol uses `animations[8]` for the same reason -
its weapon block is 2x2x2=8). Changed `animations[1]` -> `animations[4]`; rebuilt to
100.00 (report.json top-level fuzzy_match_percent, report-changes shows 99.98 -> 100.00,
1 improved, 0 regressed). The two `call ?end@buffer_string` reloc-target diffs were
benign ICF-fold churn (pistol carries the identical pair at 100.0) and did not block it.
