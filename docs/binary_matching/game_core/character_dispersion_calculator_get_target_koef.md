...
in shape. Blocker: the `empty_stub` inlined/stripped call in the prologue
(frame-size + stack-slot shift) is an LTCG artifact, not steerable from this
function's source. Regressions caused: none (3 functions improved, 0 regressed).
Inlining: `empty_stub` prologue call inlined in target only; shared with
get_broken_hands_penalty (same prologue) - not matched here.
