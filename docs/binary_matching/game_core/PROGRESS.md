...
- weapon_core_show_state (concrete WeaponCore show-state subclass, 5 fns; ONE unit, branch
  match/game_core-weapon_core_show_state off origin/int/game_core):
    ...
    weapon_core_show_state::new_object                      99.98% PARTIAL  (REVIEW CORRECTION: was mislabeled DONE/reloc-artifact. The 0.02% is a real index bug - target line 90 does `add ecx,10h` = animations[4] (first user anim; ctor puts weapon anims 0-3, user anims 4-7), base wrote animations[1] (add ecx,4). NEXT STEP: animations[1] -> animations[4], rebuild; expect 100.0 like pistol (which uses animations[8]). The two ?end@buffer_string reloc diffs are benign ICF churn - pistol has them at 100.0.)
