...
- player_logic_sprint_state (player_logic_base_state subclass with two boost::function
  ...
  match/game_core-player_logic_sprint_state off origin/int/game_core):
    ...
    player_logic_sprint_state::selected_animations   3.32%  None     (lexeme/expression mixing wall; placeholder VOSTOK_UNREACHABLE_CODE() keeps the pair-returning override compilable so the anchor can construct the state. Carcass preserved.)
