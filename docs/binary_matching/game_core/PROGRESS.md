...
- weapon_core_chamber_a_round_aimed_state (concrete WeaponCore chamber-a-round-AIMED-state
  subclass, 5 fns; ONE unit, branch match/game_core-weapon_core_chamber_a_round_aimed_state
  off origin/int/game_core). Structural twin of weapon_core_chamber_a_round_state (non-aimed)
  - identical 5-fn shape, identical two cross-cutting walls; resumed a cut-off matcher whose
  header had everything public WITHOUT friends, so EVERY fn compiled with the wrong access
  char and objdiff could not pair them (unit 8.4% / 1-of-5, per-fn measure None). The fix was
  pure access-specifier + scaffolding; no body changes were needed.
    weapon_core_chamber_a_round_aimed_state::ctor                        100%   DONE     (report.json 100.0; mnemonic stream byte-identical to target. ASSERT_CMP_U(count,==,8); two 2x2 loops sharing animation_index into m_weapon_animations(148h) then m_user_animations(158h) via intrusive_ptr::operator=; trailing ASSERT(UNKNOWN_EXPRESSION). @@IAE protected.)
    weapon_core_chamber_a_round_aimed_state::get_weapon_lexeme_pair      100%   DONE     (report.json 100.0; IDENTICAL mnemonic stream vs target. id="weapon-chamber_a_round_aimed"; selected=m_weapon_animations[view][user==type_crouch]; set_animation_to_wait(selected); get_weapon_lexeme_pair_impl(buffer,id,selected,&m_weapon,m_animation_playback_state,8,m_animation_timescale,play_once_and_freeze_at_end,linear_interpolator(s_aim_transition_time)). @@ABE private const.)
    weapon_core_chamber_a_round_aimed_state::new_object (cook)           100%   DONE     (new(buffer.c_ptr()) ctor(params->weapon,1.0f,animations,count). report.json scores a clean 100.0 after the access fix + relink; the earlier 99.84%/computed_*-LTCG note was a stale pre-relink reading. @@AAE private.)
    weapon_core_chamber_a_round_aimed_state::weapon_and_hands_expression 83.52% PARTIAL  (lexeme_pair=get_weapon_lexeme_pair; hands=get_user_hands_expression(offset,...); return hands+main+offset. NO extra sprint/jump branch (target asm checked - identical to non-aimed sibling). WALL: target resolves the + chain via expression-returning operator+ family (expression::expression<animation_lexeme>, operator+<animation_lexeme>(expression&,..), operator+(expression&,expression&)) absent from shared mixing_addition_lexeme headers; same 83.52% wall as the non-aimed sibling. @@EBE private virtual const.)
    weapon_core_chamber_a_round_aimed_state::get_user_hands_expression   74.02% PARTIAL  (if(user_state_id==type_sprint) return weapon_lexeme; ASSERT(UNKNOWN_EXPRESSION); idx=user==type_crouch; captions{"stand_chamber_a_round_aimed","crouch_chamber_a_round_aimed","jump_chamber_a_round_aimed"}; animation_lexeme_parameters(...).animated_object(m_weapon.get_user()).bones_mask(2).playback_type(play_once_and_freeze_at_end). WALL: target keeps the lexeme_parameters setters + parameters dtor OUT-OF-LINE, our /Od build inlines them - whole-program inline decision, same 74.02% wall as the non-aimed sibling. @@ABE private const.)
  Header (weapon_core_chamber_a_round_aimed_state.h): rewrote to the sibling layout - ctor protected (@@IAE), weapon_and_hands_expression private virtual const (@@EBE), get_weapon_lexeme_pair/get_user_hands_expression private const (@@ABE); added `namespace vostok { void use_game_core_weapon_core_chamber_a_round_aimed_state(); }` fwd-decl + `template<T> friend class weapon_core_state_cook_template;` + `friend void ::vostok::use_game_core_..._aimed_state();`. New anchor use_game_core_weapon_core_chamber_a_round_aimed_state in temp_include_all.cpp (+include +dispatcher call); weapon_core_state_cook_template.h fwd-decl + friend (new_object is private). state_base header m_animation_timescale made protected (read by get_weapon_lexeme_pair). cpp body unchanged from the prior matcher.
  Regressions: none - report-changes.json after the access fix: 0 regressed, 4 improved, 0 removed, 0 added. Unit 8.4%/1-of-5 -> 90.63%/3-of-5 (matches the non-aimed sibling exactly).
- base_player leaf + lifecycle (player-death subscriber leaves + lifecycle; 7 base_player
  STUB fns batched, ONE unit; branch match/game_core-base_player off origin/int/game_core).
  Detail: docs/binary_matching/game_core/base_player_leaf_and_lifecycle.md.
    base_player::subscribe_on_player_death       100%   DONE     (ASSERT(UNKNOWN_EXPRESSION) [finalize_impl @0x09 lone-eater] then m_player_death_subscribers.push_back(subscriber); push_back 2nd arg defaults NULL = target push 0; list @0x88. QAE public.)
    base_player::unsubscribe_from_player_death   100%   DONE     (same leading ASSERT then m_player_death_subscribers.erase(subscriber). QAE public.)
    call_player_death_subscriber_callback (free) 100%   DONE     (subscriber->subscription_callback() = boost::function0<void>::operator(). KEY: target symbol is INTERNAL-linkage (demangled-only, no COMDAT) - must be declared `static` or the external COMDAT name fails to pair (0% despite byte-identical RVA diff). static -> 100%.)
    base_player::on_player_death                 100%   DONE     (m_player_death_subscribers.for_each(call_player_death_subscriber_callback); fn name decays to lvalue binding for_each(Predicate&), Predicate=void(player_death_subscriber const*) = target void_predicate_ref<void __cdecl(...)>. Trailing empty-fn fold dummy::nonnull vs finalize_impl is byte-identical at link. IAE protected.)
    base_player::~base_player                    72.19% PARTIAL  (body = ASSERT(UNKNOWN_EXPRESSION) [line 36, +69.79->72.19]; rest compiler-gen. Residual structural+cross-class: our build INLINES base-subobject dtors (loose_ptr_base/hit_receiver/hit_initiator/collision_user/inventory_holder) and lays all 4 base vtables at TOP; target calls e.g. hit_receiver::~hit_receiver OUT-OF-LINE. dummy::nonnull vs finalize_impl = folded-empty cosmetics. NEXT: match base-class dtors out-of-line. UAE.)
    base_player::tick_active_object              3.38%  INPROGRESS (large ~616B virtual-dispatch fn on active object + weapon_core::could_be_used + inventory::{get_active_slot,item_in_slot,action}. Now PAIRED [was unpaired] via protected access [IAE] + anchor. NEXT: map active-object vtable offsets [0x1c/0x20/0x28/0x2c/0x54/0x70/0x7c exceed interactive_object's own table -> derived type] and reconstruct the two nested guarded if-blocks. IAE protected.)
    base_player::base_player (ctor)              0%     INPROGRESS (body empty {}; all member-init list. Decoded copies: recoil_params<-params+0x10, dispersion_params<-params+0x20, breath_holding_params<-params+0x58, damage_model<-params+0x114, movement_speed_factor<-clear_value, 4 bools=0; init list updated to verified copies. UNPAIRED - abstract class not yet anchored [needs concrete derived stub + constructed creation_params]; full match also gated on base-class CTORS out-of-line, like the dtor. QAE.)
  Header (base_player.h): set tick_active_object + on_player_death to protected: (target IAE);
  added `friend void ::vostok::use_game_core_base_player()` (+ fwd-decl of the anchor in
  namespace vostok) so the anchor can ODR-use the protected leaves. temp_include_all.cpp:
  new use_game_core_base_player anchor (null-ref qualified calls to the 4 leaves + escape)
  registered in IncludeAll::IncludeAll.
...
- damage_zone_core shape-distance helpers (4 free geometric helpers, ONE unit;
  ...
  Regressions: none - report-changes 0 regressed / 0 improved / 0 removed / 0 added.
- double_barreled_weapon_core_show_state (DoubleBarreled WeaponCore show-state subclass; 5 fns, ONE
  unit; branch match/game_core-double_barreled_weapon_core_show_state off origin/int/game_core).
  Sibling of the matched pistol_weapon_core_show_state (#185) and weapon_core_show_state; identical
  5-function shape. m_weapon_animations[2][2][3] + m_user_animations[2][2], 16 animations, size 0x190.
    double_barreled_weapon_core_show_state::ctor                       99.96% DONE     (m_time_scale=animation_timescale; ASSERT_CMP_U(weapon.get_magazine_capacity(),==,2); ASSERT_CMP_U(count,==,16); triple 2x2x3 loop into m_weapon_animations + double 2x2 loop into m_user_animations sharing animation_index; trailing ASSERT(UNKNOWN_EXPRESSION). Residual = weapon staged for get_magazine_capacity() into eax(target)/ecx(base) - call-boundary arg-reg. @@IAE protected.)
    double_barreled_weapon_core_show_state::new_object (cook)          100%   DONE     (animations[0]/animations[12] cubic_spline length_in_frames -> time_scale; new(buffer.c_ptr()) ctor(params->weapon, time_scale, animations, count, params->shown). count 16. @@AAE private.)
    double_barreled_weapon_core_show_state::get_weapon_lexeme_pair     99.93% DONE     (trailing-leading ASSERT(UNKNOWN_EXPRESSION); captions {"show_both_barrels_empty","show_one_barrel_loaded","show_two_barrels_loaded"}; weapon_animation_index=ammo_in_magazine() (direct, not ==0); m_weapon_animations[is_third_view!=false][user==type_crouch][index]; set_animation_to_wait; get_weapon_lexeme_pair_impl(group 6, m_time_scale, play_once_and_freeze_at_end, instant_interpolator()). Residual = inlined ammo_in_magazine() [ecx+128h] read landing in eax(target)/ecx(base) - call-boundary/inline-result reg, same as pistol sibling 99.92%. @@ABE private const.)
    double_barreled_weapon_core_show_state::weapon_and_hands_expression 83.52% PARTIAL  (lexeme_pair=get_weapon_lexeme_pair(...); hands=get_user_hands_expression(lexeme_pair.offset_lexeme,...); return hands + main + offset. Residual = shared-header operator+ wall: target uses dedicated expression operator+(expression&,animation_lexeme&)/operator+(expression&,expression&) overloads absent from mixing_addition_lexeme_inline.h; base falls back to generic addition_lexeme& operator+ (operator+<expression,animation_lexeme> + operator+<addition_lexeme,...> + expression(addition_lexeme)). Identical residual/score to pistol_weapon_core_show_state::weapon_and_hands_expression. Out of unit scope. @@EBE private virtual const.)
...
- double_barreled_weapon_core_aimed_fire_state (Double-Barreled WeaponCore AIMED-fire-state
  subclass, 6 fns; ONE unit, branch match/game_core-double_barreled_weapon_core_aimed_fire_state
  off origin/int/game_core). Union of double_barreled_weapon_core_fire_state (#203 - 12 animations
  8 weapon[2][2][2]+4 user[2][2], m_weapon_animation_index, magazine_capacity==2 ASSERT) and the
  aimed variant (s_aim_transition_time linear_interpolator, aimed captions). Modeled directly on #203.
    double_barreled_weapon_core_aimed_fire_state::ctor                       99.96% DONE     (: weapon_core_aimed_fire_state_base(weapon,animation_time_scale), m_weapon_animation_index(-1); ASSERT_CMP_U(weapon.get_magazine_capacity(),==,2); ASSERT_CMP_U(count,==,12); 2x2x2 loop into m_weapon_animations then 2x2 into m_user_animations sharing animation_index; trailing ASSERT(UNKNOWN_EXPRESSION). Residual = get_magazine_capacity() __thiscall this in eax(target)/ecx(base), LTCG arg-reg, same as #203. @@IAE protected.)
    double_barreled_weapon_core_aimed_fire_state::initialize                 99.76% DONE     (weapon_core_aimed_fire_state_base::initialize(); ASSERT(UNKNOWN_EXPRESSION); m_weapon_animation_index = m_weapon.ammo_in_magazine()!=2. Residual = ammo_in_magazine() this in eax(target)/ecx(base), LTCG arg-reg, same as #203. @@MAE protected virtual.)
    double_barreled_weapon_core_aimed_fire_state::get_weapon_lexeme_pair     100%   DONE     (captions {"aimed_shot_first_barrel","aimed_shot_second_barrel"}; selected=m_weapon_animations[is_third_view!=false][user==crouch][m_weapon_animation_index]; set_animation_to_wait; get_weapon_lexeme_pair_impl(group 1, m_animation_timescale, mixing::play_once_and_freeze_at_end, linear_interpolator(s_aim_transition_time)). @@ABE private const.)
    double_barreled_weapon_core_aimed_fire_state::weapon_and_hands_expression 83.90% PARTIAL  (ASSERT(UNKNOWN_EXPRESSION); lexeme_pair=get_weapon_lexeme_pair(...); hands=get_user_hands_expression(offset,buffer,is_third_view,user_state_id,weight_driving_animation); return main + expression(offset) + hands. Differs from #203 only by the LEADING ASSERT. Residual = operator+ inline-vs-out-of-line /GL template-selection wall, identical to #203's 83.18%. @@EBE private virtual const.)
    double_barreled_weapon_core_aimed_fire_state::get_user_hands_expression  75.61% PARTIAL  (if(user_state_id==type_sprint) return expression(weapon_lexeme); user_animation_index=user==crouch; selected=m_user_animations[view][idx]; if(pinned_ptr_const<cubic_spline_skeleton_animation>(selected)->animation_type()!=additive) return expression(weapon_lexeme); captions {"aimed_stand_shot","aimed_crouch_shot"}; linear_interpolator(s_aim_transition_time) LOCAL (built between captions and the param build, constructed-only, NOT a ctor arg); animation_lexeme_parameters(buffer,captions[idx],selected,&weapon_lexeme,&weight_driving_animation).animated_object(m_weapon.get_user()).playback_type(play_once_and_freeze_at_end).additivity_priority(1) -> animation_lexeme override_lexeme -> expression. vs #203: adds the aimed interpolator local + aimed captions. Residual = lexeme_parameters setter inline-vs-call /GL wall (target keeps animated_object/playback_type out-of-line, additivity_priority inline; base inverts) + COMDAT-folded animation_type() getter, same class as #203's 73.17%. @@ABE private const.)
    double_barreled_weapon_core_aimed_fire_state::new_object (cook)          86.50% PARTIAL  (new(buffer.c_ptr()) ctor(params->weapon, computed_shooting_animation_time_scale(*animations,params->rounds_per_second), animations, count). Residual = STUB callee computed_shooting_animation_time_scale folded to fldz here vs target out-of-line xmm0-return LTCG calling convention; call-boundary wall, identical to all sibling new_objects (#203 86.5%, #199 86.5%, #182 92.08%). @@AAE private.)
  Header (double_barreled_weapon_core_aimed_fire_state.h): access layout copied verbatim from double_barreled_weapon_core_fire_state.h (identical mangled access chars: ctor IAE->protected; initialize MAE->protected virtual; weapon_and_hands EBE->private virtual const; get_weapon_lexeme_pair/get_user_hands ABE->private const) + sibling-anchor scaffolding (use_game_core_double_barreled_weapon_core_aimed_fire_state fwd-decl, template friend, friend use_*). New anchor in temp_include_all.cpp (construct + exercise all 6 fns) + include + dispatcher call; cook_template.h fwd-decl + friend (new_object @@AAE private). cpp #includes mirror #203 (mixing_addition_lexeme.h, mixing_animation_lexeme_parameters.h, cubic_spline_skeleton_animation.h, linear_interpolator.h, resources_pinned_ptr.h, weapon_animations_timescale_inline.h); per-file static s_aim_transition_time=0.3f.
  Regressions: none real - report-changes 22 regressed (all ->0.0 except 6 partial drops) / 29 improved / 0 removed / 0 added. The churn is COMDAT-fold/thunk representative re-attribution from enabling this TU (network::world vcalls, sound thunks, btHashMap/btSoftBody dtors, boost::_bi storage, particle/Scaleform dtors); all disjoint from this unit (none contain double_barreled/weapon_core), several names appear in BOTH lists. Net-neutral; no source-caused regression.
...
- double_barreled_weapon_core_hide_state (5 fns, ONE unit; branch
  match/game_core-double_barreled_weapon_core_hide_state off origin/int/game_core).
  HIDE-state sibling of double_barreled_weapon_core_show_state (#210), pistol_weapon_core_hide_state
  (#204), weapon_core_hide_state (#197). Modeled directly on those; matched on the FIRST build, no
  iteration. Combines the double-barreled 16-anim [2][2][3]/[2][2] layout (3 weapon states) with
  hide semantics (linear_interpolator(s_aim_transition_time), time-sync group 7, "hide_*" captions).
    double_barreled_weapon_core_hide_state::ctor                       99.96% DONE     (m_time_scale first; ASSERT_CMP_U(weapon.get_magazine_capacity(),==,2); ASSERT_CMP_U(count,==,16); 2x2x3 loop + 2x2 loop sharing animation_index; trailing ASSERT(UNKNOWN_EXPRESSION). NO m_body_part_mask_for_user store. Residual = weapon staged for get_magazine_capacity() into eax(target)/ecx(base) - call-boundary arg-reg. @@IAE protected. Same residual as show_state ctor 99.96%.)
    double_barreled_weapon_core_hide_state::new_object (cook)          100%   DONE     (animations[0]/animations[12] cubic_spline length_in_frames -> time_scale; new(buffer.c_ptr()) ctor(params->weapon, time_scale, animations, count, params->shown). count 16. @@AAE private.)
    double_barreled_weapon_core_hide_state::get_weapon_lexeme_pair     99.93% DONE     (leading ASSERT(UNKNOWN_EXPRESSION); captions {"hide_both_barrels_empty","hide_one_barrel_loaded","hide_two_barrels_loaded"}; weapon_animation_index=ammo_in_magazine() direct; m_weapon_animations[is_third_view!=false][user==type_crouch][index]; set_animation_to_wait; get_weapon_lexeme_pair_impl(group 7, m_time_scale, play_once_and_freeze_at_end, linear_interpolator(s_aim_transition_time)). Residual = inlined ammo_in_magazine() [ecx+128h] read landing in eax(target)/ecx(base) - call-boundary/inline-result reg. @@ABE private const. Same as show_state 99.93%.)
    double_barreled_weapon_core_hide_state::weapon_and_hands_expression 83.52% PARTIAL  (lexeme_pair=get_weapon_lexeme_pair(...); hands=get_user_hands_expression(lexeme_pair.offset_lexeme,...); return hands + main + offset. Residual = shared-header operator+ wall: target uses dedicated expression-returning operator+(expression&,animation_lexeme&)/operator+(expression&,expression&) overloads absent from mixing_addition_lexeme_inline.h; base falls back to generic addition_lexeme& operator+. Identical residual/score to show_state and pistol hide siblings. Out of unit scope. @@EBE private virtual const.)
    double_barreled_weapon_core_hide_state::get_user_hands_expression  72.12% PARTIAL  (if(user_state_id==type_sprint) return weapon_lexeme; user_state_index=user==type_crouch; captions {"stand_hide","crouch_hide"}; animation_lexeme_parameters builder(.animated_object(m_weapon.get_user()).bones_mask(2).playback_type(play_once_and_freeze_at_end)) -> animation_lexeme override_lexeme -> expression. NO linear_interpolator local (verified from asm - differs from pistol hide sibling which has one). Residual = shared-header whole-program inline wall: target keeps get_user() + the 3 lexeme_parameters setters OUT-OF-LINE (frame 0x110), base inlines. Same class as show_state/pistol hide siblings. Out of unit scope. @@ABE private const.)
  Scaffolding: header rewritten to match show_state header (protected ctor @@IAE / private virtual
  weapon_and_hands @@EBE / private const get_* @@ABE / public virtual dtor; template-friend cook +
  friend use fn; size 0x190; [2][2][3]/[2][2] layout). temp_include_all.cpp: include + anchor
  use_game_core_double_barreled_weapon_core_hide_state() + dispatcher call. weapon_core_state_cook_template.h:
  fwd-decl + friend (new_object private -> C2248 without it).
