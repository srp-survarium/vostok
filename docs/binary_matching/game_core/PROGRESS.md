...
- weapon_user_animations_selector remaining hub + weapon_user_animations_container_cook batch
  (ONE unit, branch match/game_core-selector_remaining_and_container_cook off origin/int/game_core):
    ...
    weapon_user_animations_selector::deactivate                 34.88% PARTIAL  (set_initial_state(NULL)+unsubscribe args/structure match; L122 unsubscribe_animation_player resolves base vtable+58 vs target+54 - 1-slot MSVC overloaded-virtual ordering delta, base_player.h reorder is out of unit files; L124 same operator* deref-idiom wall as current_state())
