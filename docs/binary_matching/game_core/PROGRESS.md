...
- jump_logic.cpp batch3 (8 fns, parallel non-stacked off fc3aadf9):
    survarium::get_move_direction                100%   DONE     (4 mask-bit bools fwd/bwd/left/right; two cancel-ifs; nested dispatch tree; diff footer 99/99=100% on committed index; report.json per-fn measures null for the whole unit - symbol-map artifact)
    survarium::get_jump_animation_index          100%   DONE     (no-bounds jump table -> contiguous switch 0..8 + default:NODEFAULT(); on_site = on_site_jump+anim_part; others = (jump_from_right_leg ? X_right : X_left)+anim_part; anon enum of 19 named consts. NOTE: rich-diff can't pair (target params const-qualified); body length matches target)
    jump_logic::does_need_land_and_run           63.7%  INPROGRESS (REVIEWER: banked 100% did NOT hold - re-measured 102/160 on the committed index. Body+per-case ORs byte-identical but target has 1 extra trailing block: every leaf jmp .17 (base) vs .18 (target), target struct = 13 stmts ending L259. STEERABLE: result-temp + single tail return, NOT return-per-case (same class as movement_animation_index None->100%). Carcass restored. See jump_logic.md)
    jump_logic::set_user                         41.9%  PARTIAL  (REVIEWER: banked 76.55%, re-measured 18/43. (1) ai-fsm inline-vs-call wall: states()/front() out-of-line in target (states() folds 0x03f210), inlined base. (2) a STEERABLE cast-temp residual in the loop body (target materializes the static_cast into its own slot before the arg push) - flagged for re-match, distinct from the wall. Blocked on ai fsm type)
    jump_logic::deactivate                       26.9%  PARTIAL  (REVIEWER: banked 45.13%, re-measured 7/26. SAME ai-fsm out-of-line-vs-inline wall (states()/front()), blocked on ai fsm type)
