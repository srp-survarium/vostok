...
- pistol_weapon_core_aimed_fire_state (PR #182, base = match/game_core-weapon_core_fire_state_base; reviewed):
    ...
    pistol_weapon_core_aimed_fire_state::initialize               PARTIAL   (~92.62% est; 1 LTCG this-reg for ammo_in_magazine + 1 SOURCE-SHAPE boolize residual on the m_weapon_animation_index store - try `last_shot ? 1u : 0u`)
    pistol_weapon_core_aimed_fire_state::weapon_and_hands_expression  INPROGRESS  (stub return; addition_lexeme/operator+ machinery)
    pistol_weapon_core_aimed_fire_state::get_user_hands_expression    INPROGRESS  (stub return; lexeme machinery)
  ACCESS FIXES OWED (NOT applied by this PR - flagged by review): ctor/initialize/get_weapon_lexeme_pair/get_user_hands_expression/weapon_and_hands_expression are all declared `public:` in the .h but the target wants ctor IAE(protected), initialize MAE(protected-virtual), get_weapon_lexeme_pair ABE(private), get_user_hands_expression ABE(private), weapon_and_hands_expression EBE(private-virtual). objdiff cannot pair them so report.json omits the 100%/PARTIAL functions entirely - only new_object (AAE, matching) scores. The matcher mislabeled this absence as "ICF folding"; it is the documented access-specifier pairing failure (same fix the weapon_core_fire_state_base PR made). Fix the .h access specifiers (no rebuild done in review).
