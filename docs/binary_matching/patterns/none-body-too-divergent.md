...
symptoms: fuzzy_match_percent omitted, mangled name matches exactly, symbol present in base obj, objdiff-cli match_percent 0.0 while vostok-pdb inspect --view diff reports most instructions equal
confidence: 7/10
...
vostok-pdb inspect --target ... --base ... --function <fn> --view diff       # N/M equal
vostok-pdb inspect ... --view structure-diff                                             # statement shape
```
...
report.json prints no percent and match_score counts it unpaired. `vostok-pdb inspect --view diff`
...
Corollary: for macro-repetitive functions treat vostok-pdb inspect's diff as the quality signal and
