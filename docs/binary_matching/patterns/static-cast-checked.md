...
```
Evidence: weapon_user_animations_selector::get_current_state_id 71.5->100, current_state 70.26->80.2; serialize/deserialize +0x6 temps -> byte-match; medkit::load all 6 rows closed (0x55c both sides); sushi@TODO in game_material_manager_cook.cpp suspected it. breath_vibration_calculator (2026-06-13): tick `static_cast<breath_state*>(m_logic.current_state())` 94.2->100, set_breath_holding_params for-body+multiplier casts 84.5->96.6 (residual = front() knob, not the cast) - the t1[->t2] temp chain off the cast was the tell. weapon_core (2026-06-14): the header inline `current_base_state()` (`*static_cast<weapon_core_base_state*>(m_logic->current_state())`) was spelled with plain static_cast - switching to static_cast_checked took get_body_part_mask_for_user 73->100, get_weapon_and_hands_animation_expression 91->100, and lifted target_and_animation_ended_predicate / the *_and_animation_ended_predicate family by ~5pp each (residual there is has_animation_ended inline-vs-call). serialize's `static_cast<weapon_core_base_state const*>(...)->serialize(packet)` cast +6 -> matched (64.6->67.8). NOTE: a static_cast on a RAW pointer (`.c_ptr()`) is non-polymorphic so check{} is empty - static_cast_checked adds NO temps there (deserialize m_ammunition cast got 2 bytes WORSE; reverted) - only worth it when the source is a polymorphic ref/ptr.
