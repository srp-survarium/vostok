...
ret
```
Steerable: write `if ( ptr )`, NOT `if ( ptr.c_ptr() )` or `if ( ptr != NULL )` (those give a plain `cmp`/`sete` — see intrusive-ptr-compare-sete-leaf.md). Usually inlined at the test site. cite: victory_item.cpp:27 `if ( m_model )`; symbol intrusive_ptr<...inventory_item...>::operator inventory_item*.
