...
ret   4
```
Steerable: keep the idiomatic `lhs = rhs;` — the two-`dec` + one-`increment` skeleton is forced by copy-and-swap; do NOT hand-roll a manual inc/dec. cite: booby_trap_set.cpp:167 `m_current_rendering_model = model;`; symbol intrusive_ptr<...weapon_ammunition...>::operator=.
