...
- [The comma operator is invisible: `(e1, e2);` == `e1; e2;` byte-for-byte](comma-operator-invisible.md) — c10 — cpp:comma | asm:call asm:mov | topic:wall topic:convention — no distinguishing bytes vs the statement sequence, comma in for-increment just sequences
...
- [`VOSTOK_NEW`/`VOSTOK_DELETE` family: strip_pointer feeding new_helper<T>::call / delete_helper<T>](vostok-memory-macros.md) — c8 — cpp:new-delete cpp:template cpp:macro | asm:call asm:push | topic:codegen-idiom topic:convention — call strip_pointer then new_helper<T>::call then a null-guarded ctor; allocator pushed as OFFSET g_*_allocator
