...
sar   edx, 3
```
Steerable: the `add reg,<sizeof T>` stride is exact — a wrong element type yields the wrong immediate. This is the /Ox world (tu-optimization-level-od-vs-ox.md); a /Od vector loop instead keeps the iterator in a slot. cite: vostok::math::convex::test_inexact math_convex.cpp:234-258.
