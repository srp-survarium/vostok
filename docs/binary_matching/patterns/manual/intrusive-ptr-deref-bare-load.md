...
ret
```
Not a steering lever (3 bytes, folded) — note it so a 0%/ICF-named `operator->` symbol isn't mistaken for a structure bug. cite: victory_item.cpp:28 `m_model->m_render_model`; symbols intrusive_ptr<...inventory_item...>::operator-> and ...booby_trap_core...::operator*.
