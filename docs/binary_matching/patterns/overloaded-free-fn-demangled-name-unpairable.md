# Overloaded free function: delinker drops the param list, both overloads share one bare demangled name (unpairable)
...
`unpaired`/`None` in report.json no matter how correct the bytes are.
...
```
The two-sided diff pairs by RVA, so it scores each overload honestly even though objdiff's
report cannot. Bank the result from the RVA diff, leave a `claude@NOTE`, and report the
objdiff `unpaired` as a delinker artifact (not a residual to chase).
