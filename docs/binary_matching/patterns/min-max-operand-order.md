...
```
Evidence: generic_anomaly_core::dec_energy (100% reference); legs_ik_processor set_{heel,toe}_transition_time 83.69 -> 100 swapping to member-first; bullet::tick wanted min(m_life_time, g_bullet_tracer_exposition), collide_front_face min(1.0f, max(...)) - both recovered from the register shuffle alone. breath_state_normal/shortbreathing::tick 79/63 -> 100: target had xmm0=max_breath_holding_time, xmm1=reserve(+dt) so the source is min(reserve+dt, max), i.e. the param-limit goes SECOND (xmm0) even though intuition writes the clamp limit first.
