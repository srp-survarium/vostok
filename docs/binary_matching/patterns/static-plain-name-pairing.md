...
linkage) AT THE SCOPE the plain name shows: no namespace prefix = global scope, a
`vostok::network::` prefix = static inside that namespace. The anchor must then live
IN-TU - park the volatile-fn-pointer block inside a same-TU stub the target also has (an
extern temp_include_all declaration no longer links), or reference it from its real
caller (e.g. the dtor's bind) to survive /OPT:REF.

```cpp
static float distance_from_box_center_to_point_on_shape( ... );   // internal linkage pairs
```
Evidence: damage_zone_core shape helpers (sphere/box/capsule/cylinder) None -> 100/86.5/82.5/76.6, +8 report improvements, 0 regressions; call_item_serialize precedent; network destroy_client (tcp_packet_client.cpp, global scope) + destroy_http_client (http_client.cpp, namespaced), both None -> scored. Same shape pending on booby_trap_set_core trap_is_active / find_free_trap_predicate / create_place_matrix_for_looking_point.
