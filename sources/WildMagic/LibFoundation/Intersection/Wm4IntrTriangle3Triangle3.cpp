    ...
    const Vector3<Real>& rkEnd0, const Vector3<Real>& rkEnd1)
{
    ...
    Vector2<Real> kProjDir = kProjEnd1 - kProjEnd0;
    fMax = kProjDir.Normalize();
    LinComp2<Real> kProjSeg(kProjEnd0, kProjDir, (Real)0, fMax);

    IntrLinComp2Triangle2<Real> kCalc(kProjSeg,kProjTri);
    if (!kCalc.Find())
    {
        return false;
    }

    const LinComp2<Real>& rkComponent = kCalc.GetIntersectionComponent();
    ...
    if (kCalc.GetIntersectionType() == IT_SEGMENT)
    {
        ...
        m_iQuantity = 2;
        akIntr[0] = rkComponent.Origin +
            rkComponent.GetMin()*rkComponent.Direction;
        akIntr[1] = rkComponent.Origin +
            rkComponent.GetMax()*rkComponent.Direction;
    }
    else
    {
...
void IntrTriangle3Triangle3<Real>::GetEdgeEdgeIntersection (
    const Vector3<Real>&, const Vector3<Real>&,
    const Vector3<Real>&, const Vector3<Real>&)
{
    // TO DO.
}
