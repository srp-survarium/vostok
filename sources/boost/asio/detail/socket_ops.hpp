...
{
  clear_error(ec);
#if (defined(BOOST_WINDOWS) || defined(__CYGWIN__)) && !defined(_XBOX)
...
#if defined(_XBOX)
  if (!xbox360_check_option_support(optname))
  {
    ec = boost::asio::error::invalid_argument;
    return -1;
  }
    ...
    size_t* optlen, boost::system::error_code& ec)
{
  ...
  ec = boost::asio::error::fault;
  return -1;
#elif (defined(BOOST_WINDOWS) || defined(__CYGWIN__)) && !defined(_XBOX)
  clear_error(ec);
...
# if defined(BOOST_WINDOWS)
  if (result == 0)
	clear_error(ec);
# endif //if defined(BOOST_WINDOWS)
  return result;
#endif
}

#if (defined(BOOST_WINDOWS) || defined(__CYGWIN__) \
  || (defined(__MACH__) && defined(__APPLE__))) && !defined(_XBOX)
