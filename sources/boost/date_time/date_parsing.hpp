...
namespace date_time {
  ...
  convert_to_lower(vostok::network::std_string inp)
  {
#if !defined(BOOST_DATE_TIME_NO_LOCALE)
    const std::locale loc(std::locale::classic());
#endif
    vostok::network::std_string::size_type i = 0, n = inp.length();
    for (; i < n; ++i) {
      inp[i] =
#if defined(BOOST_DATE_TIME_NO_LOCALE)
    ...
    month_str_to_ushort(vostok::network::std_string const& s) {
      if((s.at(0) >= '0') && (s.at(0) <= '9')) {
        return boost::lexical_cast<unsigned short>(s);
      }
      else {
        vostok::network::std_string str = convert_to_lower(s);
    ...
    date_type
    parse_date(const vostok::network::std_string& s, int order_spec = ymd_order_iso) {
      vostok::network::std_string spec_str;
      ...
      typedef typename vostok::network::std_string::traits_type traits_type;
    ...
    template<class date_type>
    date_type
    parse_undelimited_date(const vostok::network::std_string& s) {
      int offsets[] = {4,2,2};
      int pos = 0;
      typedef typename date_type::year_type year_type;
      ...
      typedef typename boost::tokenizer<boost::offset_separator,
                                        vostok::network::std_string::const_iterator,
                                        vostok::network::std_string > tokenizer_type;
    ...
    from_stream_type(iterator_type& beg,
                     iterator_type const& /* end */,
                     vostok::network::std_string const&)
    {
