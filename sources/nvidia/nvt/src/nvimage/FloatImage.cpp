...
Image * FloatImage::createImageGammaCorrect(float gamma/*= 2.2f*/) const
{
	...
	for(uint i = 0; i < size; i++)
	{
		const uint8 r = nv::clamp(int(255.0f * pow(rChannel[i]>0.0f ?rChannel[i]:0.0f, 1.0f/gamma)), 0, 255);
		const uint8 g = nv::clamp(int(255.0f * pow(gChannel[i]>0.0f ?gChannel[i]:0.0f, 1.0f/gamma)), 0, 255);
		const uint8 b = nv::clamp(int(255.0f * pow(bChannel[i]>0.0f ?bChannel[i]:0.0f, 1.0f/gamma)), 0, 255);
