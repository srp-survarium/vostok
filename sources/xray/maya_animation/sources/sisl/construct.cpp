...
*/
{
  ...
  if ((qnew = newarray (1, SISLbox)) != SISL_NULL)
      ...
      for (ki = 0; ki < 3; ki++)
	{
      ...
      if ((qnew->emax = newarray (knum, sisl_double)) == SISL_NULL)
	{
	  freearray (qnew);
	  qnew = SISL_NULL;
	}
      else if ((qnew->emin = newarray (knum, sisl_double)) == SISL_NULL)
	{
	  freearray (qnew->emax);
	  freearray (qnew);
	  qnew = SISL_NULL;
...
#if defined(SISLNEEDPROTOTYPES)
SISLCurve *newCurve (int in, int ik, sisl_double *et, sisl_double *ecoef,
...
*/
{
  ...
  sisl_double *st = SISL_NULL;		/* Copy of knotvector.          */
  sisl_double *rcoef = SISL_NULL;		/* Copy of vertices in rational case.  */
  sisl_double *scoef = SISL_NULL;		/* Copy of vertices.            */
  ...
  if (k1 > 0)
  {
     memcopy(ecoef,ecoef+k1*kdim,(in-k1)*kdim,sisl_double);
     memcopy(et,et+k1,in+ik-k1,sisl_double);
  }
  in -= (k1+k2);
  ...
  if (icopy == 1)
    {
      ...
      if ((st = newarray (in +ik, sisl_double)) == SISL_NULL ||
	  (scoef = newarray (in *kdim, sisl_double)) == SISL_NULL)
	goto err101;

      /* Copy contents of arrays.  */

      memcopy (st, et, in +ik, sisl_double);
      memcopy (scoef, ecoef, in *kdim, sisl_double);
    }
  else
    {
      st = et;
  ...
  if (ikind == 2 || ikind == 4)
    {
      ...
      rcoef = newarray (in *idim, sisl_double);
      if (rcoef == SISL_NULL)
	goto err101;
  ...
  if ((qnew = newarray (1, SISLdir)) != SISL_NULL)
    {
      ...
      if ((qnew->ecoef = newarray (idim, sisl_double)) == SISL_NULL)
	freearray (qnew);
    }
  return (qnew);
}
...
SISLIntcurve *newIntcurve (int ipoint, int ipar1, int ipar2,
			   sisl_double *epar1, sisl_double *epar2, int itype)
#else
SISLIntcurve *
...
#if defined(SISLNEEDPROTOTYPES)
SISLIntpt *newIntpt (int ipar, sisl_double *epar, sisl_double adist)
#else
SISLIntpt *
...
*/
{
  ...
  if (ipar > 0)
    {
      pnew->epar = newarray (ipar, sisl_double);
...
*/
{
  ...
  pnew->epar = stpar1 = newarray (ipar * ipoint, sisl_double);
  if (pnew->epar == SISL_NULL)
    goto err101;
...
#if defined(SISLNEEDPROTOTYPES)
SISLIntpt *
hp_newIntpt (int ipar, sisl_double *epar, sisl_double adist, int itype,
	     int ileft1, int iright1, int ileft2, int iright2,
	     int size_1, int size_2, sisl_double egeom1[], sisl_double egeom2[])
#else
SISLIntpt *
...
*/
{
  ...
  if ((pnew->pnext = newarray (pnew->no_of_curves_alloc, SISLIntpt *)) == SISL_NULL)
    goto err101;
  if ((pnew->curve_dir = newarray (pnew->no_of_curves_alloc, sisl_int)) == SISL_NULL)
    goto err101;
  if ((pnew->left_obj_1 = newarray (pnew->no_of_curves_alloc, sisl_int)) == SISL_NULL)
    goto err101;
  if ((pnew->left_obj_2 = newarray (pnew->no_of_curves_alloc, sisl_int)) == SISL_NULL)
    goto err101;
  if ((pnew->right_obj_1 = newarray (pnew->no_of_curves_alloc, sisl_int)) == SISL_NULL)
    goto err101;
  if ((pnew->right_obj_2 = newarray (pnew->no_of_curves_alloc, sisl_int)) == SISL_NULL)
    goto err101;
  ...
  if (size_1 > 0)
    {
      pnew->geo_data_1 = newarray (size_1, sisl_double);
      pnew->size_1 = size_1;
      memcopy (pnew->geo_data_1, egeom1, size_1, sisl_double);
    }
  else
    {
      pnew->geo_data_1 = SISL_NULL;
  ...
  if (size_2 > 0)
    {
      pnew->geo_data_2 = newarray (size_2, sisl_double);
      pnew->size_2 = size_2;
      memcopy (pnew->geo_data_2, egeom2, size_2, sisl_double);
    }
  else
    {
      pnew->geo_data_2 = SISL_NULL;
...
newTrack (SISLSurf * psurf_1, SISLSurf * psurf_2, SISLCurve * pcurve_3d,
	...
	int ideg, sisl_double eimpli[], int sing_start, int sing_end, int turned)
#else
SISLTrack *
...
*/
{
  ...
  pnew->ideg = ideg;
  if (pnew->ideg)
    memcopy (pnew->eimpli, eimpli, 16, sisl_double);
...
#if defined(SISLNEEDPROTOTYPES)
SISLPoint *newPoint (sisl_double *ecoef, int idim, int icopy)
#else
SISLPoint *
...
*/
{
  ...
  sisl_double *scoef = SISL_NULL;		/* Copy of coordinates          */
  ...
  if (icopy == 1)
    {
      ...
      else if ((scoef = newarray (idim, sisl_double)) == SISL_NULL) goto err101;

      /* Copy contents of arrays.  */

      memcopy (scoef, ecoef, idim, sisl_double);
    }
  else
    {
      scoef = ecoef;
...
#if defined(SISLNEEDPROTOTYPES)
SISLSurf *
newSurf (int in1, int in2, int ik1, int ik2, sisl_double *et1, sisl_double *et2,
	 sisl_double *ecoef, int ikind, int idim, int icopy)
#else
SISLSurf *
...
*              ecoef  - vertices_type of surface.
...
*/
{
  ...
  sisl_double *st1 = SISL_NULL, *st2 = SISL_NULL;	/* Copy of knotvectors.        */
  sisl_double *rcoef = SISL_NULL;		/* Copy of vertices in rational case.  */
  sisl_double *scoef = SISL_NULL;		/* Copy of vertices.                   */
  sisl_double *ucoef = SISL_NULL;         /* Utility coefficient array.          */
  ...
  if (k1 > 0 || k2 > 0)
  {
     ucoef = newarray(in1*in2*kdim, sisl_double);
     s6chpar(ecoef, in1, in2, kdim, ucoef);
  }
  if (k1 > 0)
  {
     memcopy(ucoef, ucoef + k1*in2*kdim, (in1 - k1)*in2*kdim, sisl_double);
     memcopy(et1, et1 + k1, in1 + ik1 - k1, sisl_double);
  }
  in1 -= (k1 + k2);
  if (k1 > 0 || k2 > 0)
  {
  ...
  if (k1 > 0)
  {
     memcopy(ecoef, ecoef + k1*in1*kdim, (in2 - k1)*in1*kdim, sisl_double);
     memcopy(et2, et2 + k1, in2 + ik2 - k1, sisl_double);
  }
  in2 -= (k1 + k2);
  ...
  if (icopy == 1)
    {
      ...
      st1 = newarray (in1 + ik1, sisl_double);
      st2 = newarray (in2 + ik2, sisl_double);
      scoef = newarray (in1 * in2 * kdim, sisl_double);
      if (st1 == SISL_NULL || st2 == SISL_NULL || scoef == SISL_NULL)
	goto err101;

      /* Copy contents of arrays.  */
      memcopy (st1, et1, in1 + ik1, sisl_double);
      memcopy (st2, et2, in2 + ik2, sisl_double);
      memcopy (scoef, ecoef, in1 * in2 * kdim, sisl_double);
    }
  else
    {
      st1 = et1;
  ...
  if (ikind == 2 || ikind == 4)
    {
      /* Calculate the weighted control points if the object is rational  */
      rcoef = newarray (in1 * in2 * idim, sisl_double);
      if (rcoef == SISL_NULL)
	goto err101;
...
*/
{
  ...
  if (pcurve->pbox != SISL_NULL)
  {
	...
	goto err101;
     ...
     memcopy(qc->pbox->emin, pcurve->pbox->emin, knum, sisl_double);
     memcopy(qc->pbox->emax, pcurve->pbox->emax, knum, sisl_double);
     memcopy(qc->pbox->etol, pcurve->pbox->etol, 3, sisl_double);
  ...
  if (pcurve->pdir != SISL_NULL)
  {
	...
	goto err101;
     ...
     memcopy(qc->pdir->ecoef, pcurve->pdir->ecoef, pcurve->idim, sisl_double);
     
     if (pcurve->pdir->esmooth != SISL_NULL)
     {
	if ((qc->pdir->esmooth = newarray(qc->in*qc->idim,sisl_double)) == SISL_NULL)
	{
	   freeCurve(qc);
	   qc = SISL_NULL;
	   goto err101;
...
*/
{
  ...
  if (psurf->pbox != SISL_NULL)
  {
	...
	goto err101;
     ...
     memcopy(qs->pbox->emin, psurf->pbox->emin, knum, sisl_double);
     memcopy(qs->pbox->emax, psurf->pbox->emax, knum, sisl_double);
     memcopy(qs->pbox->etol, psurf->pbox->etol, 3, sisl_double);
  ...
  if (psurf->pdir != SISL_NULL)
  {
	...
	goto err101;
     ...
     memcopy(qs->pdir->ecoef, psurf->pdir->ecoef, psurf->idim, sisl_double);
     
     if (psurf->pdir->esmooth != SISL_NULL)
     {
	if ((qs->pdir->esmooth = newarray(qs->in1*qs->in2*qs->idim,sisl_double)) 
	    == SISL_NULL)
	{
	   freeSurf(qs);
	   qs = SISL_NULL;
	...
	memcopy(qs->pdir->esmooth, psurf->pdir->esmooth, 
		qs->in1*qs->in2*qs->idim, sisl_double);
     }
  }
