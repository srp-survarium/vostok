...
public:
	...
	virtual size_t		write			(pvoid handle, pcvoid data, size_t size);
	virtual size_t		read			(pvoid handle, pvoid data, size_t size);

	virtual bool		seek			(pvoid handle, file_size_type offset, seek_file_enum origin);
	virtual file_size_type	tell		(pvoid handle);

	virtual void		setvbuf			(pvoid handle, char * buffer, int mode, size_t size);
