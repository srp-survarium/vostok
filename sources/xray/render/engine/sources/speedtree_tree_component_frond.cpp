	...
	speedtree_tree_component(parent) 
{
	...
	for (s32 lod_index=0; lod_index<num_lods; lod_index++)
	{
        ...
        for (s32 vertex_index=0; vertex_index<lod->m_nNumVertices; vertex_index++)
        {
			...
			// LOD position
			xray::memory::copy(p_vertex->m_afLodCoord, sizeof(p_vertex->m_afCoord), lod->m_pLodCoords ? lod->m_pLodCoords + vertex_index * 3 : lod->m_pCoords + vertex_index * 3, sizeof(p_vertex->m_afLodCoord));

			// normal
			xray::memory::copy(p_vertex->m_aucNormal, sizeof(p_vertex->m_afCoord), lod->m_pNormals + vertex_index * 3, sizeof(p_vertex->m_aucNormal));
			...
			// tangent
			xray::memory::copy(p_vertex->m_aucTangent, sizeof(p_vertex->m_afCoord), lod->m_pTangents + vertex_index * 3, sizeof(p_vertex->m_aucTangent));
			...
			if (lod->m_pWindData)
			{
				...
				xray::memory::copy(p_vertex->m_aucWindData, sizeof(p_vertex->m_afCoord), lod->m_pWindData + vertex_index * 6, sizeof(p_vertex->m_aucWindData));
			}
			else
			{
