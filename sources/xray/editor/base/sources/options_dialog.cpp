...
void options_dialog::unregister_page( System::String^ full_name )
{
	...
	ASSERT					(found_node);
	xray::editor_base::options_page^ page		= dynamic_cast<xray::editor_base::options_page^>(found_node->Tag);
	if(page)
		delete page;
...
void options_dialog::register_page( System::String^ full_name, xray::editor_base::options_page^ page )
...
Void options_dialog::tree_view_AfterSelect(System::Object^  sender, System::Windows::Forms::TreeViewEventArgs^  e)
{
	...
	xray::editor_base::options_page^ page			= nullptr;

	if( curr_node )
		page = dynamic_cast<xray::editor_base::options_page^>(curr_node->Tag);
...
void options_dialog::initialize_all( TreeNodeCollection^ nodes )
{
	...
	{
		xray::editor_base::options_page^ page = dynamic_cast<xray::editor_base::options_page^>( nodes[i]->Tag ); 
		if( page )
		{
			page->activate_page			( );
