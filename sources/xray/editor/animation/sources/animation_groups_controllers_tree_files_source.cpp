...
using xray::animation_editor::animation_groups_controllers_tree_files_source;
...
animation_groups_controllers_tree_files_source::animation_groups_controllers_tree_files_source()
{
	...
	m_watcher_subscribe_id	=	xray::resources::subscribe_watcher(unmanaged_string( animation_editor::animation_controllers_path ).c_str( ), boost::bind(&file_watcher_delegate::callback, m_callback, _1));
}
...
void animation_groups_controllers_tree_files_source::fs_callback(xray::vfs::vfs_notification const& info)
{
...
void animation_groups_controllers_tree_files_source::on_fs_iterator_ready(xray::vfs::vfs_locked_iterator const & fs_it)
{
...
void animation_groups_controllers_tree_files_source::process_fs(xray::vfs::vfs_iterator const & in_it, String^ file_path)
{
