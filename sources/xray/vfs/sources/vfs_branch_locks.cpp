...
bool   has_lock							(base_node<> * node, lock_type_enum lock_type)
{
	if ( node->is_folder() )
		...
		return								node->get_parent()->has_lock(lock_type);
...
bool   lock_branch						(base_node<> * node, lock_type_enum lock_type, lock_operation_enum operation)
{
	...
	lock_type_enum const soft_lock		=	node->is_folder() ? soften_lock(lock_type) : lock_type;
...
void   upgrade_branch					(base_node<> * node, lock_type_enum from_lock, lock_type_enum to_lock)
{
	...
	upgrade_branch							(node_cast<base_node>(parent), 
											 node->is_folder() ? soften_lock(from_lock) : from_lock, 
											 node->is_folder() ? soften_lock(to_lock)	: to_lock);
