 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
HybridModel::HybridModel(Opcode::allocator_type* allocator) :
	BaseModel		( allocator ),
	mNbLeaves		(0),
	mNbPrimitives	(0),
	mTriangles		(null),
...
void HybridModel::Release()
{
	...
	struct Internal
	{
		Internal(allocator_type* allocator) :
...
bool HybridModel::Build(const OPCODECREATE& create)
{
	...
	Internal Data(m_allocator);

	// 2) Build a generic AABB Tree.
	mSource = NEW ( AABBTree )( m_allocator );
	CHECKALLOC(mSource);

	// 2-1) Setup a builder. Our primitives here are triangles from input mesh,
	// so we use an AABBTreeOfTrianglesBuilder.....
	{
		AABBTreeOfTrianglesBuilder TB(m_allocator);
	...
	Data.mLeaves = NEW_A ( AABB, Data.mNbLeaves );			CHECKALLOC(Data.mLeaves);
	mTriangles = NEW_A ( LeafTriangles, Data.mNbLeaves );	CHECKALLOC(mTriangles);
	...
	// Handle source indices
	{
		...
		if(MustKeepIndices)
		{
			...
			mIndices = NEW_A ( udword, mNbPrimitives );
	...
	LeafTree = NEW ( AABBTree )( m_allocator );
	CHECKALLOC(LeafTree);
	{
		AABBTreeOfAABBsBuilder TB(m_allocator);	// Now using boxes !
		TB.mSettings		= create.mSettings;
