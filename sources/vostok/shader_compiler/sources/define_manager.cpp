		...
		|| conflicts_config.get_type()==configs::t_table_named)
	{
			...
			++it )
		{
				...
				++conf_it )
			{
				...
				// table with values
				if (define_table.get_type()==vostok::configs::t_table_indexed)
				{
						...
						++define_values_it )
					{
						...
						if (def_value.get_type()==vostok::configs::t_integer)
						{
							define_value* def_v = pair.define[define_index]->add_value("UNDEFINED");
							def_v->set_undefined(true);
						}
						else if (def_value.get_type()==vostok::configs::t_string)
						{
...
void define_manager::merge_with_declarated_defines( vostok::render::shader_declarated_macroses_list const& declared_defines)
{
	for (defines_list_type::iterator def_it=m_defines.begin(); def_it!=m_defines.end(); ++def_it)
	{
		...
		bool found = false;
		for (vostok::render::shader_declarated_macroses_list::const_iterator decl_it=declared_defines.begin(); decl_it!=declared_defines.end(); ++decl_it)
		{
			pcstr current_decl_name = (*decl_it).c_str();
			
			if (vostok::strings::compare(to_find, current_decl_name)==0)
			{
				found = true;
				break;
			}
...
define_manager::~define_manager( )
{
	...
	{
		VOSTOK_DELETE_IMPL(	::vostok::shader_compiler::g_allocator, *it );
	}
	for (std::vector<conflict_pair>::iterator it = m_conflicts.begin(); it!=m_conflicts.end(); ++it)
	{
		VOSTOK_DELETE_IMPL(	::vostok::shader_compiler::g_allocator, it->define[0] );
		VOSTOK_DELETE_IMPL(	::vostok::shader_compiler::g_allocator, it->define[1] );
	}
}
