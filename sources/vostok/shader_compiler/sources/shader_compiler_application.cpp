...
static vostok::command_line::key   s_vs_output	( "vs_output",	"", "", "outputs to file (for Visual Studio)");
...
using vostok::shader_compiler::application;

vostok::memory::doug_lea_allocator_type	vostok::shader_compiler::g_allocator;

class shader_compiler_core_engine : public vostok::core::simple_engine
{
public:
...
static vostok::uninitialized_reference< shader_compiler_core_engine >	s_core_engine;
...
//	bool show_define_name = g_show_define_names.is_set();
	...
	while ( ++f_batcher )
	{
		...
		{
			vostok::render::shader_declarated_macroses_list found_declared_macroses;
			vostok::render::found_shader_declarated_macroses(file_data.c_str(), found_declared_macroses);
...
//			printf(".");
			...
			if ( !errors.empty() )
			{
				...
				for (u32 index=0; index<errors.size(); index++) {
					...
					sprintf_s				( temp, "%s%s(%d) : %s", ".\\"/**f_batcher.get_general_path().c_str()/**/, vostok::fs_new::file_name_from_path( fs_new::virtual_path_string(current.file_name) ), current.line, current.error_message );
			...
			if ( !warnings.empty() )
			{
				...
				for (u32 index=0; index<warnings.size(); index++) {
					...
					sprintf_s				( temp, "%s%s(%d) : %s", ".\\"/**f_batcher.get_general_path().c_str()/**/, 
											  vostok::fs_new::file_name_from_path(vostok::fs_new::virtual_path_string::convert(current.file_name)), 
											  current.line, 
