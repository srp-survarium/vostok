...
#pragma warning( pop )
{
	...
	m_logic_world_user			= VOSTOK_NEW_IMPL( g_allocator, world_user )( this, &logic_allocator );
	R_ASSERT					( m_logic_world_user );
#ifndef MASTER_GOLD
	if ( editor_allocator )
	{
		m_editor_world_user		= VOSTOK_NEW_IMPL( g_allocator, world_user )( this, editor_allocator );
...
//	s_test_time_factor_cc.subscribe_on_change				( boost::bind( &sound_world::test_timer, this, _1 ));
	...
	if ( m_is_audio_device_exist )
	{
		...
		VOSTOK_CONSTRUCT_REFERENCE		( m_sound_voices_allocator, sound_voices_allocator )( m_unmanaged_resources_ptr->buffer + sound_voices_offset, sound_voices_size );
...
#pragma message( VOSTOK_TODO("dimetcm 2 dimetcm: avoid this") )
	static u32 i					= 0;
												...
												sound_spl_ptr const& sound_spl )
{
	...
	sound_voice* new_sound_voice		= VOSTOK_NEW_IMPL( m_sound_voices_allocator.c_ptr(), sound_voice )( playing_offset, before_playing_offset, after_playing_offset, *proxy, emitter, sound_spl ); 
...
void sound_world::delete_sound_voice( sound_scene& scene, sound_voice* voice )
{
	...
	if ( voice->can_be_deleted( ) )
		VOSTOK_DELETE_IMPL			( m_sound_voices_allocator.c_ptr(), voice );
	else
