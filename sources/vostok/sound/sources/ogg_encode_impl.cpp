...
static void filt_sinc(float *dest, int N, int step, double fc, double gain, int width)
{
    ...
    while (N--)
    {
		*dest = static_cast<float>((x ? sin(x * vostok::math::pi * s) / (x * vostok::math::pi) * step : fc) * gain);
        x++;
        dest += width;
        if (dest >= endpoint)
            dest = ++base;
...
int res_drain(res_state *state, SAMPLE **dstlist)
{
	...
	vostok::memory::zero(tail, state->taps * sizeof(SAMPLE));
...
long wav_read(void *in, fs_new::synchronous_device_interface const & device, float **buffer, int samples)
{
	...
	vostok::memory::zero(buf, samples*sampbyte*f->channels);
...
input_format *open_audio_file(fs_new::file_type *in, fs_new::synchronous_device_interface const & device)
{
	...
	vostok::memory::zero(buf, buf_size);
					...
					int new_bitrate)
{
	...
	vostok::memory::zero(wav, sizeof(wavfile));
	/////////////////////////////////////////////
	// open wav
	////////////////////////////////////////////
	//{
	...
	//    if(len>40)len=40;
				 ...
				 format.samplesize == 16 || format.samplesize == 8)) 
		{
			...
			vostok::memory::zero(wav->channel_permute, wav->channels * sizeof(int));
			if (wav->channels <= 8)
	...
	vostok::memory::zero(rs.resampler_state.table, outfreq * taps * sizeof(float));
	vostok::memory::zero(rs.resampler_state.pool, channels * taps * sizeof(SAMPLE));
    ...
    for(c=0; c < channels; c++)
	{
		...
		vostok::memory::zero(rs.bufs[c], sizeof(float) * rs.bufsize);
	}
