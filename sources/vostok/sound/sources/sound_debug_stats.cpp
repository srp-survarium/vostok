...
VOSTOK_DECLARE_LINKAGE_ID					( sound_debug_stats_id );
	...
	m_actual_statistic			( -1 )
{
	...
	m_progress_bars								= VOSTOK_NEW_ARRAY_IMPL( allocator, ui::progress_bar*, max_propagator_statistics_count );
	...
	sound_order* const order			= VOSTOK_NEW_IMPL( m_world_user.get_allocator(), functor_order )
...
void sound_debug_stats::clear_resources	( world_user& user )
{
	...
	VOSTOK_DELETE_ARRAY_IMPL		( m_allocator, m_progress_bars );
...
void sound_debug_stats::create_statistic	( )
{
	...
	sound_response* const response		= VOSTOK_NEW_IMPL( g_allocator, functor_response )
...
void sound_debug_stats::draw_overall_stats	( render::scene_ptr scene )
{
	...
	pvoid buffer									= VOSTOK_ALLOCA_IMPL( 64 * vostok::Kb );
	text_tree text_tree_view						( buffer, 64 * vostok::Kb, "sound scene statistic" );
...
void sound_debug_stats::draw_detail_stats	( render::scene_ptr scene, u32 proxy_id )
{
	...
	while ( stats )
	{
		...
		{
			pvoid buffer							= VOSTOK_ALLOCA_IMPL( 64 * vostok::Kb );
			text_tree text_tree_view				( buffer, 64 * vostok::Kb, "sound proxy statistic" );
...
void sound_debug_stats::update_window		( vostok::strings::text_tree_item* item )
{
	R_ASSERT								( m_main_window );
	R_ASSERT								( item );
