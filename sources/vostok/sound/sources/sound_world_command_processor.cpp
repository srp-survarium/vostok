...
void sound_world::initialize_command_processor( )
{
	...
	m_xaudio_callback_orders.push_null_node		( MT_NEW(sound_command)(&::vostok::memory::g_mt_allocator) );
	...
	m_end_user_orders.push_null_node			( VOSTOK_NEW_IMPL(m_logic_allocator, sound_command)(m_logic_allocator) );

// logic
	m_logic_orders_to_delete.set_push_thread_id	( );
	m_logic_orders_to_delete.push_null_node		( VOSTOK_NEW_IMPL(m_logic_allocator, sound_command)(m_logic_allocator) );

	m_logic_callbacks_orders.set_push_thread_id	( );
	m_logic_callbacks_orders.push_null_node		( VOSTOK_NEW_IMPL(g_allocator, sound_command)(g_allocator) );
...
// editor
	...
	m_editor_orders_to_delete.push_null_node	( VOSTOK_NEW_IMPL(m_editor_allocator, sound_command)(m_editor_allocator) );

	m_editor_callbacks_orders.set_push_thread_id( );
	m_editor_callbacks_orders.push_null_node	( VOSTOK_NEW_IMPL(g_allocator, sound_command)(g_allocator) );
}
...
void sound_world::clear_resources	( )
{
	...
	// clear editor own commands
	{
		...
		ASSERT(cmd->m_creator == &::vostok::memory::g_mt_allocator);
		VOSTOK_DELETE_IMPL			( cmd->m_creator, cmd );
...
void sound_world::process_sound_thread_orders( )
{
	...
	while ( !m_xaudio_callback_orders.empty( ) ) 
	{
		...
		order->execute				( );
		VOSTOK_DELETE_IMPL			( to_delete->m_creator, to_delete );
	}
