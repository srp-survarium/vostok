...
{
	playback_command* cmd = VOSTOK_NEW_IMPL(thread_allocator(), playback_command)(thread_allocator(), playback_command::ev_play, this);
	g_sound_world->add_end_user_command( cmd );
}

void sound_object_impl::stop( )
{
	playback_command* cmd = VOSTOK_NEW_IMPL(thread_allocator(), playback_command)(thread_allocator(), playback_command::ev_stop, this);
	g_sound_world->add_end_user_command( cmd );
}

void sound_object_impl::set_position( float3 const& p )
{
	VOSTOK_UNREFERENCED_PARAMETER	( p );
}
