...
void world_user::initialize				( )
{
	if ( m_allocator )
	{
		m_deleted_producers			= VOSTOK_NEW_IMPL( m_allocator, vectora<u64> ) ( m_allocator );
		m_deleted_receivers			= VOSTOK_NEW_IMPL( m_allocator, vectora<u64> ) ( m_allocator );
	}

	m_channel.orders.owner_initialize	(
		VOSTOK_NEW_IMPL( m_allocator, sound_order ) ( ),
		VOSTOK_NEW_IMPL( m_allocator, sound_order ) ( )
	);
...
void world_user::finalize				( )
{
	if ( m_deleted_producers )
		VOSTOK_DELETE_IMPL				( m_allocator, m_deleted_producers );

	if ( m_deleted_receivers )
		VOSTOK_DELETE_IMPL				( m_allocator, m_deleted_receivers );
...
{
	VOSTOK_UNREFERENCED_PARAMETER		( mute );
}
...
void world_user::register_receiver	( sound_scene_ptr& scene, sound_receiver& receiver )
{
	...
	functor_order* const order	= VOSTOK_NEW_IMPL( m_allocator, functor_order )
	(
		boost::bind
		( 
...
void world_user::unregister_receiver( sound_scene_ptr& scene, sound_receiver& receiver )
{
	...
	functor_order* const order =
		VOSTOK_NEW_IMPL( m_allocator, functor_order )	( 
...
void world_user::set_time_scale_factor				( float factor )
{
	...
	sound_order* const order				= VOSTOK_NEW_IMPL
