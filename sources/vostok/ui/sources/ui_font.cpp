...
ui_font::~ui_font( )
{
	VOSTOK_FREE_IMPL		( allocator(), m_char_map );
}

void ui_font::init_font(pcstr name)
{
	VOSTOK_UNREFERENCED_PARAMETER	(name);
	...
	VOSTOK_FREE_IMPL		( allocator(), m_char_map );
	m_char_map			= VOSTOK_ALLOC_IMPL( allocator(), math::float3, m_char_count );
...
{
	vostok::ui::parse_word(str, this, out_length, out_next_word);
}
