...
static vostok::command_line::key	s_memory_leaks_detection_key("memory_leaks_detection", "", "memory", "turns on memory leaks detection");
...
private:
	...
	typedef hash_multiset<	allocation_info, allocation_info *, & allocation_info::next_hash_node, vostok::detail::fixed_size_policy<16384>	>	allocation_info_container;
...
void   leak_detector::on_allocate (pvoid& pointer, pcstr const description)
{
	...
	allocation_info info;
	u32				stacktrace_hash;
	vostok::debug::call_stack::get_stack_trace (info.stacktrace, array_size(info.stacktrace), 62, &stacktrace_hash);
...
void   leak_detector::dump_leaks ()
{
											  ...
											  ++it )
	{
		...
		vostok::debug::call_stack::iterate		(NULL, 
												 info->stacktrace, 
...
void initialize			( )
{
	VOSTOK_CONSTRUCT_REFERENCE		(s_leak_detector, leak_detector);
}
...
void finalize			( )
{
	if ( enabled() )
		s_leak_detector->dump_leaks	( );
	VOSTOK_DESTROY_REFERENCE			( s_leak_detector );
}
...
{
	VOSTOK_UNREFERENCED_PARAMETER		( allocation_size );
