...
#include <vostok/os_include.h>						// for LoadLibrary and etc
#include <dbghelp.h>								// for MiniDump flags
#include <BugTrap/BugTrap.h>						// for BugTrap functionality
#include "logging.h"
#include "debug_internal.h"

using vostok::core::debug::bugtrap_usage;
using vostok::core::debug::error_mode;
...
static error_mode		s_error_mode				= vostok::core::debug::error_mode_verbose;
static bugtrap_usage	s_bugtrap_usage				= vostok::core::debug::native_bugtrap;
...
{
	if ( s_bugtrap_usage == vostok::core::debug::no_bugtrap ) {
		result						= 0;
		return;
	}
	...
	if ( !result )
		s_bugtrap_usage				= vostok::core::debug::no_bugtrap;
}
...
static void load_library							( )
{
	...
	s_bugtrap_handle				= LoadLibrary( s_bugtrap_usage == vostok::core::debug::native_bugtrap ? s_bugtrap_native_id : s_bugtrap_dotnet_id );
	if ( !s_bugtrap_handle ) {
...
{
	if ( s_bugtrap_usage == vostok::core::debug::native_bugtrap )
		return						message;
	...
	MultiByteToWideChar				(
		...
		MB_PRECOMPOSED,
		message,
		vostok::strings::length(message) + 1,
		&output[0],
		max_count
	);

	return							vostok::pointer_cast<pstr>(&output[0]);
}
...
static void install									( )
{
	...
	vostok::core::debug::bugtrap::setup_unhandled_exception_handler( );

	char const message[]			=
		"This is " VOSTOK_ENGINE_ID " crash reporting client. "
	...
	sprintf_s						( application_name, "%s-#%d", vostok::core::application_name(), vostok::core::debug::build_station_build_id() );
