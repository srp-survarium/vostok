...
{
	//bool const is_similar_determinant	= vostok::math::is_similar( determinant, other.determinant( ), vostok::math::epsilon_5 );
...
float4x4 vostok::math::invert4x3							( float4x4 const& other )
{
	R_ASSERT	( other.valid( ) );
	return		invert_impl( other, other.determinant4x3() );
}

bool vostok::math::try_invert4x4			( float4x4 const& matrix_to_invert, float4x4& out_result )
{
...
float4x4 vostok::math::invert4x4			( float4x4 const& matrix_to_invert )
{
	float4x4 out_result;
...
float4x4 vostok::math::mul4x4						( float4x4 const& left, float4x4 const& right )
{
	float4x4	result;
...
float4x4 vostok::math::create_perspective_projection	( float4x4::type vertical_field_of_view, float4x4::type aspect_ratio, float4x4::type near_plane, float4x4::type far_plane )
{
...
float4x4 vostok::math::create_orthographic_projection	( float4x4::type w, float4x4::type h, float4x4::type near_plane, float4x4::type far_plane )
{
...
float4x4 vostok::math::create_camera_direction		( float3 const& from, float3 const& view, float3 const& world_up )
{
...
float4x4 vostok::math::create_camera_at				( float3 const& from, float3 const& at, float3 const& up )
{
	return		( create_camera_direction ( from, ( at - from ).normalize( ), up ) );
}

float4x4 vostok::math::create_rotation				( float3 const& axis, float const angle )
{
	float4x4	result;
...
bool vostok::math::try_solve_linear_equations_system	(
		float3 const& first,
		float3 const& second,
		float3 const& third,
		float3 const& b,
