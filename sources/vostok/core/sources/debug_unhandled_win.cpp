...
static void save_minidump						( pcstr const output_file_name, _EXCEPTION_POINTERS* const exception_information )
{
	...
	vostok::strings::join			( file_name, vostok::core::current_directory(), library_id );
	...
	vostok::fs::make_dir_r		( output_file_name, false );
	// create the file
...
void vostok::core::debug::platform::save_minidump		( SYSTEMTIME const& date_time, _EXCEPTION_POINTERS* exception_information )
{
	string_path					temp;
...
{
	__try {
		if ( !vostok::core::debug::platform::error_after_dialog( ) )
			vostok::debug::dump_call_stack	( "debug", vostok::logging::error, 0, 0, exception_information );
