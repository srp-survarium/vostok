...
#else
   // This is trivial :)
	vostok::strings::copy( lpszOut, (u32)outSize, lpszIn );
#endif
}
...
static void GetStackTrace(HANDLE hThread, /**DWORD64 ranOffsets[][2],/**/ _EXCEPTION_POINTERS* pointers, vostok::core::debug::call_stack::Callback const& callback, bool invert_order )
{
...
//			OutputDebugStringFormat( _T("Call stack info(thread=0x%X) GetThreadContext failed.\n") );
		...
		vostok::memory::copy	( &context, sizeof(context), pointers->ContextRecord, sizeof(CONTEXT) );
...
static void WriteStackTrace ( DWORD64 ranOffsets[][2], u32 num_call_stack_lines, vostok::core::debug::call_stack::Callback const& callback, bool invert_order )
{
	TCHAR module_name[256] = _T("?");
	TCHAR file_name[2*260] = _T("?");
	TCHAR function[s_buffer_size] = _T("?");
...
static void InitSymbolPath( PSTR lpszSymbolPath, rsize_t symPathSize, PCSTR lpszIniPath )
{
   ...
   // ".;%_NT_SYMBOL_PATH%;%_NT_ALTERNATE_SYMBOL_PATH%;%SYSTEMROOT%;%SYSTEMROOT%\System32;"
	vostok::strings::copy( lpszSymbolPath, (u32)symPathSize, "." );
...
void vostok::core::debug::call_stack::iterate	( _EXCEPTION_POINTERS*	pointers,  // can be NULL
...
void vostok::core::debug::call_stack::finalize_symbols( )
{
	if ( !s_SymCleanup )
		return;
...
void vostok::core::debug::call_stack::get_stack_trace (void* stacktrace[], u32 const stacktrace_max, u32 const num_to_capture, u32* out_hash)
{
	initialize							();
