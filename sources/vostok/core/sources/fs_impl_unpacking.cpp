...
bool   file_system_impl::unpack (pcstr logical_path, pcstr dest_dir_path, db_callback callback)
{
	...
	u32		hash;
	fat_node<> *	unpack_root		=	find_node_no_mount(logical_path, & hash);
...
bool   file_system_impl::unpack_node (saving_environment & env, fat_node<> * cur_node, path_string & dest_dir)
{
	...
	else if ( !is_root )
	{
		...
		seek_file				(f_in, cur_node->get_file_offs(), SEEK_SET);
