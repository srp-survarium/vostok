	...
	)
{
	...
	vostok::fs::path_string	log_file_name;
	generate_log_file_name (& log_file_name, "log");
	vostok::fs::path_string	log_file_directory;
	vostok::fs::directory_part_from_path	(& log_file_directory, log_file_name.c_str());

	vostok::fs::make_dir_r	(log_file_directory.c_str());

	FILE * file			=	NULL;
	if ( vostok::fs::open_file (& file,	vostok::fs::open_file_create	| 
										vostok::fs::open_file_write	| 
										vostok::fs::open_file_truncate, log_file_name.c_str(), false) )
	{
		if ( file )
		{
