														...
														bool					file_exist)
{
	...
	mutable_buffer const out_buffer			(VOSTOK_ALLOC_IMPL(allocator, char, resource_size), resource_size);
...
void   test_resource_cook::destroy_resource (unmanaged_resource * resource)
{
	...
	if ( m_create_allocates_destroy_deallocates )
	{
		...
		VOSTOK_FREE_IMPL						(s_test_allocators[m_allocator_index].allocator(), final_buffer);
	}
}
...
{
	TEST_ASSERT								(!m_create_allocates_destroy_deallocates);
	VOSTOK_FREE_IMPL							(s_test_allocators[m_allocator_index].allocator(), buffer);
}
