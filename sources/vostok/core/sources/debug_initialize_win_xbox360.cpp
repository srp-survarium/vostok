...
{
	VOSTOK_UNREFERENCED_PARAMETER		( reason_string );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( expression, function, file, line );
#ifdef DEBUG
	string4096						temp;
...
{
	VOSTOK_UNREFERENCED_PARAMETER		( signal );
...
static void floating_point_handler		(int signal, int error_code)
{
	...
	string256						error;
	vostok::sprintf					( error, "floating point error ( %s ) ", description );
	handler_base					( error );
}
...
void vostok::debug::change_bugtrap_usage	( core::debug::error_mode error_mode, core::debug::bugtrap_usage bugtrap_usage )
{
...
#ifdef DEBUG
bool vostok::debug::is_fpe_enabled	( )
{
...
{
#if defined(DEBUG) && !VOSTOK_PLATFORM_WINDOWS_64
	if ( !g_fpe_disabled )
		vostok::debug::enable_fpe		( true );
#endif // #if defined(DEBUG) && !VOSTOK_PLATFORM_WINDOWS_64
}