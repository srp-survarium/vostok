...
text_tree_item::text_tree_item(vostok::strings::allocator_type& allocator, pcstr value, bool is_page_breaker)
...
{
	if(m_column_value)
		VOSTOK_FREE_IMPL(m_allocator, m_column_value);
...
{
	if(!vostok::strings::compare(m_column_value, s))
		return this;
...
{
	text_tree_item* itm = VOSTOK_NEW_IMPL(m_allocator, text_tree_item)(m_allocator, s, is_page_breaker);
...
{
	text_tree_column_item* itm = VOSTOK_NEW_IMPL(m_allocator, text_tree_column_item)();
