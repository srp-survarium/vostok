...
managed_allocator::managed_allocator (u32 const temp_arena_size, u32 const reserved_size, u32 const granularity)
: managed_allocator_base(granularity), m_temp_arena(granularity), m_arena_id(NULL)
{
...
void   managed_allocator::initialize_impl (pvoid arena, u64 size, pcstr arena_id)
{
	...
	m_temp_arena.managed_allocator_base::initialize	(temp_arena_ptr, temp_arena_size);
	managed_allocator_base::initialize		(arena_ptr, arena_size);
	m_arena_id							=	arena_id;
}

size_t	managed_allocator::usable_size_impl	( pvoid pointer ) const
{
	VOSTOK_UNREFERENCED_PARAMETER				( pointer );
	NOT_IMPLEMENTED							( return 0 );
}

pvoid   managed_allocator::call_malloc (size_t size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION)
{
...
pvoid   managed_allocator::call_realloc (pvoid pointer, size_t new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION)
{
	VOSTOK_CORE_DEBUG_PARAMETERS_UNREFERENCED_GUARD;
	VOSTOK_UNREFERENCED_PARAMETER				( VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_PARAMETER );
...
void   managed_allocator::call_free (pvoid pointer VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION)
{
	VOSTOK_CORE_DEBUG_PARAMETERS_UNREFERENCED_GUARD;
	VOSTOK_UNREFERENCED_PARAMETER				(pointer);
	NOT_IMPLEMENTED							();
...
bool   managed_allocator::can_allocate	(u32 const buffer_size) const
{
	return									m_free_size - m_reserved_size >= buffer_size;
}

managed_node *   managed_allocator::allocate (u32 const size)
{
...
//											 "not enough memory even if we defragmented");
	if ( full_size > (m_free_size - m_reserved_size) )
		return								0;

//	R_ASSERT_CMP							(full_size, <=, m_temp_arena.total_size(), "temp arena cant hold such block" );
...
void   managed_allocator::deallocate (managed_node * node)
{
	...
	check_consistency						();
	managed_allocator_base::deallocate		(node);

	check_consistency						();
}

managed_node *   managed_allocator::find_node_of_type (managed_node * const		start_node,
													   managed_node_type const	type)
{
...
managed_node *   managed_allocator::allocate_in_node_defrag  (size_t const	    full_size, 
																	   managed_node *	node,
...
bool   managed_allocator::check_valid_place_node () const
{
...
managed_node*	managed_allocator::deallocate_defrag (managed_node* node)
{
	ASSERT										(m_defragmenting);
...
void    managed_allocator::log_defragmenter_nodes (managed_allocator_base const& alloc) const
{
	...
	while ( cur_node )
	{
		...
		log_string_type						full_path;

		type_string						=	 cur_node->is_allocated() ?
											 "allocated" : "free";
		if ( !cur_node->is_free() )
			full_path					=	cur_node->owner()->log_string().c_str();
...
void   managed_allocator::notify_unmovable_changed	(managed_node* const node)
{
#ifndef MASTER_GOLD
	if ( is_defragmenting() )
	...
	{
		unmovables_t::iterator it		=	std::find_if(
												m_unmovables.begin(), 
												m_unmovables.end(), 
...
managed_allocator_base &   managed_allocator::get_node_arena (managed_node * const node)
{
...
void   managed_allocator::transfer_resource (managed_node* const	dest_node, 
													  managed_node* const	src_node)
{
	...
	managed_node_owner *  res			=	src_node->owner();
...
bool   managed_allocator::is_waiting_for_unpin (managed_node* const node) const
{
...
managed_node *   managed_allocator::allocate_duplicate_in_main_arena (managed_node * const src_node,
...
void   managed_allocator::free_unpinned (arena_t arena_type)
{
	ASSERT										(arena_type == main_arena || arena_type == temp_arena);

	managed_allocator_base &	arena		=	arena_type == main_arena ? *this : m_temp_arena;
...
void   managed_allocator::add_unpin_looked_node (managed_node* const node)
{
	managed_allocator_base &	arena		=	get_node_arena(node);
...
managed_node*   managed_allocator::to_next_ignored_unmovable (u32& index)
{
...
u32   managed_allocator::calculate_place_pos_and_free_space (buffer_vector<u32>&	looses,
																	  u32 const				start_index,
																	  pbyte					place_pos,
																	  u32&					result_index)
{
	ASSERT										(looses.max_size() >= m_defrag_unmovables->size());
	buffer_vector<unmovable>&	unmovables	=	*m_defrag_unmovables;
...
void   managed_allocator::calculate_unmovables_state (u32		const	start_index,
...
managed_node*   managed_allocator::get_next_defragmentation_node (managed_node* cur_node)
{
	if ( !cur_node )
		return									cur_node;
	...
	do
	{
		cur_node							=	find_node_of_type
												(cur_node->m_next, managed_node_allocated);
	}
...
void   managed_allocator::set_dependence (managed_node* const depend_node, 
...
size_t   managed_allocator::get_contigous_space_defrag (u32 const	unmovable_index)
{
...
void   managed_allocator::init_mode (mode_state* const mode)
{
...
void   managed_allocator::move_next_mode_node ()
{
...
void   managed_allocator::handle_unmovable (u32 const	sufficient_contigous_space, 
													 bool&		call_continue_in_main_cycle)
{
	VOSTOK_UNREFERENCED_PARAMETER					(sufficient_contigous_space);
	buffer_vector<unmovable>&	unmovables	=	*m_defrag_unmovables;
	...
	if ( unmovable_state == unmovable::waited )
	{
		...
		while ( m_mode->node->m_is_unmovable )
		{
			...
			#if TEST_UNMOVABLES
				test_unmovable_unlock_expired	(m_defragment_timer.get_elapsed_msec());
...
void   managed_allocator::calculate_nodes_place_pos_and_unmovable_state (u32 const sufficient_contigous_space)
{
...
void   managed_allocator::defragment (u32 const sufficient_contigous_space)
{
	threading::mutex_raii						raii(m_defragmentation_mutex);
	m_defragmenting							=	true;
//	g_resources_manager->mark_unmovables_before_defragmentation	();
...
#if TEST_UNMOVABLES
	test_unmovable_init							(m_defragment_timer.get_elapsed_msec());
#endif // #if TEST_UNMOVABLES
	...
	bool		enough_contiguous_space		=	false;
	...
	while ( cur_node )
	{
		...
		if ( cur_node->is_allocated() )
			ASSERT								(cur_node->m_owner->m_node == cur_node);
