...
VOSTOK_DECLARE_LINKAGE_ID(core_profiler)

using vostok::profiler;
using vostok::profile_result_portion;
using vostok::profile_portion;
using vostok::profile_stats;
using vostok::strings::text_tree_item;

#ifdef PROFILE_CRITICAL_SECTIONS
static vostok::threading::atomic64_type critical_section_counter = 0;
...
void add_profile_portion(shared_string id, const u64& time)
{
	...
	vostok::profile_result_portion temp;
	temp.m_timer_id = id;
	temp.m_time = time;
	vostok::get_profiler().add_profile_portion(temp);
}
#endif // PROFILE_CRITICAL_SECTIONS

namespace vostok {
	VOSTOK_CORE_API vostok::profiler* g_profiler = 0;
} // namespace vostok
...
inline void profiler::convert_string(vostok::shared_string str, vostok::shared_string& out)
{
	string256 m_temp;
	pcstr i, j = str.c_str();
	u32 count = 0;
	...
	vostok::strings::copy(m_temp, "");
	vostok::strings::join(m_temp, j);
	out = m_temp;
}

void profiler::setup_timer(vostok::shared_string timer_id, const u64& timer_time, const u32& call_count)
{
	string256 m_temp;
	float _time = float(timer_time) * 1000.f / vostok::timing::g_qpc_per_second;
	profile_stats_type::iterator i = m_stats.find(timer_id);
	if(i==m_stats.end()) 
	{
		vostok::strings::copy(m_temp, timer_id.c_str());
		pstr j, k = m_temp;
		while((j=strchr(k,'/'))!=0) 
		{
			*j = 0;
	...
	{
		profile_stats& current = (*i).second;
		current.m_min_time = vostok::math::min(current.m_min_time,_time);
		current.m_max_time = vostok::math::max(current.m_max_time,_time);
		current.m_total_time += _time;
		++current.m_count;
		current.m_call_count += call_count;
	}
...
{
#ifdef PROFILE_CRITICAL_SECTIONS
	while(vostok::threading::interlocked_exchange(critical_section_counter, 1))
...
#ifdef PROFILE_CRITICAL_SECTIONS
	vostok::threading::interlocked_exchange(critical_section_counter, 0);
#endif // PROFILE_CRITICAL_SECTIONS
}
...
{
#ifdef PROFILE_CRITICAL_SECTIONS
	if(vostok::threading::interlocked_exchange(critical_section_counter, 1))
		return;
