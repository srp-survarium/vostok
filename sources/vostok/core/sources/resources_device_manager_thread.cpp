											...
											fs_new::synchronous_device_interface const &	device)
{
	...
	if ( is_save_query )
	{
		...
		else
		{
			...
			R_ASSERT						(resource);
			VOSTOK_CONSTRUCT_REFERENCE		(resource_to_save, pinned_ptr_const<u8>)(resource);
	...
	if ( resource_to_save.initialized() )
		VOSTOK_DESTROY_REFERENCE				(resource_to_save);
...
bool   device_manager::do_async_operation (fs_new::file_type *							file, 
										   ...
										   bool											sector_aligned)
{
	VOSTOK_UNREFERENCED_PARAMETER				(sector_aligned);
