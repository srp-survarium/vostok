	...
	m_sync_device	(hdd.get_device())	
{
	...
	if ( enable_fs_watcher )
		VOSTOK_CONSTRUCT_REFERENCE			(m_watcher, watcher)(& m_vfs);
#else
	VOSTOK_UNREFERENCED_PARAMETERS( enable_fs_watcher );
#endif

	VOSTOK_CONSTRUCT_REFERENCE				(m_compressor, ppmd_compressor) (&memory::g_cook_allocator, 1);
	...
	VOSTOK_CONSTRUCT_REFERENCE				(s_hdd_device, hdd_manager)(512);
...
resources_manager::~resources_manager ()
{
	...
	VOSTOK_DESTROY_REFERENCE					(s_hdd_device);

	threading::tls_delete_key				(m_tls_key_thread_local_data);

	VOSTOK_DESTROY_REFERENCE					(m_compressor);

#if VOSTOK_FS_NEW_WATCHER_ENABLED
	if ( m_watcher.initialized() )
		VOSTOK_DESTROY_REFERENCE				(m_watcher);
#endif
}
...
void   resources_manager::on_mounted (vfs::base_node<> * const node)
{
	...
	vfs_sub_fat_resource * const sub_fat	=	VOSTOK_NEW_IMPL(helper_allocator(), vfs_sub_fat_resource);
