...
using vostok::memory::stack_arena_allocator;
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( arena_id, arena_size );
...
pvoid stack_arena_allocator::malloc_impl	( size_t const raw_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
...
//	printf						( "allocating %d block %d\n", size, u32( res ) );
	...
	return						on_malloc( result, raw_size, 0, VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_PARAMETER );
}

pvoid stack_arena_allocator::realloc_impl	( pvoid const pointer, size_t const new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	ASSERT						( initialized ( ) );

	pvoid new_ptr			=	malloc_impl ( new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION VOSTOK_CORE_DEBUG_PARAMETERS );
...
void stack_arena_allocator::free_impl		( pvoid block VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	VOSTOK_CORE_DEBUG_PARAMETERS_UNREFERENCED_GUARD;
...
pvoid stack_arena_allocator::call_malloc	( size_t size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return							( malloc_impl( size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION VOSTOK_CORE_DEBUG_PARAMETERS ) );
}

pvoid stack_arena_allocator::call_realloc	( pvoid pointer, size_t new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return							( realloc_impl( pointer, new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION VOSTOK_CORE_DEBUG_PARAMETERS ) );
}

void stack_arena_allocator::call_free		( pvoid pointer VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	free_impl						( pointer VOSTOK_CORE_DEBUG_PARAMETERS );
}
