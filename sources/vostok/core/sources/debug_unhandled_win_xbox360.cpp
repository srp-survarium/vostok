...
static vostok::core::debug::error_mode			s_error_mode = vostok::core::debug::error_mode_verbose;
...
void vostok::core::debug::detail::generate_file_name					(
...
{
	vostok::core::debug::platform::format_message	( );
	...
	vostok::core::debug::platform::prologue_dump_call_stack	( exception_information );
	vostok::core::debug::detail::generate_file_name			( temp, date_time, vostok::core::application_name(), "error_report", ".log");

	string_path					log_file_name;
	vostok::strings::copy			( log_file_name, temp );

	string_path					portable_log_file_name;
	vostok::strings::copy			( portable_log_file_name, log_file_name );
	vostok::logging::flush		( vostok::fs::convert_to_portable_in_place(portable_log_file_name) );

	vostok::core::debug::bugtrap::add_file		( log_file_name );
	vostok::core::debug::platform::save_minidump	( date_time, exception_information );
}

static void epilogue											( _EXCEPTION_POINTERS* const exception_information )
{
	vostok::core::debug::call_stack::finalize_symbols	( );

	if ( s_previous_handler )
		s_previous_handler		( exception_information );

	vostok::core::debug::platform::terminate	( "", 2 );
}
...
#endif // #if !defined(NDEBUG) || !VOSTOK_PLATFORM_XBOX_360
}

static LONG WINAPI unhandled_exception_handler					( _EXCEPTION_POINTERS* const exception_information )
{
	if ( !vostok::core::initialized( ) )
	{
		vostok::core::debug::platform::prologue_dump_call_stack	( exception_information );
		vostok::debug::terminate	( "" );
	}

	if ( !vostok::core::debug::bugtrap::initialized( ) )
		vostok::core::debug::bugtrap::initialize	( );
...
int   vostok::debug::unhandled_exception_filter	( int const exception_code, _EXCEPTION_POINTERS* const exception_information )
{
#ifdef DEBUG
...
#				undef PREFIX
	...
	if ( vostok::testing::is_testing() ) {
		vostok::testing::on_exception	(vostok::assert_untyped, description, exception_information, false);
	...
	if ( show_dialog_for_unhandled_exceptions( ) )
	{
		...
		vostok::core::debug::platform::on_error	(NULL, buffer, NULL, exception_information, vostok::core::debug::platform::error_type_unhandled_exception);
	}
...
void vostok::debug::protected_call							( protected_function_type* function_to_call, pvoid argument )
{
...
void vostok::core::debug::platform::setup_unhandled_exception_handler	( error_mode const error_mode )
{
	s_error_mode				= error_mode;
	s_previous_handler			= SetUnhandledExceptionFilter( &unhandled_exception_handler );
}

void vostok::core::debug::platform::change_unhandled_exception_handler ( error_mode const error_mode )
{
	s_error_mode				= error_mode;
}

void vostok::core::debug::bugtrap::setup_unhandled_exception_handler	( )
{
