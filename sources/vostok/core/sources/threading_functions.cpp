...
static vostok::command_line::key		s_fpe_disabled("fpe_disabled", "", "math", "turns off fpu exceptions");

void on_thread_spawn		( tasks_awareness const tasks_awareness )
{
#if defined(DEBUG) && VOSTOK_PLATFORM_32_BIT
	vostok::debug::enable_fpe				( !s_fpe_disabled && !command_line::key_is_set("editor") );
#endif // #if defined(DEBUG) && VOSTOK_PLATFORM_32_BIT

	vostok::math::on_thread_spawn			( );

	if ( tasks_awareness == threading::tasks_aware )
		tasks::on_thread_spawn			( );

#ifndef MASTER_GOLD
	vostok::threading::increase_thread_priority	( );
...
void thread_entry				( pvoid argument )
{
	...
	VOSTOK_MEMORY_BARRIER_EXCEPT_READS_BEFORE_WRITES	( );
	threading::interlocked_exchange			( params_raw->processed, 1 );

	set_current_thread_affinity				( params.hardware_thread );

	vostok::threading::on_thread_spawn		( params.tasks_awareness );
...
{
	VOSTOK_UNREFERENCED_PARAMETER			( name_for_debugger );
