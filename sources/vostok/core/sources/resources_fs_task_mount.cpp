...
void   fs_task_mount::execute_may_destroy_this	()
{
	...
	#pragma message							(VOSTOK_TODO("temporary solution. Remember to make it well"))
	...
	if ( mount_result.mount )
	{
		...
		fs_task_unmount * const unmount_task	=	VOSTOK_NEW_IMPL(helper_allocator(), fs_task_unmount)
													(sub_fat_resource);
