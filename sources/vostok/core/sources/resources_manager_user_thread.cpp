...
queries_result *   resources_manager::create_queries_result (query_resource_params const & params)
{
	...
	queries_result * const queries		=	(queries_result *)
											VOSTOK_ALLOC_IMPL(params.allocator, 
															char, 
...
#pragma message(VOSTOK_TODO("Lain 2 Lain: revisit to check transactions are working in presence of fs iterator requests"))
	...
	void   on_fs_iterator_ready (vfs::vfs_locked_iterator const & iterator )
	{
		if ( iterator.is_end() )
		{
			...
			queries_result*	queries		=	(queries_result*)
											VOSTOK_ALLOC_IMPL(*m_params.allocator, queries_result, 1);
			...
			VOSTOK_FREE_IMPL					(m_params.allocator, queries);
			return;
		}
		...
		fs_new::virtual_path_string * requests_pathes_ptr	=	VOSTOK_ALLOC_IMPL(m_params.allocator, 
		...
		VOSTOK_FREE_IMPL						(m_params.allocator, requests_pathes_ptr);

		this->~query_resources_helper		();
		query_resources_helper*	this_ptr	=	this;
		VOSTOK_FREE_IMPL						(m_params.allocator, this_ptr);
	}
...
void   resources_manager::query_resources_by_mask (query_resource_params const & params)
{
	...
	query_resources_helper * const helper	=	VOSTOK_ALLOC_IMPL
												(params.allocator,
												 query_resources_helper, 
													   ...
													   bool const	finalizing_thread)
{
	...
	while ( it_task )
	{
		...
		VOSTOK_DELETE_IMPL					(allocator, it_task);
...
void   resources_manager::translate_query (query_result * const query)
{
	...
	VOSTOK_UNREFERENCED_PARAMETER				(translated_query);
...
{
	VOSTOK_UNREFERENCED_PARAMETER				(finalizing_thread);
