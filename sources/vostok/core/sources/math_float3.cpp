...
void vostok::math::generate_orthonormal_basis	( float3 const& direction, float3& up, float3& right )
{
...
float3 vostok::math::normalize_precise		( float3 const& object )
{
	...
	if (aa1 > aa0) {
		...
		float3		result;
		result.x	= float(a0*l);
		result.y	= float(VOSTOK_COPY_SIGN(l,a1));
		result.z	= float(a2*l);
		return		result;
	}
...
aa2_largest:	// aa2 is largest
		...
		result.x	= float(a0*l);
		result.y	= float(a1*l);
		result.z	= float(VOSTOK_COPY_SIGN(l,a2));
		return		result;
	}
	...
	float3			result;
	result.x		= float(VOSTOK_COPY_SIGN(l,a0));
	result.y		= float(a1*l);
	result.z		= float(a2*l);
	return			result;
}
