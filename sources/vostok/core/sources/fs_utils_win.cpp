...
#include "pch.h"
#include <vostok/fs_path.h>						// for fs::convert_to_native
...
static pcstr get_current_directory	( )
{
	...
	vostok::fs::convert_to_native_in_place( s_current_directory );
	s_initialized					= true;
	return							s_current_directory;
}
...
#pragma message(VOSTOK_TODO("Lain - revisit this after merge with Andy: get_current_directory() should not end with /"))
	out_absolute.rtrim				('\\');
	out_absolute.rtrim				('/');
