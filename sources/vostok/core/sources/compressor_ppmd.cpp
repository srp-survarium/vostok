...
void _STDCALL PrintInfo		(_PPMD_FILE* DecodedFile,_PPMD_FILE* EncodedFile)
{
	VOSTOK_UNREFERENCED_PARAMETERS	(DecodedFile, EncodedFile);
}
...
public:
	...
	typedef vostok::ppmd_compressor::model_restoration_enum	MR_METHOD;

							ppmd_compressor_impl (vostok::memory::base_allocator* allocator, 
												  u32 sub_allocator_size,
												  MR_METHOD mr_method);

	virtual void			compress		(vostok::const_buffer src, vostok::mutable_buffer dest, u32& out_size);
	virtual void			decompress		(vostok::const_buffer src, vostok::mutable_buffer dest, u32& out_size);
								  ...
								  model_restoration_enum const		model_restoration)
{
	...
	m_impl				=	VOSTOK_NEW_IMPL(m_allocator, ppmd_compressor_impl) 
...
{
	VOSTOK_DELETE_IMPL		(m_allocator, m_impl);
}
...
void   ppmd_compressor_impl::compress (vostok::const_buffer src, vostok::mutable_buffer dest, u32& out_size)
{
...
void   ppmd_compressor_impl::decompress (vostok::const_buffer src, vostok::mutable_buffer dest, u32& out_size)
{
	typedef		compression::ppmd::stream	stream;
...
ppmd_compressor_impl::ppmd_compressor_impl (vostok::memory::base_allocator* const allocator, 
...
void   ppmd_compressor_impl::RestoreModelRare (PPM_CONTEXT* pc1,PPM_CONTEXT* MinContext, PPM_CONTEXT* FSuccessor)
{
	...
	if (MRMethod > vostok::ppmd_compressor::model_restoration_freeze ) {
        MaxContext=FSuccessor;              m_allocator.GlueCount += !( m_allocator.BList[1].Stamp & 1);
    } else if (MRMethod == vostok::ppmd_compressor::model_restoration_freeze ) {
	...
	} else if (MRMethod == vostok::ppmd_compressor::model_restoration_restart || m_allocator.GetUsedMemory() < (m_allocator.SubAllocatorSize >> 1)) {
...
void   PPM_CONTEXT::rescale (ppmd_compressor_impl* impl)
{
    ...
    p->Freq += 4;                           SummFreq += 4;
    EscFreq=SummFreq-p->Freq;
    Adder=(impl->OrderFall != 0 || impl->MRMethod > vostok::ppmd_compressor::model_restoration_freeze);
