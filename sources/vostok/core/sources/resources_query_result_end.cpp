...
static vostok::command_line::key s_assert_on_cook_failure( "assert_on_cook_failure", "", "debug", "", "" );
...
{	
	...
	if ( m_parent )
		m_parent->on_child_query_end	(this, get_error_type() == error_type_unset && m_create_resource_result != cook_base::result_error);
}
...
{
	VOSTOK_UNREFERENCED_PARAMETER			(refered);
...
void   query_result_for_cook::finish_query (cook_base::result_enum result, bool const assert_on_cook_failure)
{
	if ( result == cook_base::result_error )
	{
		if ( m_parent->assert_on_file_not_found() )
			LOGI_WARNING					("resources", "cook of '%s' failed (class id: %d)", 
											 get_requested_path(),
											 m_class_id);
	...
	if ( result == cook_base::result_out_of_memory )
	{
		...
		this_ptr->m_out_of_memory_type	=	this_ptr->is_translate_query() ? 
			query_result::out_of_memory_on_translate_query : query_result::out_of_memory_on_create_resource;

		set_error_type						( error_type_out_of_memory );

		if ( g_resources_manager->get_out_of_memory_callback() )
		{
			this_ptr->set_create_resource_result			(result);
			g_resources_manager->push_out_of_memory_query	(this_ptr);
			return;
		}
		else
...
query_result::~query_result	()
{
	...
	if ( has_flag(flag_should_free_request_path) )
	{
		...
		VOSTOK_FREE_IMPL						(* m_user_allocator, m_request_path);
