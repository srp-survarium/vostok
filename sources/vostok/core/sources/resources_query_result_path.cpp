...
void   query_result::translate_request_path ()
{
	...
	{
		m_request_path					=	(pstr)VOSTOK_ALLOC_IMPL(* m_user_allocator, char, new_path.length()+1);
...
{
	VOSTOK_UNREFERENCED_PARAMETER			(assert_on_fail);
	...
	if ( is_replication_type() )
		{;}
#pragma message(VOSTOK_TODO("when implementing replication, make here a case"))
