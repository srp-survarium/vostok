...
pvoid allocate_region							( u64 const size, pvoid const address, u32 const additional_flags, bool assert_on_failure = true )
{
	...
	if ( result && s_fill_arenas_with_garbage.is_set() )
		vostok::memory::fill32( result, (size_t)size, vostok::memory::uninitialized_value<u32>(), (size_t)size/sizeof(u32) );
...
void vostok::memory::platform::free_region		( pvoid buffer, u64 const buffer_size )
{
	...
	)
{
	...
	total_size					= vostok::math::align_up( total_size, (u64)allocation_granularity( ) );
#if VOSTOK_PLATFORM_WINDOWS_32
	total_size					= vostok::math::min( total_size, (u64)4*Gb - (u64)allocation_granularity( ) );
	...
	if ( s_fill_arenas_with_garbage.is_set() )
		vostok::memory::fill32	( arena, (size_t)pure_total_size, vostok::memory::uninitialized_value<u32>(), (size_t)pure_total_size/sizeof(u32) );
#endif // #ifndef MASTER_GOLD
...
void iterate_regions		( u32 const start_address, u32 const allocation_granularity, u64 const min_buffer_size, P& predicate )
{
...
#elif VOSTOK_PLATFORM_64_BIT // #if VOSTOK_PLATFORM_WINDOWS_32
	...
	for ( ; i < max_address; ) {
		...
		if ( memory_info.RegionSize < min_buffer_size ) {
			i					+= vostok::math::align_up( memory_info.RegionSize, (SIZE_T)allocation_granularity );
			continue;
		}
...
static u64 select_best_region	(
		...
		u64 largest_but_one,
		u64 largest,
		vostok::memory::platform::regions_type& resource_arenas,
		u64 const allocation_granularity
	)
{
	vostok::memory::platform::region& minimum	= resource_arenas.front();
	vostok::memory::platform::region& maximum	= resource_arenas.back();
	...
	u64 const test_size			= vostok::math::align_up( (u64)(largest_but_one*share), allocation_granularity );
	if ( test_size > largest )
		return					largest + vostok::math::align_up( (u64)(largest*share), allocation_granularity );

	return						largest_but_one + test_size;
}

static vostok::memory::platform::regions_type::iterator& select_best_region(
		u64 const largest_but_two_size,
		vostok::memory::platform::regions_type::iterator& largest_but_one,
		vostok::memory::platform::regions_type::iterator& largest,
		u64 const size,
		vostok::memory::platform::regions_type& resource_arenas,
...
struct regions_count {
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS	( size, address );
		++m_region_count;
	}
...
struct regions_filler : private boost::noncopyable {
	...
	inline	void	operator( )	( u64 const size, pvoid const address )
	{
		vostok::memory::platform::region const value = { size, address, 0 };
	...
	)
{
	...
	if ( (*i).size >= resource_arenas_size ) {
		...
		if ( !maximum.address ) {
			vostok::memory::platform::free_region	( minimum.address, minimum.size );
			minimum.address		= 0;
			return				false;
		}
	...
	if ( (regions.size() == 1) || ((*i).size*share >= (*j).size) ) {
		minimum.size			= vostok::math::align_down( (u64)((*i).size*share), (u64)allocation_granularity );
	...
	if ( ((*i).size + (*j).size) >= resource_arenas_size ) {
		if ( (*i).size > maximum.size ) {
			...
			maximum.size		= vostok::math::align_down( resource_arenas_size - (*j).size, (u64)allocation_granularity );
			maximum.address		= allocate_region( maximum.size, (*i).address, 0, false );
			if ( !maximum.address ) {
				vostok::memory::platform::free_region	( minimum.address, minimum.size );
				minimum.address	= 0;
		...
		minimum.size			= vostok::math::align_down( resource_arenas_size - (*i).size, (u64)allocation_granularity );
		minimum.address			= allocate_region( minimum.size, (*j).address, 0, false );
		if ( !minimum.address )
			return				false;
...
static u64 get_minimum_kernel_memory	( )
{
	...
	vostok::memory::zero			( &os_version_info, sizeof(os_version_info) );
	...
	)
{
	...
	u64 useful_memory			= vostok::math::min( stats.total_available_memory, stats.physical_memory);
	...
	useful_memory				= vostok::math::min( useful_memory, address_space );
	...
	if ( s_max_resources_size.is_set_as_number( &maximum_resources_size ) ) {
		...
		maximum_resources_size	= vostok::math::max( maximum_resources_size, minimum_resources_size );
	}
	useful_memory				= vostok::math::min( useful_memory, maximum_resources_size );
...
{
	size_to_reduce				= vostok::math::align_up( size_to_reduce, granularity );
	...
	u64 const new_managed_size	= vostok::math::align_up( u64( managed_to_unmanaged_share/(managed_to_unmanaged_share+1.f)*new_size ), granularity );
	...
	if ( new_managed_size <= previous_size ) {
		managed_arena.size		= new_managed_size;
		vostok::memory::platform::free_region	( managed_arena.address, previous_size );
	...
	if ( new_unmanaged_size <= previous_size ) {
		unmanaged_arena.size	= new_unmanaged_size;
		vostok::memory::platform::free_region	( unmanaged_arena.address, previous_size );
...
void vostok::memory::platform::allocate_arenas	(
	...
	)
{
	...
	vostok::platform::unload_delay_loaded_library	( "psapi.dll" );
	...
	for (;;) {
		...
		stats.video_memory		= minimal_local_video_memory_size;
		string4096				temp;
		vostok::sprintf			( temp, "Assuming minimum local video memory size: %I64d", stats.video_memory );
		MessageBox				( 0, temp, VOSTOK_ENGINE_ID, MB_ICONINFORMATION );
		break;
	}
...
void vostok::memory::on_after_memory_initialized	( )
{
#ifndef _DLL
...
void vostok::memory::register_debug_allocator	( u64 const maximum_memory_size )
{
	...
	vostok::debug::g_mt_allocator.do_register( memory_size, "debug multithreaded allocator" );
}
