...
{
	VOSTOK_UNREFERENCED_PARAMETER	(message_initiator);
...
pcstr script_engine_wrapper::get_file_name	( int const file_type, pcstr file_name, pstr const result, u32 const result_size, bool add_extension )
{
	...
	vostok::buffer_string dest(result, result_size);
	dest					= folder;
	dest					+= file_name;
	dest					+= extension;
...
{
	VOSTOK_UNREFERENCED_PARAMETERS(file_type, file_name, file_buffer, buffer_size);
