...
{
	vostok::debug::protected_call		( &thread_entry, argument );
	return							0;
}
	...
	)
{
	...
	if ( !handle ) {
		...
		pstr error_message		=	vostok::debug::platform::fill_format_message	( error_code );
		LOGI_ERROR					( "debug", "CreateThread failed with error_code %d: %s", error_code, error_message );
		vostok::debug::platform::free_format_message	( error_message );
	}
...
#pragma message(VOSTOK_TODO("Lain 2 Lain: return back"))
