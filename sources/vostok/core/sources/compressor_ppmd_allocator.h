#ifndef VOSTOK_PPMD_ALLOCATOR_H_INCLUDED
#define VOSTOK_PPMD_ALLOCATOR_H_INCLUDED
...
{
public:
	ppmd_allocator(vostok::memory::base_allocator* const allocator, u32 SASize) : m_allocator(allocator)
	{
		SubAllocatorSize				=	0;
...
void _STDCALL   ppmd_allocator::StopSubAllocator () 
{
    if ( SubAllocatorSize ) 
	{
		...
		VOSTOK_FREE_IMPL						(m_allocator, HeapStart);
    }
}
...
BOOL _STDCALL   ppmd_allocator::StartSubAllocator (UINT SASize)
{
    ...
    StopSubAllocator						();
	HeapStart							=	VOSTOK_ALLOC_IMPL(m_allocator, BYTE, t);
