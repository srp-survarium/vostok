...
bool   vostok::build::print_build_id_command_line ()
{
	static u32 s_out_result						=	u32(-1);
	if ( s_out_result == u32(-1) )
	{
		if ( vostok::command_line::initialized() )
			s_out_result						=	s_print_build_id;
		else
			s_out_result						=	vostok::command_line::key_is_set(s_print_build_id_string);
	}
...
static u32 build_id				( u32 const day, u32 const month, u32 const year, pcstr current_date )
{
	...
	string256					buffer;
	vostok::strings::copy			( buffer, current_date );
	VOSTOK_SSCANF					( buffer, "%s %d %d", VOSTOK_SCANF_STRING(month_string, sizeof(month_string)), &current.day, &current.year );
	...
	current.month				= 0;
	for ( int i=0; i<12; i++ ) {
		if ( vostok::strings::compare_insensitive( month_id[ i ], month_string ) )
			continue;
