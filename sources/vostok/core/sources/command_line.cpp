...
u32	build_station_build_id	( )
{
	...
	initialized							= true;
	if ( VOSTOK_SSCANF(vostok::command_line::s_finger_print, "Build %d", &build_station_build_id) != 1 ) {
		build_station_build_id			= vostok::build::calculate_build_id( vostok::build::build_date() );
	}
