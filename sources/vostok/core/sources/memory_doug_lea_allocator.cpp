...
using vostok::memory::doug_lea_allocator;
using vostok::memory::thread_id_const_bool;
...
{
	VOSTOK_UNREFERENCED_PARAMETER	( parameter );
...
{
	VOSTOK_UNREFERENCED_PARAMETER	( arena_id );
...
inline bool doug_lea_allocator::initialized	( ) const
{
	...
	u32 const current_thread_id	= vostok::threading::current_thread_id ( );
	VOSTOK_UNREFERENCED_PARAMETER	( current_thread_id );
	R_ASSERT_CMP				(
		current_thread_id, ==, m_user_thread_id,
		"thread[%s] calls allocator for thread[%s]",
		vostok::threading::current_thread_logging_name( ),
		m_user_thread_logging_name
	);
	return						( true );
}

pvoid doug_lea_allocator::malloc_impl	( size_t const size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	if ( m_out_of_memory )
		return					0;
	...
	pvoid const result			= vostok_mspace_malloc( m_arena, real_size );
	if ( !result )
	{
		if ( !m_return_null_after_out_of_memory )
			LOG_ERROR			( "out of memory!!!! (%s)", arena_id() ? arena_id() : "" );
	}
	m_out_of_memory				= m_return_null_after_out_of_memory & !result;
	return						( on_malloc	( result, size, 0, VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_PARAMETER ) );
}

pvoid doug_lea_allocator::realloc_impl	( pvoid pointer, size_t const new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	...
	pvoid const result			= vostok_mspace_realloc( m_arena, pointer, real_size );
	return
		on_malloc(
			result,
			new_size,
...
void doug_lea_allocator::free_impl		( pvoid pointer VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	VOSTOK_CORE_DEBUG_PARAMETERS_UNREFERENCED_GUARD;
...
pvoid doug_lea_allocator::call_malloc	( size_t size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return						( malloc_impl( size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION VOSTOK_CORE_DEBUG_PARAMETERS ) );
}

pvoid doug_lea_allocator::call_realloc	( pvoid pointer, size_t new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION_DECLARATION VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	return						( realloc_impl( pointer, new_size VOSTOK_CORE_DEBUG_PARAMETERS_DESCRIPTION VOSTOK_CORE_DEBUG_PARAMETERS ) );
}

void doug_lea_allocator::call_free		( pvoid pointer VOSTOK_CORE_DEBUG_PARAMETERS_DECLARATION )
{
	free_impl					( pointer VOSTOK_CORE_DEBUG_PARAMETERS );
}
...
size_t doug_lea_allocator::total_size	( ) const
{
	...
	ASSERT						( initialized ( ) );
	size_t const result			= vostok_mspace_mallinfo( m_arena ).usmblks;
...
size_t doug_lea_allocator::allocated_size( ) const
{
	...
	ASSERT						( initialized ( ) );
	u32 result					= (u32)vostok_mspace_mallinfo( m_arena ).uordblks;
