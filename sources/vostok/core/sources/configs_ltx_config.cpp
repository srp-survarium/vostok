...
typedef	vostok::vectora<binary_config_value*>	values_type;
...
static bool parse_item_value(memory::base_allocator* allocator, binary_config_value& itm, pcstr const value)
{
	...
	case t_string:
	{
		...
		itm.data			= (pbyte)VOSTOK_ALLOC_IMPL(allocator, char, length);
	...
	case t_float2:
	{
		itm.data			= (pbyte)VOSTOK_ALLOC_IMPL( *allocator, float2, 1);
		float2* val			= (float2*)(static_cast<void const*>(itm.data));
		val->set			(0.0f, 0.0f);
		VOSTOK_SSCANF			(value, "%f,%f", &val->x, &val->y);
		itm.count			= sizeof(float2);
		break;
	}
	case t_float3:
	{
		itm.data			= (pbyte)VOSTOK_ALLOC_IMPL( *allocator, float3, 1);
		float3* val			= (float3*)(static_cast<void const*>(itm.data));
		val->set			(0.0f, 0.0f, 0.0f);
		VOSTOK_SSCANF			(value, "%f,%f,%f", &val->x, &val->y, &val->z);
...
static void parse_item_name(memory::base_allocator* allocator, binary_config_value& itm, pcstr id)
{
	if(id)
	{
		...
		itm.id					= VOSTOK_ALLOC_IMPL( *allocator, char, str_len);
...
static void save_bin(memory::base_allocator* allocator, pbyte* dest, u32& dest_size, values_type const& data, binary_config_value* root)
{
	...
	pbyte plain_data_begin	= (pbyte)VOSTOK_MALLOC_IMPL(*allocator,data_mem_size, "ltx_plain_data");
	...
	*dest					= (pbyte)VOSTOK_MALLOC_IMPL(*allocator,out_ptr_size, "ltx_plain_data");
	pbyte out_ptr			= *dest;
	...
	VOSTOK_UNREFERENCED_PARAMETER	( sz_written );
...
//	out_ptr_size				-= data_mem_size;
	...
	VOSTOK_FREE_IMPL				(*allocator,plain_data_begin);
}

void vostok::core::configs::parse_string_data(memory::reader F, memory::base_allocator* allocator, pbyte* dest, u32& dest_size)
{
	values_type				load_data(*allocator);
	binary_config_value* root				= VOSTOK_NEW_IMPL( *allocator, binary_config_value)();
	...
	while (!F.eof())
	{
		...
		if (str[0] && (str[0]=='[')) //new section ?
		{
			...
			*strchr(str,']') 	= 0;
			vostok::strlwr			(str+1, sizeof(str)-1);

			Current					= VOSTOK_NEW_IMPL( *allocator, binary_config_value)();
			Current->type			= t_max;
		...
		else // id = value
		{
			if (Current)
			{
				...
				binary_config_value* value_itm		= VOSTOK_NEW_IMPL( *allocator, binary_config_value)();
	...
	for(; it!=it_e; ++it)
	{
		...
		if(t->id)
		{
			...
			VOSTOK_FREE_IMPL( *allocator, p);
		}
		if(t->type>t_embedded_types)
			VOSTOK_FREE_IMPL( *allocator, const_cast<pvoid&>((pcvoid&)(t->data)));
		
		VOSTOK_DELETE_IMPL( *allocator, t);
	}
}

vostok::core::configs::binary_config* vostok::core::configs::create_from_ltx_text(pcstr text, memory::base_allocator* allocator, pcstr fn)
{
	vostok::memory::reader R		((u8 const*)text, vostok::strings::length(text));
	pbyte						binary_buff = NULL;
	u32							binary_buff_size = 0;

	vostok::core::configs::parse_string_data(R, allocator, &binary_buff, binary_buff_size);
	...
	vostok::core::configs::binary_config*	result	= VOSTOK_NEW_IMPL(*allocator, vostok::core::configs::binary_config) (binary_buff, binary_buff_size, allocator);
	VOSTOK_FREE_IMPL	(*allocator, binary_buff);
	return			result;
}