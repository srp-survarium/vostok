...
{
	fs_task_composite *	const new_task	=	VOSTOK_NEW_IMPL(* allocator, fs_task_composite) (allocator);
...
void   query_mount 						(pcstr const					mount_id,
										 ...
										 memory::base_allocator * const	allocator)
{
	VOSTOK_NEW_IMPL							(allocator, mount_by_config_helper)
...
{
	fs_task_mount * const new_task		=	VOSTOK_NEW_IMPL(* allocator, fs_task_mount)
												(virtual_path,
												 physical_path,
												 descriptor,
												 watcher_enabled,
