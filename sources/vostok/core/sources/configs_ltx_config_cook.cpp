													 ...
													 bool								file_exist)
{
	...
	return									mutable_buffer (VOSTOK_MALLOC_IMPL(vostok::debug::g_mt_allocator, sizeof(vostok::configs::binary_config), "lua"), 
															sizeof(vostok::configs::binary_config));
}
...
{
	VOSTOK_FREE_IMPL					(vostok::debug::g_mt_allocator, buffer);
}
...
{
	vostok::memory::reader reader		((u8 const*)raw_file_data.c_ptr(), raw_file_data.size());

	pbyte	binary_buff				= NULL;
	u32		binary_buff_size		= 0;

	parse_string_data				(reader, & vostok::debug::g_mt_allocator, & binary_buff, binary_buff_size);

	binary_config * const config	= new (in_out_unmanaged_resource_buffer.c_ptr()) binary_config 
									(binary_buff, binary_buff_size, & vostok::debug::g_mt_allocator);
