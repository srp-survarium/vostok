...
VOSTOK_ALIGN(VOSTOK_MAX_CACHE_LINE_SIZE)
...
static VOSTOK_DEFAULT_ALIGN char			s_task_type_list_buffer				[sizeof(task_type_list_container)];
...
struct inplace_constructor
{
	...
	{
		new (pointer) tasks_types_allocator( /*VOSTOK_MAX_CACHE_LINE_SIZE*/ );
		pointer->initialize	( s_task_types_arena, sizeof(s_task_types_arena), "tasks type");
	}
};

task_type *   create_new_task_type (pcstr description, enum_flags<enum vostok::tasks::task_type_flags_enum> flags)
{
...
//	if ( task_type* new_type = VOSTOK_NEW_IMPL( s_task_types_allocator, task_type )( description, flags ) )
	...
	task_type* new_type					= VOSTOK_NEW_IMPL( s_task_types_allocator, task_type )( description, flags );
	if ( new_type )
	{
