...
struct malloc_arena {
  ...
  /* Space for mstate.  The size is just the minimum such that
     create_vostok_mspace_with_base can be successfully called.  */
...
_int_new_arena( size_t size)
{
  ...
  m = create_vostok_mspace_with_base((char*)a + MSPACE_OFFSET,
...
malloc_starter( size_t sz, const void *caller)
{
  ...
  victim = vostok_mspace_malloc(arena_to_mspace(&main_arena), sz);
...
memalign_starter( size_t align, size_t sz, const void *caller)
{
  ...
  /*ptmalloc_init_minimal();*/
  victim = vostok_mspace_memalign(arena_to_mspace(&main_arena), align, sz);
  THREAD_STAT(++main_arena.stat_starter);
...
//    return vostok_mspace_malloc(arena_to_mspace(&main_arena), sz);
//  } else {
...
//  vostok_mspace_free(arena_to_mspace(ar_ptr), mem);
//  if(vptr != ATFORK_ARENA_PTR)
...
# endif /* USE_STARTER & 1 */
  ...
  mspace = create_vostok_mspace_with_base((char*)&main_arena + MSPACE_OFFSET,
...
public_mALLOc(size_t bytes)
{
  ...
  victim = vostok_mspace_malloc(arena_to_mspace(ar_ptr), bytes);
...
public_rEALLOc(void* oldmem, size_t bytes)
{
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  newp = vostok_mspace_realloc(arena_to_mspace(ar_ptr), oldmem, bytes);
...
public_mEMALIGn( size_t alignment, size_t bytes)
{
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  p = vostok_mspace_memalign(arena_to_mspace(ar_ptr), alignment, bytes);
...
public_vALLOc( size_t bytes)
{
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  p = vostok_mspace_memalign(arena_to_mspace(ar_ptr), 4096, bytes);
...
#define HALF_INTERNAL_SIZE_T \
  ...
  if (ar_ptr != &main_arena)
    bytes += FOOTER_OVERHEAD;
  mem = vostok_mspace_calloc(arena_to_mspace(ar_ptr), bytes, 1);
...
public_iCALLOc( size_t n, size_t elem_size, void* chunks[])
{
  ...
  m = vostok_mspace_independent_calloc(arena_to_mspace(ar_ptr), n, elem_size, chunks);
...
public_iCOMALLOc( size_t n, size_t sizes[], void* chunks[])
{
  ...
  if (ar_ptr != &main_arena) {
    ...
    m_sizes = vostok_mspace_malloc(arena_to_mspace(ar_ptr), n*sizeof(size_t));
    if (!m_sizes) {
      (void)mutex_unlock(&ar_ptr->mutex);
      return 0;
    }
    ...
    if (!chunks) {
      chunks = vostok_mspace_malloc(arena_to_mspace(ar_ptr),
			     n*sizeof(void*)+FOOTER_OVERHEAD);
      if (!chunks) {
	vostok_mspace_free(arena_to_mspace(ar_ptr), m_sizes);
	(void)mutex_unlock(&ar_ptr->mutex);
	return 0;
      }
  ...
  m = vostok_mspace_independent_comalloc(arena_to_mspace(ar_ptr), n, m_sizes, chunks);
...
public_mTRIm( size_t s)
{
  ...
  (void)mutex_lock(&main_arena.mutex);
  result = vostok_mspace_trim(arena_to_mspace(&main_arena), s);
...
{
  int result;
  result = vostok_mspace_mallopt(p, v);
  return result;
}
...
public_mALLINFo(  )
{
  ...
  for (i=0, ar_ptr = &main_arena;; ++i) {
	result.uordblks += vostok_mspace_mallinfo(arena_to_mspace(ar_ptr)).uordblks;
    ar_ptr = ar_ptr->next;
    if (ar_ptr == &main_arena)
      break;
  }
...
public_mSTATs(  )
{
  ...
  for (i=0, ar_ptr = &main_arena;; ++i) {
    ...
    vostok_mspace_malloc_stats(msp);
#if THREAD_STATS
