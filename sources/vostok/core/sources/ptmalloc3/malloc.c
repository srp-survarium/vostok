...
* MSPACES
  ...
  this file also defines vostok_mspace_malloc, vostok_mspace_free, etc. These
  ...
  ONLY_MSPACES and then do something like...
    ...
    #define mymalloc(bytes)  vostok_mspace_malloc(mymspace, bytes)
  ...
  mspaces as thread-locals. For example:
    ...
    void*  tlmalloc(size_t bytes) {
      ...
      return vostok_mspace_malloc(tlms, bytes);
    }
    void  tlfree(void* mem) { vostok_mspace_free(tlms, mem); }
...
/*
  ...
  region of space that supports vostok_mspace_malloc, etc.
*/
typedef void* mspace;
...
/*
  ...
  dynamically as needed to service vostok_mspace_malloc requests.  You can
...
/*
  create_vostok_mspace_with_base uses the memory supplied as the initial base
...
*/
	...
	mspace create_vostok_mspace_with_base(void* base, unsigned long long capacity, int locked, out_of_memory_handler_type handler, out_of_memory_handler_parameter_type parameter);
#else // #ifdef USE_OUT_OF_MEMORY_HANDLER
	mspace create_vostok_mspace_with_base(void* base, unsigned long long capacity, int locked);
#endif // #ifdef USE_OUT_OF_MEMORY_HANDLER

/*
  vostok_mspace_malloc behaves as malloc, but operates within
  the given space.
*/
void* vostok_mspace_malloc(mspace msp, size_t bytes);

/*
  vostok_mspace_free behaves as free, but operates within
  the given space.

  If compiled with FOOTERS==1, vostok_mspace_free is not actually needed.
  free may be called instead of vostok_mspace_free because freed chunks from
  any space are handled by their originating spaces.
*/
void vostok_mspace_free(mspace msp, void* mem);

/*
  vostok_mspace_realloc behaves as realloc, but operates within
  the given space.

  If compiled with FOOTERS==1, vostok_mspace_realloc is not actually
  needed.  realloc may be called instead of vostok_mspace_realloc because
  realloced chunks from any space are handled by their originating
  spaces.
*/
void* vostok_mspace_realloc(mspace msp, void* mem, size_t newsize);

/*
  vostok_mspace_calloc behaves as calloc, but operates within
  the given space.
*/
void* vostok_mspace_calloc(mspace msp, size_t n_elements, size_t elem_size);

/*
  vostok_mspace_memalign behaves as memalign, but operates within
  the given space.
*/
void* vostok_mspace_memalign(mspace msp, size_t alignment, size_t bytes);

/*
  vostok_mspace_independent_calloc behaves as independent_calloc, but
  operates within the given space.
*/
void** vostok_mspace_independent_calloc(mspace msp, size_t n_elements,
                                 size_t elem_size, void* chunks[]);

/*
  vostok_mspace_independent_comalloc behaves as independent_comalloc, but
  operates within the given space.
*/
void** vostok_mspace_independent_comalloc(mspace msp, size_t n_elements,
                                   size_t sizes[], void* chunks[]);

/*
  vostok_mspace_footprint() returns the number of bytes obtained from the
  system for this space.
*/
size_t vostok_mspace_footprint(mspace msp);

/*
  vostok_mspace_max_footprint() returns the peak number of bytes obtained from the
  system for this space.
*/
size_t vostok_mspace_max_footprint(mspace msp);


#if !NO_MALLINFO
/*
  vostok_mspace_mallinfo behaves as mallinfo, but reports properties of
  the given space.
*/
struct mallinfo vostok_mspace_mallinfo(mspace msp);
#endif /* NO_MALLINFO */

/*
  vostok_mspace_malloc_stats behaves as malloc_stats, but reports
  properties of the given space.
*/
void vostok_mspace_malloc_stats(mspace msp);

/*
  vostok_mspace_trim behaves as malloc_trim, but
  operates within the given space.
*/
int vostok_mspace_trim(mspace msp, size_t pad);

/*
  An alias for mallopt.
*/
int vostok_mspace_mallopt(int, int);
...
/* segment bit set in create_vostok_mspace_with_base */
...
/*
  ...
  * If EXTERN_BIT set, then we did not allocate this segment,
    ...
    into create_vostok_mspace_with_base.)
...
#if ONLY_MSPACES
#define internal_malloc(m, b) vostok_mspace_malloc(m, b)
#define internal_free(m, mem) vostok_mspace_free(m,mem);
#else /* ONLY_MSPACES */
#if MSPACES
#define internal_malloc(m, b)\
   (m == gm)? dlmalloc(b) : vostok_mspace_malloc(m, b)
#define internal_free(m, mem)\
   if (m == gm) dlfree(mem); else vostok_mspace_free(m,mem);
#else /* MSPACES */
...
void* vostok_mspace_realloc(mspace msp, void* oldmem, size_t bytes) {
  if (oldmem == 0)
    return vostok_mspace_malloc(msp, bytes);
#ifdef REALLOC_ZERO_BYTES_FREES
  if (bytes == 0) {
    vostok_mspace_free(msp, oldmem);
    return 0;
  }
#endif /* REALLOC_ZERO_BYTES_FREES */
  else {
...
size_t vostok_mspace_usable_size(void* mem) {
  if (mem != 0) {
    mchunkptr p = mem2chunk(mem);
    if (cinuse(p))
      return chunksize(p) - overhead_for(p);
...
int vostok_mspace_mallopt(int param_number, int value) {
  return change_mparam(param_number, value);
}
...
History:
    V2.8.4 (not yet released)
      * Fix bad error check in vostok_mspace_footprint
    ...
    V2.8.3 Thu Sep 22 11:16:32 2005  Doug Lea  (dl at gee)
      ...
      * Avoid concatenating segments with the one provided
        in create_vostok_mspace_with_base
      * Rename some variables to avoid compiler shadowing warnings
      * Use explicit lock initialization.
      * Better handling of sbrk interference.
      * Simplify and fix segment insertion, trimming and vostok_mspace_destroy
