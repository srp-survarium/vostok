...
#include <vostok/fs_path_iterator.h>
					  ...
					  async_callbacks_data *	callbacks_data)
{
	...
	VOSTOK_DELETE_IMPL								(&memory::g_resources_helper_allocator, link_results);
}
							 ...
							 async_callbacks_data *	callbacks_data)
{
	...
	find_results_struct * const link_results =	VOSTOK_NEW_IMPL(&memory::g_resources_helper_allocator, find_results_struct)
...
{
	VOSTOK_UNREFERENCED_PARAMETER					(it);
	R_ASSERT									(!find_results_to_delete->pin_iterator);
	parent_callbacks_data->on_callback_may_destroy_this	();	
	VOSTOK_DELETE_IMPL							(&memory::g_resources_helper_allocator, find_results_to_delete);
}
...
void   query_recursive_mount_of_sub_tree (fat_node<> *				sub_tree_root, 
										  ...
										  enum_flags<find_enum>		find_flags)
{
	find_results_struct * const sub_tree_results	=	VOSTOK_NEW_IMPL(& memory::g_resources_helper_allocator, find_results_struct)
																(find_results_struct::type_pin);
	
	sub_tree_results->callback			=	boost::bind(& on_recursive_tree_loaded, _1, parent_callbacks_data, sub_tree_results);
	
	async_callbacks_data * const callbacks_data	=	VOSTOK_NEW_IMPL(& memory::g_resources_helper_allocator, 
...
void   query_recursive_mount_of_tree	(fat_node<> *			tree_root, 
										 ...
										 enum_flags<find_enum>	find_flags)
{
	async_callbacks_data * const callbacks_data	=	VOSTOK_NEW_IMPL(& memory::g_resources_helper_allocator, async_callbacks_data)
																	(root_find_results);
