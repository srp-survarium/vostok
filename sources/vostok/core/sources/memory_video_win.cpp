...
{
	VOSTOK_UNREFERENCED_PARAMETER	( lpDriverDescription );
...
static u64 get_local_video_memory_size_impl	( )
{
    ...
    for( UINT iAdapter = 0; iAdapter < dwAdapterCount; iAdapter++ )
    {
		...
		if( SUCCEEDED( GetVideoMemoryViaWMI( hMonitor, &dwAdapterRAM ) ) ) {
			maximum_video_memory_size	= vostok::math::max( maximum_video_memory_size, guess_exact_memory_size(dwAdapterRAM) );
			continue;
		}

        u64 dwAvailableVidMem;
        if( SUCCEEDED( GetVideoMemoryViaDirectDraw( hMonitor, &dwAvailableVidMem ) ) )
			maximum_video_memory_size	= vostok::math::max( maximum_video_memory_size, guess_exact_memory_size(dwAvailableVidMem) );
    }
...
u64 vostok::platform::get_minimal_local_video_memory_size	( )
{
	return				256*Mb;
}