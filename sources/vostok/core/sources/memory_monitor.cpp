...
#include <vostok/fs/synchronous_device_interface.h>	// for vostok::core::current_directory
#include <vostok/core_entry_point.h>
#include <vostok/fs/path_string_utils.h>	// for vostok::core::current_directory
...
static inline bool enabled		( )
{
	...
	return						s_enabled;
#else // #if VOSTOK_USE_MEMORY_MONITOR
	return						false;
...
void initialize	( )
{
	...
	strings::copy				( output_folder, vostok::fs_new::get_current_directory().c_str() );
...
void on_alloc	( pvoid& allocation_address, size_t& allocation_size, size_t const previous_size, pcstr const allocation_description )
{
	...
	device->write				( s_file, allocation_description, ( vostok::strings::length( allocation_description ) + 1 )*sizeof( char ) );
