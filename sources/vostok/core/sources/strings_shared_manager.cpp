...
using vostok::strings::shared::manager;
using vostok::shared_string;
using vostok::strings::shared::profile;

static vostok::uninitialized_reference<manager>	s_manager;

void vostok::strings::initialize			( )
{
	VOSTOK_CONSTRUCT_REFERENCE	( s_manager, manager );
}

void vostok::strings::finalize			( )
{
	VOSTOK_DESTROY_REFERENCE		( s_manager );
}
...
void vostok::strings::shared::remove		( profile* profile )
{
...
struct remove_predicate : private boost::noncopyable {
	vostok::threading::mutex&	m_mutex;

	inline		remove_predicate		( vostok::threading::mutex&	mutex ) :
		m_mutex					( mutex )
	{
	}
