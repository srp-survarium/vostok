...
void vostok::memory::platform::free_region	( pvoid const buffer, u64 const buffer_size )
{
	...
	)
{
	...
	u64 const test_size			= vostok::math::align_up( (u64)(largest_but_one*share), resources_arena_granularity );
	if ( test_size > largest )
		return					largest + vostok::math::align_up( (u64)(largest*share), resources_arena_granularity );
		...
		for ( ; ; ) {
			...
			if ( (regions.size() == 1) || ((*i).size*share >= (*j).size) ) {
				*(u32*)(*i).data	+= (u32)(*i).size;
				minimum.size		= vostok::math::align_down( (u64)((*i).size*share), (u64)resources_arena_granularity );
			...
			u64 const test_size		= vostok::math::align_up( (u64)((*j).size*proportion), (u64)resources_arena_granularity );
			if ( test_size > (*i).size ) {
				minimum.size		= vostok::math::align_up( (u64)((*i).size*proportion), (u64)resources_arena_granularity );
				minimum.address		= (*j).address;
				*(u32*)(*j).data	+= (u32)minimum.size;
...
void vostok::memory::register_debug_allocator	( u64 const maximum_memory_size )
{
	...
	VOSTOK_UNREFERENCED_PARAMETER		( maximum_memory_size );
