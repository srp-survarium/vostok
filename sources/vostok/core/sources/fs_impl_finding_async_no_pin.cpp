									  		 ...
									  		 enum_flags<find_enum>	find_flags)
{
	...
	VOSTOK_DELETE_IMPL							(memory::g_resources_helper_allocator, link_find_results);
	VOSTOK_FREE_IMPL								(memory::g_resources_helper_allocator, allocated_link_path);
}
...
void   find_async_across_link (find_env & env)
{
	...
	find_results_struct * link_find_results	=	VOSTOK_NEW_IMPL(memory::g_resources_helper_allocator, find_results_struct)
