...
using vostok::core::script_engine_wrapper;

static vostok::uninitialized_reference<script_engine_wrapper>		s_wrapper;
static vostok::uninitialized_reference<vostok::threading::mutex>	s_mutex;

static cs::script::world *	s_world		= 0;
static vostok::core::engine *	s_engine	= 0;

static pcstr s_super_tables_ids_table_id	= "__super_tables";
static pcstr s_super_tables_table_id		= "__super_tables__";

static vostok::fs_new::native_path_string s_lua_config_device_folder_to_save_to;

static void convert_to_virtual_path	( vostok::fs_new::virtual_path_string& path, pcstr const file_name )
{
	pstr lower_case_file_name	= 0;
	STR_DUPLICATEA				( lower_case_file_name, file_name );
	vostok::strlwr				( lower_case_file_name, (vostok::strings::length(lower_case_file_name) + 1)*sizeof(char) );

	if ( !strchr(lower_case_file_name, ':') && !vostok::strings::starts_with(lower_case_file_name, "..") ) {
		path					= vostok::fs_new::virtual_path_string::convert(lower_case_file_name);
		return;
	}

	bool const result			= vostok::resources::convert_physical_to_virtual_path( &path, lower_case_file_name, NULL );
	if ( result )
		return;
...
static void get_modules_file_name			( vostok::fs_new::virtual_path_string & path, pcstr const file_name )
{
	convert_to_virtual_path		( path, vostok::fs_new::native_path_string::convert(file_name).c_str() );

#if 1
	char const prefix[]		= "resources/";
	u32 const prefix_length	= vostok::array_size( prefix ) - 1;
	if ( vostok::strings::starts_with( path.c_str(), prefix ) ) {
		pstr temp;
	...
	u32 const extension_length	= vostok::array_size( extension ) - 1;
	if ( vostok::strings::ends_with( path.c_str(), file_name_length, extension, extension_length ) )
...
public:
	...
	inline	~lua_stack_guard		( )
	{
		...
		VOSTOK_UNREFERENCED_PARAMETER	( top );
...
void vostok::configs::export_enum	( pcstr const table_id, enum_tuple const* const begin, enum_tuple const* const end )
{
...
vostok::configs::lua_config* create_lua_config	( pcstr file_name, memory::reader& reader );
...
void initialize						( vostok::core::engine& engine, pcstr lua_config_device_folder_to_save_to )
{
	...
	VOSTOK_CONSTRUCT_REFERENCE		( s_wrapper, script_engine_wrapper ) ( );
	VOSTOK_CONSTRUCT_REFERENCE		( s_mutex, threading::mutex );
...
void finalize						( )
{
	...
	VOSTOK_DESTROY_REFERENCE			( s_wrapper );
...
//	vostok::configs::lua_config* const before_lua_config	= vostok::core::configs::create_lua_config( "test_binary_config", reader );
//	before_lua_config->save_as		("e:/before.lua");
//	vostok::configs::binary_config_ptr binary_config	= vostok::configs::create_binary_config( *before_lua_config );
//	vostok::configs::lua_config_ptr const after_lua_config	= vostok::configs::create_lua_config( binary_config );
...
using vostok::configs::lua_config;

////////////////////////////////////////////////////////////////////////////
//	class vostok::configs::lua_config
////////////////////////////////////////////////////////////////////////////
...
void lua_config::save_as			( pcstr file_name, vostok::configs::save_target target, vostok::configs::lua_config_value::save_as_option const option ) const
{
	m_root.save_as					( file_name, target, option );
}

void lua_config::save				( vostok::configs::save_target target, vostok::configs::lua_config_value::save_as_option const option ) const
{
	save_as							( m_file_name, target, option );
}

void lua_config::save				( vostok::strings::stream& stream ) const
{
	string4096						indent;
	indent[0]						= 0;
...
static vostok::configs::lua_config* new_config			( pcstr file_name );
static vostok::configs::lua_config* new_config_inplace	( vostok::mutable_buffer const & lua_config_buffer, pcstr file_name );
...
vostok::configs::lua_config* create_lua_config	( pcstr file_name, memory::reader& reader )
{
	...
	vostok::fs_new::virtual_path_string		modules_file_name;
...
vostok::configs::lua_config*   create_lua_config_inplace	( mutable_buffer const & in_out_buffer, pcstr const file_name, memory::reader& reader )
{
	lua_stack_guard					guard;
...
void destroy_lua_config						( vostok::resources::unmanaged_resource* config )
{
...
using vostok::configs::lua_config_ptr;

static vostok::configs::lua_config_value get_value	(
		vostok::configs::lua_config_value& result,
		vostok::configs::binary_config_value::const_iterator const i,
		vostok::configs::binary_config_value const& value,
		u32 const index
	)
{
	if ( value.type == vostok::configs::t_table_indexed )
		return						result[index];

	R_ASSERT						( value.type == vostok::configs::t_table_named );
	return							result[ (*i).id ];
}

static void construct				( vostok::configs::lua_config_value result, vostok::configs::binary_config_value const& value )
{
	...
	for (u32 j=0; i != e; ++i, ++j ) {
		switch ( (*i).type ) {
			...
			case t_float2 : {
				get_value( result, i, value, j)	= (vostok::math::float2 const&)*i;
				break;
			}
			case t_float3 : {
				get_value( result, i, value, j)	= (vostok::math::float3 const&)*i;
				break;
			}
			case t_float4 : {
				get_value( result, i, value, j)	= (vostok::math::float4 const&)*i;
				break;
			}
			default : NODEFAULT();
		}
...
vostok::configs::lua_config_ptr vostok::configs::create_lua_config	( binary_config_ptr const& config )
{
...
vostok::configs::lua_config_value vostok::configs::create_lua_config_value	( binary_config_value const& value )
{
	lua_config_ptr result			= create_lua_config( );
...
vostok::configs::lua_config_ptr vostok::configs::create_lua_config_from_string ( pcstr string )
{
	lua_stack_guard					stack_guard;
...
using vostok::configs::binary_config_ptr;

typedef vostok::vectora<binary_config_value>	items_type;

static void process_table			( vostok::configs::lua_config_value const& value, binary_config_value& item, items_type& items );

static void construct				( vostok::configs::lua_config_value const& value, binary_config_value& item )
{
	lua_mutex_guard					mutex_guard;
	...
	switch ( item.type ) {
		...
		case t_float2 : {
			item.data				= value.cast_user_data_impl< vostok::math::float2 >( );
			item.count				= sizeof(vostok::math::float2);
			break;
		}
		case t_float3 : {
			item.data				= value.cast_user_data_impl< vostok::math::float3 >( );
			item.count				= sizeof(vostok::math::float3);
			break;
		}
		case t_float4 : {
			item.data				= value.cast_user_data_impl< vostok::math::float4 >( );
			item.count				= sizeof(vostok::math::float4);
			break;
		}
		case t_int2 : {
			item.data				= value.cast_user_data_impl< vostok::math::int2 >( );
			item.count				= sizeof(vostok::math::int2);
			break;
		}
		case t_table_indexed :
		case t_table_named : {
			R_ASSERT_CMP					( value.size(), <, 65536, "table \"%s\" has more than 65535 values", value.get_field_id() ? value.get_field_id() : "<root>" );
			items_type*	const items			= DEBUG_NEW( items_type )( vostok::debug::g_mt_allocator );
...
using vostok::configs::lua_config_value;
...
static void construct				( binary_config_value& value, vostok::memory::stream& fat, vostok::memory::stream& data, int const offset )
{
	using namespace vostok::configs;
	...
	if ( !iterate_only ) {
		...
		if ( id ) {
			value.id				= vostok::horrible_cast< size_t, pcstr >( data.get_buffer_size() ).second;
			data.append				( id );
		}
	}
	...
	switch ( value.type ) {
		...
		case t_string : {
			...
			(pcvoid&)(value.data)	= vostok::horrible_cast< size_t, pcvoid >( buffer_size ).second;
			fat.append				( &value, sizeof(value) );
			break;
		}
		case t_float2 : {
			u32 const buffer_size	= data.get_buffer_size();
			data.append				( static_cast<pcvoid>(value.data), sizeof(vostok::math::float2) );
			(pcvoid&)(value.data)	= vostok::horrible_cast< size_t, pcvoid >( buffer_size ).second;
			fat.append				( &value, sizeof(value) );
			break;
		}
		case t_float3 : {
			u32 const buffer_size	= data.get_buffer_size();
			data.append				( static_cast<pcvoid>(value.data), sizeof(vostok::math::float3) );
			(pcvoid&)(value.data)	= vostok::horrible_cast< size_t, pcvoid >( buffer_size ).second;
			fat.append				( &value, sizeof(value) );
			break;
		}
		case t_float4 : {
			u32 const buffer_size	= data.get_buffer_size();
			data.append				( static_cast<pcvoid>(value.data), sizeof(vostok::math::float4) );
			(pcvoid&)(value.data)	= vostok::horrible_cast< size_t, pcvoid >( buffer_size ).second;
			fat.append				( &value, sizeof(value) );
			break;
		}
		case t_int2 : {
			u32 const buffer_size	= data.get_buffer_size();
			data.append				( static_cast<pcvoid>(value.data), sizeof(vostok::math::int2) );
			(pcvoid&)(value.data)	= vostok::horrible_cast< size_t, pcvoid >( buffer_size ).second;
			fat.append				( &value, sizeof(value) );
			break;
		}
		...
		case t_table_named : {
			...
			binary_config_value* binary_value = vostok::pointer_cast<binary_config_value*>(fat.get_buffer() + offset);
			(pcvoid&)(binary_value->data)	= vostok::horrible_cast< size_t, pcvoid >( fat.get_buffer_size() ).second;
			R_ASSERT_CMP			( items->size(), <, 65536 );
...
static void convert					( T& value, u32 const offset )
{
	...
	value							= vostok::horrible_cast< size_t, T >( vostok::horrible_cast<T, size_t>(value).second + offset ).second;
}
...
static void fill_info_for_fix_up		( binary_config_value& value, pbyte const buffer, size_t const value_offset, u32 const offset )
{
	...
	binary_config_value* const serialized = vostok::pointer_cast<binary_config_value*>( buffer + value_offset );
	if ( value.id )
	...
	switch ( value.type ) {
		...
		case t_table_named : {
			...
			items_type::iterator const	e = items->end( );
			for (u32 j=0; i != e; ++i, ++j )
				fill_info_for_fix_up( *i, buffer, vostok::horrible_cast<pcvoid,size_t>(serialized->data).second + j*sizeof(binary_config_value), offset );
...
void vostok::configs::create_binary_config_buffer	( lua_config_value value, vostok::memory::stream& fat )
{
	lua_mutex_guard					mutex_guard;
	...
	vostok::memory::stream			data( &debug::g_mt_allocator );
	data.append						( u32(0) );
...
binary_config_ptr vostok::configs::create_binary_config	( lua_config_value value )
{
...
static void save_stream				( StreamType const& stream, pcstr const portable_file_name )
{
	...
	synchronous_device_interface const &	device	=	vostok::core::get_core_synchronous_device( );
...
void vostok::configs::lua_config_value::save_as_binary	( pcstr const file_name ) const
{
	vostok::memory::stream			stream( &debug::g_mt_allocator ); 
...
static void process_table			( vostok::configs::lua_config_value const& value, binary_config_value& item, items_type& items )
{
	...
	for ( ; i != e; ++i ) {
		...
		sub_item.id					= i.key( );
		if ( vostok::strings::equal(s_super_tables_table_id, sub_item.id) || vostok::strings::equal(s_super_tables_ids_table_id, sub_item.id) )
			continue;

		boost::crc_32_type			processor;
		processor.process_block		( sub_item.id, sub_item.id + vostok::strings::length(sub_item.id) );
		...
		for ( ; j != items_e; ++j ) {
			...
			if ( !vostok::strings::equal( (*j).id, sub_item.id) )
				continue;
...
static vostok::configs::lua_config *   new_config_inplace	( vostok::mutable_buffer const & in_out_config, pcstr file_name )
{
	R_ASSERT						( in_out_config.size() >= sizeof(vostok::configs::lua_config) );
	...
	vostok::configs::lua_config * const out_result	=	new (in_out_config.c_ptr()) vostok::configs::lua_config ( file_name, object );
	return							out_result;
}

lua_config_ptr vostok::configs::create_lua_config	( mutable_buffer const & buffer, pcstr file_name )
{
	lua_stack_guard					guard;
...
lua_config_ptr vostok::configs::create_lua_config	( pcstr file_name )
{
	return
		create_lua_config(
			mutable_buffer(
...
lua_config_ptr vostok::configs::create_lua_config	( pcstr file_name, lua_config_value const& value )
{
	lua_stack_guard					guard;
...
lua_config_value::lua_config_value				( ::luabind::object const& object) :
	m_object						( vostok::pointer_cast< luabind::object* >(&m_object_fake[0]) ),
	m_table_object					( vostok::pointer_cast< luabind::object* >(&m_table_object_fake[0]) ),
	m_field_id						( 0 ),
	m_fixed_up						( true )
{
	lua_mutex_guard					mutex_guard;
	...
	m_fixed_up						( field_id ? !strchr( field_id, '.' ) : true )
{
	...
	m_field_id						= field_id ? vostok::strings::duplicate( vostok::debug::g_mt_allocator, field_id ) : 0;
...
lua_config_value::~lua_config_value				( )
{
	if ( !s_mutex.initialized() ) {
		R_ASSERT					( vostok::command_line::key_is_set("editor") );
		return;
	}
	...
	m_fixed_up						( other.m_field_id ? !strchr( other.m_field_id, '.' ) : true )
{
	...
	m_field_id						= other.m_field_id ? vostok::strings::duplicate( vostok::debug::g_mt_allocator, other.m_field_id ) : 0;
...
static pbyte process_string			( pbyte const buffer, u32 const buffer_size, pcstr const value, u32 const length )
{
	...
	pcbyte							i = vostok::pointer_cast<pcbyte>(value);
	pcbyte							e = vostok::pointer_cast<pcbyte>(value) + length;
	pbyte							j = vostok::pointer_cast<pbyte>(buffer);
...
static void process_string			( vostok::strings::stream& stream, pcstr const value, u32 const length )
{
...
static bool is_valid_identifier			( pcstr const string)
{
	...
	pcstr const	string_e			= string + vostok::strings::length(string);
	...
	if ( !initialized ) {
		...
		lua_key_values_crc* const values_e	= lua_key_values + vostok::array_size(lua_key_values);
		for ( ; values_i != values_e; ++values_i ) {
			boost::crc_32_type		processor;
			processor.process_block	((*values_i).first, (*values_i).first + vostok::strings::length((*values_i).first));
	...
	u32 const crc					= processor.checksum();
	{
		...
		lua_key_values_crc* const e	= lua_key_values + vostok::array_size(lua_key_values);
		for ( ; i != e; ++i ) {
			if ( (*i).second != crc )
				continue;

			if ( vostok::strings::equal(string, (*i).first) )
				return				false;
		}
	}
...
static vostok::configs::enum_types get_type_impl	( lua_State* const state )
{
	lua_stack_guard					guard;
	...
	switch ( lua_type(state, -1) ) {
		...
		case LUA_TTABLE				: {
			...
			for ( int i=0; lua_next( state, -2 ); ++i ) {
				if ( !lua_typenumber(state, -2) ) {
					...
					if ( !vostok::strings::equal( string, s_super_tables_ids_table_id) && !vostok::strings::equal( string, s_super_tables_table_id) ) {
						lua_pop		( state, 2 );
		...
		case LUA_TUSERDATA			: {
			...
			if ( vostok::strings::equal( class_rep->name(), "float2") )
				return				t_float2;
			
			if ( vostok::strings::equal( class_rep->name(), "float3") )
				return				t_float3;
			
			if ( vostok::strings::equal( class_rep->name(), "float4") )
				return				t_float4;
...
typedef vostok::buffer_vector< key_type >	keys_type;
...
static void write_float					( vostok::strings::stream& stream, float const number )
{
	R_ASSERT			( vostok::math::valid(number) );
	if ( !vostok::math::is_infinity(number) )
...
{
	using namespace vostok::configs;
	namespace strings = vostok::strings;
	...
	switch ( type ) {
		...
		case t_float2				: {
			...
			luabind::object			object( luabind::from_stack(state,-1) );
			vostok::math::float2 const vector = *luabind::object_cast<vostok::math::float2*>( object );
		...
		case t_float3				: {
			lua_stack_guard			stack_guard_local;
			luabind::object			object( luabind::from_stack(state,-1) );
			vostok::math::float3		vector = *luabind::object_cast<vostok::math::float3*>( object );
		...
		case t_float4				: {
			lua_stack_guard			stack_guard_local;
			luabind::object			object( luabind::from_stack(state,-1) );
			vostok::math::float4		vector = *luabind::object_cast<vostok::math::float4*>( object );
		...
		case t_int2				: {
			lua_stack_guard			stack_guard_local;
			luabind::object			object( luabind::from_stack(state,-1) );
			vostok::math::int2		vector = *luabind::object_cast<vostok::math::int2*>( object );
		...
		case t_table_named			: {
			...
			for ( int i=0; lua_next(state,-2); ++i ) {
				if ( lua_typestring(state,-2) ) {
					...
					if ( lua_typenumber(state, -2) ) {
						...
						R_ASSERT		(
							(type == t_table_indexed) || (lua_tonumber(state, -1) < 1) || (lua_tonumber(state, -1) != vostok::math::floor( (float)lua_tonumber(state, -1)) ),
			...
			struct string_comparer {
				...
				{
					return			vostok::strings::compare( left.first, right.first ) < 0;
				}
			}; // struct string_comparer
...
void lua_config_value::save					( vostok::strings::stream& stream, pstr indent, u32 indent_size, u32 indent_level ) const
{
	lua_stack_guard					guard;
...
void lua_config_value::save_as		( pcstr file_name, vostok::configs::save_target target, save_as_option const option ) const
{
	lua_mutex_guard					mutex_guard;
...
struct table_exist_predicate {
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS( index, last, length );
...
struct construct_new_string_predicate : private boost::noncopyable {
	...
	inline bool	operator()				( int index, pcstr string, u32 length, bool last ) const
	{
		VOSTOK_UNREFERENCED_PARAMETERS( index );

		if ( is_valid_identifier(string) ) {
			R_ASSERT_CMP			( (size_t(m_current - m_buffer) + length + 1 + 1), <, m_buffer_size );
			if ( m_current > m_buffer )
				*m_current++		= '.';
			vostok::memory::copy		( m_current, m_buffer_size - (m_current - m_buffer), string, length*sizeof(char) );
			m_current				+= length;
			if ( last )
				*m_current			= 0;
			return					true;
...
struct table_object_predicate {
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS( &index, length, last );
...
lua_config_value::operator vostok::math::float2	( ) const
{
	refresh_value_from_parent		( );

	vostok::math::float2 val = *cast_user_data_impl< vostok::math::float2 >( );
...
lua_config_value::operator vostok::math::float3	( ) const
{
	refresh_value_from_parent		( );

	vostok::math::float3 val = *cast_user_data_impl< vostok::math::float3 >( );
...
lua_config_value::operator vostok::math::float4	( ) const
{
	refresh_value_from_parent		( );

	vostok::math::float4 val = *cast_user_data_impl< vostok::math::float4 >( );
...
lua_config_value::operator vostok::math::int2	( ) const
{
	refresh_value_from_parent		( );
	return							*cast_user_data_impl< vostok::math::int2 >( );
}
...
static inline bool equal					( luabind::object const& object, pcstr const value )
{
	...
	bool const result				= vostok::strings::equal( lua_tostring(state, -1), value );
	lua_pop							( state, 1 );
	return							result;
}
...
static inline bool equal					( luabind::object const& object, vostok::math::float2 const& value )
{
	lua_stack_guard					guard;
...
static inline bool equal					( luabind::object const& object, vostok::math::float3 const& value )
{
	lua_stack_guard					guard;
	...
	vostok::math::float3 const vector	= *luabind::object_cast<vostok::math::float3*>( object );
	bool const result				= vector.is_similar(value);
	lua_pop							( state, 1 );
	return							result;
}

static inline bool equal					( luabind::object const& object, vostok::math::float4 const& value )
{
	lua_stack_guard					guard;
	...
	vostok::math::float4 const vector	= *luabind::object_cast<vostok::math::float4*>( object );
	bool const result				= vector.is_similar(value);
	lua_pop							( state, 1 );
	return							result;
}

static inline bool equal					( luabind::object const& object, vostok::math::int2 const& value )
{
	lua_stack_guard					guard;
	...
	vostok::math::int2 const vector	= *luabind::object_cast<vostok::math::int2*>( object );
...
lua_config_value lua_config_value::operator =	( vostok::math::float2 const& value )
{
	vostok::math::float2 val = value;
...
lua_config_value lua_config_value::operator =	( vostok::math::float3 const& value )
{
	vostok::math::float3 val = value;
...
lua_config_value lua_config_value::operator =	( vostok::math::float4 const& value )
{
	vostok::math::float4 val = value;
...
lua_config_value lua_config_value::operator =	( vostok::math::int2 const& value )
{
	return							assign( value );
}
...
void lua_config_value::rename				( pcstr new_field_id )
{
	...
	DEBUG_FREE						( m_field_id );
	m_field_id						= strings::duplicate( vostok::debug::g_mt_allocator, new_field_id );
}
...
static void adjust_inheritance				( lua_State* const state, luabind::object& parent_table, pcstr const field, luabind::object& value )
{
	...
	if ( field ) {
		...
		if ( is_number )
			value				= parent_table[vostok::math::floor( static_cast<float>(number) )];
		else
...
vostok::configs::enum_types lua_config_value::get_type( ) const
{
	lua_stack_guard					guard;
...
using vostok::configs::lua_config_iterator;

lua_config_iterator::lua_config_iterator				( ::luabind::object const& container_table, ::luabind::iterator const& iterator ) :
	m_container_table				( vostok::pointer_cast< luabind::object* >(&m_table_object_fake[0]) ),
	m_iterator						( vostok::pointer_cast< luabind::iterator* >(&m_iterator_fake[0]) )
{
	COMPILE_ASSERT					( sizeof(luabind::object) == sizeof(m_table_object_fake), Error_luabind_object_size_changed );
	...
	struct platform_helper {
		static pcstr platform_id( )
		{
			return
				...
				"win64"
#else // #elif VOSTOK_PLATFORM_WINDOWS_64
