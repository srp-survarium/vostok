...
bool vostok::fs::file_system_impl::mount_disk_file (fat_folder_node<> * const	work_folder, 
										  		  pcstr const				physical_path, 
										  		  u32 const					hash)
{
	VOSTOK_UNREFERENCED_PARAMETERS(work_folder, physical_path, hash);
	NOT_IMPLEMENTED(return false);
}

void vostok::fs::file_system_impl::mount_disk_folder_impl (
