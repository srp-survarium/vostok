...
//#pragma message(VOSTOK_TODO("turned off links"))
		if ( duplicate ) //&& vostok::identity(0) )
		{
	...
	* out_pos								=	env.offset_for_file_positions + tell_file(env.db_file);
	...
	{
		R_ASSERT								(env.alloc);
		pstr src_data						=	(pstr)VOSTOK_ALLOC_IMPL(*env.alloc, char, file_size);
		u32 const dest_size					=	128 + (2 * file_size);
		pstr dest_data						=	(pstr)VOSTOK_ALLOC_IMPL(*env.alloc, char, dest_size);
		...
		VOSTOK_FREE_IMPL							(* env.alloc, src_data);
		VOSTOK_FREE_IMPL							(* env.alloc, dest_data);
	...
	{
		* out_inlined_data					=	memory::buffer((pstr)VOSTOK_ALLOC_IMPL(* env.alloc, char, file_size), file_size);
