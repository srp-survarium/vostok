...
struct helper {
		...
		)
	{
		...
		vostok::fixed_string4096	string;
		if ( line_number > 0 )
...
void vostok::core::debug::platform::on_error	( bool* do_debug_break, char* const message, u32 const message_size, bool* ignore_always, _EXCEPTION_POINTERS* const exception_information, error_type_enum error_type )
{
	...
	pstr buffer						= message + vostok::strings::length( message );
	
	error_helper					helper (error_type == error_type_assert ? 3 : 0, buffer, u32 ( sizeof( message ) - ( buffer - message ) ) );
	vostok::core::debug::call_stack::iterate	( exception_information, NULL, vostok::core::debug::call_stack::Callback( &helper, &error_helper::predicate), false );

	buffer							= message + vostok::strings::length( message );
...
void vostok::core::debug::platform::on_error_message_box	( bool* do_debug_break, char* const message, u32 const message_size, bool* ignore_always, _EXCEPTION_POINTERS* const exception_information, error_type_enum error_type )
{
