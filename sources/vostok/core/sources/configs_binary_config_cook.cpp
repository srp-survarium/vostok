														 	 ...
														 	 bool						file_exist)
{
	...
	pvoid const buffer				= VOSTOK_MALLOC_IMPL( unmanaged_allocator(), sizeof(binary_config), "binary config");
	if ( !buffer ) {
...
{
	VOSTOK_FREE_IMPL					(unmanaged_allocator(), buffer);
}
...
void   binary_config_cook::on_lua_config_loaded	(queries_result & data, query_result_for_cook * parent_query)
{
	...
	vostok::configs::lua_config_ptr source_config_ptr		= static_cast_resource_ptr< vostok::configs::lua_config_ptr >
...
void binary_config_cook::on_binary_config_loaded( queries_result& data, query_result_for_cook* parent_query )
{
	...
	vostok::configs::binary_config_ptr resource_config_ptr = static_cast_resource_ptr<vostok::configs::binary_config_ptr>(result.get_unmanaged_resource());
