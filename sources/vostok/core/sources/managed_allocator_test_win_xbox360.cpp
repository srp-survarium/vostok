...
#include <vostok/math_randoms_table.h>
...
void   managed_allocator::test_speed ()
{
	u32 const test_arena_size				=	128 * (1024 * 1024);
	u32 const test_temp_arena_size			=	64	* (1024 * 1024);
	...
	for ( u32 test=0; test<2; ++test )
	{
		...
		while ( vostok::identity(true) )
		{
			...
			while ( do_deallocate || free_size - m_reserved_size < allocation_size )
			{
				...
				if ( test == 0 )
				{
					g_resources_manager->free_managed_resource(s_resources[index]);
				...
				else
				{
					...
					VOSTOK_FREE_IMPL				(dl_alloc, res);
					VOSTOK_FREE_IMPL				(dl_alloc, node);
				}
			...
			if ( !do_deallocate )
			{
				...
				if ( test == 0 )
				{
					res						=	g_resources_manager->allocate_managed_resource(allocation_size - sizeof(managed_node), raw_data_class);
					R_ASSERT					(free_size == m_free_size);
				}
				else
				{
					managed_node*	node	=	(managed_node*)VOSTOK_ALLOC_IMPL(dl_alloc, char, 
												allocation_size);
					...
					new (node)					managed_node(managed_node_allocated, allocation_size);
					res						=	VOSTOK_ALLOC_IMPL(dl_alloc, managed_resource, 1);
					...
					new (res)					managed_resource(allocation_size, raw_data_class);
					res->m_node				=	node;
				}
		...
		if ( test == 0 )
		{
			...
			{
				g_resources_manager->free_managed_resource	(s_resources[i]);
			}
		}
		else
		{
	...
	new ( get_first_node() )					managed_node(managed_node_free, m_arena_size);
...
void   managed_allocator::log_test_resources () const
{
	for ( u32 i=0; i<s_resources.size(); ++i )
	{
		managed_resource *	res				=	s_resources[i];
...
//			bool catcher = true;
		...
		if ( !do_deallocate )
		{
			managed_resource * res			=	g_resources_manager->allocate_managed_resource(allocation_size - sizeof(managed_node), raw_data_class);
			R_ASSERT							(res);
			s_mutex->lock();
			s_resources.push_back				(res);
			s_mutex->unlock();
...
void   managed_allocator::remove_test_resource (u32 const index)
{
	...
	g_resources_manager->free_managed_resource	(res);
}
...
static DWORD __stdcall	helper_thread_proc (pvoid)
{
	...
	{
		threading::mutex_raii					raii(*s_mutex);
		u32 const cur_time					=	timer.get_elapsed_msec();
...
void   run_helper_process ()
{
	...
	s_helper_thread_handle					=	CreateThread(0, 
															 ...
															 CREATE_SUSPENDED, 
															 &s_helper_thread_id);
	R_ASSERT								( s_helper_thread_handle );
...
void   managed_allocator::test_defragment ()
{	
	//logging::push_filter							("core:resources:defrag", logging::silent);
...
void   managed_allocator::test_unmovable_init (u32 const start_ms)
{
	u32 num_nodes							=	0;
...
void   managed_allocator::test_unmovable_unlock_expired (u32 const cur_ms)
{
	for ( u32 i=0; i<s_unmovables_test_nodes.size(); ++i )
	{
...
void   managed_allocator::test_unmovables_list_valid () const
{
	for ( u32 i=0; i<s_unmovables_test_nodes.size(); ++i )
	{
