...
{
	fs_task *	new_task			=	VOSTOK_NEW_IMPL(* allocator, fs_task)(fs_task::type_mount_composite);
...
private:
	...
	void	delete_this ()
	{
		...
		VOSTOK_DELETE_IMPL			(m_allocator, this_ptr);
	}
...
void   query_mount (pcstr const						mount_id,
					...
					memory::base_allocator * const	allocator)
{
	VOSTOK_NEW_IMPL(allocator, mount_by_config_helper)(mount_by_config_helper::type_mount, mount_id, callback, allocator);
...
void   query_unmount (pcstr const						mount_id,
					  ...
					  memory::base_allocator * const	allocator)
{
	VOSTOK_NEW_IMPL(allocator, mount_by_config_helper)(mount_by_config_helper::type_unmount, mount_id, callback, allocator);
	// memory will be freed by the object itself
}
								  ...
								  memory::base_allocator *	allocator)
{
	...
	fs_task * const	new_task		=	VOSTOK_NEW_IMPL(* allocator, fs_task)(fs_task::type_mount_disk);
...
void   query_mount_db (pcstr const						logical_path, 
					   ...
					   fs_task * const					parent_composite_task)
{
	fs_task * const	new_task			=	VOSTOK_NEW_IMPL(* allocator, fs_task)(fs_task::type_mount_db);
...
void   query_unmount_disk (pcstr const 						logical_path, 
						   ...
						   fs_task * const					parent_composite_task)
{
	fs_task * const	new_task		=	VOSTOK_NEW_IMPL(* allocator, fs_task)(fs_task::type_unmount_disk);
...
void   query_unmount_db (pcstr const						logical_path, 
						 ...
						 fs_task * const					parent_composite_task)
{
	fs_task * new_task				=	VOSTOK_NEW_IMPL(* allocator, fs_task)(fs_task::type_unmount_db);
