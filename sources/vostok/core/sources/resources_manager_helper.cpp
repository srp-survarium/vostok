...
vostok::core::configs::lua_config_cook& resources_manager::get_lua_config_cook	()
{
	static vostok::core::configs::lua_config_cook	cook;
...
void   resources_manager::do_mount_mounts_path ()
{
	...
	m_mounts_ptr						=	VOSTOK_NEW_IMPL(helper_allocator(), fs_task_unmount)
												(sub_fat_resource);
	...
	m_mounts_converted_ptr				=	VOSTOK_NEW_IMPL(helper_allocator(), fs_task_unmount)
...
{
	VOSTOK_UNREFERENCED_PARAMETER				(file_path);
	//^ if ( is_hdd path ... )
	return									false;
}
...
#pragma message(VOSTOK_TODO("wake up watcher thread here correctly"))
...
{
	VOSTOK_UNREFERENCED_PARAMETERS			(fat_it, resource, callback);
