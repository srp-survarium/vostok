...
static vostok::core::log_file_usage s_log_file_usage	=	vostok::core::uninitialized_log;
...
vostok::memory::doug_lea_allocator_type vostok::logging::g_allocator;

typedef std::pair<char*, std::pair<int, u32> >	initiator_rule;
typedef vostok::logging::vector<initiator_rule>	rules_stack;

using vostok::logging::rule_tree::node;
...
static u32			s_ready_for_use_tls_key	= vostok::threading::tls_get_invalid_key( );

static vostok::uninitialized_reference<mutex>	s_log_mutex;
...
void cc_logging_rule::execute( pcstr args )
{
	...
	vostok::logging::verbosity t_verb = string_to_value(verb);
...
{
	if ( !vostok::threading::tls_is_valid_key( s_ready_for_use_tls_key ) )
		return					false;

	pvoid const value			= vostok::threading::tls_get_value( s_ready_for_use_tls_key );
	return						!!*vostok::pointer_cast< u32 const* >( &value );
}
...
static void set_ready_for_use					( bool const value )
{
	...
	vostok::threading::tls_set_value( s_ready_for_use_tls_key, *vostok::pointer_cast< pvoid const* >(&new_value) );
...
void vostok::logging::preinitialize				( )
{
	s_ready_for_use_tls_key		= threading::tls_create_key();
	set_ready_for_use			( false );
}

void vostok::logging::on_thread_spawn				( )
{
	set_ready_for_use			( true );
}
...
void   generate_log_file_name (vostok::fs::path_string * out_result, pcstr extension)
{
	ASSERT						(extension);
	ASSERT						(out_result);
	* out_result				=  vostok::core::user_data_directory();
	* out_result				+= "/";
	* out_result				+= vostok::core::application_name();
	vostok::fs::path_string user_name	=	vostok::core::user_name();
	if ( user_name.length() )
		out_result->appendf		("_%s", user_name.c_str());

	out_result->appendf			(".%s", extension);
	* out_result				= vostok::fs::convert_to_portable(out_result->c_str());
}

void vostok::logging::initialize					( core::log_file_usage log_file_usage, int const rule4root )
{
	check_not_ready_for_use		( );

	VOSTOK_CONSTRUCT_REFERENCE	( s_log_mutex, mutex );
...
void vostok::logging::finalize				( )
{
	...
	VOSTOK_DESTROY_REFERENCE		( s_log_mutex );
...
vostok::logging::verbosity vostok::logging::string_to_verbosity ( pcstr in_verbosity )
{
...
pcstr vostok::logging::verbosity_to_string	( int const verbosity )
{
	switch ( verbosity )
	{
...
void vostok::logging::push_rule			( pcstr initiator, int const verbosity, u32 const thread_id )
{
...
bool vostok::logging::ready_for_use		( )
{
	return						s_initiator_tree && ( get_ready_for_use( ) );
}

void vostok::logging::flush				( pcstr file_name )
{
...
void vostok::logging::ready_for_use		( bool const value )
{
	bool const backup			= get_ready_for_use( );
...
pcstr vostok::logging::log_file_name	( )
{
...
vostok::logging::log_file* vostok::logging::get_log_file( )
{
	return						( s_log_file );
}
...
namespace vostok {
	namespace fs {
		FILE * file_type_to_FILE(vostok::fs::file_type * file);
	} // namespace fs
} // namespace vostok


void   vostok::logging::write_exit_code_file ( int exit_code )
{
	if ( !get_log_file() )
		return;
