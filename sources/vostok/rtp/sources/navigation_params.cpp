...
void	navigation_tree_params::render( const navigation_tree_params& to, render::scene_ptr const& scene, vostok::render::debug::renderer& r , math::color& c  )const
{
	VOSTOK_UNREFERENCED_PARAMETERS( &to, &r, &c );
...
animation::e_gait navigation_world_base_params::path_length_to_gait( float2 const & path, bool jump )
{
	...
	if( jump && vostok::identity( animation::gait_walk_jump <= max_gait ) )
	{
...
void	navigation_theta_params::render( const navigation_theta_params&,  vostok::render::scene_ptr const&, vostok::render::debug::renderer& r, math::color const& color   )const
{
	
	VOSTOK_UNREFERENCED_PARAMETERS( &r, &color  );
