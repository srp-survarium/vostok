...
using vostok::rtp::test_grasping_action;
using vostok::rtp::action_base;
using vostok::float2;
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( my_blend, to_blend );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &af, &from_sample );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &afrom, &from_sample, &step );
...
void	test_grasping_action::render( const test_grasping_world_params& a, vostok::render::scene_ptr const& scene,  vostok::render::debug::renderer& renderer ) const
{
	math::color c;
	if( b_grasping )
		c.set( 255, 0, 0, 255 );
...
#ifndef MASTER_GOLD
void	test_grasping_action::save( vostok::configs::lua_config_value cfg )const
{
	super::save( cfg );
...
void	test_grasping_action::load( const vostok::configs::binary_config_value &cfg )
{
	super::load( cfg );
