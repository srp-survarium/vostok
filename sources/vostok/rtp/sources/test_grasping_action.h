...
public:	
	...
	virtual		void	render				( const grasping_space_params& a, vostok::render::scene_ptr const& scene,  vostok::render::debug::renderer& renderer ) const;
	virtual		void	render				( const test_grasping_world_params& a, vostok::render::scene_ptr const& scene,  vostok::render::debug::renderer& renderer ) const;
...
public:
			void	debug_render_trajectory						(	math::color const& c, u32 id,
																...
																vostok::render::scene_ptr const&,
																vostok::render::debug::renderer&
																)const
			{
				VOSTOK_UNREFERENCED_PARAMETERS( &c, &id, &current_world, &current, &current_reward, &next, &next_action, &next_reward  );
			};
