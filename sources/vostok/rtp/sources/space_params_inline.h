...
#ifndef	MASTER_GOLD
template< u32 dimension, class FinalType >
void	tree_space_param<dimension, FinalType >::save	( vostok::configs::lua_config_value cfg )const
{
	for( u32 i = 0; i < dimension; ++i )
		cfg[i] = params[i];
}
...
template< u32 dimension, class FinalType >
void	tree_space_param<dimension, FinalType >::load	( const vostok::configs::binary_config_value &cfg )
{
	for( u32 i = 0; i < dimension; ++i )
		 params[i] = cfg[i];
}
...
#ifndef MASTER_GOLD
template< u32 separate_dimensions, u32 discrete_size, class TreeParamType, class FinalType >
void	space_param<separate_dimensions, discrete_size, TreeParamType, FinalType>::save			( vostok::configs::lua_config_value cfg )const
{
		for(u32 i = 0; i < separate_dimensions; ++ i)
...
template< u32 separate_dimensions, u32 discrete_size, class TreeParamType, class FinalType >
void					space_param<separate_dimensions, discrete_size, TreeParamType, FinalType>::load			( const vostok::configs::binary_config_value &cfg )
{
	for(u32 i = 0; i < separate_dimensions; ++ i)
...
{
	for( u32 i = 0; i < separate_dimensions ; ++i )
		value[i] = vostok::memory::uninitialized_value<float>();
}
