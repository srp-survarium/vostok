...
#include <vostok/tasks_thread_tls.h>
...
template< class space_params, class world_space_params, class action >
inline animation_action<space_params,world_space_params,action>::animation_action( 
	vostok::rtp::action_base<action> &ab
	):
	super( ab ),
...
void	animation_action<space_params,world_space_params,action>::on_cached_data_loaded	( resources::queries_result& data )
{
	...
	vostok::configs::binary_config_ptr config	= static_cast_resource_ptr<vostok::configs::binary_config_ptr>(data[0].get_unmanaged_resource());

	vostok::configs::binary_config_value const cfg= config->get_root()["data"];
...
void	animation_action<space_params,world_space_params,action>::save_cached_data( ) const
{
	...
	vostok::configs::lua_config_value v_cfg	=(*cfg)["data"];
...
template< class space_params, class world_space_params, class action >
inline void	animation_action<space_params,world_space_params,action>::save( vostok::configs::lua_config_value &cfg )const
{
	super::save( cfg );
...
template< class space_params, class world_space_params, class action >
inline void	animation_action<space_params,world_space_params,action>::load( const vostok::configs::binary_config_value &cfg )
{
	super::load( cfg );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( blend );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( id_transform, &transform );
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( from_action_id,  from_blend_id  );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( &weights, &lfrom );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( &lfrom );
...
inline animation_global_actions_params::animation_global_actions_params( vostok::animation::i_animation_controller_set *set ):
m_set( set )
{
	//float4x4 m;
