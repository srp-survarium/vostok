...
public:
		animation_controller ( pcstr name, vostok::animation::i_animation_controller_set *set );
		animation_controller ( pcstr name, animation::world &w, pcstr animation_set_path );

		animation_controller ( pcstr name, animation::world &w, pcstr animation_set_path, const vostok::configs::binary_config_ptr &cfg );
		animation_controller ( pcstr name, animation::world &w, const vostok::configs::binary_config_ptr &cfg );
...
public:
	...
	virtual		void	move_control				( float2 const& move_direction, float2 const& torso_direction, animation::e_gait gait, bool jump ){ VOSTOK_UNREFERENCED_PARAMETERS( &move_direction, &torso_direction, &gait, jump ); }
	virtual		void	dbg_move_control			( float2 const& move_direction, float2 const& torso_direction, bool jump ){ VOSTOK_UNREFERENCED_PARAMETERS( &move_direction, &torso_direction, jump ); }
...
public:
#ifndef	MASTER_GOLD
	virtual	void		save					( vostok::configs::lua_config_value cfg, bool training_trees )const;
#endif	// MASTER_GOLD

	virtual	void		load					( const vostok::configs::binary_config_value &cfg );
