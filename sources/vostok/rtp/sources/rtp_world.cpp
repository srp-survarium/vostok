...
#include <vostok/animation/i_animation_controller_set.h>
...
vostok::math::random32_with_count	vostok::rtp::rtp_world::m_random;

rtp_world::rtp_world(
		vostok::rtp::engine& engine,
		vostok::animation::world& animation,
	...
	m_scene				( scene ),
	m_resource_path		( resources_vostok_path ),
...
base_controller	*rtp_world::create_navigation_controller( pcstr name, const vostok::configs::binary_config_ptr &cfg )
{
...
u32		rtp_world::query_controller_sets			( )
{
		...
		resources::request	resources[]	= {
					{ controller_set,	vostok::resources::animation_controller_class },
		};
...
u32	rtp_world::query_learned_controllers( )
{
		...
		resources::request	resources[]	= {
					//{ controller_set,	vostok::resources::binary_config_class },
					{ controller_set,	vostok::resources::animation_controller_class },
		};
...
void rtp_world::save_controllers( const vector< animation_controller_ptr > &controllers, bool training )
{
	...
	for( ; i != e  ; ++i )
	{
		...
		vostok::configs::lua_config_value v_cfg	=*cfg;
		( *i )->save(v_cfg, training );
		//pcstr tp = ( *i )->type();
		//string16 n;
		//vostok::sprintf(n,"%d",u32( i - b ));
		//pcstr name = 0;
	...
	VOSTOK_UNREFERENCED_PARAMETERS( &controllers, training );
	NODEFAULT();
#endif // #ifndef MASTER_GOLD
}
...
void	rtp_world::testing( )
{
	...
	//vostok::memory::doug_lea_allocator* 
	//	allocator = static_cast_checked<vostok::memory::doug_lea_allocator*> ( g_allocator );
