...
{
	VOSTOK_UNREFERENCED_PARAMETERS(  &af, &from_sample, &blend_sample );
...
void	navigation_theta_action::save( vostok::configs::lua_config_value cfg ) const
{
	super::save( cfg );
}
#endif // MASTER_GOLD
...
void	navigation_theta_action::load_config( )
{
	...
	{
		vostok::configs::binary_config_value restrictions = cfg["animation_group"]["restrictions"];
...
void	navigation_theta_action::load							( const vostok::configs::binary_config_value &cfg )
{
	super::load( cfg );
...
void	navigation_theta_action::debug_render_trajectory	(	math::color const& c, u32 id,
																...
																vostok::render::debug::renderer& r
																)const
{
		VOSTOK_UNREFERENCED_PARAMETERS( &c, id, &next_reward );
