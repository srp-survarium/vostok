...
VOSTOK_DECLARE_LINKAGE_ID( rtp_tester );
...
{
	return VOSTOK_LIBRARY_NAME( navigation_0, controller );
...
#ifndef MASTER_GOLD
void test_float( vostok::configs::lua_config_value const &v_cfg, u32 size, u32 level )
{
	
	for( u32 i = 0; i < size; ++i  )
	{	
		vostok::configs::lua_config_value v_data = v_cfg["data"];
...
#else
	VOSTOK_UNREFERENCED_PARAMETER					( ctrl );
...
void	testing::load_test_controller( )
{
		...
		resources::request	resources[]	= {
			{ name.c_str(),	vostok::resources::binary_config_class },
		};
