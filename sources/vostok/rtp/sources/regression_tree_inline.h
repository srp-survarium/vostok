...
inline void regression_tree<space_param_type>::save( vostok::configs::lua_config_value cfg, const  node_save_load_data<space_param_type>& data  )const
{
...
template< class space_param_type > 
inline void	regression_tree<space_param_type>::load( const vostok::configs::binary_config_value &cfg,  node_save_load_data<space_param_type>& data  )
{
...
#ifndef MASTER_GOLD
template< class space_param_type > 	
void	node< space_param_type >::save( vostok::configs::lua_config_value cfg, const  node_save_load_data< space_param_type >& data  )const
{

		cfg["leaf"] = is_leaf();
		if( !is_leaf() )
		{
			if( vostok::identity( space_param_type::dimensions_number > 1 ) )
...
template< class space_param_type > 
void	node< space_param_type >::load( const vostok::configs::binary_config_value &cfg, node_save_load_data< space_param_type >& data  )
{
		bool leaf = cfg["leaf"];
		if( !leaf )
		{
