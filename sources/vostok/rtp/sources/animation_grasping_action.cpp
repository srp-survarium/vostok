...
animation_grasping_action::animation_grasping_action( 
	vostok::rtp::action_base<animation_grasping_action> &ab
	):
	super( ab )
{
...
#ifndef	MASTER_GOLD
void	animation_grasping_action::save( vostok::configs::lua_config_value cfg )const
{
	super::save( cfg );
...
void	animation_grasping_action::load( const vostok::configs::binary_config_value &cfg )
{
	super::load	( cfg );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( my_blend, &a, to_blend );
	if( is_target_action() )
...
{
	VOSTOK_UNREFERENCED_PARAMETERS(  &a, blend );
	if( target( a ) )
...
void	animation_grasping_action::run( animation_grasping_space_params& from, const animation_grasping_action& af, u32 from_sample, u32 blend_sample  )const
{
	...
	VOSTOK_UNREFERENCED_PARAMETERS( &from, &af, &from_sample, &blend_sample );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( &wfrom, &afrom, &from_sample, &blend_sample, &time, &step );
