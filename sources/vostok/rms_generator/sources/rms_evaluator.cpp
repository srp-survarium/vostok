...
static vostok::command_line::key			s_output_text		("output_text",			"txt", "RMS Generator", "set output file binary mode or text mode. Default binary.");
static vostok::command_line::key			s_output_filename	("output_file_name",	"o", "RMS Generator", "set output file name. Default input file name.");
static vostok::command_line::key			s_discr_frequency	("discr_frequency",		"d", "RMS Generator", "set discretization frequency(floats per second). Default 10.");
...
//		printf					("[%d]%3.3f\n", rms_count, in_rms);
	...
	void process_file(const char * filename)
	{
		...
		{
			char buffer[256];
			vostok::buffer_string output_file_name(&buffer[0], array_size(buffer));
