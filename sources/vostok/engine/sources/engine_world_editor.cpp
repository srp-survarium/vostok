...
#	endif
	...
	s_editor_module		= LoadLibrary( VOSTOK_EDITOR_FILE_NAME );

	if (!s_editor_module) {
		LOG_WARNING		( "cannot load library \"%s\"", VOSTOK_EDITOR_FILE_NAME );
		CHECK_OR_EXIT	( s_editor_module != 0, "Cannot load editor library - exiting.\r\nSee log file for details." );
		return;
	}

	R_ASSERT			( !s_create_world );
	s_create_world		= (vostok::editor::create_world_ptr)GetProcAddress(s_editor_module, "create_world");
	R_ASSERT			( s_create_world );

	R_ASSERT			( !s_destroy_world );
	s_destroy_world		= (vostok::editor::destroy_world_ptr)GetProcAddress(s_editor_module, "destroy_world");
	R_ASSERT			( s_destroy_world );

	R_ASSERT			( !s_memory_allocator);
	s_memory_allocator	= (vostok::editor::memory_allocator_ptr)GetProcAddress(s_editor_module, "set_memory_allocator");
	R_ASSERT			( s_memory_allocator );
...
#if defined(DEBUG) && VOSTOK_PLATFORM_WINDOWS_32
		vostok::debug::enable_fpe	( false );
#endif
		return;
	}
...
vostok::engine::console* engine_world::create_editor_console( vostok::ui::world& uw )
{
	return VOSTOK_NEW_IMPL(m_editor_allocator, vostok::engine::editor_console)( uw, m_editor_allocator );
}