...
#include <vostok/core/sources/fs_macros.h>
...
vostok::memory::doug_lea_allocator_type	g_fs_pack_allocator;
...
void   application::initialize (u32 argc, pstr const* argv)
{
	...
	vostok::core::initialize					( "../../resources/sources/", "fs_pack", core::perform_debug_initialization );
}
...
void   application::execute ()
{
	...
	if ( pack_command )
	{
		...
		if ( inline_config_command.is_set_as_string(& config_file) )
		{
			...
			vostok::resources::query_resource_and_wait	(convert_to_portable(query_path.c_str()).c_str(), vostok::resources::lua_config_class, & on_inline_config_loaded, & g_fs_pack_allocator);
		...
		{
			ppmd_compressor* const ppmd	=	VOSTOK_NEW_IMPL(g_fs_pack_allocator, ppmd_compressor) 
		...
		VOSTOK_DELETE_IMPL					(g_fs_pack_allocator, compressor);
	}
	else
	{
		//-----------------------------------------------------------------------------------
