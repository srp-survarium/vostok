...
MStatus body_node::initialize()
{
	...
	//status = inheritAttributesFrom( "vostok_constraint" );CHK_STAT_R(status);
...
lua_config_value  find_body( lua_config_value bodies_table, MString name )
{
	...
	vostok::configs::lua_config_ptr fake_cfg		= vostok::configs::create_lua_config();
...
MStatus	body_node::do_export_node	( vostok::configs::lua_config_value cfg )
{
