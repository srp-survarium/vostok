...
const MString createCollisionCmd::Name	( "vostok_collision" );
...
{ 
	...
	if(argData.isFlagSet(typeFlag))
	{
		...
		if(argData.isFlagSet(autoFlag))
		{
			...
			vostok::maya::vector< Wm4::Vector3<float> > verts;
			u32 point_idx = 0;
...
MMatrix set_cylinder_matrix( const Wm4::Box3<float> &box )
{
	...
	u8 ax[3] = {0,1,2};
	vostok::math::float_sorted<u8> ax_sorted( ax[0], ax[1], ax[2], float( vbox_scale.x ),float(  vbox_scale.y ) ,float(  vbox_scale.z ) );
...
MStatus createCollisionCmd::auto_calculate_shape( const vostok::maya::vector< Wm4::Vector3<float> >& verts, short type, MObject& object )
{
	MStatus	stat;
