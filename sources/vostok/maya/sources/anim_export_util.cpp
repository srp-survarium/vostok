...
#include <vostok/animation/i_editor_animation.h>
#include <vostok/animation/sources/bi_spline_skeleton_animation.h>
...
using	namespace vostok::maya_animation;

using	vostok::maya_animation::discrete_data;
using	vostok::maya::maya_engine;
using	vostok::maya::g_maya_engine;

MStatus		export_bi_splines		( MSelectionList &all_joints, const anim_exporter_options &opts, vostok::configs::lua_config_value & ,vostok::configs::lua_config_value &  );
MStatus		export_animation		( const anim_exporter_options &opts, MString path  );



const MString anim_export_cmd::Name	( "vostok_anim_export" );
...
MStatus	anim_export_cmd::doIt( const MArgList& args )
{
	...
	synchronous_device_interface const & device	=	vostok::resources::get_synchronous_device();
	...
	vostok::memory::dump_statistics( true );
...
MStatus export_animation( const anim_exporter_options &options, MString path  )
{
	...
	vostok::configs::lua_config_ptr cfg_splines		= vostok::configs::create_lua_config();
	vostok::configs::lua_config_value  v_cfg_splines  = *cfg_splines;

	vostok::configs::lua_config_ptr	cfg_check_data		= vostok::configs::create_lua_config();
	vostok::configs::lua_config_value  v_cfg_check_data  = *cfg_check_data;
	...
	cfg_splines->save_as	( path.asChar(), vostok::configs::target_sources );
	cfg_check_data->save_as	( ( path + "_check_data" ).asChar(), vostok::configs::target_sources );
...
{
	frm.rotation = vostok::float3( float( rot.x ),float( rot.y ),float( rot.z ));
	rotation_maya_to_vostok( frm.rotation );

	frm.translation = vostok::float3(float( translation.x ),float( translation.y ),float( translation.z ));
	translation_maya_to_vostok( frm.translation );
	frm.scale = vostok::float3(float( scale[0] ),float( scale[1] ),float( scale[2] ));

	ASSERT( vostok::math::valid( frm.translation.x ) );
	ASSERT( vostok::math::valid( frm.translation.y ) );
	ASSERT( vostok::math::valid( frm.translation.z ) );
	ASSERT( vostok::math::valid( frm.rotation.x ) );
	ASSERT( vostok::math::valid( frm.rotation.y ) );
	ASSERT( vostok::math::valid( frm.rotation.z ) );
	ASSERT( vostok::math::valid( frm.scale.x ) );
	ASSERT( vostok::math::valid( frm.scale.y ) );
	ASSERT( vostok::math::valid( frm.scale.z ) );
}
...
void get_joint_local_from_global_frame( MFnIkJoint const &jnt, frame &frm )
{
	...
	vostok::float4x4 par = frame_matrix( pfrm );
	par.try_invert( vostok::float4x4 ( par ) );

	vostok::float4x4 m = frame_matrix( frm );
	vostok::math::float3 s1 = m.get_scale();
	vostok::math::float3 s2 = par.get_scale();
...
typedef vostok::maya::vector< MObject >	joints_vector_type;
...
vostok::animation::i_editor_animation* create_base_animation ( const anim_exporter_options &opts, MStatus* stat  )
{
	if( stat )
	...
	bool right_extension	= MString( vostok::fs_new::extension_from_path( opts.additive_animation_path().asChar() ) ) == "clip";
	bool no_extension		= MString( vostok::fs_new::extension_from_path( opts.additive_animation_path().asChar() ) ) == "";
	...
	vostok::resources::convert_physical_to_virtual_path( 
		&virtual_path, 
		vostok::fs_new::native_path_string::convert(opts.additive_animation_path().asChar()).make_lowercase(), 
		vostok::resources::exported_mount );
	...
	//vostok::animation::i_editor_animation* anim = create_editor_animation( opts.additive_animation_path().asChar() );
	vostok::animation::i_editor_animation* anim = create_editor_animation( virtual_path.c_str() );
...
vostok::animation::frame get_base_frame( pcstr bone, float time, const vostok::animation::i_editor_animation* base_anim )
{
	if( !base_anim )
		return zero;
	
	if( base_anim->bone_index( bone ) == vostok::animation::bone_index_type( -1 ) ) 
		return zero;

	vostok::animation::frame f;
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( opts );
	...
	vostok::animation::i_editor_animation *base_animation = create_base_animation( opts, &status );
	...
	vostok::math::clamp( frameLast, 0, max_frame );
	...
	//int from_frame = vostok::math::max( opts.min_frame, startFrame );
	//int to_frame = !opts.cycled ? vostok::math::min( opts.max_frame, frameLast ) : opts.max_frame;
...
static vostok::threading::mutex_tasks_unaware s_build_animation_data_mutex;
...
void build_animation_data_thread( const discrete_data *data, bi_spline_skeleton_animation *animation_data )
{
	...
	u32 id_thread = vostok::maya::g_allocator.user_thread_id();
	vostok::maya::g_allocator.user_current_thread_id();

	g_maya_engine.maya_animation_world->build_animation_data( *data, *animation_data );
	
	vostok::maya::g_allocator.user_thread_id( id_thread );
...
void run_build_animation_data_thread( const discrete_data *data, bi_spline_skeleton_animation *animation_data )
{
	...
	vostok::threading::spawn( boost::bind( &build_animation_data_thread, data, animation_data ), "build_animation_data_thread", "build_animation_data_thread", 0, 0, vostok::threading::tasks_unaware ); ;
...
using vostok::animation::enum_channel_id;
MStatus export_bi_splines(	MSelectionList &all_joints, 
							const anim_exporter_options &options, 
							vostok::configs::lua_config_value &cfg_splines, 
							vostok::configs::lua_config_value &cfg_check_data  )
{
	...
	vostok::animation::bi_spline_skeleton_animation *skel = g_maya_engine.animation_world->create_skeleton_animation_data( 0 );

	{
		vostok::configs::lua_config_value tmp = cfg_check_data["discrete_data"] ;
		data->save( tmp );
	}

//	{
//		using vostok::configs::lua_config_ptr;
//		lua_config_ptr temp_config	= vostok::configs::create_lua_config();
//		*temp_config				= v;
//
...
//
//		buffer[file_size]			= 0;
//		lua_config_ptr const temp	= vostok::configs::create_lua_config_from_string( buffer );
//		FREE						( buffer );
//
...
//	}
	...
	{
		vostok::configs::lua_config_value tmp = cfg_splines["splines"];
		skel->save( tmp );
	}
