...
void size_maya_to_vostok( vostok::math::float3_pod& pt )
{
...
void translation_maya_to_vostok(vostok::math::float3_pod& pt)
{
	size_maya_to_vostok( pt );
	pt.z = -pt.z;
}

void rotation_maya_to_vostok(vostok::math::float3_pod& pt)
{
	pt.z = -pt.z;
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( arglist );
    // Optimize texture coordinates
    // 1. Calc bounds
    vostok::math::float2 		Tdelta;
    vostok::math::float2 	Tmin, Tmax;
...
{
	MString						temp_path;
	temp_path					= vostok::maya::g_maya_engine.get_resources_path();
