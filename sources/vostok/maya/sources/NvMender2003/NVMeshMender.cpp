...
public:
	...
	virtual bool CanSmooth( MeshMender::Triangle* t1, MeshMender::Triangle* t2 )
	{
		...
		vostok::math::float3 tmp1 = t1->normal;
		vostok::math::float3 tmp2 = t2->normal;
		tmp1.normalize();
		tmp2.normalize();
...
public:
	...
	virtual bool CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2 )
	{
		...
		//for checking the angle, we want these to be normalized,
		//they may not be for whatever reason
		vostok::math::float3 tmp1 = t1->tangent;
		vostok::math::float3 tmp2 = t2->tangent;
		tmp1.normalize();
		tmp2.normalize();
...
public:
	...
	virtual bool CanSmooth(MeshMender::Triangle* t1, MeshMender::Triangle* t2 )
	{
		...
		//for checking the angle, we want these to be normalized,
		//they may not be for whatever reason
		vostok::math::float3 tmp1 = t1->binormal;
		vostok::math::float3 tmp2 = t2->binormal;
		tmp1.normalize();
		tmp2.normalize();
...
//		vector<D3DXVECTOR3> groupTangentVectors;
		vostok::math::float3 gtang(0,0,0);
		vostok::math::float3 last(0,1,0);
...
//		vector<D3DXVECTOR3> groupBinormalVectors;
		...
		vostok::math::float3 gbinormal(0,0,0);
		vostok::math::float3 last(0,1,0);
		for( u32 i=0; i<neighborGroups.size(); ++i)
		{
			for(u32 t = 0; t < neighborGroups[i].size(); ++t)//for each triangle in the group, 
...
bool MeshMender::SharesEdge(	
							...
							vector< Vertex > const& theVerts)
{
	VOSTOK_UNREFERENCED_PARAMETERS(theVerts);
								  ...
								  const NormalCalcOption computeNormals)
{
	...
	{ 
		vostok::math::float3 edge0 = verts[t.indices[1]].pos - verts[t.indices[0]].pos;
		vostok::math::float3 edge1 = verts[t.indices[2]].pos - verts[t.indices[0]].pos;
		...
		if( WeightNormalsByArea < 1.0f )
		{
			...
			vostok::math::float3 normalizedNorm	= t.normal;
			normalizedNorm.normalize();
			vostok::math::float3 finalNorm = (normalizedNorm * (1.0f - WeightNormalsByArea))
...
bool MeshMender::OrthogonalizeTangentsAndBinormals( vector< Vertex >&   theVerts )
{
	...
	for(u32 i = 0 ; i < len ; ++ i )
	{
		...
		vostok::math::float3 tmpNorm = theVerts[i].normal;
		vostok::math::float3 tmpTan = theVerts[i].tangent;
		vostok::math::float3 tmpBin = theVerts[i].binormal;

		R_ASSERT(valid(theVerts[i].normal));
		R_ASSERT(valid(theVerts[i].tangent));
		R_ASSERT(valid(theVerts[i].binormal));

		vostok::math::float3 newT = tmpTan -  (tmpNorm.dot_product(tmpTan)  * tmpNorm );
		
		vostok::math::float3 newB = tmpBin - (tmpNorm.dot_product(tmpBin) * tmpNorm)
		...
		{	
			...
			else
			{
				...
				vostok::math::float3 xAxis( 1.0f , 0.0f , 0.0f);
				vostok::math::float3 yAxis( 0.0f , 1.0f , 0.0f);
...
void MeshMender::GetGradients( const MeshMender::Vertex& v0,
                               ...
                               vostok::math::float3& tangent,
                               vostok::math::float3& binormal) const
{
	...
	//let P = v1 - v0
	vostok::math::float3 P = v1.pos - v0.pos;
	//let Q = v2 - v0
	vostok::math::float3 Q = v2.pos - v0.pos;
	float s1 = v1.s - v0.s;
	float t1 = v1.t - v0.t;
	float s2 = v2.s - v0.s;
	float t2 = v2.t - v0.t;
	...
	float tmp = 0.0f;
	if(vostok::math::abs(s1*t2 - s2*t1) <= 0.0001f)
	{
