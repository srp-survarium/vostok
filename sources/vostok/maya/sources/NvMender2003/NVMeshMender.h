...
class MeshMender
{
	...
	public:
		...
		{
		public:
			vostok::math::float3 pos;
			vostok::math::float3 normal;
			float				s;
			float				t;
			vostok::math::float3 tangent;
			vostok::math::float3 binormal;
			enum 
			{
				FVF = D3DFVF_XYZ |
					  D3DFVF_NORMAL |
...
//		typedef map<D3DXVECTOR3, TriangleList > VertexChildrenMap;
		typedef map<Vertex*, TriangleList > VertexChildrenMap;
		...
		bool OrthogonalizeTangentsAndBinormals( vector< Vertex >&   theVerts );
		...
		void UpdateIndices(	const u32 oldIndex , 
							const u32 newIndex , 
		...
		void AppendToMapping(	const u32 oldIndex,
