...
const MString build_collision_cmd::Name	( "vostok_build_collision" );
...
MStatus	build_collision_cmd::build(MDagPath const& joint_path, bool b_reciursive, MString const& parent_name)
{
	...
	{
		cmd 			= "vostok_constraint -edit ";//-t fixed -n constraint
		cmd				+= " -link ";
		cmd				+= parent_name;
		cmd				+= " -link ";
		cmd				+= body_name;
