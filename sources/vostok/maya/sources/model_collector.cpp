...
//.			item.name					= locator_path.partialPathName();
			...
			translation_maya_to_vostok	( item.position );
			...
			rotation_maya_to_vostok	( item.rotation );
...
{
	// collision
	F.open_chunk		( vostok::render::model_chunk_collision_v );
...
{
	F.open_chunk		( vostok::render::model_chunk_collision_i );
