...
const MString skinned_visual_exporter::Name	( "vostok_skin_export" );
...
//	clear						( );
	...
	if( !nodb )
	{
		vostok::fs_new::native_path_string	file_path;
		vostok::fs_new::file_name_with_no_extension_from_path	(&file_path, vostok::fs_new::native_path_string::convert(m_folder_name.asChar()).c_str() );
		...
		vostok::fs_new::create_folder_r		( device, temp_path.asChar(), true );
	}
	else
	{
	...
	}
	{
		...
		vostok::fs_new::synchronous_device_interface const & device	=	vostok::resources::get_synchronous_device();
		vostok::fs_new::create_folder_r		( device, fn.asChar(), true );
		...
		for( u32 idx = 0; idx < surfaces_count; ++idx )
		{
			...
			vostok::fs_new::synchronous_device_interface const & device	=	vostok::resources::get_synchronous_device();
			vostok::fs_new::create_folder_r		( device, fns.asChar(), true );
	...
	if( !nodb )
	{
		...
		vostok::fs_new::synchronous_device_interface & device	=	vostok::resources::get_synchronous_device();
