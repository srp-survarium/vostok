...
MStatus render_skinned_surface::save_properties( configs::lua_config_value& value )  const
{
	...
	{
		case 1: 	
			value["type"]		= vostok::render::mt_skinned_submesh_1w; break;
		case 2:
			value["type"]		= vostok::render::mt_skinned_submesh_2w; break;
		case 3:
			value["type"]		= vostok::render::mt_skinned_submesh_3w; break;
		case 4:
			value["type"]		= vostok::render::mt_skinned_submesh_4w; break;
		default:
			NODEFAULT();
	}
...
{
	vostok::render::vert_boned_4w		vertex;
...
{
	vostok::render::vert_boned_3w		vertex;
	u32 vcount			= m_vertices.size();
	for(u32 vidx=0; vidx<vcount; ++vidx)
	{
		vertex.invalidate			();
...
{
	vostok::render::vert_boned_2w		vertex;
	u32 vcount			= m_vertices.size();
	for(u32 vidx=0; vidx<vcount; ++vidx)
	{
		vertex.invalidate			();
...
{
	vostok::render::vert_boned_1w		vertex;
	u32 vcount			= m_vertices.size();
	for(u32 vidx=0; vidx<vcount; ++vidx)
	{
		vertex.invalidate			();
