...
using vostok::ai::navigation::navigation_triangle;
using vostok::ai::navigation::is_marked_predicate;
using vostok::ai::navigation::filter_non_passable_predicate;
	...
	)
{
	...
	if ( vostok::math::is_zero(denominator) ) {
		float const numerator = (ay - cy)*(bx - ax) - (ax - cx)*(by - ay);
		if ( !vostok::math::is_zero(numerator) )
			return			0;
...
static inline bool is_between	(
		vostok::math::float3 const& test_vertex,
		vostok::math::float3 const& vertex0,
		vostok::math::float3 const& vertex1
	)
{
	...
	)
{
	...
	float maximum = -vostok::math::float_max;
	vertices_type::const_iterator i			= vertices.begin( );
	vertices_type::const_iterator const e	= vertices.end( );
	for ( ; i != e; ++i ) {
		float const local_maximum	= vostok::math::max( (*i).x, vostok::math::max( (*i).y, (*i).z) );
...
//	float3 const* vertices[3];
	...
	for (u32 iteration = 0; ; ++iteration ) {
		...
		if ( vostok::math::is_zero( s0_cross_s1_t0.length() ) ) {
...
//	R_ASSERT					( vostok::math::is_zero( (normalize(b_minus_a ^ c_minus_a) | triangle.normal) - 1.f, .1f ) );
