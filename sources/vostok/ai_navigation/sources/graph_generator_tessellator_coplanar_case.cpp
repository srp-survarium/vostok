...
using vostok::ai::navigation::graph_generator_tessellator;
using vostok::ai::navigation::delaunay_triangulator;
using vostok::ai::navigation::triangles_mesh_type;
...
typedef vostok::buffer_vector< u32 >	indices_type;
typedef vostok::buffer_vector< bool >	markers_type;
...
bool is_on_segment	(
		u32 const (&coordinate_indices)[2],
		vostok::math::float3 const& testee,				// t
		vostok::math::float3 const& first_segment_vertex,	// v0
		vostok::math::float3 const& second_segment_vertex,// v1
		float epsilon
	);
...
u32 vostok::ai::navigation::segment_intersects_segment	(
		u32 const (&coordinate_indices)[2],
		float3 const& v0,
		float3 const& v1,
		float3 const& u0,
	...
	)
{
	...
	float3 u					= u1 - u0;
	u							/= vostok::math::sqrt( vostok::math::sqr(u[ coordinate_indices[0] ]) + vostok::math::sqr(u[ coordinate_indices[1] ]) );
	float3 const& u2			= u0 + u;
	float const& dx_big			= u2[ coordinate_indices[0] ];
	float const& dy_big			= u2[ coordinate_indices[1] ];

	float3 v					= v1 - v0;
	v							/= vostok::math::sqrt( vostok::math::sqr(v[ coordinate_indices[0] ]) + vostok::math::sqr(v[ coordinate_indices[1] ]) );
	...
	are_on_the_same_line		= false;
	bool are_parallel			= vostok::math::is_zero(denominator_big, vostok::math::epsilon_6) || ( denominator == 0 );

	//if ( !are_parallel && vostok::math::is_zero(denominator_big, vostok::math::epsilon_5) ) {
	...
	if ( are_parallel ) {
		...
		if ( !vostok::math::is_zero(numerator_big, vostok::math::epsilon_5) )
			return				try_on_segment( coordinate_indices, v0, v1, u0, u1, result0 );

		are_on_the_same_line	= true;
		bool should_compare_x	= vostok::math::abs(ax - bx) >= vostok::math::abs(ay - by);
	...
	float const epsilon			= vostok::math::epsilon_7;

	r0							= vostok::math::is_similar( r0, 1.0f, epsilon ) ? 1.0f : r0;
	r1							= vostok::math::is_similar( r1, 1.0f, epsilon ) ? 1.0f : r1;
	r0							= vostok::math::is_similar( r0, 0.0f, epsilon ) ? 0.0f : r0;
	r1							= vostok::math::is_similar( r1, 0.0f, epsilon ) ? 0.0f : r1;
	...
	if ( vostok::math::abs(numerator0) >= vostok::math::abs(numerator1) )
		result0					= v0 + r0*(v1 - v0);
	else
		result0					= u0 + r1*(u1 - u0);
...
using vostok::ai::navigation::segment_intersects_segment;
...
struct equal_by_coordinate_indices {
	...
	{
		return
			vostok::math::is_relatively_similar( left[m_coordinate_indices[0]], right[m_coordinate_indices[0]], vostok::math::epsilon_6) &&
			vostok::math::is_relatively_similar( left[m_coordinate_indices[1]], right[m_coordinate_indices[1]], vostok::math::epsilon_6);
	}
	...
	)
{
	...
	vostok::ai::navigation::fill_coordinate_indices	( normal, coordinate_indices );
	...
	if ( (vertex_count > 1) && vostok::math::is_similar( result_intersections[0], result_intersections[1]) )
		return				1;

	return					vertex_count;
}

typedef vostok::buffer_vector< float3 >	vertices_type;
typedef vostok::buffer_vector< u32 >		indices_type;
...
bool is_on_segment	(
		u32 const (&coordinate_indices)[2],
		vostok::math::float3 const& testee,
		vostok::math::float3 const& first_segment_vertex,
		vostok::math::float3 const& second_segment_vertex,
		float epsilon
	);
	...
	)
{
	if ( check_null_square ) {
		static float const epsilon = vostok::math::epsilon_3;
		...
		/*		
		vostok::math::float3 const b_minus_a	= normalize( triangle_vertex1 - triangle_vertex0 );
		vostok::math::float3 const c_minus_a	= normalize( triangle_vertex2 - triangle_vertex0 );
...
VOSTOK_AI_NAVIGATION_API bool vostok::ai::navigation::is_triangle_inside_triangle (
		u32 const (&coordinate_indices)[2],
		float3 const& v0,
		float3 const& v1,
		float3 const& v2,
	...
	*/
	...
	for ( u32 i = 0; i < vertices.size(); ++i ) {
		for ( u32 j = i+1; j < vertices.size(); ++j ) {
			if ( is_similar( vertices[i], vertices[j], vostok::math::epsilon_3 ) ) {
...
void filter_duplicated_vertices(u32 const (&coordinate_indices)[2],  VertexContainer& vertices )
{
	for ( u32 i = 0; i < vertices.size(); ++i ) {
		for ( u32 j = i+1; j < vertices.size(); ++j ) {
			if ( vostok::math::is_similar( vertices[i][coordinate_indices[0]], vertices[j][coordinate_indices[0]], vostok::math::epsilon_3 )
				&& vostok::math::is_similar( vertices[i][coordinate_indices[1]], vertices[j][coordinate_indices[1]], vostok::math::epsilon_3 ) )
			{
				vertices.erase( vertices.begin() + j );
				--j;
			}
...
public:
	...
	inline bool operator( )							( u32 const triangle_id ) const
	{
		...
		vostok::math::float3 const& v0	= m_triangles_mesh->vertices[ indices0[0] ];
		vostok::math::float3 const& v1	= m_triangles_mesh->vertices[ indices0[1] ];
		vostok::math::float3 const& v2	= m_triangles_mesh->vertices[ indices0[2] ];

		u32 const* indices1		= &*m_triangles_mesh->indices.begin() + 3*triangle_id;
		vostok::math::float3 const& u0	= m_triangles_mesh->vertices[ indices1[0] ];
		vostok::math::float3 const& u1	= m_triangles_mesh->vertices[ indices1[1] ];
		vostok::math::float3 const& u2	= m_triangles_mesh->vertices[ indices1[2] ];
