...
class VOSTOK_AI_NAVIGATION_API delaunay_triangulator : private core::noncopyable {
public:
...
void VOSTOK_AI_NAVIGATION_API fill_coordinate_indices	( float3 const& normal, u32 (&coordinate_indices)[2] );
u32 VOSTOK_AI_NAVIGATION_API segment_intersects_segment	(
		u32 const (&coordinate_indices)[2],
		float3 const& v0,
		float3 const& v1,
		float3 const& u0,
...
bool VOSTOK_AI_NAVIGATION_API is_on_segment	(
		...
		float epsilon = vostok::math::epsilon_6
	);

bool VOSTOK_AI_NAVIGATION_API is_similar ( 
		u32 const (&coordinate_indices)[2],
		vostok::math::float3 const& v0,
		vostok::math::float3 const& v1,
		float epsilon = vostok::math::epsilon_6
	);

float VOSTOK_AI_NAVIGATION_API cross_product				( u32 const (&coordinate_indices)[2], vostok::math::float3 const& vertex0, vostok::math::float3 const& vertex1 );
float VOSTOK_AI_NAVIGATION_API dot_product				( u32 const (&coordinate_indices)[2], vostok::math::float3 const& vertex0, vostok::math::float3 const& vertex1 );
bool  VOSTOK_AI_NAVIGATION_API is_point_inside_triangle	( u32 const (&coordinate_indices)[2], vostok::math::float3 p, vostok::math::float3 a, vostok::math::float3 b, vostok::math::float3 c );
