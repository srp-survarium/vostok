	...
	)
{
	...
	float3 const& maximum			= 1.01f * math::max( vostok::math::abs(min_position), vostok::math::abs(max_position), float3( math::epsilon_3, math::epsilon_3, math::epsilon_3 ) );
	...
	m_indices						( indices )
{
	...
	for ( u32 i = 0; i < vertices_count; ++i ) {
		...
		if ( vostok::math::is_relatively_similar( distance0, distance0 + min_distance, math::epsilon_5 ) ) {
	...
	bool done = true;
	vostok::math::random32 random		= vostok::math::random32( 0 );
