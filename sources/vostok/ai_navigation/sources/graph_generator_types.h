...
#ifndef GRAPH_GENERATOR_TYPES_H_INCLUDED
#define GRAPH_GENERATOR_TYPES_H_INCLUDED
...
struct triangle_to_test {
	math::plane			plane;
	u32 const*			indices;
	bool				is_walkable;
}; // struct triangle_to_test

typedef debug::vector< triangle_to_test >	navigation_triangles_type;

struct navigation_triangle {
	inline navigation_triangle	( ) :
		color		( vostok::math::color( 255, 255, 255, 64 ) ),
		is_marked	( false )
	{
		for ( u32 i = 0; i < 3; ++i )
			neighbours[i] = u32(-1);
	...
	{
		R_ASSERT( edge_id < 3 );
		return !vostok::math::is_zero( obstructions[ edge_id ] );
	}
...
#endif // #ifndef GRAPH_GENERATOR_TYPES_H_INCLUDED