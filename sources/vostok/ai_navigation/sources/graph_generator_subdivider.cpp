...
using vostok::ai::navigation::graph_generator_subdivider;
using vostok::ai::navigation::triangles_mesh_type;
using vostok::ai::navigation::triangles_type;
...
graph_generator_subdivider::buffer_indices_type::iterator const graph_generator_subdivider::select_triangle (
		...
		vostok::math::float3& segment0,
		vostok::math::float3& segment1
		)
{
...
bool graph_generator_subdivider::try_subdivide_triangle	(
		...
		vostok::math::float3 const& triangle_vertex0,
		vostok::math::float3 const& triangle_vertex1,
		vostok::math::float3 const& triangle_vertex2,
...
{
	if ( is_similar(vertex0, testee, vostok::math::epsilon_3 ) ) {
		R_ASSERT				( vertex1 != testee );
		R_ASSERT				( vertex2 != testee );
		return					&vertex0;
	}
	
	if ( is_similar(vertex1, testee, vostok::math::epsilon_3) ) {
		R_ASSERT				( vertex2 != testee );
		return					&vertex1;
	}

	if ( is_similar(vertex2, testee, vostok::math::epsilon_3) )
		return					&vertex2;
...
void graph_generator_subdivider::correct_segment_vertex(float3& vertex, u32 const triangle_id )
{
	...
	static const float epsilon = vostok::math::epsilon_3;
...
public:
	inline filter_obstructions_predicate2	( triangles_mesh_type const& generator, vostok::math::plane const& plane ) :
		m_triangles_mesh		( &generator ),
		m_plane					( &plane )
	{
	}

		inline bool operator( )	( vostok::collision::triangle_result const& triangle_result ) const
	{
		...
		if ( vostok::math::abs(triangle.plane.normal | m_plane->normal) < 1.f )
			return				false;

		if ( vostok::math::is_similar(triangle.plane.d, m_plane->d, vostok::math::epsilon_3) )
			return				true;
...
private:
	triangles_mesh_type const*	m_triangles_mesh;
	vostok::math::plane const*	m_plane;
}; // struct filter_obstructions_predicate
...
void push_triangle( triangles_mesh_type& triangles_mesh, u32 parent_id, float3 const v0, float3 const v1, float3 const v2 )
{
	...
	vostok::math::float3 const b_minus_a	= normalize( v1 - v0 );
	vostok::math::float3 const c_minus_a	= normalize( v2 - v0 );
