...
#include <vostok/ai/search/dijkstra.h>
#include <vostok/ai/search/search_restrictor_generic.h>

using vostok::ai::navigation::path_find_channel;
using vostok::ai::navigation::triangles_mesh_type;
using vostok::ai::navigation::path_find_funnel;
...
public:
	...
	inline	bool	accessible	( vertex_id_type const& neighbour_vertex_id, vertex_id_type const& current_vertex_id, triangles_mesh_graph_wrapper::const_edge_iterator const& edge_iterator ) const
	{
		...
		vostok::ai::navigation::navigation_triangle const& triangle = m_graph.data[ current_vertex_id ];
...
struct triangle_path_constructor : public vostok::ai::path_constructor::vertex {
	typedef vostok::ai::path_constructor::vertex	super;
	...
	{
	public:
		typedef vostok::ai::path_constructor::vertex::impl<VertexType, PathType>	super;
...
//	typedef vostok::ai::dijkstra								algorithm_type;
	...
	struct vertex_type_helper<
			vostok::ai::dijkstra::vertex_helper< distance_type >::vertex_impl
		> : public vostok::ai::dijkstra::vertex_helper< distance_type >::vertex_impl
	{
	...
	typedef vostok::debug::vector< vertex_id_type >				path_type;
...
public:
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER	( target_vertex_id );
	}
	...
	float distance_to_segment					( float3 const& v, float3 const& s0, float3 const& s1 ) const
	{
		...
		R_ASSERT						( !vostok::math::is_zero( s1_minus_s0_length ) );
		...
		) const
	{
		...
		return						acos( vostok::math::clamp_r( cosine, -1.0f, 1.0f ) );
	}
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER		( iterator );
