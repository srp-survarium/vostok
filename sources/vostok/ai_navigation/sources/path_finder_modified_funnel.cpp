	...
	)
{
	...
	float const projection_on_ab	= vostok::math::sqr(radius)/middle_a_b_minus_a_length;
	R_ASSERT_CMP					( radius, >=, projection_on_ab, "it is impossible to construct valid tangents; path channel is non-passable!");
	float const projection2			= vostok::math::sqrt( vostok::math::sqr(radius) - vostok::math::sqr(projection_on_ab) );
	return
		std::make_pair(
	...
	)
{
	...
	float const projection_on_ab	= vostok::math::sqr(radius)/b_minus_a_length;
	R_ASSERT_CMP					( radius, >=, projection_on_ab, "it is impossible to construct valid tangents; path channel is non-passable!");
	float const projection2			= vostok::math::sqrt( vostok::math::sqr(radius) - vostok::math::sqr(projection_on_ab) );
	return
		std::make_pair(
