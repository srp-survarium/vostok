...
using vostok::ai::navigation::vertices_type;
using vostok::math::float3;
using vostok::math::float4x4;

void graph_generator::fill_triangles			( vostok::collision::geometry_instance* geometry, triangles_mesh_type& result, float4x4 transform )
{
...
struct remove_vertices_by_unique_indices_predicate {
		...
		m_unique_vertices_indices	( &unique_vertices_indices )
	{
		...
		inline bool operator() ( vostok::math::float3 const& vertex ) const
	{
	...
	)
{
	...
	vostok::math::float3 const b_minus_a	= normalize( vertex1 - vertex0 );
	vostok::math::float3 const c_minus_a	= normalize( vertex2 - vertex0 );
...
void graph_generator::remove_duplicates ( triangles_mesh_type& triangles_mesh )
{
	...
	for ( u32 i = 0; i < 3; ++i ) {
		...
		if ( !is_similar( middle_vertices[0], middle_vertices[1], vostok::math::epsilon_3 ) )
			break;
...
void fill_sample_triangles					( triangles_mesh_type& triangles_mesh )
{
	...
	for (u32 i = 0; i < width; ++i )
		for ( u32 j = 0; j < height; ++j ) {
			triangles_mesh.vertices.push_back( vostok::math::float3( (float)i, 0.f, (float)j ) );
		}
...
void push_triangle ( triangles_mesh_type& triangles_mesh, u32 v0, u32 v1, u32 v2) {
	...
	triangles_mesh.data.push_back ( vostok::ai::navigation::navigation_triangle() );
}
...
void load_obj_file ( const char* fname, triangles_mesh_type& triangles_mesh )
{
	...
	while ( device->read( file, buffer, 1024) ) {
		if ( buffer[0] == 'v' ) {
			float3 vertex;
			VOSTOK_SSCANF	( buffer, "v %f %f %f", &vertex.x, &vertex.y, &vertex.z );
		...
		if ( buffer[0] == 'f' ) {
			u32 indices[3];
			VOSTOK_SSCANF		( buffer, "f %d %d %d",  indices, indices+1, indices+2 );
...
{
	vostok::math::random32 random(10);
...
typedef vostok::buffer_vector< u32 > vertices_indices_type;
...
#else
//	fill_triangles									( m_geometry, m_input_triangles, vostok::math::create_translation( float3( 0, 0, 0)) );
//	fill_triangles									( m_geometry, m_input_triangles, vostok::math::create_translation( float3( 15, 0, 0)) );
//	fill_triangles									( m_geometry, m_input_triangles, vostok::math::create_translation( float3(  0, 3, 30)) );
//	fill_triangles									( m_geometry, m_input_triangles, vostok::math::create_translation( float3( 15, 3, 30)) );
