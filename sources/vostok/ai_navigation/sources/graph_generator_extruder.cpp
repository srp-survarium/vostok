...
using vostok::ai::navigation::graph_generator_extruder;
...
static inline vostok::math::float3 extrude_vertex( vostok::math::float3 const& vertex, float const extrude_length )
{
	vostok::math::float3 result	= vertex;
	result.y					-= extrude_length;
	return						result;
}

static inline bool valid_triangle( vostok::debug::vector<vostok::math::float3> const& vertices, u32 const index0, u32 const index1, u32 const index2 )
{
	vostok::math::float3 const& vertex0 = vertices[ index0 ];
	vostok::math::float3 const& vertex1 = vertices[ index1 ];
	vostok::math::float3 const& vertex2 = vertices[ index2 ];
	if ( vertex0.is_similar(vertex1, vostok::math::epsilon_3) )
		return					false;

	if ( vertex0.is_similar(vertex2, vostok::math::epsilon_3) )
		return					false;

	if ( vertex1.is_similar(vertex2, vostok::math::epsilon_3) )
		return					false;

	vostok::math::float3 const& normal_10 = normalize(vertex1 - vertex0);
	vostok::math::float3 const& normal_20 = normalize(vertex2 - vertex0);
	if ( normal_10.is_similar(normal_20, vostok::math::epsilon_3) )
		return					false;
