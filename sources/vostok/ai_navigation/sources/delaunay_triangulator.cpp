...
float VOSTOK_AI_NAVIGATION_API cross_product	(
...
float VOSTOK_AI_NAVIGATION_API dot_product	(
	u32 const (&coordinate_indices)[2],
	float3 const& vertex0,
	float3 const& vertex1
	)
...
void vostok::ai::navigation::fill_coordinate_indices	( float3 const& normal, u32 (&coordinate_indices)[2] )
{
	float3 const& normal_abs		= float3( vostok::math::abs(normal.x), vostok::math::abs(normal.y), vostok::math::abs(normal.z) );
	if ( normal_abs.x > normal_abs.y ) {
		if ( normal_abs.x > normal_abs.z ) {
			coordinate_indices[0]	= 1;
			coordinate_indices[1]	= 2;

			if ( vostok::math::sign(normal.x) == 1.f )
			...
			if ( vostok::math::sign(normal.z) == 1.f )
				std::swap			( coordinate_indices[0], coordinate_indices[1] );
		}
	}
	else {
		if ( normal_abs.z > normal_abs.y ) {
			...
			if ( vostok::math::sign(normal.y) == -1.f )
				std::swap			( coordinate_indices[0], coordinate_indices[1] );
		}
	}
}
	...
	float matrix[3][3] = {
		{
			...
			vostok::math::sqr( matrix[0][0] ) + vostok::math::sqr( matrix[0][1] ),
		},
		{
			...
			vostok::math::sqr( matrix[1][0] ) + vostok::math::sqr( matrix[1][1] ),
		},
		{
			...
			vostok::math::sqr( matrix[2][0] ) + vostok::math::sqr( matrix[2][1] ),
		},
	};
	...
	)
{
	...
	char const sign			= vostok::math::sign( cross_product(coordinate_indices, vertex10, vertex21 ) );
	float3 const& vertex32	= vertex3 - vertex2;
	if ( sign != vostok::math::sign( cross_product(coordinate_indices, vertex21, vertex32 ) ) )
		return				false;

	float3 const& vertex03	= vertex0 - vertex3;
	if ( sign != vostok::math::sign( cross_product(coordinate_indices, vertex32, vertex03 ) ) )
		return				false;

	if ( sign != vostok::math::sign( cross_product(coordinate_indices, vertex03, vertex10 ) ) )
		return				false;
	...
	)
{
	...
	float const v0_to_t_len_mul_v1_to_t_length	= vostok::math::sqrt( v0_to_t_squared_length * v1_to_t_squared_length );
	float const a2_plus_2ab_plus_b2				= v0_to_t_squared_length + 2 * v0_to_t_len_mul_v1_to_t_length + v1_to_t_squared_length;
	return
		vostok::math::is_relatively_similar (
			a2_plus_2ab_plus_b2,
...
bool VOSTOK_AI_NAVIGATION_API is_on_segment	(
		u32 const (&coordinate_indices)[2],
		vostok::math::float3 const& testee,				// t
		vostok::math::float3 const& first_segment_vertex,	// v0
		vostok::math::float3 const& second_segment_vertex,// v1
...
{
	return	is_on_segment( testee, first_segment_vertex, second_segment_vertex, vostok::math::epsilon_7 );
}
...
{
	return
		vostok::math::sqr( vector[ coordinate_indices[0] ] ) +
		vostok::math::sqr( vector[ coordinate_indices[1] ] );
}

bool VOSTOK_AI_NAVIGATION_API is_similar ( 
		u32 const (&coordinate_indices)[2],
		float3 const& v0,
		float3 const& v1,
		float epsilon
...
{
	return vostok::math::is_similar( v0[coordinate_indices[0]], v1[coordinate_indices[0]], epsilon ) &&
			vostok::math::is_similar( v0[coordinate_indices[1]], v1[coordinate_indices[1]], epsilon );
}
...
float distance_to_segment_squared( u32 const (&coordinate_indices)[2], float3 v, float3 a, float3 b )
{
	...
	a[ third_index ] = 0;
	b[ third_index ] = 0;
	return vostok::math::abs(cross_product( coordinate_indices, v - a, normalize(b - a) ));
}
