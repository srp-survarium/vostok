...
inline int vostok::strings::compare_insensitive	( pcstr left, pcstr right )
{
#if defined(_MSC_VER)
...
inline pstr vostok::strings::copy			( pstr destination, u32 destination_size, pcstr source )
{
#ifdef _MSC_VER
...
template < int count >
inline pstr vostok::strings::copy			( char (&destination)[count], pcstr source )
{
	return				( copy(&destination[0], count, source) );
}

inline pstr vostok::strings::copy_n		( pstr const destination, size_t const destination_size, pcstr const source, size_t const count )
{
#ifdef _MSC_VER
...
inline pstr vostok::strings::duplicate	( allocator_type& allocator, pcstr const string )
{
	u32 const size		= (length( string ) + 1)*sizeof(char);
	pstr const result	= ( char* )VOSTOK_MALLOC_IMPL(allocator, size, "strings::duplicate" );
	memory::copy		( result, size, string, size );
	return				( result );
}

template <typename predicate_type>
inline bool	vostok::strings::iterate_items( pcstr string, u32 length, predicate_type const& predicate, char const separator )
{
	pcstr I				= string;
...
template <typename predicate_type>
inline bool	vostok::strings::iterate_items( pcstr string, predicate_type const& predicate, char const separator )
{
	return				( iterate_items ( string, length( string ), predicate, separator ) );
}

#endif // #ifndef VOSTOK_STRINGS_FUNCTIONS_INLINE_H_INCLUDED