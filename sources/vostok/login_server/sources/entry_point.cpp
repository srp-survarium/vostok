...
static bool setup_program_options	( int argc, char* argv[], boost::program_options::variables_map& options )
{
	...
	sign_in_options.add_options()
		( "tolerant_attempts_count",	po::value<u32>( &vostok::sign_in_tolerant_attempts_count )->default_value( 3 ),	"set tolerant attempts count" )
		( "tolerant_wait_time",			po::value<u32>( &vostok::sign_in_tolerant_wait_time )->default_value( 1 ),		"set tolerant wait interval for sign in attempt, in seconds" )
		( "intolerant_wait_time",		po::value<u32>( &vostok::sign_in_intolerant_wait_time )->default_value( 10 ),		"set intolerant wait interval for sign in attempt, in seconds" )
	;
...
static void run_server				( boost::program_options::variables_map const& options )
{
	try	{
		...
		{
			vostok::login_server server( io_service, vostok::login_tcp_port, options );
			io_service.run		( );
		}
	}
	catch ( std::exception const& exception ) {
