...
#include <vostok/fs/fs_portable_path_string.h>
#include <vostok/fs/fs_file_type.h>
#include <vostok/fs/fs_platform_configuration.h>
...
class VOSTOK_NOVTABLE device_file_system_interface
{
public:
	...
	virtual size_t		write			(pvoid handle, pcvoid data, size_t size) = 0;
	virtual size_t		read			(pvoid handle, pvoid data, size_t size) = 0;

	virtual bool		seek			(pvoid handle, file_size_type offset, seek_file_enum origin) = 0;
	virtual file_size_type	tell		(pvoid handle) = 0;

	virtual void		setvbuf			(pvoid handle, char * buffer, int mode, size_t size) = 0;
