...
#include <vostok/fs/fs_device_file_system_interface.h>
...
class VOSTOK_FS_API device_file_system_proxy_base
{
public:
	...
	size_t			write				(file_type * file, pcvoid data, size_t size) const;
	size_t			read				(file_type * file, pvoid data, size_t size) const;

	bool			seek				(file_type * file, file_size_type offset, seek_file_enum origin) const;
	file_size_type	tell				(file_type * file) const;
	
	void			setvbuf				(file_type * stream, char * buffer, int mode, size_t size) const;
