...
#include "pch.h"
#include <vostok/fs/fs_asynchronous_device_commands.h>
#include <vostok/fs/fs_synchronous_device_interface.h>
#include <vostok/fs/fs_asynchronous_device_interface.h>
...
void   synchronize_device_query::execute () 
{
	...
	m_synchronization_ended_event.wait		(threading::event::wait_time_infinite);
}

void   synchronize_device_query::wait_synchronization ()
{
	m_synchronization_started_event.wait	(threading::event::wait_time_infinite);
}
...
void   open_file_device_query::execute	()
{
	...
	VOSTOK_UNREFERENCED_PARAMETER				(open_file_result);
}
