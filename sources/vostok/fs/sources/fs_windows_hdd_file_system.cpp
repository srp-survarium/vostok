...
#include "pch.h"
#include <vostok/fs/fs_windows_hdd_file_system.h>
...
#pragma message(VOSTOK_TODO("fix it"))
void   log_last_error (pcstr initiator, pcstr header)
{
	VOSTOK_UNREFERENCED_PARAMETERS(initiator, header);
	/*pstr message					=	core::debug::platform::fill_format_message	((int)GetLastError());
	fixed_string<128>	full_initiator	=	VOSTOK_LOG_MODULE_INITIATOR;
...
void   windows_hdd_file_system::setvbuf (pvoid const handle, char * buffer, int mode, size_t size)
{
...
size_t	  windows_hdd_file_system::read	(pvoid const handle, pvoid data, size_t size)
{
	DWORD	bytes_read					=	0;
...
size_t   windows_hdd_file_system::write (pvoid const handle, pcvoid data, size_t size)
{
	R_ASSERT								(handle);
