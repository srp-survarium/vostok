...
#include <vostok/memory_allocated_buffer.h>
#include <vostok/fs/fs_file_type_pointer.h>
#include <vostok/fs/fs_device_utils.h>
#include <vostok/vfs/vfs_virtual_file_system.h>
#include "vfs_saving_info_tree.h"
#include <vostok/vfs/vfs_unpack.h>
#include "vfs_test.h"
#include "vfs_free_node.h"
...
bool   make_patch						(patch_args & args)
{
	...
	virtual_file_system * from_vfs		=	VOSTOK_NEW_IMPL(args.allocator, virtual_file_system);
	...
	u32 const to_sources_size			=	args.to_sources.count_of(';') + 1;
 	...
 	virtual_file_system * to_vfs		=	VOSTOK_NEW_IMPL(args.allocator, virtual_file_system);
	...
	R_ASSERT_U								(to_result == result_success);
 	...
 	virtual_file_system	* patch_vfs		=	VOSTOK_NEW_IMPL(args.allocator, virtual_file_system);
	...
	VOSTOK_DELETE_IMPL						(args.allocator, from_vfs);
	VOSTOK_DELETE_IMPL						(args.allocator, to_vfs);
	VOSTOK_DELETE_IMPL						(args.allocator, patch_vfs);

	VOSTOK_FREE_IMPL							(args.allocator, to_vfs_info_buffer);
	VOSTOK_FREE_IMPL							(args.allocator, from_vfs_info_buffer);
