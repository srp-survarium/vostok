...
#include "pch.h"
#include <vostok/vfs/vfs_virtual_file_system.h>
#include <vostok/linkage_helper.h>
#include <vostok/fs/fs_device_utils.h>
#include <vostok/vfs/vfs_mount_history.h>
#include "vfs_saving.h"
#include "vfs_mount_archive.h"
#include <vostok/vfs/vfs_watcher.h>
#include "vfs_mount_physical_path.h"
#include "vfs_branch_locks.h"
#include "vfs_find.h"
#include "vfs_mount_archive.h"
#include "vfs_unmount.h"
#include "vfs_mount_physical_path.h"

VOSTOK_INCLUDE_TO_LINKAGE					(vfs_test);
...
bool   virtual_file_system::try_reference_to_pending_mount_unsafe (query_mount_arguments & args, bool * out_of_memory)
{
	* out_of_memory						=	false;
	...
	mount_referer * referer				=	VOSTOK_NEW_WITH_CHECK_IMPL(args.allocator, referer, mount_referer);
	if ( !referer )
	{
		* out_of_memory					=	true;
		return								false;
...
void   virtual_file_system::query_mount_impl (query_mount_arguments & args)
{
	...
	pending_mount_list::mutex_raii	raii	(pending_mounts);
	...
	archive_mounter	* mounter			=	VOSTOK_NEW_WITH_CHECK_IMPL(args.allocator, mounter, archive_mounter)(args, * this);
	if ( !mounter )
	{
...
struct filter_by_descriptor
{
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS		(physical_path, virtual_path);
		return								!descriptor || 
											!* descriptor || 
...
void   on_mounted						(vfs::mount_result result, vfs::mount_result * out_result, bool * finished)
{
	* out_result						=	result;
	* finished							=	true;
}
...
void   destroy_temp_physical_node		(base_node<> * node)
{
	...
	VOSTOK_FREE_IMPL							(allocator, mount_root);
}

static void on_hot_mounted				(vfs_locked_iterator *					out_iterator, 
										 ...
										 memory::base_allocator *				allocator)
{
	...
	query_hot_mount_and_wait				(file_system, physical_path, & virtual_path, out_iterator, allocator);
}
