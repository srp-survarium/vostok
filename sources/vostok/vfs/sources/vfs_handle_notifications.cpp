...
struct query_notification_operation : core::noncopyable
{
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER			(descriptor);
	...
	bool   try_lock_mount_root			()
	{
		virtual_path_string					path_to_mount_root;
		node_cast<base_node>(m_mount_root)->get_full_path	(& path_to_mount_root);

		if ( !m_file_system.hashset.find_and_lock_branch(m_lock_node, 
											 			 path_to_mount_root.c_str(), 
														 lock_type_write, 
														 m_lock_operation) )
		{
