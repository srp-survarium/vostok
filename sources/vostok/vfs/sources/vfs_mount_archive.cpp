...
#include <vostok/fs/fs_synchronous_device_interface.h>
#include "vfs_fat_header.h"
	...
	:	mounter(args, vfs_data), m_mount_root_base(NULL), m_reverse_byte_order(false), m_nodes_buffer(NULL), m_branch_lock(NULL)
{
	...
	if ( m_args.submount_node )
	{
		...
		m_mount_ptr->parent				=	parent_mount;
	}
												 ...
												 synchronous_device_interface &		device)
{
	...
	m_nodes_buffer						=	(pstr)VOSTOK_MALLOC_IMPL(m_args.allocator, header.buffer_size + 1, "archive nodes");
	if ( !m_nodes_buffer )
	{
