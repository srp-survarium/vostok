...
#include "pch.h"
#include "vfs_find.h"
#include "vfs_find_async_callbacks.h"
...
void	free_nodes_to_expand			(node_to_expand_list & list, memory::base_allocator * allocator)
{
	while ( !list.empty() )
	{
		...
		VOSTOK_DELETE_IMPL					(allocator, node);
	}
}
													 ...
													 u32						increment)
{
						...
						it_node			=	it_node->get_next_overlapped() )
	{
		...
		if ( it_node->is_sub_fat() || need_physical_mount(& is_out_of_memory, it_node, find_flags, traverse_type, allocator) )
		{
			...
			node_to_expand * new_item	=	VOSTOK_NEW_WITH_CHECK_IMPL(allocator, new_item, node_to_expand) 
...
{
	async_callbacks_data * async_data	=	(async_callbacks_data *)VOSTOK_MALLOC_IMPL(env.allocator, sizeof(async_callbacks_data) + 
