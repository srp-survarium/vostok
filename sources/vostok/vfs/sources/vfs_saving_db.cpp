										 ...
										 memory::base_allocator *		allocator)
{
	...
	out_allocated_nodes_buffer			=	VOSTOK_ALLOC_IMPL(allocator, char, allocator_for_info_size);
...
bool   archive_saver::save_db			()
{
	...
	pstr dest_buffer					=	VOSTOK_ALLOC_IMPL(m_args.allocator, char, max_buffer_size);
	...
	VOSTOK_FREE_IMPL							(m_args.allocator, dest_buffer);

	if ( m_root_save_db )
		m_env.node_info_set->clear			();

	if ( info_buffer )
		VOSTOK_FREE_IMPL						(m_args.allocator, info_buffer);
