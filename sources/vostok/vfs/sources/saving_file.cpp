									  	 ...
									  	 u32								sub_fat_size)
{
	...
	* out_pos							=	device->tell(m_env.db_file);
		 ...
		 !node->has_flags(vfs_node_is_compressed) )
	{
		...
		pstr src_data					=	(pstr)VOSTOK_ALLOC_IMPL(* m_args.allocator, char, file_size);
		u32 const dest_size				=	128 + (2 * file_size);
		pstr dest_data					=	(pstr)VOSTOK_ALLOC_IMPL(* m_args.allocator, char, dest_size);
		...
		VOSTOK_FREE_IMPL						(* m_args.allocator, src_data);
		VOSTOK_FREE_IMPL						(* m_args.allocator, dest_data);
	...
	{
		* out_inlined_data				=	memory::buffer((pstr)VOSTOK_ALLOC_IMPL(* m_args.allocator, char, file_size), file_size);
