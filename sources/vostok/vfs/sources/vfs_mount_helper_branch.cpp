										 ...
										 base_folder_node<> *			parent)
{
	...
	if ( node_is_submount_root )
	{
		...
		if ( !overlapper && m_args.root_write_lock == overlapped )
		{
			...
			R_ASSERT						(parent->has_lock(lock_type_write));
			parent->upgrade_lock			(lock_type_write, lock_type_soft_write);
		}
...
bool   mounter::allocate_mount_branch	(buffer_vector< mount_helper_node<> * > * out_helper_nodes)
{
								...
								it		!=	it_end; )
	{
		...
		mount_helper_node<> * const new_node	=	(mount_helper_node<> *)VOSTOK_MALLOC_IMPL(m_args.allocator, helper_node_size, "mount_helper_node");
		if ( !new_node )
		{
			out_of_memory				=	true;
			break;
...
void   mounter::free_mount_branch		(buffer_vector< mount_helper_node<> * > & helper_nodes)
{
	for ( u32 i=0; i<helper_nodes.size(); ++i )
		if ( helper_nodes[i] )
			VOSTOK_FREE_IMPL					(m_args.allocator, helper_nodes[i]);
}
