...
#include <vostok/vfs/vfs_mount_args.h>
#include "vfs_mount_helper_node.h"
#include "vfs_branch_locks.h"
#include "vfs_mount_utils.h"
#include <vostok/vfs/vfs_virtual_file_system.h>
#include <vostok/fs/fs_path_string_utils.h>
#include <vostok/fs/fs_path_part_iterator.h>
#include <vostok/fs/fs_path_string_utils.h>
#include <vostok/fs/fs_physical_path_iterator.h>
	...
	: mounter(args, vfs_data), m_device(NULL)
{
	...
	if ( try_find_in_mount_history() )
		return;

	if ( args.submount_node == NULL )	// if root physical mount?
	{		
		vfs_mount * mount_ptr_raw		=	VOSTOK_NEW_WITH_CHECK_IMPL(m_args.allocator, mount_ptr_raw, vfs_mount)(NULL, m_args.allocator);
		if ( !mount_ptr_raw )
		{
...
#pragma message(VOSTOK_TODO("how about checking for lazy mount ? mount_flags_enum"))
																	 ...
																	 native_path_string &		absolute_path)
{
	...
	ASSERT									(folder_size);
	mutable_buffer children_arena			(VOSTOK_MALLOC_IMPL(m_args.allocator, folder_size, "children_nodes_arena"), folder_size);
	if ( !children_arena )
	{
		m_result						=	result_out_of_memory;
		free_node_list						(new_nodes);
	...
	while ( !new_nodes.empty() )
	{
		...
		if ( source_node->is_file() )
		{
			...
			VOSTOK_FREE_IMPL					(m_args.allocator, source_file);
		}
		else
		{
			...
			VOSTOK_FREE_IMPL					(m_args.allocator, source_folder);
		}
