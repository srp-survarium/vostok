...
void   create_to_db						()
{
	...
	virtual_file_system	* to_vfs		=	VOSTOK_NEW_IMPL(m_allocator, virtual_file_system);
	...
	VOSTOK_DELETE_IMPL						(m_allocator, to_vfs);
}
...
void   create_random_patch_resources	()
{
	...
	for ( u32 i=0; i<files_total; ++i )
	{
		...
		if ( m_synchronous_device->get_physical_path_info(child_from).is_valid() )
			continue;
		if ( m_synchronous_device->get_physical_path_info(child_to).is_valid() )
			continue;
...
void   vfs_test::test_patch				()
{
	...
	{
		LOG_INFO							(logging::log_force_verbosity, "VFS:PATCH TEST ITERATION %d", i);
