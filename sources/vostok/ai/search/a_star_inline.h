...
#ifndef VOSTOK_AI_SEARCH_A_STAR_INLINE_H_INCLUDED
#define VOSTOK_AI_SEARCH_A_STAR_INLINE_H_INCLUDED
...
inline void a_star::on_before_search(
	...
	)
{
	...
	for ( u32 i = 0; i < vertices_count; ++i )
	{
		...
		start.h()					= heuristics.estimate( 0, start_vertex_id );
		start.g()					= distance_type( 0 );
...
TEMPLATE_SPECIALIZATION
inline void a_star::on_after_search	(
		priority_queue_type&		queue,
		graph_wrapper_type&			graph,
		path_constructor_type&		path,
		path_heuristics_type&		heuristics,
...
TEMPLATE_SPECIALIZATION
inline void a_star::process_neighbour	(
		priority_queue_type&		queue,
		graph_wrapper_type&			graph,
		path_constructor_type&		path,
		path_heuristics_type&		heuristics,
	...
	)
{
	...
	if ( !queue.visited( neighbour_id ) )
	{
		...
		neighbour.h()				= heuristics.estimate( &best.id(), neighbour_id );
		neighbour.g()				= heuristics.evaluate( best, neighbour, i );
	...
	{
		if ( heuristics.metric_euclidian() )
			return;

		distance_type const& g		= heuristics.evaluate( best, neighbour, i );
		if ( neighbour.g() <= g )
			return;

		neighbour.g()				= g;
		path.assign_parent			( neighbour, &best, graph.edge_id( best.id(), i ) );
		path.update_successors		( neighbour );
		return;
	}
...
{
	typedef typename priority_queue_type::vertex_type			vertex_type;
	typedef typename graph_wrapper_type::const_edge_iterator	const_edge_iterator;
...
#endif // #ifndef VOSTOK_AI_SEARCH_A_STAR_INLINE_H_INCLUDED