...
disturbance_target_selector::disturbance_target_selector( ai_world& world, working_memory const& memory, blackboard& board, pcstr name ) :
...
{
	// after every 1000 milliseconds confidence of memory object must be considered as decreased by 0.03
	return object->confidence - ( ( world.get_current_time_in_ms() - object->update_time ) * 0.03f / 1000.f );
}
...
void disturbance_target_selector::tick			( )
{
	find_highest_confidence_predicate			find_predicate( m_world );
	m_working_memory.for_each					( percept_memory_object_type_disturbance, find_predicate );
	
	if ( percept_memory_object* memory_object = find_predicate.object_with_highest_confidence )
	{
		m_blackboard.set_current_disturbance	( c_ptr( memory_object->object ) );
	}
}

planning::object_type disturbance_target_selector::get_target	( u32 const target_index )
{
	VOSTOK_UNREFERENCED_PARAMETER					( target_index );
	return										std::make_pair( pcvoid( 0 ), "" );
}

pcstr disturbance_target_selector::get_target_caption( u32 const target_index ) const
{
	VOSTOK_UNREFERENCED_PARAMETER					( target_index );
	return										"";
}

void disturbance_target_selector::dump_state	( npc_statistics& stats ) const
{
	VOSTOK_UNREFERENCED_PARAMETER					( stats );
}

void disturbance_target_selector::fill_targets_list	( )
{
}

void disturbance_target_selector::clear_targets		( )
{
}
