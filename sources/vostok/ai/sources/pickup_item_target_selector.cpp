...
pickup_item_target_selector::pickup_item_target_selector( ai_world& world, working_memory const& memory, blackboard& board, pcstr name ) :
...
{
	// after every 1000 milliseconds confidence of memory object must be considered as decreased by 0.08
	return object->confidence - ( ( world.get_current_time_in_ms() - object->update_time ) * 0.08f / 1000.f );
}
...
void pickup_item_target_selector::tick			( )
{
	find_highest_confidence_predicate			find_predicate( m_world );
	m_working_memory.for_each					( percept_memory_object_type_pickup_item, find_predicate );
	
	if ( percept_memory_object* memory_object = find_predicate.object_with_highest_confidence )
	{
		m_blackboard.set_current_pickup_item	( c_ptr( memory_object->object ) );
	}
}

planning::object_type pickup_item_target_selector::get_target	( u32 const target_index )
{
	VOSTOK_UNREFERENCED_PARAMETER					( target_index );
	return										std::make_pair( pcvoid( 0 ), "" );
}

void pickup_item_target_selector::fill_targets_list		( )
{
}

void pickup_item_target_selector::clear_targets			( )
{
}

pcstr pickup_item_target_selector::get_target_caption	( u32 const target_index ) const
{
	VOSTOK_UNREFERENCED_PARAMETER					( target_index );
	return										"";
}

void pickup_item_target_selector::dump_state	( npc_statistics& stats ) const
{
