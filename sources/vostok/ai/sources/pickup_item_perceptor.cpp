...
pickup_item_perceptor::pickup_item_perceptor	( npc& npc, brain_unit& brain, working_memory& memory )
	:	perceptor_base							( npc, brain, memory ),
		m_subscription							( boost::bind( &pickup_item_perceptor::on_sensed_object_retrieval, return_this(), _1 ) )
{
	m_brain_unit.subscribe_on_sensors_events	( m_subscription );
}

pickup_item_perceptor::~pickup_item_perceptor	( )
{
	m_brain_unit.unsubscribe_from_sensors_events( m_subscription );
}

inline pickup_item_perceptor* pickup_item_perceptor::return_this	( )
{
	return this;
}

void pickup_item_perceptor::on_sensed_object_retrieval	( sensors::sensed_object const& memory_object )
{
	...
	{
		case sensors::sensed_object_type_interaction :
		{
			if ( memory_object.confidence == 1.f )
				m_working_memory.forget_all_about_object( *memory_object.object );
			
			percept_memory_object* memory_fact	= m_working_memory.create_memory_object( percept_memory_object_type_pickup_item );
