...
using vostok::network::client_impl;
using boost::asio::ip::tcp;

client_impl::client_impl			( boost::asio::io_service& io_service, vostok::network::network_world& world ) :
	m_socket						( io_service ),
	...
	)
{
	...
	if ( error_code ) {
		...
		VOSTOK_DELETE_IMPL			( g_allocator, resolver );
		m_connection_state			= host_name_is_unresolved;
...
void client_impl::send				( vostok::network::packet const& packet )
{
	m_packet_socket.send			( packet );
}

void client_impl::on_error			( vostok::network::client_error_codes_enum const client_error_code, boost::system::error_code const error_code )
{
	m_connection_state				= host_name_is_unresolved;
	if ( m_on_error )
