...
using vostok::network::client;

client::client						( vostok::network::world& world ) :
...
void client::destroy_client			( vostok::network::client_impl* client_to_destroy )
{
...
void client::send					( vostok::network::packet const* packet )
{
	m_world.add_order	(
		VOSTOK_NEW_IMPL( m_world.orders_allocator(), send_order ) (
			boost::bind( &vostok::network::client_impl::send, m_client, _1 ),
			boost::cref( *packet ),
			boost::ref(m_world.orders_allocator())
		)
	);
...
void client::on_packet_received_impl( vostok::network::packet const& packet )
{
...
void client::on_packet_received		( vostok::network::packet const& packet )
{
	if ( !m_on_packet_received )
		return;

	vostok::network::packet* cloned_packet	= m_world.new_packet( );
