...
#include <vostok/network/server.h>	// for connection_action_enum only

using vostok::network::server_impl;
using boost::asio::ip::tcp;

server_impl::server_impl						( boost::asio::io_service& io_service, vostok::network::network_world& world ) :
...
server_impl::~server_impl						( )
{
	for ( ; m_first_client; ) {
		...
		VOSTOK_DELETE_IMPL			( g_allocator, m_first_client );
		m_first_client				= next_client_session;
	}
}

void server_impl::delete_client_session			( vostok::network::client_session* client_session )
{
	VOSTOK_DELETE_IMPL				( g_allocator, client_session );
...
void server_impl::process_client				(
		vostok::network::client_session* const new_client,
...
void server_impl::send_broadcast				( vostok::network::packet const& packet )
{
	for ( client_session* i = m_first_client; i; i = i->next )
		(*i).send					( packet );
}