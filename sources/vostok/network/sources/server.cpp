...
using vostok::network::server;

server::server							( vostok::network::world& world ) :
...
void server::on_packet_received_impl	( vostok::network::client_session& client, vostok::network::packet const& packet )
{
...
void server::on_packet_received			( vostok::network::client_session& client, vostok::network::packet const& packet )
{
	if ( !m_on_packet_received )
		return;
...
vostok::network::connection_action_enum server::on_connection_requested_impl	( vostok::network::client_session& client )
{
...
vostok::network::connection_action_enum server::on_connection_requested		( vostok::network::client_session& client )
{
	return							permit_connection;
	//if ( !m_on_connection_requested )
	//	return						permit_connection;
...
void server::on_disconnected_impl	( vostok::network::client_session& client )
{
	if ( m_on_disconnected )
		m_on_disconnected			( client );
}

void server::on_disconnected		( vostok::network::client_session& client )
{
	if ( !m_on_disconnected )
		return;
...
void server::destroy_server				( vostok::network::server_impl* server_to_destroy )
{
	VOSTOK_DELETE_IMPL				( g_allocator, server_to_destroy );
}
...
void server::send						( vostok::network::client_session& client, vostok::network::packet const* const packet )
{
	m_world.add_order				(
		VOSTOK_NEW_IMPL( m_world.orders_allocator(), send_order ) (
			boost::bind( &vostok::network::client_session::send, &client, _1 ),
			boost::cref( *packet ),
...
void server::send_broadcast				( vostok::network::packet const* const packet )
{
	m_world.add_order				(
		VOSTOK_NEW_IMPL( m_world.orders_allocator(), send_order ) (
			boost::bind( &vostok::network::server_impl::send_broadcast, m_server, _1 ),
			boost::cref( *packet ),
			boost::ref( m_world.orders_allocator() )
		)
	);
