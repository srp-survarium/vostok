...
namespace
{
  ...
  public:
    ...
    vostok::network::std_string    message( int ev ) const;
  };
  ...
  vostok::network::std_string generic_error_category::message( int ev ) const
  {
    static vostok::network::std_string unknown_err( "Unknown error" );
  ...
  # if defined(BOOST_WINDOWS_API) || defined(__hpux) || defined(__sun)\
     ...
     || (defined(__QNXNTO__))
      ...
      return  c_str
        ? vostok::network::std_string( c_str )
        : unknown_err;
  # else  // use strerror_r
      char buf[64];
      char * bp = buf;
  ...
  #  endif
          ...
          if ( (bp = static_cast<char*>(std::malloc( sz ))) == 0 )
            return vostok::network::std_string( "ENOMEM" );
        }
      }
      vostok::network::std_string msg;
      try
      {
...
# if !defined( BOOST_WINDOWS_API )
  ...
  vostok::network::std_string system_error_category::message( int ev ) const
  {
...
# if !defined(BOOST_NO_ANSI_APIS) && !defined(_XBOX)
    ...
    if (retval == 0)
        return vostok::network::std_string("Unknown error");
        
    vostok::network::std_string str( static_cast<LPCSTR>(lpMsgBuf) );
    ::LocalFree( lpMsgBuf ); // free the buffer
# elif defined(_XBOX)
	char tmp_buff[64];
	sprintf(tmp_buff, "unknown system error: %08x", ev);
	vostok::network::std_string str(tmp_buff);
# else  // WinCE workaround
    ...
    vostok::network::std_string str( narrow_buffer );
    ::LocalFree( lpMsgBuf ); // free the buffer
# endif
    while ( str.size()
      && (str[str.size()-1] == '\n' || str[str.size()-1] == '\r') )
