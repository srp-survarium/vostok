...
public:
	...
	virtual	~input_engine( );
			...
			void register_action	( vostok::editor::input_action^  action );

			// Registers an input action with keys combination
			void register_action	( vostok::editor::input_action^  action, System::String^ keys );
...
//			int	get_current_keys_string	( System::String^& keys);
			...
			void load					( /*vostok::configs::lua_config_value const& cfg, bool reset_all */);
			void save					( /*vostok::configs::lua_config_value const& cfg*/ );
private:
			//
...
private:
	...
	{
	public:
		action_item( 	vostok::editor::input_action^	action,	
		...
		vostok::editor::input_action^	m_action;
	};
