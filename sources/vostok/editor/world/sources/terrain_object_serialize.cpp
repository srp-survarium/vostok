...
// compress it!
	...
	u8* const compressed_buff	= (u8*)VOSTOK_MALLOC_IMPL(debug::g_mt_allocator, len, "compressed terrain" );
	...
	w.close_chunk				( );
	VOSTOK_FREE_IMPL				( debug::g_mt_allocator, compressed_buff );
...
void terrain_node::load_vertices_from_reader( memory::reader& F )
{
	...
	VOSTOK_UNREFERENCED_PARAMETERS(phys_size);
...
//			generate_for_all = (res==System::Windows::Forms::DialogResult::OK);
	...
	if(chunk.chunk_exists(render::model_chunk_terrain_data_compressed1))
	{
		...
		u8* uncompressed_buffer	= (u8*)VOSTOK_MALLOC_IMPL(debug::g_mt_allocator, uncompressed_max_size, "uncompressed terrain");
...
//.		R_ASSERT(d_stream.total_out==uncompressed_max_size);
		...
		VOSTOK_FREE_IMPL(debug::g_mt_allocator, uncompressed_buffer);
	}else
	{
