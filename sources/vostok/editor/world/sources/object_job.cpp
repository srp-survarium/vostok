...
object_job::object_job( tool_scripting^ t )
	:super(t)
{
	image_index				= vostok::editor_base::npc_tree_icon;
...
void object_job::load_props( configs::lua_config_value const& t )
{
	...
	if ( t.value_exists("job_resource") && !has_static_resource( ) ){
		if ( parent_composite == nullptr )	
			tool->get_level_editor()->get_project()->query_project_item(gcnew System::String(t["job_resource"]), gcnew vostok::editor::queried_object_loaded(this, &object_job::on_job_resource_loaded) );
		else{
...
//on_parent_scene_changed += gcnew vostok::editor::on_parent_scene_changed( this, &object_job::add_cached_links );
}
...
void object_job::initialize_logic_view		( )
{
	...
	for each( job_behaviour_base^ behaviour in m_behaviours_list ){
		...
		prop_descriptor->dynamic_attributes->add( gcnew vostok::editor::wpf_controls::hypergraph::node_property_attribute( false, false ));
