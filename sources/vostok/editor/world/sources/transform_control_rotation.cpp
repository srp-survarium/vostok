...
using namespace vostok::memory;


using vostok::editor::editor_control_base;
using vostok::editor::transform_control_rotation;
using vostok::editor::transform_control_base;

using vostok::editor::collision_object_transform_control;
using vostok::editor::collision_object;
using vostok::editor::enum_transform_axis;
using vostok::editor::enum_transform_mode;
...
transform_control_rotation::transform_control_rotation( vostok::editor::level_editor^ le)
:super						( le ),
m_radius					( 4.5f ),
...
m_last_pos_segment_step		( vostok::math::pi/16 ),
m_outer_ring				( NULL ),
m_current_angle				( 0 ),
m_initialized				( false ),
...
void transform_control_rotation::initialize		( )
{
	...
	vostok::non_null<collision::geometry>::ptr geom = collision::create_triangle_mesh_geometry( g_allocator, (float3*)&vertices[0], vertices.size(), &collision_indices[0], collision_indices.size() );
...
void transform_control_rotation::draw					( vostok::math::float4x4 const& view_matrix )
{
	...
	vostok::render::editor::renderer& rend = m_level_editor->get_editor_renderer();
...
void transform_control_rotation::draw_axis_ellipse		( vostok::math::float4x4 const& draw_transform, float3 rotation, enum_transform_axis axis, bool strip_lines )
{
	vostok::render::editor::renderer& rend = m_level_editor->get_editor_renderer();
...
void transform_control_rotation::draw_inner_outer_lines ( vostok::math::float4x4 const& translation_draw_transform,  vostok::math::float4x4 const& rotate_to_view, bool strip_lines )
{
	vostok::render::editor::renderer& rend = m_level_editor->get_editor_renderer();
...
void transform_control_rotation::draw_last_position			( vostok::math::float4x4 const& view_matrix, bool transparent_mode )
{
