...
using vostok::editor::object_base;
using vostok::editor::object_compound;
using vostok::editor::tool_compound;
...
void object_compound::load(vostok::configs::lua_config_value const& t)
{
	super::load					(t);

	(*m_pivot_offset)			= t["pivot_offset"];
	
	vostok::configs::lua_config_value t_objects = t["objects"];
	...
	for(; it!=it_e; ++it)
	{
		vostok::configs::lua_config_value current = *it;
...
void object_compound::save(vostok::configs::lua_config_value t)
{
	super::save						(t);
	...
	for each (object_base^ o in m_objects)
	{
		vostok::configs::lua_config_value current = t_objects[i];
		o->save						(current);
		++i;
	}
}
...
void object_compound::initialize_collision()
{
	...
	non_null<vostok::collision::geometry>::ptr geom = collision::create_compound_geometry(g_allocator, vec);
	m_collision.create_from_geometry	(this, &*geom, vostok::editor::collision_type_dynamic);
	m_collision.insert					();
...
void object_compound::on_child_object_collision_initialized(vostok::editor::object_collision^)
{
...
{
	VOSTOK_UNREFERENCED_PARAMETER		( m );
