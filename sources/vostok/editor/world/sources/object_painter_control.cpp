...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( b_activate );
}

void object_painter_control::update( )
{}

void object_painter_control::start_input( int mode, int button, collision::object const* coll, vostok::math::float3 const& pos )
{
...
void object_painter_control::draw(render::scene_view_pointer scene, vostok::render::render_output_window_pointer )
{
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( key );
}
...
void painted_object::reset( )
{
	...
	VOSTOK_FREE_IMPL( debug::g_mt_allocator, m_vertices );
	m_vertices			= NULL;
	VOSTOK_FREE_IMPL( debug::g_mt_allocator, m_paint_data );
	m_paint_data		= NULL;
}
...
void painted_object::on_model_loaded(  vostok::resources::queries_result& data )
{
