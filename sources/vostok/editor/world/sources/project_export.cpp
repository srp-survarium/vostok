...
void on_vertices_loaded( resources::queries_result& data, 
						vostok::geometry_utils::geometry_collector* collector, 
...
void on_model_folder_fs_iterator_ready( vfs::vfs_locked_iterator const & in_fs_it, 
									   vostok::geometry_utils::geometry_collector* collector, pcstr model_path, float4x4 const* m, bool is_skeleton )
{
	if( in_fs_it.is_end() )
		return;
...
void export_model_as_obj( project^ p, 
							...
							bool is_skeleton )
{
	VOSTOK_UNREFERENCED_PARAMETERS( p );
