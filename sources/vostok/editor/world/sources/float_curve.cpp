...
using vostok::editor::float_curve;
using vostok::editor::float_curve_pair;
...
float_curve::float_curve(vostok::configs::lua_config_value curve_config)
:m_min_time(0.0f),
m_max_time(100.0f),
m_min_value(0.0f),
m_max_value(100.0f),
m_initialized_by_config(true)
{
	...
	m_curve_config = NEW(vostok::configs::lua_config_value)(curve_config);
...
void float_curve::load_keys_from_config(vostok::configs::lua_config_value keys_config){
...
void float_curve::save_keys_to_config		(vostok::configs::lua_config_value keys_config){
	Int32 i = 0;
	for each(float_curve_pair^ p in keys){
		vostok::configs::lua_config_value key = keys_config[i];
		key["key"] = p->time;
		key["value"] = p->value;
		i++;
	}
...
void		float_curve::save(vostok::configs::lua_config_value config){
 	config["min_time"]		= m_min_time;
 	config["max_time"]		= m_max_time;
 	config["min_value"]		= m_min_value;
 	config["max_value"]		= m_max_value;
	
	vostok::configs::lua_config_value keys_config = config["keys"];
	
	save_keys_to_config		(keys_config);
	
}

void		float_curve::load(vostok::configs::lua_config_value config){
...
//--- color curve
using vostok::editor::color_curve;
using vostok::editor::color_curve_pair;
...
color_curve::color_curve(vostok::configs::lua_config_value curve_config)
:m_min_time(0.0f),
m_max_time(100.0f),
m_initialized_by_config(true)
{
...
void		color_curve::save(vostok::configs::lua_config_value config){
	config["min_time"]		= m_min_time;
	config["max_time"]		= m_max_time;
...
void		color_curve::load(vostok::configs::lua_config_value config){
	m_min_time		= config["min_time"];		
	m_max_time		= config["max_time"];		
...
void color_curve::load_keys_from_config(vostok::configs::lua_config_value keys_config){
	configs::lua_config_value::iterator begin	= keys_config.begin();
	configs::lua_config_value::iterator end		= keys_config.end();
...
void color_curve::save_keys_to_config		(vostok::configs::lua_config_value keys_config){
	Int32 i = 0;
