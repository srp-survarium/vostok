...
void object_proximity_trigger::load_props ( configs::lua_config_value const& config_value )
{
	...
	if ( selected_collision_geometry != "")
		owner_tool( )->get_level_editor( )->get_project( )->query_project_item( selected_collision_geometry, gcnew vostok::editor::queried_object_loaded( this, &object_proximity_trigger::collision_geometry_loaded ) );
}
...
void object_proximity_trigger::select_collision_geometry	( wpf_controls::property_editors::property^ prop , Object^ )
{
	...
	bool result = editor_base::resource_chooser::choose(
		"project_item",
		collision_geometry,
		"vostok.editor.object_collision_geometry",
		selected,
		true
	);
	if ( result )
