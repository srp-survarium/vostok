...
object_speedtree::object_speedtree( tool_speedtree^ t, render::scene_ptr const& scene )
	...
	m_model_query	( NULL )
{
	image_index					= vostok::editor_base::node_speedtree;
	m_speedtree_instance_ptr	= NEW(vostok::render::speedtree_instance_ptr)();
}
...
void object_speedtree::on_resource_loaded( vostok::resources::queries_result& data )
{
	m_model_query = NULL;
	if (data[0].is_successful())
	{
		*m_speedtree_instance_ptr = static_cast_checked<vostok::render::speedtree_instance*>( data[0].get_unmanaged_resource().c_ptr() );
