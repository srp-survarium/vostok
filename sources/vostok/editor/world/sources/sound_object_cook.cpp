...
#pragma managed(push)
#pragma unmanaged
#include <vostok/core/sources/resources_manager.h>
#include <vostok/core/sources/resources_managed_allocator.h>
#pragma managed(pop)
...
using vostok::editor::raw_file_property_struct;
using vostok::editor::sound_editor;
using vostok::editor::unmanaged_string;
using vostok::resources::fs_iterator;

#pragma message(VOSTOK_TODO("fix cooks"))
...
namespace vostok
{
	namespace resources
	{
		...
		{
			VOSTOK_UNREFERENCED_PARAMETER(data_raw);
...
void				ogg_sound_cook_wrapper::on_fs_iterator_ready		(fs::path_string* resource_path, query_result_for_cook* parent, vostok::resources::fs_iterator fs_it)
{
	//
	...
	vostok::resources::fs_iterator fs_it_converted_options	= fs_it.find_child(converted_options_path.c_str());
...
{
	VOSTOK_UNREFERENCED_PARAMETER	( final_managed_resource_size );
...
static void					make_sound_options(fs::path_string& end_file_path)
{
	...
	fixed_string512			cl_args;
	cl_args			+= "vostok_rms_generator-debug.exe";
	cl_args 		+= " -o=\"";
	cl_args 		+= end_file_path.c_str();
	cl_args 		+= "\" -i=\"";
