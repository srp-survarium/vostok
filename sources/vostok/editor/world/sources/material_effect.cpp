...
using vostok::editor::wpf_controls::property_property_value;
using vostok::editor::wpf_controls::property_editors::attributes::value_range_and_format_attribute;
using vostok::editor::wpf_controls::property_editors::attributes::external_editor_attribute;
using vostok::editor::wpf_controls::property_editors::attributes::external_editor_event_handler;
using vostok::editor::wpf_controls::color_rgb;
...
{
	if (config_root.get_type() != vostok::configs::t_table_indexed &&
		config_root.get_type() != vostok::configs::t_table_named)
		return;
...
{
	if (search_config.get_type() != vostok::configs::t_table_indexed &&
		search_config.get_type() != vostok::configs::t_table_named)
		return;
...
void material_effect::fill_effect_property_grid(wpf_property_container^ cont, configs::lua_config_value config_root, bool material_editor_mode)
{
	...
	for( ; it!=it_e; ++it )
	{
		...
		if (current.get_type() != vostok::configs::t_table_indexed &&
			current.get_type() != vostok::configs::t_table_named)
			return;
		...
		if (!current.value_exists("shader_parameter_id"))
		{
			...
			property_descriptor^ desc		= gcnew property_descriptor( category_name, gcnew vostok::editor::wpf_controls::object_property_value<vostok::editor::wpf_controls::i_property_container^>(inner_container) );
