...
using vostok::editor::wpf_controls::i_property_value;
	...
	m_editor_world(in_editor_world)
{
	...
	vostok::resources::request requests[2] = 
	{
...
void render_options_editor_resource::load_internal( )
{
	...
	configs::lua_config_value options_table				= (*m_options_config)->get_root()["options"];
 	...
 	vostok::configs::lua_config_value const& tempalate_options_table = m_options_template_config->c_ptr()->get_root()["options"];
	...
	for (configs::lua_config_value::iterator it = options_table.begin(); it != options_table.end(); ++it)
	{
		...
		vostok::configs::enum_types property_type				= (*it).get_type();
...
render_options_editor_resource::~render_options_editor_resource( )
{
	...
	for each (vostok::editor::wpf_controls::i_property_value^ p in m_properties.Values)
		delete p;
}
