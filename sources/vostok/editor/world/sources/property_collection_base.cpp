...
void property_collection_base::CopyTo					(Array^ items, int index)
{
	...
	for (int i=index, n=collection->size(); i<n ; ++i) {
		vostok::editor::property_holder*holder_raw = collection->item(i);
...
{
	vostok::editor::property_holder*	holder_raw = collection()->item(index);
	::property_holder*			holder = dynamic_cast<::property_holder*>(holder_raw);
...
{
	vostok::editor::property_holder*	holder_raw = collection()->create();
	::property_holder*			holder = dynamic_cast<::property_holder*>(holder_raw);
	return						(holder->container());
}