...
{
	for each(vostok::editor_base::command^ cmd in m_transform_commands)
		delete cmd;
...
bool terrain_modifier_height_command::end_preview()
{
	...
	if( m_control->affect_objects_snap )
	{
		...
		vostok::collision::space_partitioning_tree* tree	= m_control->m_level_editor->get_collision_tree();
