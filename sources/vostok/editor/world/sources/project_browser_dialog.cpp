...
void project_browser_dialog::in_constructor( )
{
	...
	m_image_list_large		= vostok::editor_base::image_loader::load_images("folder_icons64x64", "vostok.editor.resources", 64, safe_cast<int>(folders_icons_count), this->GetType()->Assembly);
	m_image_list_small		= vostok::editor_base::image_loader::load_images("folder_icons16x16", "vostok.editor.resources", 16, safe_cast<int>(folders_icons_count), this->GetType()->Assembly);
...
bool check_valid_vostok_project(System::String^ folder)
{
...
public:
	...
	{
		bool is_project_first			= check_valid_vostok_project( first );
		bool is_project_second			= check_valid_vostok_project( second );
...
void project_browser_dialog::fill_current_path( )
{
	...
	for each(System::String^ folder in folders)
	{
		...
		if(item->Length)
		{
			bool b_vostok_project			= check_valid_vostok_project( folder );
			System::DateTime dt			= System::IO::Directory::GetLastWriteTime( folder );
			if(b_vostok_project)
			{
				list_item = create_folder_item(item, dt.ToString(), "vostok project", vostok_project_folder_img);
				projects_list_view->Items->Add	( list_item);
			}else
			{
...
void project_browser_dialog::projects_list_view_ItemActivate(System::Object^, System::EventArgs^)
{
	...
	if(activated_item->ImageIndex==vostok_project_folder_img && ok_button->Enabled)
	{
...
void project_browser_dialog::selected_text_box_TextChanged(System::Object^, System::EventArgs^)
{
	...
	{ // loading mode
		...
		if(activated_item && activated_item->ImageIndex==vostok_project_folder_img)
			ok_button->Enabled		= true;
	}
}
	...
	{ // saving
		...
		{	// create question
			::DialogResult result = MessageBox::Show( this, "Project directory \n'" 
														+ fullpath
														+ "'\n does not exist. Create new?", "Vostok Editor"
