...
public:
	...
	action_control_step_modify		( System::String^ action_name, level_editor^ le, vostok::editor_base::enum_transform_axis, float value );

	virtual bool		enabled		( )	override;
	virtual bool		do_it		( ) override;

private:
	vostok::editor_base::enum_transform_axis		m_axis;
	float										m_value;
	level_editor^								m_level_editor;
}; // class action_control_properties
