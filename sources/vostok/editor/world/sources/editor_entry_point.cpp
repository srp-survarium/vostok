...
using vostok::editor::editor_world;

static bool					s_initialized = false;
static char					s_world_fake[ sizeof( editor_world ) ];
static editor_world&		s_world = ( editor_world& )s_world_fake;

vostok::editor::allocator_type*	vostok::editor::g_allocator = 0;

static vostok::editor::world* create_world		( vostok::editor::engine& engine, bool fpe_enabled )
{
#ifdef DEBUG
	enable_fpe				( fpe_enabled );
#else // #ifdef DEBUG
	VOSTOK_UNREFERENCED_PARAMETER	( fpe_enabled );
#endif // #ifdef DEBUG

	R_ASSERT				( !s_initialized );
	s_initialized			= true;
	
	typedef vostok::editor::editor_world	world;
	new ( &s_world ) world	( engine );

	static vostok::editor::animated_model_instance_cook s_animated_model_cook( &s_world.get_physics_world() );
	register_cook			( &s_animated_model_cook );

	return					( &s_world );
}

static void destroy_world						( vostok::editor::world*& world )
{
	R_ASSERT				( s_initialized );
...
static void enable_fpe							( bool const value )
{
	vostok::debug::enable_fpe	( value );
}
#endif // #ifdef DEBUG

vostok::editor::world* vostok::editor::create_world	( engine& engine )
{
#ifdef DEBUG
	bool const fpe_enabled	= vostok::debug::is_fpe_enabled( );
...
void vostok::editor::destroy_world				( vostok::editor::world*& world )
{
	::destroy_world			( world );
	::collect_garbage		( );
}

void vostok::editor::set_memory_allocator			( allocator_type& allocator )
{
	install_bugtrap			( );
