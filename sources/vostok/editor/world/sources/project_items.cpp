...
void project_item_folder::locked::set(bool v)
{
	...
	if(m_tree_node_)
	{
		m_tree_node_->ImageIndex			= m_locked? vostok::editor_base::node_group_locked : vostok::editor_base::node_group;
		m_tree_node_->SelectedImageIndex	= m_locked? vostok::editor_base::node_group_locked : vostok::editor_base::node_group;
	}
...
tree_node^ make_hierrarchy( tree_node_collection^ root, System::String^ name )
{
	...
	for(int i=0; i<tokens->GetLength(0); ++i)
	{
		...
		}else
		{
			...
			result						= gcnew vostok::editor::controls::tree_node(folder);
			result->ImageIndex			= vostok::editor_base::node_group;
			result->SelectedImageIndex	= vostok::editor_base::node_group;
			result->Name				= folder;
