...
namespace vostok
{
	namespace editor
	{
		...
		void				sound_files_source::on_fs_iterator_ready	(vostok::resources::fs_iterator fs_it)
		{
			process_fs(fs_it.children_begin(), "");
		}

		void				sound_files_source::process_fs			(vostok::resources::fs_iterator it, String^ file_path)
		{
			while( !it.is_end() )
			{
				if(it.is_folder())
