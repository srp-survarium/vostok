...
typedef vostok::editor::wpf_controls::select_behavior select_behavior;
...
object_decal::object_decal(tool_base^ t, render::scene_ptr const& scene):
	...
	m_cook_data		(NEW(vostok::render::material_effects_instance_cook_data*))
{
...
{
	VOSTOK_UNREFERENCED_PARAMETERS		(bdestroy);
...
void object_decal::requery_material()
{
	...
	*m_cook_data							= 
		NEW(vostok::render::material_effects_instance_cook_data)(
			vostok::render::decal_vertex_input_type,
			NULL,
			false,
			vostok::render::cull_mode_none
		);
