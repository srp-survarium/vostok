...
{	
	...
	resources::query_create_resource(
						"#editor_project",
						vostok::memory::buffer(str_buf, strings::length(str_buf)),
...
vostok::editor::project_ui project::ui( )
{
...
void project::create_new( )
{
	...
	const vostok::render::scene_ptr		scene = m_level_editor->get_editor_world().scene();
	const vostok::render::scene_view_ptr	scene_view = m_level_editor->get_editor_world().scene_view();
...
vostok::math::random32	uid_random	(GetTickCount());
...
//
//	vostok::resources::request  requests[] = { request_path.c_str(), vostok::resources::lua_config_class };
//
...
//
//	vostok::resources::query_resources(requests, 
//		boost::bind(&query_result_delegate::callback, q, _1),
//		g_allocator);
//}

//void project::on_project_links_loaded	(vostok::resources::queries_result& data)
//{
...
void project::fill_tree_view(tree_node_collection^ nodes)
{
	...
	if(!empty())
	{
		...
		m_root_folder->m_tree_node_->ImageIndex			= vostok::editor_base::default_object_set;
		m_root_folder->m_tree_node_->SelectedImageIndex	= vostok::editor_base::default_object_set;
...
{
	m_level_editor->ide()->Text	= m_project_name + (is_modified ? " (Modified)" : "") + " - X-Ray Editor (Build #" + vostok::build::build_station_build_id() + ", " + gcnew System::String( vostok::build::build_date() ) + ")";
}
