...
#include <vostok/editor/base/collision_object_types.h>
#include <vostok/ui/world.h>

#pragma managed( push, off )
#	include <vostok/animation/anim_track_common.h>
#	include <vostok/animation/anim_track.h>
...
typedef vostok::editor::wpf_controls::animation_curve_editor animation_curve_editor_control;
typedef vostok::editor::wpf_controls::property_container container; 
typedef vostok::editor::wpf_controls::float_curve float_curve;
typedef vostok::editor::wpf_controls::float_curve_key float_curve_key;
typedef vostok::editor::wpf_controls::curve_editor::visual_curve visual_curve;
typedef System::Windows::Point Point;
typedef System::Windows::Vector Vector;

using	vostok::editor::wpf_controls::curve_editor::panel_curve_group;
...
void			object_track::selected_to_camera			( )
{
	if( m_selected_key != -1 )
	{
		vostok::editor_base::scene_view_panel^ view_window = owner_tool( )->get_level_editor( )->view_window( );
...
{
	VOSTOK_UNREFERENCED_PARAMETER( key_index );
	update_collisions( );
}
...
{
	return math::float3(
		m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].value,
		m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].value,
		m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].value
		);
}
...
{
	return math::float3(
		m_track->m_channels[vostok::animation::channel_rotation_x]->keyList[idx].value,
		m_track->m_channels[vostok::animation::channel_rotation_y]->keyList[idx].value,
		m_track->m_channels[vostok::animation::channel_rotation_z]->keyList[idx].value
		);
}

float			object_track::key_time						( u32 idx )
{
	return m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].time;
}

curve_key_type	object_track::key_type						( vostok::animation::enum_channel_id channel, u32 idx )
{
	return (curve_key_type)m_track->m_channels[channel]->keyList[idx].type;
}

math::float2	object_track::channel_key_tangent			( vostok::animation::enum_channel_id channel, u32 idx, s32 tangent_idx )
{
	if( tangent_idx == -1 )
	{
...
void			object_track::fill_animation_curve_editor	( )
{
	...
	if( count == 0 )
	{
		...
		vostok::animation::EtCurve* time_channel		= m_track->get_channel( (vostok::animation::enum_channel_id)vostok::animation::anm_track::m_time_channel_id, false );
	...
	vostok::animation::EtCurve* time		= m_track->get_channel( (vostok::animation::enum_channel_id)vostok::animation::anm_track::m_time_channel_id, false );
	...
	{
		in_tangent					= channel_key_tangent( (vostok::animation::enum_channel_id)vostok::animation::anm_track::m_time_channel_id, i, -1 );
		out_tangent					= channel_key_tangent( (vostok::animation::enum_channel_id)vostok::animation::anm_track::m_time_channel_id, i, 1 );
	...
	for( int i = 0; i < count; ++i )
	{
		...
		in_tangent					= channel_key_tangent( vostok::animation::channel_translation_x, i, -1 );
		out_tangent					= channel_key_tangent( vostok::animation::channel_translation_x, i, 1 );

		x_curve->keys->Add			(	gcnew float_curve_key( Point( time, x_value ),
										key_type( vostok::animation::channel_translation_x, i ),
										Point( time, x_value ) + (Vector)Point( in_tangent.x, in_tangent.y ),
										Point( time, x_value ) + (Vector)Point( out_tangent.x, out_tangent.y ) ) );

		in_tangent					= channel_key_tangent( vostok::animation::channel_translation_y, i, -1 );
		out_tangent					= channel_key_tangent( vostok::animation::channel_translation_y, i, 1 );

		y_curve->keys->Add			(	gcnew float_curve_key( Point( time, y_value ),
										key_type( vostok::animation::channel_translation_y, i ),
										Point( time, y_value ) + (Vector)Point( in_tangent.x, in_tangent.y ),
										Point( time, y_value ) + (Vector)Point( out_tangent.x, out_tangent.y ) ) );

		in_tangent					= channel_key_tangent( vostok::animation::channel_translation_z, i, -1 );
		out_tangent					= channel_key_tangent( vostok::animation::channel_translation_z, i, 1 );

		z_curve->keys->Add			(	gcnew float_curve_key( Point( time, z_value ),
										key_type( vostok::animation::channel_translation_z, i ),
										Point( time, z_value ) + (Vector)Point( in_tangent.x, in_tangent.y ),
										Point( time, z_value ) + (Vector)Point( out_tangent.x, out_tangent.y ) ) );
		
		
		in_tangent					= channel_key_tangent( vostok::animation::channel_rotation_x, i, -1 );
		out_tangent					= channel_key_tangent( vostok::animation::channel_rotation_x, i, 1 );

		rx_curve->keys->Add			(	gcnew float_curve_key( Point( time, rx_value ),
										key_type( vostok::animation::channel_rotation_x, i ),
										Point( time, rx_value ) + (Vector)Point( in_tangent.x, in_tangent.y ),
										Point( time, rx_value ) + (Vector)Point( out_tangent.x, out_tangent.y ) ) );

		in_tangent					= channel_key_tangent( vostok::animation::channel_rotation_y, i, -1 );
		out_tangent					= channel_key_tangent( vostok::animation::channel_rotation_y, i, 1 );

		ry_curve->keys->Add			(	gcnew float_curve_key( Point( time, ry_value ),
										key_type( vostok::animation::channel_rotation_y, i ),
										Point( time, ry_value ) + (Vector)Point( in_tangent.x, in_tangent.y ),
										Point( time, ry_value ) + (Vector)Point( out_tangent.x, out_tangent.y ) ) );

		in_tangent					= channel_key_tangent( vostok::animation::channel_rotation_z, i, -1 );
		out_tangent					= channel_key_tangent( vostok::animation::channel_rotation_z, i, 1 );

		rz_curve->keys->Add			(	gcnew float_curve_key( Point( time, rz_value ), 
										key_type( vostok::animation::channel_rotation_z, i ),
...
void			object_track::init_collision				( )
{
	...
	m_collision->create_from_geometry( true, this, geom, vostok::editor_base::collision_object_type_dynamic | vostok::editor_base::collision_object_type_touch );
...
void			object_track::set_key_transform				( u32 idx, math::float4x4 const& t )
{
	...
	m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].value		= pos.x;
	m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].value		= pos.y;
	m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].value		= pos.z;
	...
	m_track->m_channels[vostok::animation::channel_rotation_x]->keyList[idx].value		= (float)-xPitch;
	m_track->m_channels[vostok::animation::channel_rotation_y]->keyList[idx].value		= (float)-yYaw;
	m_track->m_channels[vostok::animation::channel_rotation_z]->keyList[idx].value		= (float)zRoll;
...
math::float3	object_track::get_key_tangent_position		( int idx, int tangent_idx )
{
	...
	{
		return math::float3(
			m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].value + m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].inTanY,
			m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].value + m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].inTanY,
			m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].value + m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].inTanY
		);
	}
	else
	{
		return math::float3(
			m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].value + m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].outTanY,
			m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].value + m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].outTanY,
			m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].value + m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].outTanY
		);
	}
}

static void		fix_key_type								( vostok::animation::EtKey &key )
{
...
void			object_track::set_key_tangent_position		( int idx, int tangent_idx, math::float3 position )
{
	if( tangent_idx == -1 )
	{
		m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].inTanY = position.x - m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].value;
		m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].inTanY = position.y - m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].value;
		m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].inTanY = position.z - m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].value;
	}
	else
	{
		m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].outTanY = position.x - m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx].value;
		m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].outTanY = position.y - m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx].value;
		m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].outTanY = position.z - m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx].value;
	}

	fix_key_type( m_track->m_channels[vostok::animation::channel_translation_x]->keyList[idx] );
	fix_key_type( m_track->m_channels[vostok::animation::channel_translation_y]->keyList[idx] );
	fix_key_type( m_track->m_channels[vostok::animation::channel_translation_z]->keyList[idx] );
	...
	if( count != 0 )
	{
		...
		if( count > 1 )
		{
			...
			if( m_is_selected && show_frame_numbers )
			{
				...
				vostok::editor_base::scene_view_panel^ v          = owner_tool()->get_level_editor()->view_window();
				...
				vostok::ui::world* ui_world = m_tool_misc->get_level_editor( )->get_editor_world( ).get_ui_world( );
...
:super		( g_allocator, vostok::editor_base::collision_object_type_touch, geometry ),
m_owner		( o ),
m_key_idx	( key_idx )
{}
...
bool			collision_object_track_key::touch			( ) const
{
	if( !m_owner->is_selected	 )
		m_owner->get_project( )->select_id( m_owner->id( ), vostok::editor::enum_selection_method_set );
