...
static void get_children(particle_graph_node^ node, System::Collections::Generic::List<vostok::editor::controls::hypergraph::node^ >^ nods, System::Collections::Generic::List<vostok::editor::controls::hypergraph::node^ >^ p_nods){
...
:m_hypergraph(h)
{
	...
	while(begin != end)
	{
		...
		if (node_type == "action")
			new_node = gcnew particle_graph_node_action(vostok::editor_base::image_node_emitter, p_editor);
		if (node_type == "emitter")
			new_node = gcnew particle_graph_node_emitter(vostok::editor_base::image_node_emitter, p_editor);
		if (node_type == "event")
			new_node = gcnew particle_graph_node_event(vostok::editor_base::image_node_emitter, p_editor);
		if (node_type == "property")
			new_node = gcnew particle_graph_node_property(vostok::editor_base::image_node_emitter, p_editor);
 		new_node->load											(*begin);		
