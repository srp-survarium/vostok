...
#include <vostok/ai_navigation/sources/delaunay_triangulator.h>
...
using vostok::editor::triangulation_tester_canvas;
using vostok::ai::navigation::delaunay_triangulator;
using vostok::ai::navigation::constrained_delaunay_triangulator;
using vostok::ai::navigation::triangles_mesh_type;
using vostok::ai::navigation::fill_coordinate_indices;
...
triangulation_tester_canvas::triangulation_tester_canvas(void) :
	font("Aerial", 12 ),
	m_start( vostok::math::float2(0.0f, 0.0f ) ),
	m_scale( vostok::math::float2(1000000.0f, 1000000.0f ) ),
//	m_scale( vostok::math::float2(10000.0f, 10000.0f ) ),
	m_circles_scale( vostok::math::epsilon_5 ),
...
void triangulation_tester_canvas::build_lua_config ( vostok::configs::lua_config_ptr& config )
{
	config->get_root()["normal"] = float3( m_normal.x, m_normal.y, m_normal.z );
	vostok::configs::lua_config_value vertices_value = config->get_root()["vertices"];
...
void triangulation_tester_canvas::load_lua_config ( vostok::configs::lua_config_ptr& config )
{
	remove_triangulation( );
	m_vertices->clear();
	m_edges->clear();
	...
	if ( root.value_exists( "edges" ) ) {
		...
		for ( u32 i = 0; i < edges_count; ++i ) {
			...
			m_edges->push_back( vostok::ai::navigation::constrained_delaunay_triangulator::constraint_edge( edge_info[0], edge_info[1] ) );
		}
	}
