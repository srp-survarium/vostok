...
using		vostok::texture_compressor::TgaHeader;
...
static pvoid		tga_reader_allocate			( u32 size, bool is_additional_buffer )
{
	if(is_additional_buffer)
		return VOSTOK_MALLOC_IMPL( debug::g_mt_allocator, size, "tempprary for image");
...
{
	VOSTOK_FREE_IMPL( debug::g_mt_allocator, buffer);
}
