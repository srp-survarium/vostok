...
#include <vostok/editor/base/images/images104x8.h>
...
void particle_graph_node_emitter::in_constructor(){
	m_rect_images =	vostok::editor_base::image_loader::load_images("images104x8", 104, safe_cast<int>(vostok::editor_base::images104x8_count), this->GetType());	
...
void draw_nodes_container(Graphics^ g, int first_child_id, int last_child_id, particle_graph_node_collection^ children, ImageList^ images_list){
	...
	g->DrawImage(images_list->Images[vostok::editor_base::image_container_top], point_top);
	g->DrawImage(images_list->Images[vostok::editor_base::image_container_bottom], point_bottom);
	g->DrawImage(images_list->Images[vostok::editor_base::image_container_middle], destPara);
}
...
void particle_graph_node_emitter::OnPaint			(PaintEventArgs^ e)
{
	if(m_parent != nullptr)
	{
		...
		if (m_highlighted)
			e->Graphics->DrawImage(particle_node_images->Images[vostok::editor_base::image_node_highlited], Drawing::Point(0,0));
