...
vostok::render::texture_options::ETType get_texture_type_from_name(pcstr);
...
{
	m_filter_type = vostok::render::texture_options::ETType(-1);
}
	...
	while( !it.is_end() )
	{
		...
		}else
		{
			...
			if(extension == ".tga" )
			{
				...
				if(m_filter_type != vostok::render::texture_options::ETType(-1) )
				{
					if(get_texture_type_from_name(unmanaged_string(file_name).c_str()) == m_filter_type )
						m_parent->add_item	( file_path+file_name, vostok::editor_base::node_texture );
				}else
				{
					m_parent->add_item		( file_path+file_name, vostok::editor_base::node_texture );
				}
