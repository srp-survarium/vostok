...
{
	default_image_index	= vostok::editor_base::node_mesh;
...
int get_image_index( int const item_type )
{
	...
	case static_model_type:
		return vostok::editor_base::node_mesh;
		break;
	case skinned_model_type:
		return vostok::editor_base::node_joint;
		break;
	case composite_model_type:
		return vostok::editor_base::composite_tree_icon;
		break;
	default:
		NODEFAULT();
		return 0;
...
void tool_solid_visual::item_double_clicked( lib_item_base^ item )
{
	...
	vostok::editor_base::tool_window^ ed = get_level_editor()->get_editor_world().create_model_editor();
