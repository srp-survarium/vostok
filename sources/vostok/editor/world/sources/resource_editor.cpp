...
static void remove_modify_status_recurcively(controls::tree_node^ node, int resource_image_index, int modified_resource_image_index)
{
	while(node != nullptr)
	{
		...
		}else
		{
			...
			node->m_image_index_collapsed			= vostok::editor_base::folder_closed;
			node->m_image_index_expanded			= vostok::editor_base::folder_open;
			node->ImageIndex						= vostok::editor_base::folder_open;
			node->SelectedImageIndex				= vostok::editor_base::folder_open;
		}
		node = node->Parent;
	}
}
...
void resource_editor::make_wrapper_modified( resource_editor_resource^ wrapper )
{
	...
	while(node != nullptr)
	{
		if(node->m_node_type == controls::tree_node_type::single_item)
		{
			int img = wrapper->m_is_broken ? vostok::editor_base::npc_tree_icon : modified_resource_image_index;
...
{
	if(panel_name == "vostok.editor.controls.item_properties_panel_base")		
...
{
	VOSTOK_UNREFERENCED_PARAMETER( context1 );
