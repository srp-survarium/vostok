...
void terrain_painter_cursor::on_visual_ready(vostok::resources::queries_result& data)
{
...
{
	m_shown				= show;
	vostok::render::editor::renderer& r = m_owner->m_level_editor->get_editor_renderer();
...
void terrain_painter_cursor::update(terrain_core^ terrain_core, float3 const& picked_position_global)
{
	...
	typedef vectora<vostok::render::buffer_fragment>	buffer_fragments;
	buffer_fragments										fragments(g_allocator);
	fragments.resize				(1);
	vostok::render::buffer_fragment& fragment = fragments[0];
	fragment.start					= 0;
...
void export_ring(vector<vert_struct>& dest_vertices, 
				 ...
				 pcstr material_name )
{
	vostok::memory::writer	writer(g_allocator);
...
void terrain_selector_cursor::on_visual_ready(vostok::resources::queries_result& data)
{
	R_ASSERT(data.is_successful());
