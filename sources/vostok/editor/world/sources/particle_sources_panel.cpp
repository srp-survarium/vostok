...
particle_graph_node^ particle_sources_panel::add_node_to_container(particle_source_nodes_container^ container, String^ node_type, String^ node_name, vostok::editor_base::images92x25 image_type_id){
...
void particle_sources_panel::on_toolbar_panel_loaded(vostok::resources::queries_result& data){
		...
		for(;begin != end;++begin){
			...
			if ((*begin).value_exists("children")){
				...
				for(;begin_children != end_children;++begin_children){
					...
					vostok::editor_base::images92x25 image_type = vostok::editor_base::images92x25(int(node_config["image_type"]));
