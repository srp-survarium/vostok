...
#	include <vostok/ai_navigation/sources/graph_generator.h>
#pragma managed( pop )
...
editor_world::editor_world		( vostok::editor::engine& engine ) :
	m_engine					( engine ),
	m_editor_mode				( true ),
	m_paused					( false ),
...
{
	vostok::base_registry_key::company_name = VOSTOK_ENGINE_COMPANY_ID;
	vostok::base_registry_key::product_name = VOSTOK_ENGINE_ID;

	editor_base::editor_creator::set_memory_allocator(g_allocator);
	register_texture_cook		( );
	m_command_engine			= gcnew vostok::editor_base::command_engine( 100 );
	...
	m_input_engine				= gcnew editor_base::input_engine( vostok::editor_base::input_keys_holder::ref , gcnew editor_contexts_manager ( *this ) );
	...
	ide()->get_options_manager()->register_page( "General\\Input", gcnew vostok::editor_base::options_page_input( m_input_engine ) );
	ide()->get_options_manager()->register_page( "Controls\\Wpf Editor", gcnew vostok::editor_base::wpf_controls_page( ) );
...
//	vostok::ai::navigation::set_memory_allocator( *g_allocator );
...
public ref class render_mode_strip_button: public vostok::editor::controls::mode_strip_button
{
public:
...
void editor_world::load_settings( )
{
	...
	vostok::editor_base::transform_control_base::gizmo_size = convert_to_single(editor_key->GetValue( "gizmo_size", "0.8") );

	vostok::editor::wpf_controls::settings::load( product_key );
...
void editor_world::save_settings( )
{
	...
	editor_key->SetValue		( "gizmo_size", convert_to_string(vostok::editor_base::transform_control_base::gizmo_size) );
	...
	vostok::editor::wpf_controls::settings::save( product_key );
...
void editor_world::set_renderer_configuration( pcstr in_render_config_name )
{
	...
	vostok::console_commands::execute				(command_str.c_str(), vostok::console_commands::execution_filter_all);
}

void editor_world::on_fs_iterator_render_options_ready( fs_new::virtual_path_string const & /*options_path*/, vostok::vfs::vfs_locked_iterator const & fs_it )
{
...
void editor_world::load( )
{
	...
	vostok::editor::wpf_controls::library_initializer::initialize( );
...
void editor_world::tick( )
{
	...
	if ( m_editor_mode )
	{
		if( vostok::editor_base::editors_helper::is_active_form_belong_to( m_window_ide ) || engine().is_closing_timeout_set() )
		{
			execute_input						( );
...
struct render_predicate : private boost::noncopyable {
	inline			render_predicate( vostok::render::scene_ptr const& scene, vostok::render::debug::renderer& renderer ) :
		m_scene						( scene ),
		m_renderer					( renderer )
	{
	}
	...
	vostok::render::scene_ptr const&	m_scene;
	vostok::render::debug::renderer&	m_renderer;
}; // struct render_predicate


void editor_world::render( u32 frame_id, vostok::math::float4x4 view_matrix )
{
	VOSTOK_UNREFERENCED_PARAMETERS( frame_id, view_matrix );
...
{
	vostok::editor_base::tool_window^ ed = m_tool_windows->get("sound_editor");
	if(ed==nullptr)
	{
...
{
	vostok::editor_base::tool_window^ ed = m_tool_windows->get("animation_editor");
	if(ed==nullptr)
	{
...
{
	vostok::editor_base::tool_window^ ed = m_tool_windows->get("dialog_editor");
	if(ed==nullptr)
	{
...
{
	vostok::editor_base::tool_window^ ed = m_tool_windows->get("particle_editor");
...
vostok::editor_base::tool_window^ editor_world::create_model_editor( )
{
	model_editor::editor_creator::set_memory_allocator(g_allocator);
	vostok::editor_base::tool_window^ ed = model_editor::editor_creator::create_editor( engine().get_renderer_world(),
																						m_tool_windows,
...
//	for(int i=0; i<10; ++i)
	{
		vostok::editor_base::tool_window^ ed = create_model_editor( );
		ed->load_settings( base_registry_key::get() );
		ed->Show( nullptr, nullptr );
	}
}
...
{
	vostok::editor_base::tool_window^ tab	= gcnew texture_editor(*this);
	tab->Show							( nullptr, nullptr );
}

void editor_world::show_material_editor( )
{
	vostok::editor_base::tool_window^ tab	= gcnew material_editor(*this, material_editor_mode::edit_material, false );
...
{
	vostok::editor_base::tool_window^ tab	= gcnew render_options_editor(*this);
	tab->load_settings( base_registry_key::get() );
	tab->Show							( nullptr, nullptr );
}
...
{
	vostok::editor_base::tool_window^ tab	= gcnew material_editor(*this, material_editor_mode::edit_instances, false );
	tab->load_settings( base_registry_key::get() );
	tab->Show							( nullptr, nullptr );
}
...
void editor_world::toggle_console( )
{
	...
	if(m_console_wrapper->get_active())
		m_console_wrapper->on_deactivate( vostok::editor_base::input_keys_holder::ref );
	else
		m_console_wrapper->on_activate( vostok::editor_base::input_keys_holder::ref );
}
...
void editor_world::on_render_resources_created(resources::queries_result& data)
{
	...
	*m_camera_view_output_window	= static_cast_resource_ptr< render::render_output_window_ptr >( data[3].get_unmanaged_resource() );
	...
	m_ai_navigation_world			= vostok::ai::navigation::create_world( *this, *m_scene, get_renderer().debug() );
