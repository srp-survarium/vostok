...
using vostok::editor_base::editor_control_base;

static vostok::command_line::key	s_world_key		 ("world", "", "", "project for loading");
...
{	
	...
	view_window()->subscribe_on_camera_move	( gcnew vostok::editor_base::camera_moved_delegate(this, &level_editor::on_camera_move));
...
//	m_tools_tab->add_tab		("scripting", p_tool_scripting->ui()->frame());
	...
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(tool_sv) );// static

	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_light) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_composite) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_particle) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_misc) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_scripting) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_sound) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(tool_st) );
	vostok::editor_base::resource_chooser::register_chooser( gcnew library_class_chooser(p_tool_animations) );
...
// Initializing standard set of transformation controls
	...
	m_transform_control_translation		= gcnew vostok::editor_base::transform_control_translation	( m_transform_control_helper );
	m_transform_control_scaling			= gcnew vostok::editor_base::transform_control_scaling		( m_transform_control_helper );
	m_transform_control_rotation		= gcnew vostok::editor_base::transform_control_rotation		( m_transform_control_helper );
...
{	
	...
	vostok::fixed_string512		project_path;
...
WeifenLuo::WinFormsUI::Docking::IDockContent^ level_editor::find_dock_content(System::String^ type_name)
{
	...
	if(type_name=="vostok.editor.project_tab")
	{
		result = m_project_tab;
	}else
	if(type_name=="vostok.editor.tools_tab")
	{
		result = m_tools_tab;
	}else
	if(type_name=="vostok.editor.object_inspector_tab")
	{
		result = m_object_inspector_tab;
	}else
	if(type_name=="vostok.editor.animation_curve_editor_panel")
	{
		result = m_animation_curve_editor_panel;
	}else
	if(type_name=="vostok.editor.triangulation_tester")
	{
		result = m_triangulation_tester;
	}else
	if(type_name=="vostok.editor_base.camera_view_panel")
	{
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( tool );
}
...
void level_editor::set_transform_object( vostok::editor_base::transform_control_object^ d )
{
	m_transform_control_helper->m_object	= d;
	m_transform_control_helper->m_changed	= true;
}

vostok::editor_base::scene_view_panel^ level_editor::view_window( )
{
	return m_editor_world.view_window();
}

vostok::editor_base::camera_view_panel^ level_editor::camera_view_panel	( )
{
...
{
	return 		System::Windows::Forms::MessageBox::Show( ide()->wnd(), message, "Vostok Editor", buttons, icon );
}
