...
using vostok::editor::controls::property_value;
...
void property_restrictor::load(vostok::configs::lua_config_value const& lv)
{
	reset();
	if(!lv.value_exists("restrictions"))
		return;

	vostok::configs::lua_config_value t = lv["restrictions"];


	vostok::configs::lua_config::const_iterator	it		= t.begin();
	vostok::configs::lua_config::const_iterator	it_e	= t.end();
	for ( ;it!=it_e; ++it) 
	{
		pcstr prop_name		= *it;
...
void property_restrictor::save(vostok::configs::lua_config_value lv)
{
	vostok::configs::lua_config_value t = lv["restrictions"];
	for each (ro_property_wrapper^ w in m_restricted_items)
	{
		...
		vostok::editor::unmanaged_string us		(w->m_property_name);
...
{
	vostok::editor::property_holder* ph	= m_property_holder;
	DELETE								(ph);
	m_property_holder					= NULL;
}
