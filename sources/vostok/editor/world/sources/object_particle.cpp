...
object_particle::object_particle( tool_particle^ t, render::scene_ptr const& scene ) :
	...
	m_selected		( false )
{
	image_index			= vostok::editor_base::particle_tree_icon;
...
void object_particle::on_resource_loaded( vostok::resources::queries_result& data )
{
	R_ASSERT							( data.is_successful() );
	
	
	*m_particle_system_instance_ptr = vostok::static_cast_resource_ptr<vostok::particle::particle_system_instance_ptr>( data[0].get_unmanaged_resource() );
