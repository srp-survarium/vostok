...
bool picker::ray_query( vostok::collision::object_type const mask, collision_object const** object, bool check_priority, float3* position )
{
	vostok::math::int2 pos = m_world->get_mouse_position();
	
	return ray_query( mask, pos, object, check_priority, position);
}

bool picker::ray_query( vostok::collision::object_type const mask, vostok::math::int2 screen_xy, collision_object const** object, const bool check_priority, float3* position )
{
	if( m_cached && 
...
bool picker::ray_query( vostok::collision::object_type const mask, vostok::math::float3 origin, vostok::math::float3 direction, collision_object const** object, bool check_priority, float3* position )
{
...
bool picker::ray_query( vostok::collision::object_type const mask, vostok::math::float3 origin, vostok::math::float3 direction, collision::ray_objects_type& collision_results )
{
	if( !m_world->get_collision_tree() )
		return false;
...
bool picker::frustum_query( vostok::collision::object_type const mask, vostok::math::int2 start_xy, vostok::math::int2 end_xy, vector< collision_object const*> &objects )
{
	math::float3 origin;
	math::float3 direction;
