...
{
	vostok::editor_base::input_keys_holder::create();
	m_is_closing				= false;
	...
	m_camera_view_panel			= gcnew vostok::editor_base::camera_view_panel( );
	m_view_window->DockAreas	= WeifenLuo::WinFormsUI::Docking::DockAreas::Document;
	vostok::editor_base::input_keys_holder::ref->subscribe_on_changed( gcnew editor_base::keys_combination_changed(this, &window_ide::key_combination_changed));
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( sender, event_args );
...
{
	return vostok::editor_base::input_keys_holder::ref->get_held_keys();
}

int	window_ide::get_current_keys_string( System::String^& combination )
{
	return vostok::editor_base::input_keys_holder::ref->get_current_pressed_keys( combination );
}
...
//}
//
//vostok::math::int2 window_ide::get_mouse_position_int( )
//{
...
void window_ide::add_action_menu_item( vostok::editor_base::gui_binder^ binder, System::String^ action_name, System::String^ strip_key, int prio )
{
	binder->add_action_menu_item( menuStrip, action_name, strip_key, prio );
}

void window_ide::add_action_button_item( vostok::editor_base::gui_binder^ binder, System::String^ action_name, System::String^ strip_key, int prio )
{
	m_view_window->add_action_button_item( binder, action_name, strip_key, prio );
}

void window_ide::add_action_button_item( vostok::editor_base::gui_binder^ binder, action^ action, System::String^ strip_key, int prio )
{
...
{
	NOT_IMPLEMENTED();
	VOSTOK_UNREFERENCED_PARAMETERS(button, strip_key, prio);
...
vostok::editor_base::options_manager^ window_ide::get_options_manager() 
{
...
{
	VOSTOK_UNREFERENCED_PARAMETER		(lParam);
