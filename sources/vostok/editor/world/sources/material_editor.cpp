...
using namespace vostok::editor::wpf_controls::property_grid;
using namespace System;
using namespace System::Collections;
using namespace System::Collections::ObjectModel;

using System::Windows::Forms::ContextMenuStrip;;
using vostok::editor::wpf_controls::property_grid::value_changed_event_handler;
...
ref class material_editor_contexts_manager :public vostok::editor_base::i_context_manager
{
public:
...
void material_editor::tick( )
{
	...
	vostok::render::editor::renderer* r = m_view_window->editor_renderer();
	if( r )
	{
 		m_view_window->tick			( );
...
void material_editor::item_label_renamed( Object^ , NodeLabelEditEventArgs^ e )
{
	...
	if(r->m_is_new)
	{
		e->Node->SelectedImageIndex		= vostok::editor_base::node_material_added;
		e->Node->ImageIndex				= vostok::editor_base::node_material_added;
	}
}
...
void material_editor::remove_tool_stip_item_click( Object^ ,System::EventArgs^ )
{
	...
	for(int i=count-1; i>=0; --i)
	{
		...
		if(node->m_node_type==controls::tree_node_type::single_item)
		{
			...
			node->SelectedImageIndex	= vostok::editor_base::node_material_deleted;
			node->ImageIndex			= vostok::editor_base::node_material_deleted;
...
void material_editor::clone_tool_stip_item_click( Object^ , System::EventArgs^ )
{
	...
	tree_node^ node =				 m_material_view_panel->tree_view->add_item( new_material_name, vostok::editor_base::node_material );
	m_material_view_panel->tree_view->Sort			( );
...
{
	if(panel_name == "vostok.editor.controls.file_view_panel_base")
...
{ 
	RegistryKey^ editor_key		= vostok::base_registry_key::get_sub_key(product_key, Name );
