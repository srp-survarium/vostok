...
using vostok::resources::query_result;
using vostok::resources::query_result_for_cook;
using vostok::resources::queries_result;
using vostok::resources::query_result_for_user;
using vostok::resources::unmanaged_resource_ptr;
using vostok::resources::resource_ptr;
using vostok::resources::user_data_variant;
using vostok::memory::base_allocator;	 	

static vostok::command_line::key	s_use_editor_resource_manager	("use_editor_resource_manager",	"editor_rm", "resources", "if set, use editor resource manager");
...
private ref class manager_impl
{
	...
	private:
		...
		public:
			...
			{
				vostok::resources::managed_resource_ptr	res					= data->get_managed_resource();
				if(res.c_ptr() != NULL)				m_resource				= NEW(vostok::resources::managed_resource_ptr)(res);
		...
		public:
			vostok::resources::managed_resource_ptr*				m_resource;
	...
	private:
	void				inner_query_callback							(vostok::resources::queries_result& data)
	{
		query_holder^ query;
		if(m_is_sync_call)
		{
			...
			if(is_not_enough_memory)
			{
				...
				if(is_has_cleaned_memory)
				{
					...
					query_result_delegate* rq = NEW(query_result_delegate)(gcnew query_result_delegate::Delegate(this, &manager_impl::inner_query_callback));
 					int ret_id = vostok::resources::query_resources(
 						requests,
						data.size(),
																			...
																			query_result_for_cook *		parent		)
	{
		...
		if(is_all_requests_used)
		{
			...
			return vostok::resources::query_resources(
 				requests,
 				count,
				callback,
 				allocator,
