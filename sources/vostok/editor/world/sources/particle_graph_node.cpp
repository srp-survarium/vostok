...
#include <vostok/editor/base/images/images30x25.h>
...
using vostok::editor::particle_graph_node;
using vostok::editor::controls::hypergraph::connection_point;
using vostok::editor::controls::hypergraph::point_align;
using vostok::editor::controls::hypergraph::view_mode;
...
void					particle_graph_node::in_constructor		()
{
	...
	m_image_type					= vostok::editor_base::image_node_disabled;
	m_parent_position_offset		= Point( 0, 0 );
	m_property_holder				= NEW(::property_holder)("particle_node_props", NULL, NULL);
	m_properties_config				= NEW(vostok::configs::lua_config_value)((*vostok::configs::create_lua_config())["properties"]);
...
void					particle_graph_node::OnDragEnter		(DragEventArgs^ e){
	VOSTOK_UNREFERENCED_PARAMETERS(e);
	if (m_parent != nullptr){		
...
void					particle_graph_node::OnPaint			(PaintEventArgs^ e)
{
	...
	vostok::editor_base::images30x25 mini_image = vostok::editor_base::images30x25_emitter;
	if (m_image_type == vostok::editor_base::image_node_emitter)
		if(is_root_node())
			mini_image = vostok::editor_base::images30x25_root;
		else
			mini_image = vostok::editor_base::images30x25_emitter;
	else if (m_image_type == vostok::editor_base::image_node_render)
		mini_image = vostok::editor_base::images30x25_render;
	else if (m_image_type == vostok::editor_base::image_node_color)
		mini_image = vostok::editor_base::images30x25_color;
	else if (m_image_type == vostok::editor_base::image_node_size)
		mini_image = vostok::editor_base::images30x25_size;
	else if (m_image_type == vostok::editor_base::image_node_velocity)
		mini_image = vostok::editor_base::images30x25_velocity;
	else if (m_image_type == vostok::editor_base::image_node_rotation)
		mini_image = vostok::editor_base::images30x25_rotation;
	else if (m_image_type == vostok::editor_base::image_node_rotaion_rate)
		mini_image = vostok::editor_base::images30x25_rotaion_rate;
	else if (m_image_type == vostok::editor_base::image_node_location)
		mini_image = vostok::editor_base::images30x25_location;
	else if (m_image_type == vostok::editor_base::image_node_event)
		mini_image = vostok::editor_base::images30x25_event;
	else if (m_image_type == vostok::editor_base::image_node_misc)
		mini_image = vostok::editor_base::images30x25_misc;
	if(m_parent != nullptr){
		...
		if (!m_enabled)
			e->Graphics->DrawImage(particle_node_images->Images[vostok::editor_base::image_node_disabled], Drawing::Point(0,0));			
		
		if(m_parent->selected_nodes->Contains(this))
			e->Graphics->DrawImage(particle_node_images->Images[vostok::editor_base::image_node_selected], Drawing::Point(0,0));			
...
void					particle_graph_node::load(configs::lua_config_value config)
{
	...
	m_image_type = vostok::editor_base::images92x25(int(config["image_type"]));
	...
	for(;begin != end;++begin)
	{
		...
		if (gcnew String((*begin)["type"]) == "action")
			new_node = gcnew particle_graph_node_action(vostok::editor_base::image_node_emitter, parent_editor);
		if (gcnew String((*begin)["type"]) == "emitter")
			new_node = gcnew particle_graph_node_emitter(vostok::editor_base::image_node_emitter, parent_editor);
		if (gcnew String((*begin)["type"]) == "event")
			new_node = gcnew particle_graph_node_event(vostok::editor_base::image_node_emitter, parent_editor);
		if (gcnew String((*begin)["type"]) == "property")
			new_node = gcnew particle_graph_node_property(vostok::editor_base::image_node_emitter, parent_editor);
