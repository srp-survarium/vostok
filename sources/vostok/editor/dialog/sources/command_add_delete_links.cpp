...
using vostok::dialog_editor::command_add_links;
using vostok::dialog_editor::command_remove_links;
using vostok::dialog_editor::command_set_actions_and_preconditions;
using vostok::dialog_editor::command_change_links_connection_point;
using vostok::dialog_editor::dialog_hypergraph;
using vostok::dialog_editor::dialog_links_manager;
using vostok::editor::controls::hypergraph::link;
using vostok::editor::controls::hypergraph::connection_point;
using vostok::dialog_editor::dialog_graph_link;
using vostok::dialog_editor::dialog_link_action;
using vostok::dialog_editor::dialog_link_precondition;
////////////////////////////////////////////////////////////////////////////
...
:m_hypergraph(h)
{
	...
	for each(dialog_graph_link^ lnk in lnks)
	{
		...
		vostok::configs::lua_config_value link_cfg = cfg["dialog"]["links"][links_counter];
...
bool command_add_links::commit()
{
	...
	for(; links_it!=config_ptr->get_root()["dialog"]["links"].end(); ++links_it)
	{
		...
		if(n1 && n2)
		{
			vostok::editor::controls::hypergraph::connection_point^ src_pt = n1->get_connection_point("child");
			vostok::editor::controls::hypergraph::connection_point^ dst_pt = n2->get_connection_point("parent");
			if(src_pt==nullptr)
...
////////////////////////////////////////////////////////////////////////////
vostok::dialog_editor::dialog_expression* make_preconditions_from_nodes(dialog_link_precondition^ prec)
{
	vostok::dialog_editor::dialog_expression* root = prec->expression_copy();
	for each(dialog_link_precondition^ chld in prec->Nodes)
	{
		vostok::dialog_editor::dialog_operation* oper = dynamic_cast<vostok::dialog_editor::dialog_operation*>(root);
...
:m_hypergraph(h)
{
	...
	vostok::configs::lua_config_value precond_table = cfg["new_link"]["preconditions"];
	...
	vostok::configs::lua_config_value action_table = cfg["new_link"]["actions"];
	int i = 0;
	for each(dialog_link_action^ act in acts)
	{
		vostok::configs::lua_config_value cur_action = action_table[i];
...
////////////////////////////////////////////////////////////////////////////
command_change_links_connection_point::command_change_links_connection_point(dialog_hypergraph^ h, links^ lnks, vostok::editor::controls::hypergraph::node^ n, bool dest)
:m_hypergraph(h),
m_destination(dest)
{
