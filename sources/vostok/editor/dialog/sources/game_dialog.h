...
namespace dialog_editor {
	...
	public:
								game_dialog			(vostok::resources::queries_result& result);
		virtual					~game_dialog		();
		virtual	void			set_text			(pcstr new_text) {VOSTOK_UNREFERENCED_PARAMETER(new_text);};
	...
	private:
				u32				calculate_size		(vostok::configs::lua_config_value const& cfg);
				void			create_roots		(vostok::configs::lua_config_value const& cfg);
				void			create_childs		(dialog_node_base* parent, vostok::configs::lua_config_value const& cfg);
		dialog_expression*		create_precondition	(vostok::configs::lua_config_value const& cfg);
		dialog_node_base*		new_node			(vostok::configs::lua_config_value const& cfg);
