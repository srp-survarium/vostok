...
#pragma unmanaged
using vostok::dialog_editor::string_tables::string_table_ids_storage;
using vostok::dialog_editor::string_tables::string_table_id;
////////////////////////////////////////////////////////////////////////////
//- global functions ------------------------------------------------------------------------
////////////////////////////////////////////////////////////////////////////
static vostok::uninitialized_reference<string_table_ids_storage> g_string_table_id_storage;
string_table_ids_storage* vostok::dialog_editor::string_tables::create_string_tables()
{
	VOSTOK_CONSTRUCT_REFERENCE(g_string_table_id_storage, string_table_ids_storage)();
	return (&*g_string_table_id_storage);
}

void vostok::dialog_editor::string_tables::destroy_string_tables()
{
	VOSTOK_DESTROY_REFERENCE(g_string_table_id_storage);
}

string_table_ids_storage* vostok::dialog_editor::string_tables::get_string_tables()
{
	return (&*g_string_table_id_storage);
}
////////////////////////////////////////////////////////////////////////////
...
void string_table_ids_storage::on_loc_table_loaded(vostok::resources::queries_result& data)
{
	...
	if(data.size() > 0)
	{
		...
		vostok::resources::query_resources_by_mask(
...
void string_table_ids_storage::on_string_table_ids_loaded(vostok::resources::queries_result& data)
{
	R_ASSERT(data.is_successful());	
	...
	if(data.size() > 0)
	{
		...
		for(u16 i=0; i<data.size(); ++i)
		{
			...
			vostok::fs_new::virtual_path_string file_path;
			vostok::fs_new::file_name_with_no_extension_from_path(&file_path, data[i].get_requested_path());
