...
namespace dialog_editor {
	...
	public:
		...
		void	request_subresources			(vostok::resources::query_result_for_cook* parent);

	private:
		void	on_loaded						(vostok::resources::queries_result& data);

	private:
		u32							m_visited_counter;
		vostok::resources::request	m_request[255];
	}; // class dialog_node_gatherer_visitor

	class dialog_node_giver_visitor : public boost::noncopyable
	{
	public:
				dialog_node_giver_visitor	(vostok::resources::queries_result& data)
					:m_resources(data),
					m_visited_counter(0)		{};
	...
	private:
		u32									m_visited_counter;
		vostok::resources::queries_result&	m_resources;
