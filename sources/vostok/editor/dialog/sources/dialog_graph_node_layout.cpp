...
dialog_graph_node_layout::dialog_graph_node_layout(vostok::configs::lua_config_value cfg)
{
	m_layout = layout_vector();
...
void dialog_graph_node_layout::save(vostok::configs::lua_config_value cfg)
{
	u32 counter = 0;
	layout_vector::const_iterator i = m_layout.begin();
	for(; i!=m_layout.end(); ++i)
	{
		vostok::configs::lua_config_value val = cfg["layout"][counter];
		val["id"] = i->first;
		val["x"] = i->second.x;
		val["y"] = i->second.y;
		++counter;
