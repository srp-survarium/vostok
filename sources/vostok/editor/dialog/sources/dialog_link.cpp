...
#pragma unmanaged
using vostok::dialog_editor::dialog_link;
...
using vostok::dialog_editor::dialog_precondition;
using vostok::dialog_editor::dialog_operation_and;
using vostok::dialog_editor::dialog_operation_or;
using vostok::dialog_editor::dialog_operation_not;
...
void dialog_link::load(vostok::configs::lua_config_value const& cfg)
{
...
void dialog_link::save(vostok::configs::lua_config_value cfg)
{
	vostok::configs::lua_config_value precond_table = cfg["preconditions"];
	if(m_root_precondition!=NULL)
		m_root_precondition->save(cfg["preconditions"]);

	vostok::configs::lua_config_value action_table = cfg["actions"];
	int i = 0;
	vector<dialog_action*>::iterator b = m_actions_list.begin();
	for(; b!=m_actions_list.end(); ++b)
	{
