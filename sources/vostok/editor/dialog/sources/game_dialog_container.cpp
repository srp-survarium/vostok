...
#pragma unmanaged
using vostok::dialog_editor::game_dialog_container;
...
{
	VOSTOK_UNREFERENCED_PARAMETER(new_text);
}
...
void game_dialog_container::on_dialog_loaded(vostok::dialog_editor::game_dialog_ptr p)
{
	(*m_resource) = p.c_ptr();
}

vostok::dialog_editor::dialog_node_base* game_dialog_container::get_root(u32 index) const
{
	if(!(*m_resource))
		return NULL;
	else
