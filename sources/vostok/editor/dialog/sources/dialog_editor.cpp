...
using namespace vostok::dialog_editor::string_tables;

dialog_editor_impl::dialog_editor_impl(System::String^ rp, vostok::editor_base::tool_window_holder^ holder )
:m_holder		( holder ),
m_closed		( false )
{
	VOSTOK_UNREFERENCED_PARAMETER(rp);
...
void dialog_editor_impl::on_references_loaded(vostok::resources::queries_result& data)
{
...
void dialog_editor_impl::on_languages_list_loaded(vostok::resources::queries_result& data)
{
	R_ASSERT(data.is_successful());	
...
void dialog_editor_impl::save_options()
{
	...
	vostok::configs::lua_config_value root = cfg->get_root()["references"];
	references_table::iterator it = m_references_table->begin();
	for(; it!=m_references_table->end(); ++it)
	{
		pcstr key = it->first;
