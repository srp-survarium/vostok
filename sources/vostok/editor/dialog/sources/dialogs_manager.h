...
namespace dialog_manager {
	...
	class dialogs_manager : private boost::noncopyable
	{
		...
		typedef vostok::dialog_editor::map<pstr, dialog_manager_callback, compare_predicate> editor_callbacks_list;

		typedef associative_vector<pcstr, game_dialog_ptr, vector, compare_predicate> game_dialogs_list;
		typedef vostok::dialog_editor::map<pstr, game_dialog_callback, compare_predicate> game_callbacks_list;
	...
	private:
		void	on_editor_dialog_loaded		(vostok::resources::queries_result& result);
		void	on_game_dialog_loaded		(vostok::resources::queries_result& result);
