...
using vostok::dialog_editor::dialog_action;

dialog::dialog(vostok::configs::lua_config_value const& cfg)
{
	m_last_id = 0;
...
void delete_preconditions(vostok::dialog_editor::dialog_expression* root)
{
	vostok::dialog_editor::dialog_precondition* root_prec = dynamic_cast<vostok::dialog_editor::dialog_precondition*>(root);
	if(root_prec)
		DELETE(root_prec);
	else
	{
		vostok::dialog_editor::dialog_operation* root_oper = dynamic_cast<vostok::dialog_editor::dialog_operation*>(root);
		if(root_oper)
		{
			vostok::dialog_editor::vector<vostok::dialog_editor::dialog_expression*>::iterator prec_vec_iter = root_oper->childs()->begin();
...
void dialog::save(vostok::configs::lua_config_value cfg)
{
//	saving nodes
	...
	{
		u32 id = (*i)->id();
		vostok::configs::lua_config_value val = cfg["dialog"]["nodes"][id];
		val["id"] = id;
...
//			dialog_node_base* p = get_node(cur_lnk->child()->id());
			...
			if(p!=m_phrases.end())
			{
				vostok::configs::lua_config_value val = cfg["dialog"]["links"][link_id];
				val["src_id"] = (*i)->id();
				val["dst_id"] = (*p)->id();
				cur_lnk->save(val);
			}
...
void dialog::remove_node(vostok::dialog_editor::dialog_node_base* n)
{
	m_phrases.erase(std::remove(m_phrases.begin(), m_phrases.end(), n), m_phrases.end());
}

void dialog::add_node(vostok::dialog_editor::dialog_node_base* n)
{
	m_phrases.push_back(n);
}

vostok::dialog_editor::dialog_node_base* dialog::get_node(u32 id)
{
