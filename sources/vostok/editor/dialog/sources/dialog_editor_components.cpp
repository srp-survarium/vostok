...
using vostok::dialog_editor::dialog_editor_impl;
using vostok::dialog_editor::dialog_document;
using vostok::dialog_editor::dialog_graph_link;
using namespace WeifenLuo::WinFormsUI::Docking;
using namespace System::Windows::Forms;
using vostok::editor_base::action_delegate;
using vostok::editor_base::execute_delegate_managed;
...
void dialog_editor_impl::initialize_components()
{
	...
	m_input_engine = gcnew vostok::editor_base::input_engine( vostok::editor_base::input_keys_holder::ref, gcnew dialog_editor_contexts_manager(this));
  	m_gui_binder = gcnew vostok::editor_base::gui_binder(m_input_engine); 

	action_delegate^ a = nullptr;
	a = gcnew action_delegate("New Document", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::new_document));
	m_input_engine->register_action(a, "Control+N(dialog_editor)");

	a = gcnew action_delegate("Save Document", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::save_active));
	m_input_engine->register_action(a, "Control+S(dialog_editor)");

	a = gcnew action_delegate("Save All", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::save_all));
	m_input_engine->register_action(a, "Control+Shift+S(dialog_editor)");

	a = gcnew action_delegate("Exit", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::exit_editor));
	m_input_engine->register_action(a, "");

	a = gcnew action_delegate("Undo", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::undo));
	m_input_engine->register_action(a, "Control+Z(dialog_editor_active_document)");

	a = gcnew action_delegate("Redo", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::redo));
	m_input_engine->register_action(a, "Control+Y(dialog_editor_active_document)");

	a = gcnew action_delegate("Cut", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::cut));
	m_input_engine->register_action(a, "Control+X(dialog_editor_active_document)");

	a = gcnew action_delegate("Copy", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::copy));
	m_input_engine->register_action(a, "Control+C(dialog_editor_active_document)");

	a = gcnew action_delegate("Paste", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::paste));
	m_input_engine->register_action(a, "Control+V(dialog_editor_active_document)");

	a = gcnew action_delegate("Select All", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::select_all));
	m_input_engine->register_action(a, "Control+A(dialog_editor_active_document)");

	a = gcnew action_delegate("Delete", gcnew execute_delegate_managed(multidocument_base, &vostok::editor::controls::document_editor_base::del));
...
{
	if(persist_string == "vostok.dialog_editor.dialog_files_view_panel")
		return m_view_panel;

	if (persist_string == "vostok.dialog_editor.dialog_item_properties_panel")
		return m_properties_panel;

	if (persist_string == "vostok.dialog_editor.dialog_control_panel")
		return m_dialog_tool_bar;

	if (persist_string == "vostok.dialog_editor.dialog_language_panel")
...
void dialog_editor_impl::on_selection_changed(vostok::dialog_editor::dialog_graph_node_base^ n)
{
...
vostok::editor::controls::document_base^ dialog_editor_impl::on_document_creating()
{
...
{
	vostok::editor::wpf_controls::property_grid::property_grid_control^ g = safe_cast<vostok::editor::wpf_controls::property_grid::property_grid_control^>(sender);
...
void dialog_editor_impl::update_node_text(vostok::dialog_editor::dialog_graph_node_phrase^ n, System::String^ id, System::String^ txt)
{
...
void dialog_editor_impl::show_text_editor(vostok::dialog_editor::dialog_graph_node_phrase^ n)
{
