...
#pragma unmanaged
using vostok::dialog_editor::game_dialog;
using vostok::buffer_vector;
using namespace vostok::resources;
using vostok::dialog_editor::dialog_manager::dialogs_manager;
...
u32 calculate_preconditions_size(vostok::configs::lua_config_value const& cfg)
{
	u32 sz = 0;
	int t = (int)cfg["type"];
	switch(t)
	...
	case 0: 
		{
			sz += sizeof(vostok::dialog_editor::dialog_precondition);
			sz += vostok::math::align_up(vostok::strings::length((pcstr)cfg["param1"]) + 1, u32(4));
			break;
		}
	case 1: 
		{
			...
			{
				sz += sizeof(vostok::dialog_editor::dialog_operation_and);
				vostok::configs::lua_config::const_iterator i = cfg["childs"].begin();
	...
	case 2: 
		{
			if(cfg.value_exists("childs"))
			{
				sz += sizeof(vostok::dialog_editor::dialog_operation_or);
				vostok::configs::lua_config::const_iterator i = cfg["childs"].begin();
				for(; i!=cfg["childs"].end(); ++i) 
					sz += calculate_preconditions_size(*i);
			}
	...
	case 3: 
		{
			if(cfg.value_exists("childs"))
			{
				sz += sizeof(vostok::dialog_editor::dialog_operation_not);
				vostok::configs::lua_config::const_iterator i = cfg["childs"].begin();
				for(; i!=cfg["childs"].end(); ++i) 
					sz += calculate_preconditions_size(*i);
			}
...
u32 game_dialog::calculate_size(vostok::configs::lua_config_value const& cfg)
{
	u32 dialog_size = 0;
	...
	for(; i!=cfg["nodes"].end(); ++i) 
	{
		...
		{
			dialog_size += sizeof(vostok::dialog_editor::dialog_node_base*);
			dialog_size += math::align_up(strings::length((pcstr)(*i)["string_table"]) + 1, u32(4));
			++m_roots_counter;
		}
		
		if((bool)(*i)["is_dialog"]==true)
			dialog_size += sizeof(vostok::dialog_editor::game_dialog_container);
		else
		{
			dialog_size += sizeof(vostok::dialog_editor::dialog_phrase);
			dialog_size += math::align_up(strings::length((pcstr)(*i)["string_table"]) + 1, u32(4));
		}
	}
...
vostok::dialog_editor::dialog_node_base* game_dialog::new_node(vostok::configs::lua_config_value const& cfg)
{
...
void game_dialog::create_roots(vostok::configs::lua_config_value const& cfg)
{
...
void game_dialog::create_childs(dialog_node_base* parent, vostok::configs::lua_config_value const& cfg)
{
...
vostok::dialog_editor::dialog_expression* game_dialog::create_precondition(vostok::configs::lua_config_value const& cfg)
{
	int t = (int)cfg["type"];
	switch(t)
	{
...
void game_dialog::delete_preconditions(vostok::dialog_editor::dialog_expression* root)
{
...
vostok::dialog_editor::dialog_node_base* const& game_dialog::get_root(u32 index) const
{
