...
using vostok::dialog_editor::e_precondition_functions;
...
void dialog_precondition::load(vostok::configs::lua_config_value const& cfg)
{
...
void dialog_precondition::save(vostok::configs::lua_config_value cfg)
{
	cfg["type"] = 0;
...
{
	VOSTOK_UNREFERENCED_PARAMETER(info);
	return true;
}

bool dialog_precondition::predicate_has_item(pcstr item_name)
{
	VOSTOK_UNREFERENCED_PARAMETER(item_name);
	return true;
}
