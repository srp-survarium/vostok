...
particle_tree_node_holder::particle_tree_node_holder(vostok::editor::controls::tree_node^ parent, particle_editor^ parent_editor)
{
	m_tree_node = parent;
	m_parent_editor = parent_editor;
}

void particle_tree_node_holder::on_tree_node_config_loaded(vostok::resources::queries_result& data)
{
	...
	while (config->value_exists((pcstr)unmanaged_string("LOD "+i).c_str())){
		vostok::editor::controls::tree_node^ node	= m_tree_node->add_node_file_part("LOD "+i);
		m_parent_editor->add_context_menu_to_tree_view_node(node);
		i++;
	}
}

void particle_tree_node_holder::on_tree_node_new_lod_config_loaded(vostok::resources::queries_result& data)
{
	R_ASSERT(data.is_successful());
...
void particle_tree_node_holder::on_tree_node_delete_lod_config_loaded(vostok::resources::queries_result& data)
{
	R_ASSERT(data.is_successful());
