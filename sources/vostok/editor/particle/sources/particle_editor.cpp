...
namespace particle_editor {
	static bool process_clipboard( )
	{
		...
		pstr string				= vostok::os::paste_from_clipboard( *g_allocator );
		...
		bool is_copy	= vostok::strings::starts_with( string, "tree_view_clipboard_copy");
		bool is_cut		= vostok::strings::starts_with( string, "tree_view_clipboard_cut");
...
m_is_use_skybox		( true )
{
	...
	m_output_window						= NEW(vostok::render::render_output_window_ptr);
...
void particle_editor::on_render_resources_ready(resources::queries_result& data)
{
	...
	*m_output_window			= static_cast_resource_ptr<vostok::render::render_output_window_ptr>(data[2].get_unmanaged_resource());
...
static void draw_text(vostok::ui::world& ui_world, pcstr str, float pos_x, float pos_y, u32 clr)
{
	float2 line_size		(100.0f, 10.0f);
	
	vostok::ui::text* txt		= ui_world.create_text();
	txt->set_text			( str );
...
void particle_editor::draw_statistics()
{
	...
	vostok::particle::preview_particle_emitter_info* props = VOSTOK_ALLOC_IMPL(g_allocator, vostok::particle::preview_particle_emitter_info, num_fields);
	...
	VOSTOK_FREE_IMPL					( g_allocator, props );
}
...
void particle_editor::initialize( )
{
	...
	m_properties_panel								= gcnew vostok::editor::controls::item_properties_panel_base(multidocument_base);
	m_particles_panel								= gcnew vostok::editor::controls::file_view_panel_base(multidocument_base);
	m_toolbar_panel									= gcnew vostok::editor::controls::toolbar_panel_base(multidocument_base);
	...
	m_view_window			= gcnew vostok::editor_base::scene_view_panel();
	...
	ImageList^ particle_image_list		= vostok::editor_base::image_loader::load_images("particle_editor_images16x16", "particle_editor.resources", 16, safe_cast<int>(vostok::editor::particle_editor_images16x16_count), this->GetType()->Assembly);
	...
	use_skybox_Button->ImageScaling				= System::Windows::Forms::ToolStripItemImageScaling::SizeToFit;
 	...
 	m_input_engine		= gcnew vostok::editor_base::input_engine( vostok::editor_base::input_keys_holder::ref , gcnew particle_contexts_manager( this ));
 	m_gui_binder		= gcnew vostok::editor_base::gui_binder( m_input_engine ); 
	...
	vostok::editor_base::input_keys_holder::ref->subscribe_on_changed( gcnew vostok::editor_base::keys_combination_changed(this,&particle_editor::keys_combination_changed));
	
	m_time_layout_panel->KeyPreview = true;

	m_options_dialog = gcnew vostok::editor_base::options_dialog();

	m_options_dialog->register_page("Input", gcnew vostok::editor_base::options_page_input(m_input_engine));
...
{
	//test
	vostok::editor_base::action_delegate^ a				= nullptr;
	
	a				= gcnew vostok::editor_base::action_delegate("New Document", gcnew vostok::editor_base::execute_delegate_managed(this, &particle_editor::on_create_new_file_from_tool_strip ));
	m_input_engine->register_action( a, "Control+N(Global)" );

	a				= gcnew vostok::editor_base::action_delegate("Save Document", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::save_active ));
	m_input_engine->register_action( a, "Control+S(Global)" );

	a				= gcnew vostok::editor_base::action_delegate("Save All", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::save_all ));
	m_input_engine->register_action( a, "Control+Shift+S(Global)" );

	a				= gcnew vostok::editor_base::action_delegate("Exit", gcnew vostok::editor_base::execute_delegate_managed( this, &particle_editor::exitToolStripMenuItem_Click ));
	m_input_engine->register_action( a, "Alt+X(Global)" );

	a				= gcnew vostok::editor_base::action_delegate("Undo", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::undo ));
	m_input_engine->register_action( a, "Control+Z(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Redo", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::redo ));
	m_input_engine->register_action( a, "Control+Y(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Cut", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::cut ));
	m_input_engine->register_action( a, "Control+X(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Copy", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::copy ));
	m_input_engine->register_action( a, "Control+C(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Paste", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::paste ));
	m_input_engine->register_action( a, "Control+V(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Select All", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::select_all ));
	m_input_engine->register_action( a, "Control+A(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Delete", gcnew vostok::editor_base::execute_delegate_managed( m_multidocument_base, &document_editor_base::del ));
	m_input_engine->register_action( a, "Delete(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Disable Nodes", gcnew vostok::editor_base::execute_delegate_managed( this, &particle_editor::disable_selected_nodes ));
	m_input_engine->register_action( a, "Control+D(ActiveDocument)" );

	a				= gcnew vostok::editor_base::action_delegate("Delete in TreeView", gcnew vostok::editor_base::execute_delegate_managed( this, &particle_editor::on_tree_view_remove_document ));
	m_input_engine->register_action( a, "Delete(TreeView)" );
	a				= gcnew vostok::editor_base::action_delegate("Copy in TreeView", gcnew vostok::editor_base::execute_delegate_managed( this, &particle_editor::on_tree_view_copy_to_clipboard ));
	m_input_engine->register_action( a, "Control+C(TreeView)" );
	a				= gcnew vostok::editor_base::action_delegate("Paste in TreeView", gcnew vostok::editor_base::execute_delegate_managed( this, &particle_editor::on_tree_view_paste_from_clipboard ));
	a->set_enabled	(gcnew vostok::editor_base::enabled_delegate_managed(&process_clipboard));
	m_input_engine->register_action( a, "Control+V(TreeView)" );
	a				= gcnew vostok::editor_base::action_delegate("Cut in TreeView", gcnew vostok::editor_base::execute_delegate_managed( this, &particle_editor::on_tree_view_cut_to_clipboard ));
	m_input_engine->register_action( a, "Control+X(TreeView)" );
	
	a				= gcnew vostok::editor_base::action_delegate("Restart System", gcnew vostok::editor_base::execute_delegate_managed(this, &particle_editor::restart_particle_system ));
	m_input_engine->register_action( a, "F5(ActiveDocument)" );
	
	// Maya style mouse actions 
	m_input_engine->register_action( gcnew	vostok::editor_base::mouse_action_view_move_xy	( "view maya move xy",	m_view_window ), "Alt+MButton(View)" );
	m_input_engine->register_action( gcnew	vostok::editor_base::mouse_action_view_move_z		( "view maya move z",	m_view_window ), "Alt+RButton(View)" );
	m_input_engine->register_action( gcnew	vostok::editor_base::mouse_action_view_rotate_around( "view maya rotate",	m_view_window ), "Alt+LButton(View)" );
	
	// Old LE style mouse actions 
	m_input_engine->register_action( gcnew	vostok::editor_base::mouse_action_view_rotate		( "view LE rotate",		m_view_window ), "Space+LButton+RButton(View)" );
	m_input_engine->register_action( gcnew	vostok::editor_base::mouse_action_view_move_y_reverse( "view LE move y",	m_view_window ), "Space+RButton(View)" );
	m_input_engine->register_action( gcnew	vostok::editor_base::mouse_action_view_move_xz_reverse( "view LE move xz",m_view_window ), "Space+LButton(View)" );
	...
	vostok::editor_base::editor_control_base^ ec = gcnew vostok::editor_base::transform_control_translation(m_transform_control_helper);
	m_input_engine->register_action( gcnew	action_select_active_control( "select translate", this, ec ), "W(View)") ;

	ec = gcnew vostok::editor_base::transform_control_rotation(m_transform_control_helper);
	m_input_engine->register_action( gcnew	action_select_active_control( "select rotate", this, ec ), "E(View)") ;

	ec = gcnew vostok::editor_base::transform_control_scaling(m_transform_control_helper);
...
void particle_editor::load_particles_library			( )
{
	...
	vostok::resources::query_resource(
		...
		vostok::resources::lua_config_class,
		boost::bind(&query_result_delegate::callback, rq, _1),
		g_allocator
		);
}

void particle_editor::on_particles_library_loaded		( vostok::resources::queries_result& data )
{
...
void  particle_editor::on_tree_view_expand_node(Object^, TreeViewCancelEventArgs^ e){
	...
	tree_node^ t_node = safe_cast<vostok::editor::controls::tree_node^>(e->Node);
...
{
	if (panel_full_name == "vostok.editor.controls.file_view_panel_base")
		return			m_particles_panel;
	else if (panel_full_name == "vostok.editor.controls.item_properties_panel_base")
		return			m_properties_panel;
	else if (panel_full_name == "vostok.editor.controls.toolbar_panel_base")
		return			m_toolbar_panel;
	else if (panel_full_name == "vostok.editor_base.scene_view_panel" || 
		panel_full_name == "vostok.particle_editor.particle_preview_panel") // remove next commit
		return			m_view_window;
	else if (panel_full_name == "vostok.particle_editor.particle_data_sources_panel")
		return			m_data_sources_panel;
	else if (panel_full_name == "vostok.particle_editor.particle_time_based_layout_panel")
...
vostok::editor::controls::document_base^	particle_editor::on_create_document	()
{
...
// 	}
	...
	vostok::editor_base::input_keys_holder::ref->reset(this , EventArgs::Empty);
}

void			particle_editor::on_tree_view_paste_from_clipboard	( )
{
	vostok::editor::controls::tree_node^ t_node = safe_cast<vostok::editor::controls::tree_node^>(m_particles_panel->tree_view->selected_nodes[0]);
	if ( t_node->m_node_type == tree_node_type::single_item )
		t_node = safe_cast<vostok::editor::controls::tree_node^>(t_node->Parent);
	else if ( t_node->m_node_type == tree_node_type::file_part_item )
		t_node = safe_cast<vostok::editor::controls::tree_node^>(t_node->Parent->Parent);

	pcstr string				= vostok::os::paste_from_clipboard( *g_allocator );
	// clipboard doesn't contain text information
	if ( !string ) {
		// we could use function vostok::os::is_clipboard_empty() to check
	...
	vostok::editor::controls::tree_node^ node_to_copy = m_particles_panel->tree_view->get_node(node_to_copy_path);
		...
		if (IO::File::Exists(new_node_path+s_file_extension)){
			...
			if (Windows::Forms::MessageBox::Show(message, "Paste File", MessageBoxButtons::YesNo) == Windows::Forms::DialogResult::Yes)
			{
				IO::File::Copy(	old_node_path+s_file_extension,
							...
							true);		
				vostok::editor::controls::tree_node^ node = m_particles_panel->tree_view->get_node(t_node->FullPath+"/" + node_to_copy->Text);
				node->nodes->Clear();
				add_context_menu_to_tree_view_node(node->add_node_single("LOD 0", vostok::editor_base::particle_tree_icon));
				node->Collapse();
			...
			IO::File::Copy(	old_node_path+s_file_extension,
						new_node_path+s_file_extension,
						true);
			vostok::editor::controls::tree_node^ added_node = t_node->add_node_single(node_to_copy->Text, vostok::editor_base::particle_tree_icon);
...
void			particle_editor::on_tree_view_copy_to_clipboard		( )
{
	...
	vostok::os::copy_to_clipboard	( unmanaged_string(to_clipboard).c_str() );
}
...
{
	Boolean is_file				= safe_cast<vostok::editor::controls::tree_node^>(e->Node)->m_node_type == tree_node_type::single_item;
	if(e->Node->Text == " ")
	{
...
{
	vostok::editor::controls::tree_node^ node = m_particles_panel->tree_view->add_item("particles/ ", vostok::editor_base::particle_tree_icon);
	m_particles_panel->tree_view->select_node(node,true,true);
	vostok::editor::controls::tree_node^ lod_node = node->add_node_file_part("LOD 0");
...
{
	vostok::editor::controls::tree_node^ node = m_particles_panel->tree_view->add_group("particles/ ", vostok::editor_base::folder_closed, vostok::editor_base::folder_open);
	m_particles_panel->tree_view->select_node(node,true,true);
	add_context_menu_to_tree_view_node(node);
	node->BeginEdit();
}
...
void 			particle_editor::on_document_add_new_lod					(Object^ , EventArgs^){
	...
	vostok::editor::controls::tree_node^ node = safe_cast<vostok::editor::controls::tree_node^>(m_particles_panel->tree_view->selected_nodes[0]);
...
void 			particle_editor::on_tree_view_folder_new_document			(Object^ , EventArgs^ )
{
	...
	if(safe_cast<vostok::editor::controls::tree_node^>(m_particles_panel->tree_view->selected_nodes[0])->m_node_type == tree_node_type::single_item)
	...
	vostok::editor::controls::tree_node^ node	= m_particles_panel->tree_view->add_item(path, vostok::editor_base::particle_tree_icon);
	node->EnsureVisible	();
	m_particles_panel->tree_view->select_node(node,true,true);
	vostok::editor::controls::tree_node^ lod_node = node->add_node_file_part("LOD 0");
	add_context_menu_to_tree_view_node(node);
...
{
	Boolean is_lod		= safe_cast<vostok::editor::controls::tree_node^>(m_particles_panel->tree_view->selected_nodes[0])->m_node_type == tree_node_type::file_part_item;
	Boolean is_file		= safe_cast<vostok::editor::controls::tree_node^>(m_particles_panel->tree_view->selected_nodes[0])->m_node_type == tree_node_type::single_item;
...
void 			particle_editor::on_tree_view_folder_new_folder				(Object^ , EventArgs^ )
{
	...
	vostok::editor::controls::tree_node^ node	= m_particles_panel->tree_view->add_group(path, vostok::editor_base::folder_closed, vostok::editor_base::folder_open);
	node->EnsureVisible	();
	m_particles_panel->tree_view->select_node(node,true,true);
	add_context_menu_to_tree_view_node(node);
	node->BeginEdit();
...
{
	vostok::editor::controls::tree_node^ node = safe_cast<vostok::editor::controls::tree_node^>(e->Node);
...
void			particle_editor::load_document					(vostok::editor::controls::tree_node^ node)
{
...
void particle_editor::add_context_menu_to_tree_view_node(vostok::editor::controls::tree_node^ node)
{
...
{
	RegistryKey^ editor_key		= vostok::base_registry_key::get_sub_key(product_key, m_name);
...
{
	int mode = safe_cast<mode_strip_button^>( sender )->current_mode;
	m_editor_renderer->scene().set_particles_render_mode( 0, vostok::particle::enum_particle_render_mode( mode ) );
}
