...
using vostok::editor::wpf_controls::color_curve;

namespace vostok {
namespace particle_editor {
	
	extern vostok::configs::lua_config_value create_changes_config_from_string_path(String^ path, vostok::configs::lua_config_value cfg);
	
	particle_color_curve_holder::particle_color_curve_holder(vostok::editor::wpf_controls::property_container^ container,configs::lua_config_value const& config, particle_graph_node^ node, String^ property_name,String^ property_description ,String^ property_category, u32 def_value, String^ property_path)
		:m_default_value(def_value)
	{
		...
		m_config				= CRT_NEW(vostok::configs::lua_config_value)(config);
	...
	void particle_color_curve_holder::synchronize_config(Object^ sender, EventArgs^ ){
		...
		for each(vostok::editor::wpf_controls::color_curve_key^ p in ((color_curve^)sender)->keys){
			vostok::configs::lua_config_value key = data_source_config[unmanaged_string("key" + i).c_str()];
			key["key"] = p->position;
