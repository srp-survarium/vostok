...
using vostok::animation_editor::animation_model;
	...
	m_animation_callback( NEW(animation_managed_callback)( ) )
{
	...
	vostok::resources::query_resource(
		...
		vostok::resources::skeleton_model_instance_class,
...
vostok::render::skeleton_model_ptr animation_model::model( ) 
{
...
void animation_model::calculate_animations_events( vostok::vectora<editor_animations_event>& events )
{
	if( m_mixer )
		m_mixer->calculate_animations_events( events );
}

void animation_model::get_current_anim_states( vostok::vectora<editor_animation_state>& result, u32 current_time_in_ms )
{
	if( m_mixer )
...
void animation_model::model_loaded( vostok::resources::queries_result& result )
{
	if( m_scene == NULL )
		return;
	...
	m_model		= NEW(vostok::render::skeleton_model_ptr)( );
	(*m_model)	= static_cast_resource_ptr<vostok::render::skeleton_model_ptr>( result[0].get_unmanaged_resource( ) );
