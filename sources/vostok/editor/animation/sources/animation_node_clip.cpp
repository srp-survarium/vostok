...
using vostok::animation_editor::animation_node_clip;
using vostok::animation_editor::animation_node_clip_instance;
...
animation_node_clip::animation_node_clip(System::String^ name, 
										 vostok::animation::skeleton_animation_ptr anim, 
										 ...
										 )
{
	...
	m_animation = vostok::animation::create_editor_animation(*g_allocator, anim);
	m_config = NEW(vostok::configs::lua_config_ptr)();
	(*m_config) = cfg;
...
animation_node_clip::~animation_node_clip()
{
	...
	if(m_animation)
	{
		vostok::animation::i_editor_animation* anim = m_animation;
		vostok::animation::destroy_editor_animation(*g_allocator, anim);
	}
...
void animation_node_clip::load_intervals()
{
	...
	vostok::configs::lua_config_iterator b = m_animation->config_value().begin();
	for(; b!=m_animation->config_value().end(); ++b)
	{
		if(vostok::strings::compare((pcstr)(*b)["name"], "anim_intervals")==0)
		{
			anim_intervals_present = true;
			u32 id = 0;
			bool domains_exists = (*b).value_exists("domains");
			vostok::configs::lua_config_iterator d_b = (*b)["knots"].begin();
			if(domains_exists)
				d_b = (*b)["domains"].begin();

			vostok::configs::lua_config_iterator knots_b = (*b)["knots"].begin();
			...
			for(; knots_b!=(*b)["knots"].end(); ++knots_b)
			{
				...
				u32 partition_type = (u32)vostok::editor::wpf_controls::animation_setup::animation_channel_partition_type::undefined;
				if(domains_exists)
				{
...
{
	vostok::configs::lua_config_ptr new_cfg = vostok::configs::create_lua_config();
	new_cfg->add_super_table((*m_config)->get_root());
	return gcnew animation_node_clip_instance(this, new_cfg);
}

vostok::animation::skeleton_animation_ptr animation_node_clip::animation() 
{
	return m_animation->animation();
}

vostok::animation::i_editor_animation* animation_node_clip::editor_animation()
{
	return m_animation;
}
