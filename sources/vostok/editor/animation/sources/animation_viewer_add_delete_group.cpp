...
using vostok::animation_editor::animation_viewer_add_group;
using vostok::animation_editor::animation_viewer_remove_group;
...
bool animation_viewer_add_group::commit()
{
	...
	vostok::configs::lua_config_iterator nb = config_ptr->get_root()["scene"]["nodes"].begin();
	vostok::configs::lua_config_iterator ne = config_ptr->get_root()["scene"]["nodes"].end();
	for(; nb!=ne; ++nb)
	{
	...
	vostok::configs::lua_config_iterator lb = config_ptr->get_root()["scene"]["links"].begin();
	vostok::configs::lua_config_iterator le = config_ptr->get_root()["scene"]["links"].end();
	for(; lb!=le; ++lb)
	{
...
void animation_viewer_remove_group::init_weight_values(vostok::animation_editor::animation_viewer_graph_link_weighted^ initial_link, links^ deleting_links)
{
	...
	{
		float new_val = 0.0f;
		if(!vostok::math::is_similar(sum, 0.0f))
			new_val = l->weight + initial_link->weight*(l->weight/sum);
		else
			new_val = initial_link->weight/lnks->Count;

		new_val = (float)System::Math::Round(vostok::math::clamp_r(new_val, 0.0f, 1.0f), 3);
