...
animation_node_clip_instance::animation_node_clip_instance(animation_node_clip^ base, vostok::configs::lua_config_ptr cfg) :
	m_animation_clip	( NEW(animation::skeleton_animation_ptr) )
{
	...
	(*m_config) = cfg;
	vostok::configs::lua_config_value val = (*m_config)->get_root()["viewer_options"];
...
vostok::animation::skeleton_animation_ptr animation_node_clip_instance::animation() 
{
	return m_base->animation();
}

vostok::animation::i_editor_animation* animation_node_clip_instance::editor_animation()
{
	return m_base->editor_animation();
}

vostok::animation_editor::animation_node_interval^ animation_node_clip_instance::get_interval_by_offset_and_length(float offset, float length)
{
	for each(animation_node_interval^ interval in intervals)
	{
		if(vostok::math::is_similar(interval->offset, offset) && vostok::math::is_similar(interval->length, length))
			return interval;
	}
