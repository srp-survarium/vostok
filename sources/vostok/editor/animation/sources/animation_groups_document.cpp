...
using vostok::animation_editor::animation_groups_document;
using vostok::animation_editor::animation_groups_editor;
using vostok::animation_editor::animation_group_object;
using vostok::editor_base::command_engine;
using vostok::animation_editor::animation_node_interval;
using vostok::animation_editor::animation_node_interval_instance;
...
m_first_save(true)
{
	...
	// m_hypergraph
	m_hypergraph = gcnew vostok::editor::wpf_controls::hypergraph::hypergraph_host();
...
void animation_groups_document::save()
{
	...
	vostok::fs_new::virtual_path_string controller_path;
	...
	vostok::fs_new::virtual_path_string scene_path = controller_path;
	controller_path += ".lua";
	scene_path += ".options";
	vostok::configs::lua_config_ptr controller_cfg = vostok::configs::create_lua_config(controller_path.c_str());
	vostok::configs::lua_config_ptr scene_cfg = vostok::configs::create_lua_config(scene_path.c_str());
	vostok::configs::lua_config_value controller_root = controller_cfg->get_root()["animation_controller_set"];
	...
	vostok::configs::lua_config_value scene_root = scene_cfg->get_root()["scene"]["nodes"];
	for(int i=0; i<m_groups->Count; ++i)
	{
		...
		vostok::configs::lua_config_value val = scene_root[i];
		gr->save(val);

		vostok::configs::lua_config_ptr group_cfg = vostok::configs::create_lua_config(group_path.c_str());
...
void animation_groups_document::load()
{
	...
	vostok::resources::request arr[2] = {
		{controller_path.c_str(), vostok::resources::lua_config_class},
		{scene_path.c_str(), vostok::resources::lua_config_class}
	};
	...
	vostok::resources::query_resources(arr, 
...
void animation_groups_document::on_controller_loaded(vostok::resources::queries_result& result)
{
	if(result[0].is_successful())
	{
		is_saved = true;
		m_command_engine->set_saved_flag();
		vostok::configs::lua_config_ptr controller_cfg = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[0].get_unmanaged_resource());
		vostok::configs::lua_config_value root = controller_cfg->get_root()["animation_controller_set"];
		R_ASSERT(root.value_exists("model"));
		model = gcnew String((pcstr)root["model"]);
		root = root["animation_groups"];
		vostok::configs::lua_config_iterator it_contr = root.begin();
		vostok::configs::lua_config_iterator it_scene = root.begin();
		vostok::configs::lua_config_ptr scene_cfg;
		if(result[1].is_successful())
		{
			scene_cfg = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[1].get_unmanaged_resource());
			it_scene = scene_cfg->get_root()["scene"]["nodes"].begin();
		}

		vostok::resources::request arr[255];
		u32 index = 0;
		for(; it_contr!=root.end(); ++it_contr)
		{
			pcstr group_path = (pcstr)*it_contr;
			arr[index].path = group_path;
			arr[index].id = vostok::resources::lua_config_class;
			++index;
		...
		if(result[1].is_successful())
		{
			vostok::configs::lua_config_ptr scene_cfg = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[1].get_unmanaged_resource());
			vostok::configs::lua_config_iterator it_links = scene_cfg->get_root()["scene"]["links"].begin();
		...
		vostok::resources::query_resources(arr, index,
			boost::bind(&query_result_delegate::callback, rq, _1),
			g_allocator
		);
	}
...
void animation_groups_document::on_groups_loaded(vostok::resources::queries_result& result)
{
	if(result.is_successful())
	{
		...
		{
			vostok::configs::lua_config_ptr cfg = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[i].get_unmanaged_resource());
...
{
	vostok::editor::controls::tree_node^ n = get_editor()->animations_tree->get_node(folder_name);
	if(n->m_node_type==vostok::editor::controls::tree_node_type::group_item)
	{
		for each(vostok::editor::controls::tree_node^ child in n->nodes)
...
void animation_groups_document::on_animation_loaded(vostok::animation_editor::animation_node_clip_instance^ new_clip)
{
