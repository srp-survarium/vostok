	...
	: m_subscribe_id(0)
{
	...
	m_subscribe_id = vostok::resources::subscribe_watcher(
...
animation_collection_files_source::~animation_collection_files_source( )
{
	vostok::resources::unsubscribe_watcher( m_subscribe_id );
	DELETE( m_callback );
}

void animation_collection_files_source::fs_callback( vostok::vfs::vfs_notification const & info )
{
	switch( info.type )
	{
		case vostok::vfs::vfs_notification::type_added:
		{
		...
		case vostok::vfs::vfs_notification::type_removed:
		{
			String^ path		= ( gcnew String( info.physical_path ) )->Replace( Path::GetFullPath( animation_editor::animation_collections_absolute_path ) + "\\", "" )->Replace( animation_collection_item::extension, "" )->Replace( "\\", "/" );
			tree_node^ node		= m_parent->get_node( path );
		...
		case vostok::vfs::vfs_notification::type_renamed:
		{
...
void animation_collection_files_source::refresh( )
{
	...
	resources::query_vfs_iterator( unmanaged_string( animation_editor::animation_collections_path ).c_str( ), boost::bind( &fs_iterator_delegate::callback, q, _1 ), g_allocator, vostok::resources::recursive_true );
}

void animation_collection_files_source::on_fs_iterator_ready( vostok::vfs::vfs_locked_iterator const & fs_it )
{
...
void animation_collection_files_source::process_fs( vostok::vfs::vfs_iterator const & fs_it, String^ file_path )
{
	vostok::vfs::vfs_iterator	it	=	fs_it;
	while( !it.is_end( ) )
	{
		if( it.is_folder( ) )
		{
