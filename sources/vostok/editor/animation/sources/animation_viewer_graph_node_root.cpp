...
using vostok::animation_editor::animation_viewer_graph_node_root;
...
void animation_viewer_graph_node_root::save(vostok::configs::lua_config_value cfg)
{
	super::save(cfg);
	cfg["type"] = 0;
...
void animation_viewer_graph_node_root::load(vostok::configs::lua_config_value const& cfg, animation_viewer_hypergraph^ h)
{
	super::load(cfg, h);
...
bool node_with_name_exists(vostok::animation_editor::animation_viewer_graph_node_root^ r, System::String^ name)
{
	for each(vostok::animation_editor::animation_viewer_graph_node_base^ n in r->get_parent()->all_nodes())
	{
		if(r!=n && dynamic_cast<vostok::animation_editor::animation_viewer_graph_node_root^>(n)
			&& dynamic_cast<vostok::animation_editor::animation_viewer_graph_node_root^>(n)->caption==name)
			return true;
	}
