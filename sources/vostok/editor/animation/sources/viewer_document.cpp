...
using vostok::animation_editor::viewer_editor;
using vostok::animation_editor::animation_viewer_graph_node_base;
...
m_is_loaded(false)
{
	...
	m_hypergraph->move_mode = vostok::editor::controls::hypergraph::nodes_movability::movable;
	...
	m_hypergraph->on_selection_changed += gcnew vostok::editor::controls::hypergraph::selection_changed(this, &viewer_document::on_hypergraph_selection_changed);
...
void viewer_document::save()
{
	...
	vostok::configs::lua_config_ptr cfg = vostok::configs::create_lua_config(path.c_str());
	vostok::configs::lua_config_value scene_cfg = cfg->get_root()["scene"];
...
void viewer_document::on_focus_losted(System::Object^, System::EventArgs^)
{
	if(m_hypergraph->selected_nodes->Count>0 && !this->Disposing)
	{
		...
		for each(vostok::editor::controls::hypergraph::node^ n in m_hypergraph->selected_nodes)
			n->hypergraph_node_MouseUp(nullptr, e);
	}
}

void viewer_document::on_resources_loaded(vostok::resources::queries_result& result)
{
	if(result[0].is_successful())
	{
		vostok::strings::stream stream(g_allocator);
		vostok::configs::lua_config_ptr cfg = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[0].get_unmanaged_resource());
		cfg->save(stream);
		stream.append("%c", 0);
		m_hypergraph->add_group((pcstr)stream.get_buffer(), System::Drawing::Point(0,0));
		m_hypergraph->set_selected_node((vostok::editor::controls::hypergraph::node^)nullptr);
		m_hypergraph->clear_command_stack();
		m_hypergraph->set_saved_flag();
		vostok::configs::lua_config_value scene_cfg = cfg->get_root()["scene"];
		R_ASSERT(scene_cfg.value_exists("models"));
		vostok::configs::lua_config_iterator it = scene_cfg["models"].begin();
...
void viewer_document::copy(bool del)
{
	...
	for each(vostok::editor::controls::hypergraph::link^ lnk in lnks)
	{
...
void viewer_document::del()
{
	...
	for each(vostok::editor::controls::hypergraph::link^ lnk in m_hypergraph->selected_links)
...
void viewer_document::add_connection(vostok::editor::controls::hypergraph::node^ source, vostok::editor::controls::hypergraph::node^ destination)
{
...
vostok::animation_editor::animation_viewer_graph_link^ viewer_document::create_connection(vostok::editor::controls::hypergraph::node^ source, vostok::editor::controls::hypergraph::node^ destination)
{
...
{
	vostok::strings::stream stream(g_allocator);
	vostok::configs::lua_config_ptr cfg = configs::create_lua_config();
	vostok::configs::lua_config_value val = cfg->get_root()["scene"]["nodes"][0];
	val["id"] = 0;
...
void viewer_document::set_animation_node_new_intervals_list(animation_viewer_graph_node_base^ n, List<vostok::animation_editor::animation_node_interval^>^ lst, u32 cycle_index)
{
...
void viewer_document::on_new_model_loaded(vostok::resources::queries_result& result)
{
...
void viewer_document::save_node_as_clip()
{
	...
	if(m_save_file_dialog->ShowDialog(this)==System::Windows::Forms::DialogResult::OK)
	{
		...
		unmanaged_string path = unmanaged_string(file_path);
		vostok::configs::lua_config_ptr cfg_ptr = vostok::configs::create_lua_config(path.c_str());
		vostok::configs::lua_config_value cfg = cfg_ptr->get_root()["clip"];
