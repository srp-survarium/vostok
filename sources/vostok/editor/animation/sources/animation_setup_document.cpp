...
using vostok::animation_editor::animation_setup_document;
using vostok::animation_editor::animation_setup_manager;
using namespace vostok::animation;
using namespace System;
using namespace System::Collections;
using vostok::editor_base::command_engine;
using vostok::editor::wpf_controls::property_descriptor;
...
{
	vostok::configs::lua_config_value val = m_clip->editor_animation()->config_value();
	val.clear();
	for each(animation_channel^ ch in m_host->panel->channels)
	{
		vostok::configs::lua_config_value v = val[m_host->panel->channels->IndexOf(ch)];
		save_channel(ch, v);
	}
}
...
void animation_setup_document::save_channel(System::String^ channel_name)
{
	...
	if(ch!=nullptr)
	{
		vostok::configs::lua_config_value val = m_clip->editor_animation()->config_value()[m_host->panel->channels->IndexOf(ch)];
		save_channel(ch, val);
	}
}

void animation_setup_document::save_channel(animation_channel^ ch, vostok::configs::lua_config_value& cfg)
{
	...
	case animation_channel_type::intervals:
		{
			...
			for each(animation_channel_interval^ i in ch->objects)
			{
				...
				if(vostok::math::floor(knots_index/2 + 1.0f)!=ch->objects->Count)
...
void animation_setup_document::on_resources_loaded(vostok::animation_editor::animation_node_clip_instance^ new_clip)
{
	if(new_clip==nullptr)
		return;
	...
	{
		vostok::configs::lua_config_iterator b = m_clip->editor_animation()->config_value().begin();
...
void animation_setup_document::load_channel(vostok::configs::lua_config_value const& cfg)
{
	...
	case animation_channel_type::intervals:
		{
			...
			vostok::configs::lua_config_iterator d_b = cfg["knots"].begin();
			if(domains_exists)
				d_b = cfg["domains"].begin();

			R_ASSERT((cfg["knots"].size()%2 == 0));
			vostok::configs::lua_config_iterator b = cfg["knots"].begin();
			for(; b!=cfg["knots"].end(); ++b)
			{
				...
				if((old_max_time - start_time+interval_length) < vostok::math::epsilon_5)
					continue;
	...
	case animation_channel_type::partitions:
		{
			...
			for(; b!=cfg["knots"].end(); ++b)
			{
				...
				if((current_knot - old_max_time) > vostok::math::epsilon_3)
				{
...
void animation_setup_document::on_channel_copy(System::Object^, setup_panel_copy_event_args^ e)
{
	...
	vostok::configs::lua_config_value val = cfg->get_root()["data"]["event_channels"][0];
