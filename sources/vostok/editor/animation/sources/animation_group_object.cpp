...
using vostok::animation_editor::animation_groups_gait;
using vostok::animation_editor::animation_groups_flag;
...
void animation_group_object::save(vostok::configs::lua_config_value& cfg)
{
...
void animation_group_object::save_group_settings(vostok::configs::lua_config_value& cfg)
{
	...
	for(int index=0; index<m_intervals->Count; ++index)
	{
		...
		if(!vostok::math::is_similar(m_intervals[index]->max_weight, 1.0f))
			cfg["animations"][index]["max_weight"] = m_intervals[index]->max_weight;
		if(!vostok::math::is_similar(m_intervals[index]->min_weight, 0.0f))
			cfg["animations"][index]["min_weight"] = m_intervals[index]->min_weight;
	}
}

void animation_group_object::load(vostok::configs::lua_config_value const& cfg)
{
...
void animation_group_object::load_group_settings(vostok::configs::lua_config_value const& cfg)
{
...
void animation_group_object::on_animation_loaded(vostok::animation_editor::animation_node_clip_instance^ new_clip)
{
	if(new_clip==nullptr)
		return;
