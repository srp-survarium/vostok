...
using vostok::animation_editor::channels_document;
using vostok::animation_editor::channels_editor;
...
void channels_document::load()
{
	...
	vostok::resources::request arr[] = {
		{ source_path.c_str(), vostok::resources::animation_class },
		{ check_data_path.c_str(), vostok::resources::lua_config_class },
	};
...
void channels_document::on_resources_loaded(vostok::resources::queries_result& result)
{
	if(result[0].is_successful() && result[1].is_successful())
	{
		vostok::maya_animation::engine* meng(0);
		if(!m_allocator_inited)
		{
			vostok::maya_animation::set_memory_allocator(*g_allocator);
			m_allocator_inited = true;
		}

		vostok::maya_animation::world *maw = vostok::maya_animation::create_world(*meng);
		ASSERT(maw);

		vostok::resources::managed_resource_ptr anim_managed_ptr = static_cast_resource_ptr<vostok::resources::managed_resource_ptr>(result[0].get_managed_resource());
		vostok::configs::lua_config_value check_data_root = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[1].get_unmanaged_resource())->get_root();


		vostok::animation::i_editor_animation* anim = vostok::animation::create_editor_animation(*g_allocator, anim_managed_ptr);
		...
		for(u32 bone=0; bone<num_bones; ++bone)
		{
			...
			for(u32 ch=0; ch<9; ++ch)
			{
				...
				{
					vostok::animation::frame frame; 
					float time = vostok::math::QNaN;
