...
void animation_viewer_graph_node_base::assign_style(vostok::editor::controls::hypergraph::node_style^ style)
{
	m_style	= style;
...
void animation_viewer_graph_node_base::on_added(vostok::editor::controls::hypergraph::hypergraph_control^ h)
{
	super::on_added(h);
	h->display_rectangle = h->display_rectangle;
}

void animation_viewer_graph_node_base::on_removed(vostok::editor::controls::hypergraph::hypergraph_control^ h)
{
	super::on_removed(h);
	h->display_rectangle = h->display_rectangle;
}

void animation_viewer_graph_node_base::save(vostok::configs::lua_config_value cfg)
{
	cfg["id"] = m_id;
	cfg["position"] = float2((float)position.X, (float)position.Y);
}

void animation_viewer_graph_node_base::load(vostok::configs::lua_config_value const& cfg, animation_viewer_hypergraph^ h)
{
	...
	{
		vostok::configs::lua_config_iterator it = cfg["childs"].begin();
			...
			if(child!=nullptr)
			{
				...
				else if(this->parent!=nullptr)
				{
					...
					else
					{
						...
						vostok::editor::controls::hypergraph::connection_point^ src_pt = child->add_connection_point(animation_viewer_graph_node_base::typeid, "child");
