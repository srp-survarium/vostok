...
using vostok::animation_editor::animation_groups_toolbar;
using vostok::animation_editor::animation_groups_drag_drop_operation;
using vostok::editor::controls::drag_cursor_collection;

void animation_groups_toolbar::in_constructor(void)
{
	ImageList^ images_list = vostok::editor_base::image_loader::load_images("images24x24", "animation_editor.resources", 24, safe_cast<int>(images24x24_count), this->GetType()->Assembly);
	...
	m_cursors = gcnew vostok::editor::controls::drag_cursor_collection();
	HideOnClose = true;
	ResumeLayout(false);
}
