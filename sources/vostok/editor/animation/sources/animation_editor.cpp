...
//#	include <vostok/animation/i_animation_controller.h>
...
#	include <vostok/text_tree_log_helper.h>
#	include <vostok/ui/text_tree_draw_helper.h>
//#	include <vostok/profiler.h>
#	include <vostok/animation/i_animation_controller.h>
#	include <vostok/animation/anim_track_common.h>
#pragma managed( pop )

using namespace WeifenLuo::WinFormsUI::Docking;
using vostok::animation_editor::animation_editor;
using vostok::animation_editor::navigation_controller_query;
using vostok::animation_editor::animation_editor_form;
...
animation_editor::animation_editor(
	...
	vostok::ui::world& ui_wrld,							// for test
	vostok::editor_base::tool_window_holder^ holder
):m_renderer(NULL),
m_ui_world(ui_wrld),									// for test
m_render_world(render_world),
m_holder(holder)
{
//	vostok::g_profiler = NEW(vostok::profiler)();
//	START_PROFILE("constructor");
	...
	m_timer = NEW(vostok::timing::timer)();
	m_controller = NEW(vostok::rtp::animation_controller_ptr)();
	...
	m_controller_config = NEW(vostok::configs::lua_config_ptr)();
...
void animation_editor::on_render_resources_ready(vostok::resources::queries_result& data)
{
...
//  	START_PROFILE("initialize");
	...
	vostok::math::float3 eye_pos = float3(0.f, 1.5f, -1.5f);
	...
	bool is_loaded_normally = vostok::editor::controls::serializer::deserialize_dock_panel_root(m_form, m_form->main_dock_panel, m_name,
	...
	a = gcnew action_delegate("New scene", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::new_document));
	m_input_engine->register_action(a, "Control+N(viewer_editor)");

	a = gcnew action_delegate("Save scene", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::save_active));
	...
	a = gcnew action_delegate("Undo", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::undo));
	m_input_engine->register_action(a, "Control+Z(viewer_editor_active_doc)");

	a = gcnew action_delegate("Redo", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::redo));
	m_input_engine->register_action(a, "Control+Y(viewer_editor_active_doc)");

	a = gcnew action_delegate("Cut", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::cut));
	m_input_engine->register_action(a, "Control+X(viewer_editor_active_doc)");

	a = gcnew action_delegate("Copy", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::copy));
	m_input_engine->register_action(a, "Control+C(viewer_editor_active_doc)");

	a = gcnew action_delegate("Paste", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::paste));
	m_input_engine->register_action(a, "Control+V(viewer_editor_active_doc)");

	a = gcnew action_delegate("Select all", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::select_all));
	m_input_engine->register_action(a, "Control+A(viewer_editor_active_doc)");

	a = gcnew action_delegate("Delete", gcnew execute_delegate_managed(m_viewer_editor->multidocument_base, &vostok::editor::controls::document_editor_base::del));
	m_input_engine->register_action(a, "Delete(viewer_editor_active_doc)");

	//
	// Setup Manager
	//
	a = gcnew action_delegate("Save animation", gcnew execute_delegate_managed(m_setup_manager->multidocument_base, &vostok::editor::controls::document_editor_base::save_active));
	...
	a = gcnew action_delegate("Undo ", gcnew execute_delegate_managed(m_setup_manager->multidocument_base, &vostok::editor::controls::document_editor_base::undo));
	m_input_engine->register_action(a, "Control+Z(setup_manager_active_doc)");

	a = gcnew action_delegate("Redo ", gcnew execute_delegate_managed(m_setup_manager->multidocument_base, &vostok::editor::controls::document_editor_base::redo));
	m_input_engine->register_action(a, "Control+Y(setup_manager_active_doc)");

	//
	// Groups Editor
	//
	a = gcnew action_delegate("New controller", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::new_document));
	m_input_engine->register_action(a, "Control+N(groups_editor)");

	a = gcnew action_delegate("Save controller", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::save_active));
	...
	a = gcnew action_delegate(" Undo", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::undo));
	m_input_engine->register_action(a, "Control+Z(groups_editor_active_doc)");

	a = gcnew action_delegate(" Redo", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::redo));
	m_input_engine->register_action(a, "Control+Y(groups_editor_active_doc)");

	a = gcnew action_delegate(" Cut", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::cut));
	m_input_engine->register_action(a, "Control+X(groups_editor_active_doc)");

	a = gcnew action_delegate(" Copy", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::copy));
	m_input_engine->register_action(a, "Control+C(groups_editor_active_doc)");

	a = gcnew action_delegate(" Paste", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::paste));
	m_input_engine->register_action(a, "Control+V(groups_editor_active_doc)");

	a = gcnew action_delegate(" Select all", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::select_all));
	m_input_engine->register_action(a, "Control+A(groups_editor_active_doc)");

	a = gcnew action_delegate(" Delete", gcnew execute_delegate_managed(m_groups_editor->multidocument_base, &vostok::editor::controls::document_editor_base::del));
	m_input_engine->register_action(a, "Delete(groups_editor_active_doc)");

	//
	// Collections editor
	//
	a = gcnew action_delegate("Collection Doc Save ", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::save_active));
	m_input_engine->register_action(a, "Control+S(collections_editor)");

	a = gcnew action_delegate("Collection Doc Undo", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::undo));
	m_input_engine->register_action(a, "Control+Z(collections_editor_active_doc)");

	a = gcnew action_delegate("Collection Doc Redo", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::redo));
	m_input_engine->register_action(a, "Control+Y(collections_editor_active_doc)");

	a = gcnew action_delegate("Collection Doc Cut", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::cut));
	m_input_engine->register_action(a, "Control+X(collections_editor_active_doc)");

	a = gcnew action_delegate("Collection Doc Copy", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::copy));
	m_input_engine->register_action(a, "Control+C(collections_editor_active_doc)");

	a = gcnew action_delegate("Collection Doc Paste", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::paste));
	m_input_engine->register_action(a, "Control+V(collections_editor_active_doc)");

	a = gcnew action_delegate("Collection Doc Select all", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::select_all));
	m_input_engine->register_action(a, "Control+A(collections_editor_active_doc)");

	a = gcnew action_delegate("Collection Doc Delete", gcnew execute_delegate_managed(m_collections_editor->multidocument_base, &vostok::editor::controls::document_editor_base::del));
	...
	ImageList^ anim_editor_image_list16x16 = image_loader::load_images("images16x16", "animation_editor.resources", 16, safe_cast<int>(vostok::animation_editor::images16x16_count), this->GetType()->Assembly);
	image_loader loader;
...
bool animation_editor::close_query()
{
	...
	vostok::editor::controls::serializer::serialize_dock_panel_root(m_form, m_form->main_dock_panel, m_name);
...
{
	if(persist_string=="vostok.animation_editor.channels_editor")
		return m_channels_editor;
	else if(persist_string=="vostok.animation_editor.viewer_editor")
		return m_viewer_editor;
	else if(persist_string=="vostok.animation_editor.animation_setup_manager")
		return m_setup_manager;
	else if(persist_string=="vostok.animation_editor.animation_groups_editor")
		return m_groups_editor;
	else if(persist_string=="vostok.editor_base.scene_view_panel" )
		return m_view_window;
	else if(persist_string=="vostok.animation_editor.animation_collections_editor" )
...
void animation_editor::switch_navigation_mode()
{
	...
	else
	{
		if(m_open_file_dialog->ShowDialog()==DialogResult::OK)
		{
			...
			m_pv = NEW(vostok::resources::user_data_variant)();
			vostok::rtp::controller_resource_user_data data;
			m_pv->set<vostok::rtp::controller_resource_user_data>(data);
			...
			vostok::resources::query_resource(
				path.c_str(),
				vostok::resources::animation_controller_class,
...
void animation_editor::on_controller_loaded(vostok::resources::queries_result& result)
{
	if(result.is_successful())
	{
		(*m_controller) = static_cast_resource_ptr<vostok::rtp::animation_controller_ptr>(result[0].get_unmanaged_resource());
		if(m_pv)
		{
			vostok::rtp::controller_resource_user_data data;
			bool res = m_pv->try_get<vostok::rtp::controller_resource_user_data>(data);
			if(res && data.name)
				FREE(data.name);
...
void animation_editor::tick_navigation_mode()
{
	...
	if(learning_mode)
	{
		...
		if(!res)
		{
			...
			vostok::configs::lua_config_ptr cfg = vostok::configs::create_lua_config(save_path.c_str());
	...
	if(!m_navigation_paused && m_navigation_model_added)
	{
		...
		vostok::render::skeleton_model_ptr model = (*m_controller)->model();
...
void animation_editor::controller_set_target_callback(vostok::animation::mixing::expression const& expr, u32 current_time_in_ms)
{
	if(m_controller_config->c_ptr())
	{
		vostok::configs::lua_config_value root = (*m_controller_config)->get_root()["scene"]["nodes"][m_expressions_counter];
		...
		vostok::animation::mixing::binary_tree_visitor* wr = vostok::animation::create_binary_tree_writer(*g_allocator, root, position);
		expr.node().accept(*wr);
		m_last_target_pos_y = position.y + 100.0f;
		vostok::animation::destroy_binary_tree_writer(*g_allocator, wr);
	}
}
...
void animation_editor::start_saving_controller_scene()
{
	...
	if(m_save_file_dialog->ShowDialog(m_form)==System::Windows::Forms::DialogResult::OK)
	{
		...
		*m_controller_config = vostok::configs::create_lua_config(config_path.c_str());
		vostok::configs::lua_config_value root = (*m_controller_config)->get_root()["scene"];
...
void animation_editor::inc_time_scale()
{
	...
	m_time_scale = vostok::math::clamp_r(m_time_scale, 0.f, 100.f);
}
...
vostok::animation_editor::animation_node_clip^ animation_editor::clip_by_name(System::String^ clip_name)
{
...
long animation_editor::request_animation_clip(System::String^ name, vostok::animation_editor::animation_clip_request_callback^ callback)
{
//  	PROFILE_FUNCTION("request_animation_clip");
	...
	vostok::fs_new::virtual_path_string animation_path;
	animation_path += unmanaged_string( animation_editor::single_animations_path + "/" ).c_str( );
	animation_path += unmanaged_string(name).c_str();
	vostok::fs_new::virtual_path_string options_path = animation_path;
	options_path += ".options";

	vostok::resources::request arr[2] = {
		{animation_path.c_str(), vostok::resources::animation_class},
		{options_path.c_str(), vostok::resources::lua_config_class}
	};
	...
	return vostok::resources::query_resources(arr,
		array_size(arr),
...
void animation_editor::on_anim_clip_loaded(vostok::resources::queries_result& result, vostok::animation_editor::animation_clip_request_callback^ cb)
{
//  	PROFILE_FUNCTION("on_anim_clip_loaded");
	...
	if( result[0].is_successful( ) )
	{
		...
		vostok::configs::lua_config_ptr cfg;
		if(result[1].is_successful())
			cfg = static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[1].get_unmanaged_resource());
		else
		{
			System::String^ file_path = animation_editor::single_animations_path + "/" + requested_path + ".options";
			cfg = vostok::configs::create_lua_config(unmanaged_string(file_path).c_str());
		}

		vostok::configs::lua_config_value val = cfg->get_root();
		...
		vostok::animation::skeleton_animation_ptr anim = static_cast_resource_ptr<vostok::animation::skeleton_animation_ptr>(result[0].get_managed_resource());
...
void animation_editor::start_learning_controller(System::String^ controller_name)
{
	...
	vostok::rtp::controller_resource_user_data data;
	data.empty_set = true;
	data.name = vostok::strings::duplicate(g_allocator, name.c_str());
	m_pv->set<vostok::rtp::controller_resource_user_data>(data);
...
{
	RegistryKey^ editor_key		= vostok::base_registry_key::get_sub_key(product_key, "animation_editor");
...
{
	using vostok::strings::text_tree;
	using vostok::strings::text_tree_item;
	using vostok::strings::text_tree_log_helper;
	
	pvoid buffer = ALLOCA(64*vostok::Kb);
	text_tree m_text_tree_view(buffer, 64*vostok::Kb, " ----------- Loaded animations --------------- "); 
//	vostok::get_profiler().show_stats(&m_text_tree_view.root(), true);
	//text_tree m_text_tree_view(buffer, 64*vostok::Kb, " ----------- Loaded animations --------------- "); 
