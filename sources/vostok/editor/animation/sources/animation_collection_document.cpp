...
void animation_collection_document::in_constructor ( )
{
	...
	m_tree_view->selected_items_changed += gcnew EventHandler<vostok::editor::controls::tree_view_selection_event_args^>(this, &animation_collection_document::selection_changed);
	...
	m_editor->properties_panel->property_grid_control->property_value_changed += gcnew vostok::editor::wpf_controls::property_grid::value_changed_event_handler( this, &animation_collection_document::property_changed );
}
...
void animation_collection_document::tick ( )
{
	...
	//		vostok::animation::mixing::expression expr	( m_selected_animation_item->emit( buffer ) );
...
void animation_collection_document::drag_drop ( Object^, DragEventArgs^ e )
{
	...
	{
		if( vostok::editor::wpf_controls::dot_net_helpers::is_type<animation_collection_item^>( dest_node->Tag ) )
		{
...
void animation_collection_document::selection_changed ( Object^, vostok::editor::controls::tree_view_selection_event_args^ )
{
	...
	Boolean is_read_only =
		...
		!( vostok::editor::wpf_controls::dot_net_helpers::is_type<animation_collection_item^>( m_tree_view->selected_nodes[0]->Tag ) ) ||
		(
	...
	if( m_tree_view->selected_nodes->Count == 1 )
	{
		...
		void* buffer_memory		= ALLOCA( animation_editor::expression_alloca_buffer_size );
		mutable_buffer buffer	( buffer_memory, animation_editor::expression_alloca_buffer_size );
		vostok::animation::mixing::expression expr	( m_selected_animation_item->emit( buffer, is_last_animation ) );

		m_editor->active_model->set_target	( expr, m_current_time );
		m_timer->start( );

		if( vostok::editor::wpf_controls::dot_net_helpers::is_type<animation_collection_item^>( m_selected_animation_item ) )
			m_editor->active_model->animation_end += gcnew animation_callback( this, &animation_collection_document::animation_ended );
		else
			m_editor->active_model->animation_end -= gcnew animation_callback( this, &animation_collection_document::animation_ended );
	}
