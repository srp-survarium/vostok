...
using vostok::sound_editor::sound_scene_document;
...
void sound_scene_document::on_render_resources_ready(vostok::resources::queries_result& data)
{
...
vostok::sound::sound_scene_ptr& sound_scene_document::get_sound_scene()
{
	R_ASSERT(m_sound_scene->c_ptr()); 
	return *m_sound_scene;
}

void sound_scene_document::on_sound_scene_created(vostok::resources::queries_result& data)
{
	R_ASSERT(data.is_successful());
...
void sound_scene_document::on_resources_loaded(vostok::resources::queries_result& result)
{
	if(result[0].is_successful())
	{
		...
		vostok::configs::lua_config_value scene_cfg = cfg->get_root()["scene"];
		vostok::configs::lua_config_iterator it = scene_cfg.begin();
...
void sound_scene_document::view_window_drag_drop(Object^, DragEventArgs^ e)
{
	if(e->Data->GetDataPresent("tree_view_item"))
	{
		...
		bool bpick = m_view_window->ray_query(
			vostok::editor_base::collision_object_type_dynamic, 
			origin, 
			direction, 
			&picked_collision, 
			&picked_position
		...
		bpick |= vostok::editor_base::transform_control_base::plane_ray_intersect(
			float3(0, 0, 0),
			float3(0, 1, 0),
			origin,						
			direction,
...
void sound_scene_document::select_objects(objects_list^ lst)
{
	...
	m_editor->toolbar->enable_buttons(true);
	typedef array<vostok::editor::wpf_controls::property_container^> containers_list;
