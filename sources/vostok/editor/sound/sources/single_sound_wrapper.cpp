...
#include <vostok/sound/wav_utils.h>
#include <vostok/sound/spl_utils.h>
#include <vostok/fs_utils.h>

using vostok::sound_editor::single_sound_wrapper;
using vostok::editor::wpf_controls::float_curve_key;
using vostok::editor::wpf_controls::float_curve_key_type;
using vostok::editor::wpf_controls::float_curve_monotone_type;
using vostok::sound_editor::sound_object_instance;
...
void single_sound_wrapper::save(vostok::configs::lua_config_value& root)
{
	typedef List<float_curve_key^> keys_list;
	keys_list^ curve_keys = m_spl_curve->keys;
	vostok::math::float_curve c;
	c.reserve(curve_keys->Count);
	vostok::fixed_string<8> key_name;
	for(int i=0; i<curve_keys->Count; ++i)
	{
		...
		root[str]["position"] = vostok::math::float2((float)k->position_x, (float)k->position_y);
		root[str]["left_tangent"] = vostok::math::float2((float)k->left_tangent.X,(float)k->left_tangent.Y);
		root[str]["right_tangent"] = vostok::math::float2((float)k->right_tangent.X,(float)k->right_tangent.Y);
	}
}
...
void single_sound_wrapper::load(vostok::configs::lua_config_value const& cfg)
{
...
void single_sound_wrapper::on_single_sound_loaded(vostok::resources::queries_result& data)
{
...
void single_sound_wrapper::on_wav_options_loaded(vostok::resources::queries_result& data)
{
	...
	else
	{
		...
		vostok::fs_new::native_path_string wav_path;
		bool const convertion_result	=	vostok::resources::convert_virtual_to_physical_path(&wav_path, requested_path, resources::sources_mount);
		R_ASSERT							(convertion_result);
		vostok::fs_new::set_extension_for_path(&wav_path, "wav");
		vostok::fs_new::synchronous_device_interface const & device	=	vostok::resources::get_synchronous_device();
		wav_cfg = vostok::sound::wav_utils::create_default_config(wav_path, device);
	}

	if(data[1].is_successful())
		spl_cfg = static_cast_resource_ptr<configs::lua_config_ptr>(data[1].get_unmanaged_resource());
	else
		spl_cfg = vostok::sound::create_default_spl_config(data[1].get_primary_requested_path(), g_allocator);
...
void single_sound_wrapper::read_spl_curve(vostok::configs::lua_config_value const& cfg)
{
	m_spl_curve->keys->Clear();
	vostok::math::float_curve curve = vostok::math::float_curve();
	curve.load(cfg);
	for(u32 i=0; i<curve.get_num_points(); ++i)
	{
		float_curve_key^ key = gcnew float_curve_key();
		vostok::math::curve_point<float> pt;
