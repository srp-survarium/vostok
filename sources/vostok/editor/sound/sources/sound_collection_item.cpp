...
void sound_collection_item::save(vostok::configs::lua_config_value& cfg)
{
	...
	else
	{
		if(!is_link)
		{
			vostok::configs::lua_config_value val = cfg["collection"];
...
void sound_collection_item::load(vostok::configs::lua_config_value const& cfg)
{
...
{
	VOSTOK_UNREFERENCED_PARAMETER(obj);
}
