...
using vostok::sound_editor::tree_view_files_source;
using vostok::sound_editor::tree_view_files_node_sorter;
...
tree_view_files_source::tree_view_files_source(System::Windows::Forms::ContextMenuStrip^ context_menu)
{
	...
	m_watcher_subscriber_id	=	vostok::resources::subscribe_watcher(unm_res_path.c_str(), boost::bind(&file_watcher_delegate::callback, m_callback, _1));
...
tree_view_files_source::~tree_view_files_source()
{
	String^ res_path = sound_editor::sound_resources_path;
	unmanaged_string unm_res_path(res_path);
	vostok::resources::unsubscribe_watcher(m_watcher_subscriber_id);
	DELETE(m_callback);
}

void tree_view_files_source::fs_callback(vostok::vfs::vfs_notification const& info)
{
	vostok::fs_new::virtual_path_string file_path = info.virtual_path;
	...
	if(info.type!=vostok::vfs::vfs_notification::type_modified && !do_not_refresh)
		refresh();
...
void tree_view_files_source::refresh()
{
	String^ res_path = sound_editor::sound_resources_path;
	unmanaged_string unm_res_path(res_path);
	vostok::fs_new::virtual_path_string path = unm_res_path.c_str();
...
void tree_view_files_source::on_fs_iterator_ready(vostok::vfs::vfs_locked_iterator const &  fs_it)
{
...
void tree_view_files_source::process_fs(vostok::vfs::vfs_iterator const & in_it, String^ file_path)
{
