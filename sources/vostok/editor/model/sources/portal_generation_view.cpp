	...
	{
		{
			...
			bsp_tree_config->save_as( bsp_tree_filename.c_str(), vostok::configs::target_sources  );
		}
		{
			...
			mesh_config->save_as( mesh_filename.c_str(), vostok::configs::target_sources  );
		}
	}
}
	...
	{
		{
			...
			config->save_as( filename.c_str(), vostok::configs::target_sources  );
		}
		{
			unmanaged_string mesh_filename( bsp_node_save_dialog->FileName->Replace( ".lua", ".mesh" )->Replace( '\\', '/' ) );
			configs::lua_config_ptr mesh_config = configs::create_lua_config();
...
void portal_generation_view::set_property_container( vostok::editor::wpf_controls::property_container^ cont )
{
