...
#include <vostok\ai_navigation\sources\graph_generator_adjacency_builder.h>
...
u32 get_actual_triangle( u32 triangle_id, math::float3 const& point, triangles_mesh_type const& mesh )
{
	...
	vostok::ai::navigation::fill_coordinate_indices( triangle_plane.normal, index_pair );
...
void sectors_generator::initialize_delimited_edges()
{
	...
	for ( portals_type::const_iterator portal_it = m_portals.begin(); portal_it != portal_end; ++portal_it )
	{
		...
		for ( sector_generator_portal::outer_edges_type::const_iterator  edge_it = edges.begin(); edge_it != edge_end; ++edge_it )
		{
			...
			vostok::ai::navigation::fill_coordinate_indices( portal.get_plane().normal, coordinate_indices );

			vostok::collision::triangles_type::const_iterator triangle_end = triangles.end();
			for ( vostok::collision::triangles_type::const_iterator triangle_it = triangles.begin(); triangle_it != triangle_end; ++triangle_it )
			{
...
void sectors_generator::fill_property_container( vostok::editor::wpf_controls::property_container^ cont )
{
