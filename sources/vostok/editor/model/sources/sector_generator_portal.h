...
struct triangle_intersects_aabb : public std::unary_function<vostok::collision::triangle_result const&, bool>
{
	triangle_intersects_aabb( triangles_mesh_type const& mesh, math::float4x4 const& matrix )
		:m_mesh( &mesh ), m_matrix( matrix ){}
	bool operator()( vostok::collision::triangle_result const& triangle ) const
	{
		...
		int triBoxOverlap(float3 const& boxcenter, float3 const& boxhalfsize, float3 const (&triverts)[3] );//defined in "vostok\collision\sources\colliders_aabb_geometry_triangle_intersection.cpp"
