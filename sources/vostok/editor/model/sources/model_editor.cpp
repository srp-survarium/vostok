...
model_editor::model_editor( vostok::render::world& render_world, 
...
void model_editor::Show( System::String^ context1, System::String^ context2 )
{
	...
	bool is_loaded_normally = vostok::editor::controls::serializer::deserialize_dock_panel_root(
			m_ide, 
			m_ide->main_dock_panel, 
			registry_key_name(),
			gcnew WeifenLuo::WinFormsUI::Docking::DeserializeDockContent(this, &model_editor::find_dock_content),
...
{
	if(persist_string=="vostok.editor_base.scene_view_panel")
...
void model_editor::refresh_caption( )
{
	...
	m_ide->Text = System::String::Format("Vostok Model Assembler : {0}{1}", 
...
{
	RegistryKey^ editor_key		= vostok::base_registry_key::get_sub_key(product_key, "model_assembler");
