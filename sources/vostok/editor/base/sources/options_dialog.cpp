...
void options_dialog::unregister_page( System::String^ full_name )
{
	...
	ASSERT					(found_node);
	vostok::editor_base::options_page^ page		= dynamic_cast<vostok::editor_base::options_page^>(found_node->Tag);
	if(page)
		delete page;
...
void options_dialog::register_page( System::String^ full_name, vostok::editor_base::options_page^ page )
...
Void options_dialog::tree_view_AfterSelect(System::Object^  sender, System::Windows::Forms::TreeViewEventArgs^  e)
{
	...
	vostok::editor_base::options_page^ page			= nullptr;

	if( curr_node )
		page = dynamic_cast<vostok::editor_base::options_page^>(curr_node->Tag);

	if( m_active_page && m_active_page != page )
	{
		m_active_page->deactivate_page				( );
		page_platform_panel->Controls->Remove		( m_active_page->get_control() );
	...
	{
		m_active_page								= page;
		page_platform_panel->Controls->Add			( m_active_page->get_control() );
...
void options_dialog::initialize_all( TreeNodeCollection^ nodes )
{
	...
	{
		vostok::editor_base::options_page^ page = dynamic_cast<vostok::editor_base::options_page^>( nodes[i]->Tag ); 
		if( page )
		{
			page->activate_page			( );
