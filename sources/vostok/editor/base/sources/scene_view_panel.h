...
public:
	...
	vostok::render::debug::renderer*	debug_renderer			( );
	vostok::render::editor::renderer*	editor_renderer			( );
...
public:
	...
	bool 				ray_query			( vostok::collision::object_type const mask, vostok::collision::object const** object, float3* position );
	bool 				ray_query			( vostok::collision::object_type const mask, System::Drawing::Point screen_xy, vostok::collision::object const** object, float3* position );
	bool 				ray_query			( vostok::collision::object_type const mask, math::float3 origin, math::float3 direction, vostok::collision::object const** object, float3* position );
	bool 				ray_query			( vostok::collision::object_type const mask, math::float3 origin, math::float3 direction, collision_ray_objects^ collision_results );
	bool 				ray_query			( vostok::collision::object_type const mask, math::float3 origin, math::float3 direction, collision_ray_triangles^ collision_results );
	bool				ray_query			( vostok::collision::object_type const mask, float3& position, float3& normal );
	bool 				frustum_query		( vostok::collision::object_type const mask, System::Drawing::Point min_xy, System::Drawing::Point max_xy, collision_objects^ objects );
	...
	void				set_camera_effector			( vostok::camera_effector* e );
	...
	inline	float		vertical_fov				( ) { return 57.5f; }
			float		aspect_ratio				( );
//private:
	vostok::collision::space_partitioning_tree*		m_collision_tree;
//public:
	float											m_near_plane;
	float											m_far_plane;
...
private:
	...
	vostok::render::editor::renderer*	m_editor_renderer;
	vostok::math::float4x4*			m_inverted_view;
	...
	vostok::camera_effector*			m_camera_effector;
