...
bool command_engine::run				( vostok::editor_base::command^ command )
{
	ASSERT( m_preview_command == nullptr || m_transaction_started, "Command engine is still in preview mode." );
	return run_impl( command );
}
bool	command_engine::run_preview		( vostok::editor_base::command^ command )
{
...
bool command_engine::run_impl			( vostok::editor_base::command^ command )
{
...
int command_engine::rollback			( int steps )
{
	...
	if( steps < 0 )
	{
		...
		{
			vostok::editor_base::command^ cmd			= m_command_stack[idx];
			cmd->rollback			();
...
void command_engine::call_command_destructor( vostok::editor_base::command^ command )
{
	if( command )
		command->~command	( );
}
...
void command_engine::finalize_old		( )
{
	...
	{
		vostok::editor_base::command^ comm = m_command_stack[0];
		delete comm;
...
{
	vostok::editor_base::command_transaction^ transaction_command = gcnew vostok::editor_base::command_transaction( this, m_transaction_commands_count );
