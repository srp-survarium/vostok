...
using vostok::editor_base::action;
...
void gui_binder::update_menu_item( ToolStripMenuItem^ menu_item )
{
	...
	vostok::editor_base::checked_state item_check_state	= a->checked_state( );
	menu_item->Checked		= ( item_check_state != vostok::editor_base::checked_state::uncheckable );

	if( item_check_state != vostok::editor_base::checked_state::uncheckable )
	{
		menu_item->Checked		= true;
		menu_item->CheckState	= ( item_check_state == vostok::editor_base::checked_state::checked )	?
									CheckState::Checked : 
...
{
	VOSTOK_UNREFERENCED_PARAMETER( action_name );
	action^ a				= safe_cast<action^>( button_item->Tag );

	button_item->Enabled	= a->enabled( );
	vostok::editor_base::checked_state item_check_state	= a->checked_state( );
	button_item->Checked	= ( item_check_state == vostok::editor_base::checked_state::checked );
}

void gui_binder::update_button ( ToolStripButton^ button_item, action^ action )
{
	button_item->Enabled	= action->enabled( );
	vostok::editor_base::checked_state item_check_state	= action->checked_state( );
	button_item->Checked	= ( item_check_state == vostok::editor_base::checked_state::checked );
}
