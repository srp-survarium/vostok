...
public ref class lua_property_value: vostok::editor::wpf_controls::i_property_value
{
internal:
	lua_property_value( )
	{
...
public:
	lua_property_value( const vostok::configs::lua_config_value& config_value, String^ property_path )
	{
		m_config_value  = CRT_NEW(vostok::configs::lua_config_value)(config_value);
		m_path_steps	= property_path->Split('/');
	}
	
private:
	vostok::configs::lua_config_value*	m_config_value;
	array<String^>^				m_path_steps;

private:
	vostok::configs::lua_config_value config_value( )
	{
		vostok::configs::lua_config_value return_value = *m_config_value;

		if( m_path_steps == nullptr || ( m_path_steps->Length == 1 && m_path_steps[0] == "" ))
			return return_value;

		for each( String^ path_step in m_path_steps )
			return_value = return_value[ vostok::editor_base::unmanaged_string( path_step ).c_str( )];
...
{
public:
	lua_property_value_int( const vostok::configs::lua_config_value& config_value, String^ property_path ):
		lua_property_value<int>( config_value, property_path ){};
};
public ref class lua_property_value_float	: lua_property_value<float>
{
public:
	lua_property_value_float( const vostok::configs::lua_config_value& config_value, String^ property_path ):
		lua_property_value<float>( config_value, property_path ){};
};
public ref class lua_property_value_bool	: lua_property_value<bool>
{
public:
	lua_property_value_bool( const vostok::configs::lua_config_value& config_value, String^ property_path ):
