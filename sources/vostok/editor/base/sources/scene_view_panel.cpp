...
#include <vostok/editor/base/collision_object_control.h>
...
vostok::render::scene_view_mode text_to_render_mode( System::String^ mode_str )
{
	for(int mode = 0; mode<vostok::render::num_view_modes; ++mode)
	{
		if( mode_str == gcnew System::String(view_mode_names[mode]) )
			return (vostok::render::scene_view_mode)mode;
	}
	return vostok::render::lit_view_mode;// default
}

int get_next_render_mode(int mode)
{
	++mode;
	if(mode == vostok::render::num_view_modes)
		mode = (vostok::render::scene_view_mode)0;
...
struct pick_cache{
	...
	bool								m_cached;
	bool								m_result;
	vostok::collision::object_type		m_mask;
	vostok::collision::object const*		m_obj;
};
...
void scene_view_panel::in_constructor( )
{
	...
	m_render_view_mode			= vostok::render::lit_view_mode;
...
void scene_view_panel::apply_render_view_mode( bool text_only )
{
	...
	if(m_editor_renderer && !text_only)
		m_editor_renderer->scene().set_view_mode( *m_scene_view, (vostok::render::scene_view_mode)m_render_view_mode);
}
...
bool scene_view_panel::ray_query(	vostok::collision::object_type const mask, 
...
bool scene_view_panel::ray_query(	vostok::collision::object_type const mask, 
									   ...
									   float3* position )
{
	vostok::collision::ray_objects_type		collision_results( g_allocator );
	...
	*object = NULL;
	...
	{
		vostok::collision::collision_object const* obj = static_cast_checked<vostok::collision::collision_object const*> (collision_results[0].object);
...
{
	vostok::collision::ray_objects_type	results (g_allocator);
	m_collision_tree->ray_query(
		mask,
		origin, 
		direction, 
		1000.f,  
		results, 
		vostok::collision::objects_predicate_type( )
	);
								...
								)
{
	...
	vostok::collision::ray_triangles_type	results (g_allocator);
	m_collision_tree->ray_query(
		mask,
		origin, 
		direction, 
		1000.f,  
		results, 
		vostok::collision::triangles_predicate_type( &triangles_predicate )
	);
								...
								)
{
	...
	m_collision_tree->ray_query(
		...
		1000.f,  
		results, 
		vostok::collision::triangles_predicate_type( )
	);
...
bool scene_view_panel::frustum_query(	vostok::collision::object_type const mask, 
										...
										collision_objects^ objects )
{
	...
	if( collision_results.size() > 0)
	{
		vostok::collision::object const* obj = collision_results[0];
		ASSERT( obj, "The obj must not be NULL.");
		objects->add( obj );
...
vostok::render::editor::renderer*	scene_view_panel::editor_renderer( )
{
	 return m_editor_renderer;
}

vostok::render::debug::renderer* scene_view_panel::debug_renderer( )
{
...
void scene_view_panel::set_camera_effector( vostok::camera_effector* e )
{
	m_camera_effector				= e;
...
void scene_view_panel::window_view_Deactivate( )
{
	...
	unclip_cursor				( );
	vostok::editor_base::input_keys_holder::ref->reset	( gcnew System::Object(), gcnew System::EventArgs());
}
...
void scene_view_panel::remove_action_button_item( vostok::editor_base::gui_binder^ binder, System::String^ action_name, System::String^ strip_key )
{
...
void scene_view_panel::add_action_button_item( vostok::editor_base::gui_binder^ binder, System::String^ action_name, System::String^ strip_key, int prio )
{
	VOSTOK_UNREFERENCED_PARAMETER(prio);
	ToolStrip^ strip			= get_tool_strip(strip_key);
...
void scene_view_panel::add_action_button_item( vostok::editor_base::gui_binder^ binder, vostok::editor_base::action^ action, System::String^ strip_key, int prio )
{
	VOSTOK_UNREFERENCED_PARAMETER(prio);
	ToolStrip^ strip			= get_tool_strip(strip_key);
	ToolStripButton^ button		= nullptr;
...
bool scene_view_panel::context_fit( )
{
	...
	if( vostok::editor::wpf_controls::dot_net_helpers::is_type<WeifenLuo::WinFormsUI::Docking::FloatWindow^>( frm ) )
	{
