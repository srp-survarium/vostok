...
//#include <vostok/editor/base/converting.h>

using vostok::editor_base::float_curve;
using vostok::editor_base::float_curve_pair;
...
float_curve::float_curve(vostok::configs::lua_config_value const& curve_config)
:m_min_time(0.0f),
m_max_time(100.0f),
m_min_value(0.0f),
m_max_value(100.0f),
...
void float_curve::alter_key(float_curve_pair^ key, float time, float value)
{
	if(key!=m_container[0] && key!=m_container[m_container->Count-1])
		key->time		= vostok::math::clamp_r(time, min_time, max_time);
		key->value			= vostok::math::clamp_r(value, min_value, max_value);
...
void float_curve::load_keys_from_config(vostok::configs::lua_config_value const& keys_config){
...
void		float_curve::load(vostok::configs::lua_config_value& config){
	...
	vostok::configs::lua_config_value keys = config["keys"];
