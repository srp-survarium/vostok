...
using vostok::texture_compressor_test::application;

vostok::memory::doug_lea_allocator_type	vostok::texture_compressor_test::g_allocator;

class texture_compressor_test_core_engine : public vostok::core::simple_engine
{
public:
...
static vostok::uninitialized_reference< texture_compressor_test_core_engine >	s_core_engine;
...
static vostok::command_line::key
	s_target_format(
		"target_format",
		"tf",
		"texture compressor",
...
{
	VOSTOK_UNREFERENCED_PARAMETER		( temp_data );
...
{
//	process_folder		("e:\\survarium\\resources\\sources\\textures");
...
/*
	...
	vostok::texture_compressor::load_tga(
	...
	u8* destination_buffer			=
		create_texture(
			...
			buffer_size,
			true,
			vostok::texture_compressor::texture_2D
		);
