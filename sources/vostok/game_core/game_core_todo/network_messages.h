////////////////////////////////////////////////////////////////////////////
//	Created 	: 12.10.2025
////////////////////////////////////////////////////////////////////////////

#ifndef NETWORK_MESSAGES_H_INCLUDED
#define NETWORK_MESSAGES_H_INCLUDED

namespace survarium {

// STATE[STUB]
// vostok::network_core::udp_match_message_type_info survarium::network_packets_orderer<enum vostok::match_client_message_types_enum,enum vostok::match_server_message_types_enum>::get_sending_message_info(unsigned char)
vostok::network_core::udp_match_message_type_info network_packets_orderer<enum vostok::match_client_message_types_enum,enum vostok::match_server_message_types_enum>::get_sending_message_info( u8 message_type )
{
	// FUNCTION BODY
	// <0xa7db0>|0x000|0x000:'35'
	// ******
}

} // namespace survarium

#endif // #ifndef NETWORK_MESSAGES_H_INCLUDED
