...
class pistol_weapon_core_idle_state : public weapon_core_idle_state_base {
	...
	static resources::class_id_enum const resource_class = resources::pistol_weapon_idle_state_class;

private:
	// ctor mangles ??0...@@AAE@... -> private, non-const (objdiff pairs by mangled name)
				explicit							pistol_weapon_core_idle_state	( weapon_core& weapon, resources::managed_resource_ptr const* animations, const u32 animations_count );
