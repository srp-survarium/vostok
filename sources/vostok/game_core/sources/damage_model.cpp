...
// STATE[UNVERIFIED]
survarium::booster_damage_protector::booster_damage_protector(
	pcstr									damage_type,
	float									reduce,
	float									absorb):
	m_reduce								(reduce),
...
// STATE[UNVERIFIED]	
float survarium::booster_damage_protector::reduce_damage(
	pcstr									__formal,
	pcstr									damage_type,
	float									amount)
{
...
public:
	dump_npc_body_part_state_predicate(
		vostok::ai::npc_statistics&		npc_stats,
		u32								current_time) :
		npc_stats						(npc_stats),
...
private:
	/* offset 0x0000 */ vostok::ai::npc_statistics&           npc_stats;
...
//								  There are calls to DELETE however in the vostok source code
