...
namespace vostok
{
	...
	*/
	...
	// claude@MATCH: use_network_core_tcp_packet / use_network_core_tcp_packet_client /
	// use_network_core_http_client anchors removed - every symbol they kept is now
	// reachable from REAL matched callers: the tcp_packet ctor via network_world::
	// new_packet + network::tcp_packet_client::send; buffer_to_send/_receive_into via
	// tcp_packet_socket_inline (network_core::tcp_packet_client send/start_receiving);
	// the network_core tcp_packet_client ctor/connect/disconnect/send via network::
	// tcp_packet_client (create_client/connect_impl/order binds; io_service() is an
	// inline accessor, no symbol); the network_core http_client ctor/set_on_error/get
	// via network::http_client (create_client_impl/get_impl); read_lines_from_stream
	// via network_core http_client's own read_status_line handler.

	// claude@MATCH: pruned to the three load-bearing devices. Removed as redundant
	// (real matched callers exist): get_ip_address via network::login_client::create_client;
	// memory_allocator/initialize/finalize via network::initialize/finalize
	// (network_entry_point.cpp, reached from use_network_clients); the udp_match_stats/
	// stream/items operator>= chain via the matched enqueue_order/send_queued_order/
	// receive_udp_response execute bodies (stats op>= calls the stream op>= out-of-line;
	// items op>= inlines into it). KEPT: get_connection_info_from_string (no real caller
	// anywhere yet) and the two address-takes (their real callers live in the still-
	// unmatched game module).
	...
	// claude@MATCH: use_network_core_udp_match_connection / use_network_core_udp_match_client /
	// use_network_core_udp_network_flow_emulator anchors removed - all reachable from REAL
	// matched callers: the connection ctor + connect/enqueue/send_queued_packets/disconnect/
	// instant_disconnect/packets_count via udp_match_client (m_connection member + methods);
	// is_low_level_packet via the flow emulator's add_packet; the udp_match_client ctor/
	// connect/start_receiving/enqueue/send_queued_packets/disconnect via match_client_impl
	// (m_client member + methods); check_consistency via udp_match_client's own methods;
	// handle_receive via start_receiving's async_receive_from bind (matched since); on_error
	// via handle_receive; the emulator ctor/dtor via match_client_impl (NEW/DELETE), tick +
	// on_packet_received via udp_match_client (boost::function params, not templates - same
	// symbols from any caller).
