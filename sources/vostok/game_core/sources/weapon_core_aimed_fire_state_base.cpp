////////////////////////////////////////////////////////////////////////////
//	Created 	: 06.12.2025
////////////////////////////////////////////////////////////////////////////

#include "pch.h"
#include "weapon_core_aimed_fire_state_base.h"

namespace survarium {

// STATE[STUB]
// survarium::weapon_core_aimed_fire_state_base::weapon_core_aimed_fire_state_base(survarium::weapon_core&, float)
weapon_core_aimed_fire_state_base::weapon_core_aimed_fire_state_base( weapon_core& weapon, float animation_timescale )
{
	// FUNCTION BODY
	// <0x59e7b0>|0x000|+0x059:'21'	{
	// <0x59e809>|0x059|      :'22'	}
	// ******
}

// STATE[STUB]
// void survarium::weapon_core_aimed_fire_state_base::initialize()
void weapon_core_aimed_fire_state_base::initialize( )
{
	// CALL SITE INFO
	// <0x59e956> -> void <unknown>()
	// ******

	// FUNCTION BODY
	// <0>
	// <1>
	// <2>
	// <3>
	// <4>
	// <5>
	// <0x59e830>|0x010|+0x01b:'32'
	// <0x59e84b>|0x02b|+0x00b:'33'
	// <0x59e856>|0x036|+0x0de:'34'
	// <0x59e934>|0x114|+0x024:'35'
	// <0x59e958>|0x138|+0x0f6:'36'
	// <0x59ea4e>|0x22e|+0x035:'37'
	// ******
}

// STATE[STUB]
// void survarium::weapon_core_aimed_fire_state_base::execute()
void weapon_core_aimed_fire_state_base::execute( )
{
	// FUNCTION BODY
	// <0x59e797>|0x007|+0x008:'42'
	// <0>
	// <1>
	// <2>
	// <3>
	// <4>
	// <5>
	// <0x59e79f>|0x00f|+0x00a:'49'
	// ******
}

// STATE[STUB]
// void survarium::weapon_core_aimed_fire_state_base::finalize()
void weapon_core_aimed_fire_state_base::finalize( )
{
	// CALL SITE INFO
	// <0x59e752> -> void <unknown>()
	// ******

	// FUNCTION BODY
	// <0>
	// <1>
	// <2>
	// <3>
	// <4>
	// <5>
	// <0x59e719>|0x009|+0x008:'60'
	// <0x59e721>|0x011|+0x017:'61'
	// <0x59e738>|0x028|+0x01c:'62'
	// <0x59e754>|0x044|+0x017:'63'
	// <0>
	// <0x59e76b>|0x05b|+0x018:'65'
	// ******
}

// STATE[STUB]
// vostok::animation::callback_return_type_enum survarium::weapon_core_aimed_fire_state_base::on_shot_event(vostok::animation::animation_callback_params&)
animation::callback_return_type_enum weapon_core_aimed_fire_state_base::on_shot_event( animation::animation_callback_params& params )
{
	// FUNCTION BODY
	// <0>
	// <0x59eaa1>|0x011|+0x007:'71'
	// <0>
	// <1>
	// <2>
	// <3>
	// <4>
	// <5>
	// <0x59eaa8>|0x018|+0x00c:'78'
	// <0x59eab4>|0x024|+0x00c:'79'
	// <0>
	// <1>
	// <0x59eac0>|0x030|+0x01c:'82'
	// <0x59eadc>|0x04c|+0x077:'83'
	// <0x59eb53>|0x0c3|+0x004:'84'
	// <0>
	// <1>
	// <2>
	// <0x59eb57>|0x0c7|+0x00c:'88'
	// <0>
	// <1>
	// <0x59eb63>|0x0d3|+0x015:'91'
	// <0>
	// <0x59eb78>|0x0e8|+0x00c:'93'
	// <0>
	// <0x59eb84>|0x0f4|+0x002:'95'
	// ******
}

// STATE[STUB]
// void survarium::weapon_core_aimed_fire_state_base::on_animation_end_impl(bool&)
void weapon_core_aimed_fire_state_base::on_animation_end_impl( bool& animation_player_tick_result )
{
	// FUNCTION BODY
	// <0>
	// <0x59e687>|0x007|+0x006:'101'
	// ******
}

// STATE[STUB]
// vostok::animation::callback_return_type_enum survarium::weapon_core_aimed_fire_state_base::on_aiming_event(vostok::animation::animation_callback_params&)
animation::callback_return_type_enum weapon_core_aimed_fire_state_base::on_aiming_event( animation::animation_callback_params& params )
{
	// CALL SITE INFO
	// <0x59e6e7> -> void <unknown>()
	// <0x59e705> -> void <unknown>()
	// ******

	// FUNCTION BODY
	// <0x59e6a9>|0x009|+0x00c:'106'
	// <0x59e6b5>|0x015|+0x00c:'107'
	// <0>
	// <0x59e6c1>|0x021|+0x00c:'109'
	// <0x59e6cd>|0x02d|+0x01c:'110'
	// <0x59e6e9>|0x049|+0x002:'111'
	// <0x59e6eb>|0x04b|+0x01c:'112'
	// <0>
	// <0x59e707>|0x067|+0x002:'114'
	// ******
}

} // namespace survarium
