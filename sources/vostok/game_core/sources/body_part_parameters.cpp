...
void body_part_parameters::fill_new_stats_item( stats_item_type& new_stats_item, const u32 current_time_in_ms ) const
{
	...
	// FUNCTION BODY (kept: PARTIAL, not 100%; target --view structure rva 0x0ba3c0)
	// LOCALS
	// vostok::fixed_string<46>          new_item
	// u32                             i<1>
	// u32                             remaining_time_in_ms<2>
	// ; 18 statements, 0x228 bytes
	// 0x00  <0x10>  L285							{ (function open)
	// 0x10  <0x11>  L286							new_stats_item.caption = m_name;
	// 0x21  <0x0d>  L287							new_stats_item.caption.append( " state:" );
	// 0x2e  <0x08>  L289							vostok::fixed_string<46> new_item;
	// 0x36  <0x3c>  L290							new_item.appendf( "HP: %4.2f/%4.2f", m_health, m_max_health );
	// 0x72  <0x0f>  L291							new_stats_item.content.push_back( new_item );
	// 0x81  <0x08>  L293							new_item.clear( );
	// 0x89  <0x0d>  L294							new_item.append( "affects: " );
	// 0x96  <0x0f>  L295							new_stats_item.content.push_back( new_item );
	// 0xa5  <0x3e>  L297	[1]						for ( u32 i = 0 ; i < m_affects.size( ) ; ++i )
	// 0xe3  <0x08>  L299							new_item.clear( );
	// 0xeb  <0x67>  L300							const u32 remaining_time_in_ms = ... >= ... ? ... - ... : 0;
	// 0x152 <0x5e>  L301							new_item.appendf( "%s [%4.2f seconds left]", affects_captions[...], .../1000.0f );
	// 0x1b0 <0x0f>  L302							new_stats_item.content.push_back( new_item );
	// 0x1bf <0x05>  L303							} (loop close)
	// 0x1c4 <0x2c>  L305							if ( m_affects.empty( ) )
	// 0x1f0 <0x31>  L306							new_stats_item.content.push_back( vostok::fixed_string<46>( "none" ) );  claude@MATCH: target inlines fixed_string<46>(char const*) here; base calls it (see .md)
	// 0x221 <0x07>  L307							} (function close)
	// ******
}
...
// claude@NOTE: fill_new_stats_item call wired in so the template instantiation has its genuine
// caller; the trailing stats.<member>.push_back is still blocked (npc_statistics has no member yet).
