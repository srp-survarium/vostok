...
// STATE[91.78%|PARTIAL]: body structurally matched; residual is the inline-vs-call of the
// fixed_string<46>("none") ctor (target inlines it; base keeps it out-of-line) plus the frame/reg
// cascade that follows. Under the new guidelines this is a matching problem to re-diff against
// source, not a clean LTCG excuse - see the .md for the rich-index proof and next steps.
