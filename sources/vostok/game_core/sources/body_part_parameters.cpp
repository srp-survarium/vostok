...
// claude@NOTE: It is a template member instantiated for ai::statistics_item<46,16>. Builds a
// human-readable "<name> state:" caption plus content lines (HP, "affects:", one line per active
// affect with remaining seconds, or "none" when empty) for the npc statistics dump.
// claude@NOTE: body is a structural match; the residual diff is LTCG inline-vs-call of the
// fixed_string<46>("none") ctor. TARGET inlines fixed_string<46>(const char*) (no out-of-line
// symbol survives whole-program), exposing its body as a direct
// buffer_string::buffer_string(m_buffer, 46, "none") call + a [ebp-CCh] capacity temp; BASE keeps
// that ctor out-of-line (sym @base 0x030b00) and emits one `call`. The out-of-line base ctor body
// IS that exact 3-arg buffer_string call, so the source `fixed_string<46>("none")` is already
// correct - the inline choice is not steerable per call-site under /Od+/GL. The +0x10 frame-size
// delta (sub esp,0E8h vs 0D8h) and the cascading reg/slot renaming all follow from that one inline.
// claude@NOTE: the three `mov byte[ebp-N],0; lea eax; call empty_stub` (~+0x0c) inside the
// loop are the inlined `ASSERT( index < size() )` from buffer_vector::operator[] - they are
// ALREADY reproduced by the `m_affects[i]` subscripts (base emits the same 3 calls), so there
// is no missing standalone ASSERT to recover here; the residual is purely the fixed_string LTCG.
// STATE[91.78%|PARTIAL]: body matched; only LTCG inline-vs-call of fixed_string<46>("none") + frame/regs remain
...
// claude@NOTE: fill_new_stats_item call wired in so the private template instantiation is reachable
// (its genuine caller); the trailing stats.<member>.push_back is still blocked (npc_statistics has
// no matching body_state member yet).
