...
// STATE[99.98%|DONE]: byte-identical instruction stream; sole residual is a relocation/symbol
// artifact at the ctor call boundary (same class the pistol/idle new_object siblings accept).
...
)
{
	...
	float user_anim_length = animation::cubic_spline_skeleton_animation_pinned( animations[1] )->length_in_frames( );
