...
//using vostok::animation::anim_track_discrete;
...
template< class TrackType >
void	t_animation_data< TrackType >::bone_pose( vostok::float4x4 &pose, u32 id, float time )
{
...
template < typename T >
inline void lua_write( T const& v,  vostok::configs::lua_config_value &cfg )
{
	cfg = v; //[0]
}

template <>
inline void lua_write<float>( float const& v,  vostok::configs::lua_config_value &cfg )
{
	cfg = v; //[0]
}
...
template < typename T >
inline void lua_read( T& v, vostok::configs::lua_config_value const &cfg )
{
	v = cfg; // [0]
}

inline void lua_write( const animation_curve_data_header &h, vostok::configs::lua_config_value f )
{
	f["type"] = (u32)h.type;
	f["frequency"] = h.equidistant_frames_frequency;
}

inline void lua_read(  animation_curve_data_header &h, vostok::configs::lua_config_value const &f )
{
...
template< typename T >
inline void lua_read_vector(  vector<T> &v, vostok::configs::lua_config_value const &f )
{
	u32 size = f.size();
...
template< typename  T >
inline void lua_write_vector( vector<T> const& v, vostok::configs::lua_config_value cfg )
{
	u32 size = v.size();

	for( u32 i= 0; i< size ; ++i ) {
		vostok::configs::lua_config_value tmp_cfg = cfg[i];
