...
public  vostok::maya_animation::discrete_data
{
public:
	...
	virtual	void				save				( vostok::configs::lua_config_value &cfg ) const ;
	virtual	void				load				( vostok::configs::lua_config_value const& cfg, bool bone_names_only = false ) ;
	virtual	void				check				( vostok::configs::lua_config_value &discrete_data ) const;

	virtual void				add_frame			( u32 bone, frame const& frm, frame const& base_frm );
	virtual	void				calculate			( );

	void						fix_angles			(u32 bone );
	virtual void				get_frame			( u32 bone, u32 number, float &time, vostok::animation::frame& frame )const;
...
void	discrete_data_impl::save( vostok::configs::lua_config_value &cfg ) const 
{
	...
	vostok::configs::lua_config_value tmp_cfg = cfg["bones"];
	data.save( tmp_cfg );
	
}

void	discrete_data_impl::load( vostok::configs::lua_config_value const& cfg, bool bone_names_only /*= false*/ ) 
{
	...
	data.load( vostok::configs::lua_config_value( cfg["bones"] ) );

}

void	discrete_data_impl::check( vostok::configs::lua_config_value &discrete_data ) const
{
...
void build_channel( bi_spline_data< float > &channel, float epsilon, const float *p, u32 size,  vostok::maya_animation::raw_bi_spline_1d *raw_bi_spline )
{
...
void build_channel( u32 bone, u32 ch, float epsilon,
					...
					vostok::maya_animation::raw_bi_spline_1d *raw_bi_spline
					)
{
	vostok::animation::bi_spline_data< float > &channel =
...
static void set_fp_state( u32& previous_denormals, u32& previous_control_word )
{
...
#else
	VOSTOK_UNREFERENCED_PARAMETER			( previous_denormals );
	...
	R_ASSERT							( !error );
#else // #ifdef _MSC_VER
	VOSTOK_UNREFERENCED_PARAMETER			( previous_control_word );
#endif // #ifdef _MSC_VER
}

static void restore_fp_state			( u32 denormals, u32 control_word )
{
	VOSTOK_UNREFERENCED_PARAMETER			( denormals );
	...
	R_ASSERT							( !error );
#else // #ifdef _MSC_VER
	VOSTOK_UNREFERENCED_PARAMETER			( control_word );
#endif // #ifdef _MSC_VER
...
void	maya_animation_world::build_animation_data( const discrete_data &data, bi_spline_skeleton_animation &animation_data   )const
{
	...
	vostok::maya_animation::raw_bi_spline_1d *raw_bi_spline = create_1d_bi_spline	( );
