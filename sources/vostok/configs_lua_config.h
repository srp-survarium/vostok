...
#ifndef VOSTOK_CONFIGS_LUA_CONFIG_H_INCLUDED
#define VOSTOK_CONFIGS_LUA_CONFIG_H_INCLUDED

#include <vostok/resources.h>
#include <vostok/configs_lua_config_value.h>
...
public:
	...
	inline	void				swap				( lua_config& other )								{ m_root.swap( other.m_root ); }
			...
			void				save				( vostok::strings::stream& stream ) const;
...
VOSTOK_CORE_API	lua_config_ptr  create_lua_config	( mutable_buffer const & buffer, pcstr file_name );
VOSTOK_CORE_API	lua_config_ptr	create_lua_config	( pcstr file_name = 0 );
VOSTOK_CORE_API	lua_config_ptr	create_lua_config	( pcstr file_name, lua_config_value const& value );
VOSTOK_CORE_API	lua_config_ptr	create_lua_config_from_string ( pcstr string );
VOSTOK_CORE_API	lua_config_ptr	create_lua_config	( binary_config_ptr const& config );

typedef std::pair<pcstr, u32>	enum_tuple;
VOSTOK_CORE_API	void			export_enum			( pcstr const table_id, enum_tuple const* begin, enum_tuple const* end );

} // namespace configs
} // namespace vostok

	inline	void				swap				( vostok::configs::lua_config& left, vostok::configs::lua_config& right ) { left.swap(right); }

#ifdef MASTER_GOLD
#	error class must not be used in MASTER_GOLD
#endif

#endif // #ifndef VOSTOK_CONFIGS_LUA_CONFIG_H_INCLUDED