...
{
	T t = y / vostok::math::max(x, vostok::math::epsilon_5);
	return t;
}
...
TEMPLATE_SIGNATURE
inline u32 CURVE_LINE::save_binary( vostok::mutable_buffer& buffer, bool calc_size)
{
	u32 save_size = num_points * sizeof(point_type);
	if (!calc_size)
	{
		vostok::memory::copy(buffer.c_ptr(), save_size, points, save_size);
		buffer+=save_size;
	}
	return save_size;
}
...
TEMPLATE_SIGNATURE
inline void CURVE_LINE::load_binary( vostok::mutable_buffer& buffer )
{
...
inline void CURVE_LINE::calc_tangents()
{
	for (u32 i=0; i<num_points; i++)
	{
		...
		{
			if (i<num_points-1)
				vostok::memory::zero(&tangent_out, sizeof(T));
			else
				vostok::memory::zero(&tangent_out, sizeof(T));
		}
		else
		{
			if (i<num_points-1)
			{
				...
				vostok::memory::zero(&tangent_in, sizeof(T));
		}
	}
}
...
inline void CURVE_LINE::operator =(curve_line_points const& other)
{
	...
	reserve			(other.num_points);

	for (u32 i=0; i<other.num_points; i++)
	{
		point_type point;
		other.get_point	(i, point);
		set_point		(i, point);
	}
...
inline u32 curve_line_ranged_base::save_binary(vostok::mutable_buffer& buffer, bool calc_size)
{
	return m_upper.save_binary(buffer, calc_size) +
		m_lower.save_binary(buffer, calc_size);
}

inline void curve_line_ranged_base::load_binary(vostok::mutable_buffer& buffer)
{
