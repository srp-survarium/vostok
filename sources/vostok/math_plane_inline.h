...
#ifndef VOSTOK_MATH_PLANE_INLINE_H_INCLUDED
#define VOSTOK_MATH_PLANE_INLINE_H_INCLUDED
...
{
    R_ASSERT                            ( vostok::math::valid( normal ) && vostok::math::valid( d ) );
    R_ASSERT                            ( vostok::math::valid( position ) );
    R_ASSERT                            ( vostok::math::valid( direction ) );

    float const signed_distance         = classify( position );
    float const cosine_alpha            = normal | direction;
    const float distance                = cosine_alpha != 0.f ? signed_distance/-cosine_alpha : -vostok::math::abs( signed_distance );
	intersection						= position + direction * distance;
    return                              distance >= 0.f;

}

inline bool plane::intersect_segment				( vostok::math::float3 const& first, vostok::math::float3 const& second, vostok::math::float3& intersection_position ) const
{
    vostok::math::float3 const& second_minus_first    = second - first;
    float const signed_projection					= normal | second_minus_first;
    float const first_signed_distance				= classify( first );
    float const distance							= signed_projection != 0.f ? first_signed_distance/-signed_projection : -vostok::math::abs(first_signed_distance);
...
#endif // #ifndef VOSTOK_MATH_PLANE_INLINE_H_INCLUDED