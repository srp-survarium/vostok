										  ...
										  log_file_usage					log_file_usage )
{
	...
	{
		VOSTOK_CONSTRUCT_REFERENCE			( globals->synchronous_device, fs_new::synchronous_device_interface ) ( device );
		fs_new::native_path_string			file_name;
		generate_log_file_name				( & file_name, "log" );
		VOSTOK_CONSTRUCT_REFERENCE			( globals->log_file, log_file ) ( globals->log_file_usage, file_name.c_str(), device );
	}
...
void finalize							( )
{
	...
	if ( globals->log_file.initialized( )  )
		VOSTOK_DESTROY_REFERENCE				( globals->log_file );
	if ( globals->synchronous_device.initialized( ) )
		VOSTOK_DESTROY_REFERENCE				( globals->synchronous_device );
