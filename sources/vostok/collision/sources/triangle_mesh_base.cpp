...
void triangle_mesh_base::initialize		( float3 const* const vertices, u32 const vertex_count, u32 const* const indices, u32 const index_count )
{
	...
	m_mesh					= VOSTOK_NEW_IMPL ( m_allocator, Opcode::MeshInterface )( m_allocator );
	...
	m_model					= VOSTOK_NEW_IMPL ( m_allocator, Opcode::Model )( m_allocator );
...
{
	VOSTOK_DELETE_IMPL		( m_allocator, m_model );
	VOSTOK_DELETE_IMPL		( m_allocator, m_mesh );
}
...
void triangle_mesh_base::render			( vostok::render::debug::renderer& renderer, float4x4 const& matrix ) const
{
...
void triangle_mesh_base::generate_contacts ( vostok::collision::on_contact& c, const float4x4 &self_transform, const float4x4 &transform,  const vostok::collision::compound_geometry& og ) const	
{
	VOSTOK_UNREFERENCED_PARAMETERS	(&c, &self_transform, &transform, &og);
}
