...
inline bool check_neighbours( const vostok::buffer_vector<triangle_sphere_res> & vec, 
						   u32 v0, u32 v1, u32 v2 )
{
	if ( vec.empty() )
		return		false;

	vostok::buffer_vector<triangle_sphere_res>::const_iterator i			= vec.begin();
	vostok::buffer_vector<triangle_sphere_res>::const_iterator const e	= vec.begin() + vec.size() - 1;
	for( ; i != e; ++i )
