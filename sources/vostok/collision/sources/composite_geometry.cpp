...
composite_geometry::composite_geometry	(
	...
	m_geometry_instances				(
		VOSTOK_MALLOC_IMPL( allocator, instances.size() * sizeof( geometry_instances_type::value_type ), "geometry instances vector" ),
		instances.size(),
		instances.begin(),
		instances.end()
	)
...
//		delete_geometry_instance	( allocator, *i );
	...
	VOSTOK_FREE_IMPL					( allocator, begin );
}
...
{
	VOSTOK_UNREFERENCED_PARAMETER	( position );
	//vectora< geometry_instance const* >::const_iterator i = m_geometry_instances.begin(), e = m_geometry_instances.end();
	//for ( ; i != e; ++i ) {
	//	if ( (*i)->is_inside() )
	//		return	true;
...
{
	VOSTOK_UNREFERENCED_PARAMETER		( bounding_convex );
	NOT_IMPLEMENTED					(return 0);
}
