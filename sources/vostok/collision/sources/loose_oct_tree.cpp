...
using vostok::collision::ray_objects_type;
using vostok::collision::objects_type;
using vostok::collision::objects_predicate_type;
using vostok::collision::ray_triangles_type;
using vostok::collision::results_callback_type;
using vostok::collision::triangles_type;
using vostok::collision::triangles_predicate_type;
using vostok::collision::oct_node;
//using vostok::collision::query_mask;
...
public:
		...
		m_node_count		( 0 )
	{
		#pragma message ( VOSTOK_TODO( "use reserve_item_count here!" ) )
		VOSTOK_UNREFERENCED_PARAMETER(reserve_item_count);
	}
			...
			~vertex_allocator	( )
	{
		while ( m_nodes ) {
			...
			VOSTOK_DELETE_IMPL( m_allocator, node );
		}
	}
	...
	oct_node* allocate			( )
	{
		...
		if ( result )
			...
			result			= VOSTOK_NEW_IMPL( m_allocator, oct_node );
...
#endif // #ifdef DEBUG
	m_allocator				( VOSTOK_NEW_IMPL(allocator, vertex_allocator)( allocator, reserve_item_count) ),
	m_root					( 0 ),
#ifdef DEBUG
...
loose_oct_tree::~loose_oct_tree		( )
{
	...
	VOSTOK_DELETE_IMPL		( allocator, m_allocator );
}
...
#else // #ifdef DEBUG
	VOSTOK_UNREFERENCED_PARAMETERS	( node, object_count );
#endif // #ifdef DEBUG
}
...
void loose_oct_tree::render			( vostok::render::scene_ptr const& scene, vostok::render::debug::renderer& renderer )
{
	if ( !m_initialized )
		return;
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	(&origin, &direction, max_distance, &predicate);
	return true; 
}
