...
cylinder_geometry::cylinder_geometry( vostok::memory::base_allocator* allocator, float radius, float half_length ):
m_radius( radius ), m_half_length( half_length )
{
	VOSTOK_UNREFERENCED_PARAMETERS(allocator);
}
...
vostok::math::aabb& cylinder_geometry::get_aabb		( math::aabb& result ) const
{
	result				= 
		math::create_min_max(
...
{
	VOSTOK_UNREFERENCED_PARAMETERS		(&aabb);
	return true;
}

bool cylinder_geometry::cuboid_test		( math::cuboid const& cuboid ) const
{
	VOSTOK_UNREFERENCED_PARAMETERS		(&cuboid);
	return true;
}
