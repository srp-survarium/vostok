...
using vostok::collision::triangle_mesh_buffer;
using vostok::collision::triangle_mesh_resource;
using vostok::collision::loose_oct_tree;
...
{
	return VOSTOK_NEW_IMPL( allocator, triangle_mesh_buffer )(
			allocator,
			vertices,
			vertex_count,
			indices,
...
{
	return VOSTOK_NEW_IMPL( allocator, triangle_mesh_resource )( allocator, resource );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, box_geometry_instance ) ( matrix );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, cylinder_geometry_instance ) ( matrix, radius, half_length );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, terrain_geometry_instance ) ( matrix, physical_size, vertex_row_size, heightfield, delete_by_collision_object );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, capsule_geometry_instance )( matrix, radius, half_length );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, composite_geometry )( allocator, instances );
}
	...
	)
{
	...
	return VOSTOK_NEW_IMPL( allocator, composite_geometry_instance )( allocator, matrix, geom );
}
											...
											)
{
	...
	return VOSTOK_NEW_IMPL( allocator, triangle_mesh_geometry_instance )( matrix, mesh_geom );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, truncated_sphere_geometry_instance )( matrix, radius, planes, planes_count );
}
...
void delete_geometry		( memory::base_allocator* allocator, geometry* geometry )
{
	...
	VOSTOK_DELETE_IMPL		( allocator, geometry );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, loose_oct_tree )( allocator, min_aabb_radius, reserve_node_count );
}
...
void delete_space_partitioning_tree	( space_partitioning_tree* tree )
{
	...
	VOSTOK_DELETE_IMPL( &temp->get_allocator(), temp );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, collision_object ) ( allocator, object_type, &*geometry, user_data );
}
...
{
	return VOSTOK_NEW_IMPL( allocator, aabb_object ) ( object_type, create_aabb_center_radius(position, epsilon), user_data );
}
		...
		memory::base_allocator* allocator )
{
	...
	pvoid arena						= VOSTOK_MALLOC_IMPL( allocator, total_object_size, description );
...
void delete_animated_object	( animated_object* object_to_be_deleted, memory::base_allocator* allocator )
{
	...
	VOSTOK_FREE_IMPL							( allocator, object_to_be_deleted );
}
