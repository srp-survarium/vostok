...
gpu_counter::gpu_counter()
{
	...
	HRESULT hr = vostok::render::device::ref().d3d_device()->CreateCounter( &desc, &m_pixel_processing_counter );
	...
	for(int i=0;i<nCounters;i++) {
		...
		HRESULT h = vostok::render::device::ref().d3d_device()->CreateCounter( &d, &(Counter[ i ]) );

		FAILED(h) ? printf("\nnot supported counter (%d)",i) : printf("\n*** supported (%d)",i);
	}
	OutputCounters(vostok::render::device::ref().d3d_device());
...
#else
	vostok::render::device::ref().d3d_context()->Begin(m_pixel_processing_counter);
#endif
}
...
#else
	vostok::render::device::ref().d3d_context()->End(m_pixel_processing_counter);
#endif
}
...
#else
	while ( vostok::render::device::ref().d3d_context()->GetData(m_pixel_processing_counter, &pixel_processing_time, sizeof(pixel_processing_time), 0) == S_FALSE );
#endif
