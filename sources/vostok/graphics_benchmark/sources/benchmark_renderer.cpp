...
{
	m_geom = resource_manager::ref().create_geometry( F_L, sizeof(colored_vertex), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_low_geom = resource_manager::ref().create_geometry( F_L_24, sizeof(low_colored_vertex), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());


	m_geom_sized[0] = resource_manager::ref().create_geometry( vertex_descs[0], sizeof(colored_vertex_sized<1>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_geom_sized[1] = resource_manager::ref().create_geometry( vertex_descs[1], sizeof(colored_vertex_sized<2>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_geom_sized[2] = resource_manager::ref().create_geometry( vertex_descs[2], sizeof(colored_vertex_sized<3>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_geom_sized[3] = resource_manager::ref().create_geometry( vertex_descs[3], sizeof(colored_vertex_sized<4>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_geom_sized[4] = resource_manager::ref().create_geometry( vertex_descs[4], sizeof(colored_vertex_sized<6>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_geom_sized[5] = resource_manager::ref().create_geometry( vertex_descs[5], sizeof(colored_vertex_sized<10>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
	m_geom_sized[6] = resource_manager::ref().create_geometry( vertex_descs[6], sizeof(colored_vertex_sized<14>), vostok::render::backend::ref().vertex.buffer(), vostok::render::backend::ref().index.buffer());
...
u32 benchmark_renderer::cook_mesh(void* vertices, u32 const vertex_size, u32 const vertex_count, colored_indices_type  const& indices)
{
	...
	void* vbuffer = vostok::render::backend::ref().vertex.lock( vertex_count, vertex_size, msh.voffset);
	memory::copy( vbuffer, vertex_count*vertex_size, vertices, vertex_count*vertex_size);
	vostok::render::backend::ref().vertex.unlock();

	// Prepare index buffer
	u16* ibuffer = (u16*) vostok::render::backend::ref().index.lock( indices.size(), msh.ioffset);
	memory::copy( ibuffer, indices.size()*sizeof(u16), &indices[0], indices.size()*sizeof(u16));
	vostok::render::backend::ref().index.unlock();
...
u32 benchmark_renderer::cook_mesh(colored_vertices_type const& vertices, colored_indices_type  const& indices)
{
	...
	// Prepare ill vertex buffer.
	colored_vertex* vbuffer = (colored_vertex*) vostok::render::backend::ref().vertex.lock( vertices.size(), sizeof(colored_vertex), msh.voffset);
...
void benchmark_renderer::render_cooked_mesh(u32 mesh_index, vostok::math::float4x4 const& wvp_matrix)
{
...
{
	m_low_geom->apply();
	backend::ref().set_vs_constant( m_WVP, vostok::math::float4x4().identity());
...
void benchmark_renderer::render_3d_sphere(gpu_timer&, vostok::math::float4x4 const& wvp_matrix)
{
