...
using namespace vostok::render_dx10;
...
{ 
	...
	vostok::render_dx10::resource_manager::ref().add_shader_code((current_test_name + "simple_shader.vs").c_str(),vs_code,true);
	vostok::render_dx10::resource_manager::ref().add_shader_code((current_test_name + "simple_shader.ps").c_str(),new_ps_code,true);
	...
	if (!m_X)
	{
		...
		m_texture = vostok::render_dx10::resource_manager::ref().create_texture2d("$user$texture",128,128,0,texture_format,D3D_USAGE_DEFAULT,1);
		
		//DELETE_ARRAY(data.pSysMem);
		
		//m_texture = vostok::render_dx10::resource_manager::ref().create_texture("detail/detail_beton_det4");
