...
static vostok::render::constant_host* g_gvVars[4] = {NULL,NULL,NULL,NULL,};
static vostok::render::constant_host* g_gsVars[4] = {NULL,NULL,NULL,NULL,};
static vostok::render::constant_host* g_view_dir  = NULL;
...
bool test_simple_shader_op::initialize	( benchmark& benchmark, group_to_execute& group, test_to_execute& test, std::map<std::string,std::string>& parameters, std::string& out_comments)
{
	...
	vostok::render::resource_manager::ref().add_shader_code((current_test_name + "simple_shader.vs").c_str(),m_vs_code.c_str(),true);
	vostok::render::resource_manager::ref().add_shader_code((current_test_name + "simple_shader.ps").c_str(),m_ps_code.c_str(),true);
	...
	vostok::render::texture_slots const& tex_slots = m_sh->get_technique(0)->get_pass(0)->get_ps()->hw_shader()->data().textures;
			...
			for (u32 j=0; j<m_textures.size(); j++)
				if (m_textures[j].name==tex_name)
				{
					...
					if (value.find("DXGI_FORMAT")!=-1)
					{
						...
						m_textures[j].texture = vostok::render::resource_manager::ref().create_texture2d(user_texture_name.c_str(),1024,1024,0,get_dxgi_format(value),D3D_USAGE_DEFAULT,1);
					}
					else 
						m_textures[j].texture = vostok::render::resource_manager::ref().create_texture( value.c_str(), 0, false, false, false );
				}
		}
...
void test_simple_shader_op::add_texture( char const* name, std::string&, u32 size_x, u32 size_y, D3D_SUBRESOURCE_DATA* data, DXGI_FORMAT format)
{
	...
	texture_to_shader t2s;
	t2s.name	= name;
	t2s.texture = vostok::render::resource_manager::ref().create_texture2d(user_texture_name.c_str(),size_x,size_y,data,format,D3D_USAGE_DEFAULT,1);
