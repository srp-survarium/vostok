...
{
	//VOSTOK_DELETE_IMPL(g_allocator, m_device);
}
...
static void shader_compile_error_handler( vostok::render::enum_shader_type /*shader_type*/, char const* /*name*/, char const* error_string)
{
	printf("\n%s\n",error_string);
}
	...
	m_counters_valid	 ( initialize())
{
	...
	vostok::render::resource_manager::ref().set_compile_error_handler(shader_compile_error_handler);
...
benchmark::~benchmark()
{
	...
	{
		next = it->next;
		VOSTOK_DELETE_IMPL(g_allocator,it);
	}
...
#pragma warning(pop)
	...
	//vostok::render::backend::ref().reset_render_targets();
	if (num_rts)
		vostok::render::backend::ref().set_render_targets(
		num_rts>0 ? &*test.render_targets[0].rt : 0,
		num_rts>1 ? &*test.render_targets[1].rt : 0,
		num_rts>2 ? &*test.render_targets[2].rt : 0,
		0);
	else
		//vostok::render::backend::ref().set_render_targets(0,0,0,0);
		vostok::render::backend::ref().reset_render_targets();
	...
	if (m_benchmark_target==target_general)
	{
		for ( u32 ps=0; ps < num_tests + num_skip_tests; ps++)
		{
			...
			{
				vostok::render::device::ref().begin_frame();
			}
			...
			if (m_show_preview_window) 
				vostok::render::device::ref().d3d_context()->Flush();

			gtimer.stop();
			
			gtimer.get_elapsed_info(elapsed_time,elapsed_ticks,frequency);
			
			if (m_show_preview_window) vostok::render::device::ref().end_frame();
	...
	else if (m_benchmark_target==target_ati)
	{
		...
		//vostok::render::device::ref().end_frame();
	...
	else if (m_benchmark_target==target_nvidia)
	{
		...
		for ( u32 ps=0; ps < num_tests + num_skip_tests; ps++)
		{
			if (ps<num_skip_tests)
				continue;
			NVPMBeginExperiment(&nNumPasses);
			if (m_show_preview_window) vostok::render::device::ref().begin_frame();
			for (u32 pass_index=0; pass_index < num_passes; pass_index++) 
			{
...
void benchmark::save_results()
{
	...
	sprintf(buffer,"[%s, %s] Results for %S: (%dx%d)\n", 
		__DATE__, 
		__TIME__, 
		vostok::render::device::ref().adapter_desc().Description, 
		m_window_size_x, 
		m_window_size_y
		);
...
void benchmark::start_execution()
{
	...
	//if (!m_show_preview_window) vostok::render::device::ref().d3d_context()->Flush();
	if (!m_show_preview_window) vostok::render::device::ref().end_frame();
	...
	for ( u32 group_index=0; group_index<m_groups.size(); group_index++)
		for ( u32 test_index=0; test_index<m_groups[group_index].tests.size(); test_index++)
			if (  m_groups[group_index].tests[test_index].object )
				VOSTOK_DELETE_IMPL(g_allocator, m_groups[group_index].tests[test_index].object);
