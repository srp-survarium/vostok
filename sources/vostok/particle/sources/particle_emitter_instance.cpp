...
void particle_emitter_instance::play_child( particle_event* evt, vostok::math::float4x4 const& transform )
{
...
void particle_emitter_instance::process_event(enum_particle_event const& event_type, vostok::float3 const& location)
{
...
#pragma message( VOSTOK_TODO("Dima to IronNick: do not use dynamic_casts, use virtual function here instead") )
		...
		if (evt->get_event_type()==event_type && evt->get_visibility())
		{
			...
			if (evt->get_inherit_position())
				transf = vostok::math::create_translation(location);
...
{
	VOSTOK_UNREFERENCED_PARAMETERS(num_need_particles, time_delta);
...
{
	VOSTOK_UNREFERENCED_PARAMETERS(time_delta);
...
u32 particle_emitter_instance::calc_num_new_particles( float time_delta, bool allow_to_create_new )
{
	...
	for (u32 i=0; i<m_emitter.m_num_burst_entries; i++)
	{
		...
		{
			m_num_particles_to_create = u32(vostok::math::max( random_float(
	...
	num_new_particles			= vostok::math::floor(f_num_new);
...
void particle_emitter_instance::append_particles( float time_delta)
{
	for (u32 i=0; i<m_num_particles_to_create; i++)
	{
		...
		new_particle->m_seed	= u32(new_particle) + static_cast<u32>(vostok::timing::get_QPC());
...
void particle_emitter_instance::tick(float time_delta, bool /*create_new_particles*/, float alpha)
{
	...
	if (is_emitter_finished())
	{
		...
		{
			m_current_loop++;
			vostok::math::clamp<u32>(m_current_loop, 0, m_emitter.m_num_loops);
...
void particle_emitter_instance::on_material_loaded(vostok::resources::queries_result& result)
{
