...
inline vostok::math::float4 color_matrix::evaluate(float time, vostok::math::float4 const& default_value) const
{
	return evaluate(time, random_float(0.0f, 1.0f), default_value);
}

inline vostok::math::float4 color_matrix::evaluate(float x, float y, vostok::math::float4 const& default_value) const
{
	vostok::math::clamp(x, 0.0f, 1.0f);
	vostok::math::clamp(y, 0.0f, 1.0f);
	...
	if (vostok::math::abs(dist_x)<=vostok::math::epsilon_5)
		dist_x = 1.0f;
	
	if (vostok::math::abs(dist_y)<=vostok::math::epsilon_5)
		dist_y = 1.0f;
	...
	vostok::math::float4 const result	= bilinear_interpolation(
...
inline void color_matrix::load				(ConfigValueType config)
{
	...
	vostok::fixed_string<16> row_name;
	vostok::fixed_string<16> column_name;
	...
	for (row_index=0; row_index<num_rows; row_index++)
	{
		...
		for (column_index=0; column_index<num_columns; column_index++)
		{
			...
			point.position	= vostok::math::float2(element["position"]);
			point.color		= vostok::math::float4(element["color"]);
		}
	}
}
...
inline u32 color_matrix::save_binary		(vostok::mutable_buffer& buffer, bool calc_size)
{
	u32 save_size = m_num_rows * m_num_columns * sizeof(color_matrix_point_type);
	if (!calc_size)
	{
		vostok::memory::copy(buffer.c_ptr(), save_size, m_points, save_size);
		buffer+=save_size;
	}
	return save_size;
}

inline void color_matrix::load_binary		(vostok::mutable_buffer& buffer)
{
