...
void particle_emitter::set_defaults()
{
	...
	vostok::memory::zero(m_material_name);
...
u32 particle_emitter::save_binary(vostok::mutable_buffer& buffer, bool calc_size)
{
	...
	else
	{
		...
		vostok::memory::copy(buffer.c_ptr(), size, &*m_burst_entries, size);
		buffer += size;
	}
	return result;
}

void particle_emitter::load_binary(vostok::mutable_buffer& buffer) 
{
...
void particle_emitter::on_material_loaded(vostok::resources::queries_result& /*result*/)
{
//	if (result.is_successful())
//		m_material = static_cast_checked<vostok::render::material*>(result[0].get_unmanaged_resource().c_ptr());
}
