	...
	particle_system_instance( particle_world )
{
	...
	m_transform			 = vostok::math::float4x4().identity();
...
{
	vostok::threading::interlocked_increment(m_reference_count);
	m_pinned = true;
}

void particle_system_instance_impl::unpin()
{
	vostok::threading::interlocked_decrement(m_reference_count);
	m_pinned = false;
}
...
void particle_system_instance_impl::check_lods( vostok::math::float3 const& view_location )
{
	if (!m_use_lods)
		return;
...
void particle_system_instance_impl::set_transform(vostok::math::float4x4 const& transform)
{
...
void particle_system_instance_impl::process_lods_lerping( float time_delta )
{
	...
	if (m_lods_lerp_alpha<1.0f)
	{
		...
		vostok::math::clamp(m_lods_lerp_alpha,0.0f,1.0f);
