...
extern vostok::uninitialized_reference<particle_system_instance_cook>	s_particle_cook_object;
...
static particle_action* create_action_by_index(particle_system_lod& lod, vostok::mutable_buffer& buffer, particle_emitter* emitter, s32 index)
{
	(void)&lod;
...
void particle_system::load_lod_actions_binary( particle_system_lod& lod, vostok::mutable_buffer& buffer)
{
	for (u32 e=0; e<lod.m_num_emitters; e++)
	{
		...
		for (u32 a=0; a<emitter->m_num_actions; a++)
		{
			...
			vostok::memory::copy(&class_index, sizeof(class_index), buffer.c_ptr(), sizeof(class_index));
...
void particle_system::load_binary( vostok::mutable_buffer& buffer )
{
	vostok::memory::copy(&m_num_lods, sizeof(m_num_lods), buffer.c_ptr(), sizeof(m_num_lods));
...
u32 particle_system::save_lod_actions_binary( particle_system_lod& lod, vostok::mutable_buffer& buffer, bool cals_size )
{
	u32 save_size = 0;
	for (u32 e=0; e<lod.m_num_emitters; e++)
	{
		...
		while(action)
		{
			...
			{
				vostok::memory::copy(buffer.c_ptr(), sizeof(class_index), &class_index, sizeof(class_index));
				buffer+=sizeof(class_index);
			}
			...
			{
				vostok::memory::copy(buffer.c_ptr(), class_size, action, class_size);
				buffer+=class_size;
			}
...
u32 particle_system::save_binary( vostok::mutable_buffer& buffer, bool cals_size )
{
	u32 save_size = 0;
	...
	{
		vostok::memory::copy(buffer.c_ptr(), sizeof(m_num_lods), &m_num_lods, sizeof(m_num_lods));
		buffer+=sizeof(m_num_lods);
	}
	...
	{
		vostok::memory::copy(buffer.c_ptr(), sizeof(particle_system_lod) * m_num_lods, m_lods, sizeof(particle_system_lod) * m_num_lods);
		buffer+=sizeof(particle_system_lod) * m_num_lods;
	}
	...
	for (u32 lod_index=0; lod_index<m_num_lods; lod_index++)
	{
		...
		else
		{
			for (u32 emitter_index=0; emitter_index<lod.m_num_emitters; emitter_index++)
			{
				...
				vostok::memory::copy(buffer.c_ptr(), sizeof(particle_emitter), &emitter, sizeof(particle_emitter));
