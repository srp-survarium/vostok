...
typedef curve_line_points<vostok::math::float3_pod, float_curve_line>		curve_line_float3;
typedef curve_line_points<vostok::math::float4_pod, float_curve_line>		curve_line_float4;
typedef curve_line_points<vostok::math::float4_pod, color_curve_line>		curve_line_color_base;
...
struct curve_line_color: public curve_line_color_base
{
	...
	vostok::math::float4 evaluate(float time, vostok::math::float4 default_value, enum_evaluate_time_type time_type = range_time_type) const
	{
...
private:
	...
	inline float evaluate(float time, float default_value, enum_evaluate_type evaluate_type, enum_evaluate_time_type time_type, u32 seed)
	{
		...
		VOSTOK_UNREFERENCED_PARAMETER	( temp_value );
#else // #if 0
		for (u32 random_index=0; random_index<index; random_index++)
			rnd.random		( 65535 );
#endif // #if 0
...
struct curve_line_ranged_float
{
	...
	{
		return vostok::math::max(m_line.m_upper.curve_value_max, m_line.m_lower.curve_value_max);
	}
private:
	enum_evaluate_type		m_evaluate_type;
