...
void preview_particle_system::add_emitter(u32 lod_index, pcstr emitter_id, pcstr, vostok::configs::lua_config_value const& emitter_config)
{
	...
	{
		vostok::memory::copy(emitters, sizeof(particle_emitter) * lod.m_num_emitters, lod.m_emitters_array, sizeof(particle_emitter) * lod.m_num_emitters);
...
void preview_particle_system::remove_emitter(u32 lod_index, particle_emitter* emitter)
{
	...
	for (u32 i=0; i<lod.m_num_emitters; i++)
	{
		...
		vostok::memory::copy(&emitters[new_index], sizeof(particle_emitter), &lod.m_emitters_array[i], sizeof(particle_emitter));
...
void preview_particle_system::on_group_changed(vostok::configs::lua_config_value const& config, pcstr group_id)
{
	guid_to_object_pair* pair = m_guids_list.front();
	while (pair)
	{
...
{
	vostok::intrusive_list<exchange_order,exchange_order*,&exchange_order::next> action_order;
...
void preview_particle_system::play_preview(math::float4x4 const& transform, bool always_looping)
{
	...
	m_preview_instance->set_creation_source(
		...
		"",
		vostok::resources::memory_usage_type(vostok::resources::nocache_memory, sizeof(particle_system_instance))
		);
