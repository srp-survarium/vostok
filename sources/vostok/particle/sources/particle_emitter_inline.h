...
void particle_emitter::load( ConfigValueType const& prop_config )
{
	...
	vostok::fs_new::virtual_path_string mtl_path = read_config_value<pcstr>(prop_config, "Material", m_material_name); // in group too
	...
	if (prop_config.value_exists("BurstList"))
	{
		...
		else if (num_burst_entries==1)
		{
			...
			{
				//((vostok::configs::lua_config_value*)&burst_list_config)->save_as("f:/1.lua");
			...
			else
			{
				for (u32 i=0; i<m_num_burst_entries; i++) // Update one.
				{
					...
					if (burst_list_config.value_exists(n.c_str()))
					{
						...
						be.count			= read_config_value(value, "Count", (u32)vostok::math::abs(be.count));
		...
		else // Append new all.
		{
			...
			for (u32 i=0; i<num_burst_entries; i++)
			{
				...
				{
					m_burst_entries[i].count		  = read_config_value(burst_list_config[c], "Count", (u32)vostok::math::abs(m_burst_entries[i].count));
