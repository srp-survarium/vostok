...
vostok::math::float4x4 particle_domain_complex::get_transform() const
{
	//return m_transform;
	return vostok::math::create_scale(m_scale) * vostok::math::create_matrix(vostok::math::quaternion(m_rotate), m_translate);
}

vostok::math::float3 particle_domain_complex::to_local_space( vostok::math::float3 const& point) const
{
...
vostok::math::float3 particle_domain_complex::generate() const
{
...
bool particle_domain_complex::inside( vostok::math::float3 const& point) const
{
	switch (m_domain_type)
	{
		...
		case cone_domain:
			{
				#pragma message( VOSTOK_TODO( "cone domain inside(point)!" ) )
				return false;
			}
	}
	return false;
