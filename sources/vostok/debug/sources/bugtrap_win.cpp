...
{
	if ( s_bugtrap_usage == vostok::debug::no_bugtrap ) {
		result						= 0;
		return;
	}
	...
	if ( !result )
		s_bugtrap_usage				= vostok::debug::no_bugtrap;
}
...
static void load_library							( )
{
	...
	s_bugtrap_handle				= LoadLibrary( s_bugtrap_usage == vostok::debug::native_bugtrap ? s_bugtrap_native_id : s_bugtrap_dotnet_id );
	if ( !s_bugtrap_handle ) {
...
{
	if ( s_bugtrap_usage == vostok::debug::native_bugtrap )
		return						message;
