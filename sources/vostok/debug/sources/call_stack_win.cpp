...
#include <stdio.h>
#include <intrin.h>
#pragma intrinsic(_InterlockedIncrement, _InterlockedExchange)

#define	_T( a )						a
#define	_tcscpy_s					vostok::strings::copy
...
#else
   // This is trivial :)
	memcpy_s			( lpszOut, (u32)outSize, lpszIn, (u32)outSize );
#endif
}
...
static BOOL GetModuleNameFromAddress( DWORD64 address, LPTSTR lpszModule, rsize_t lpszModuleSize )
{
	...
	if ( s_SymGetModuleInfo64( GetCurrentProcess(), (DWORD)address, &moduleInfo ) )
	{
	   ...
	   // Not found :(
		strcpy_s( lpszModule, (u32)lpszModuleSize, _T("?") );
...
{
	VOSTOK_UNREFERENCED_PARAMETER	(stackAddress);
	BOOL              ret = FALSE;
	DWORD64             dwDisp = 0;
...
#pragma warning( pop )
   ...
   // Set the default to unknown
	strcpy_s( lpszSymbol, (u32)lpszSymbolSize, _T("?") );
...
static BOOL GetSourceInfoFromAddress( DWORD64 address, LPTSTR lpszModuleInfo, rsize_t lpszModuleInfoSize, LPTSTR lpszSourceInfo, rsize_t lpszSourceInfoSize, int& line, size_t& address_out )
{
	BOOL            ret = FALSE;
...
//	TCHAR           lpModuleInfo[s_buffer_size] = _T("");
	...
	strcpy_s		( lpszSourceInfo, (u32)lpszSourceInfoSize, _T("?") );
	strcpy_s		( lpszModuleInfo, (u32)lpszModuleInfoSize, _T("?") );
	line			= -1;
...
{
	VOSTOK_UNREFERENCED_PARAMETER	(hThread);
...
static void GetStackTrace(HANDLE hThread, /**DWORD64 ranOffsets[][2],/**/ _EXCEPTION_POINTERS* pointers, vostok::debug::call_stack::Callback const& callback, bool invert_order )
{
...
//			OutputDebugStringFormat( _T("Call stack info(thread=0x%X) GetThreadContext failed.\n") );
		...
		memcpy_s	( &context, sizeof(context), pointers->ContextRecord, sizeof(CONTEXT) );
...
static void WriteStackTrace ( DWORD64 ranOffsets[][2], u32 num_call_stack_lines, vostok::debug::call_stack::Callback const& callback, bool invert_order )
{
	TCHAR module_name[256] = _T("?");
	TCHAR file_name[2*260] = _T("?");
	TCHAR function[s_buffer_size] = _T("?");
...
static void InitSymbolPath( PSTR lpszSymbolPath, rsize_t symPathSize, PCSTR lpszIniPath )
{
   ...
   // ".;%_NT_SYMBOL_PATH%;%_NT_ALTERNATE_SYMBOL_PATH%;%SYSTEMROOT%;%SYSTEMROOT%\System32;"
	strcpy_s( lpszSymbolPath, (u32)symPathSize, "." );
...
static void load_function	( T*& result, HMODULE const module, pcstr const module_id, pcstr const function_id )
{
	...
	if ( !result ) 
	{
		...
		{
			string512		string;
			sprintf_s		( string, "can't find function %s in %s", function_id, module_id );
			log_callback	( "debug", true, false, string );
		}
		s_use_dbghelp		= false;
	}
}
...
static void load_library	( )
{
	...
	if ( !s_dbghelp_handle ) 
	{
		vostok::debug::log_callback const log_callback	=	vostok::debug::get_log_callback();
		if ( log_callback )
			log_callback	( "debug", true, false, "cannot load dbghelp library" );
		s_use_dbghelp		= false;
		return;
	}
	...
	if ( !s_psapi_handle ) 
	{
		vostok::debug::log_callback const log_callback	=	vostok::debug::get_log_callback();
		if ( log_callback )
			log_callback	( "debug", true, false, "cannot load psapi library" );
		return;
	}
...
static long s_initializer_lock		= 0;
...
static void initialize		( )
{
	...
	HMODULE const handle	= GetModuleHandle( _T("ntdll.dll") );
	R_ASSERT				( handle );
	s_pfnCaptureStackBackTrace	= ( pfnCaptureStackBackTrace )::GetProcAddress( handle, "RtlCaptureStackBackTrace" );
	if ( !s_pfnCaptureStackBackTrace )
	{
		vostok::debug::log_callback const log_callback	=	vostok::debug::get_log_callback();
		if ( log_callback )
			log_callback	( "debug", true, false, "can't find function RtlCaptureStackBackTrace in ntdll.dll" );
	}
											  ...
											  bool					invert_order)
{
	...
	if ( !pointers && s_pfnCaptureStackBackTrace ) 
	{
		...
		if ( call_stack )
		{
			...
			{
				stack[i][0]	=	(DWORD64)call_stack[i];
				stack[i][1]	=	(DWORD64)call_stack[i];
...
void vostok::debug::call_stack::get_stack_trace (void* stacktrace[], u32 const stacktrace_max, u32 const num_to_capture, u32* out_hash)
{
	::initialize						();
	VOSTOK_UNREFERENCED_PARAMETER			(stacktrace_max);
