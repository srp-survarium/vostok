...
static vostok::debug::error_mode			s_error_mode = vostok::debug::error_mode_verbose;
...
bool show_dialog_for_unhandled_exceptions	( )
{
	return	s_show_dialog_for_unhandled_exceptions && 
			(vostok::debug::debug_engine() && !vostok::debug::debug_engine()->terminate_on_error());
}
...
static void epilogue											( _EXCEPTION_POINTERS* const exception_information )
{
	...
	vostok::debug::platform::terminate	( "", 2 );
}
...
#else
#pragma message (VOSTOK_TODO("Lain 2 Dima: what return here?"))
	return						( EXCEPTION_CONTINUE_SEARCH );
...
// 	if ( !vostok::debug::debug_engine() || !vostok::debug::debug_engine()->core_initialized() )
// 	{
...
#				undef PREFIX
	...
	{
		debug_engine()->on_testing_exception(vostok::assert_untyped, description, exception_information, false);
		return					EXCEPTION_EXECUTE_HANDLER;
	}

	log_callback const log_callback	=	vostok::debug::get_log_callback();
	if ( log_callback )
	{	
	...
	if ( show_dialog_for_unhandled_exceptions( ) )
	{
		...
		strcat_s					(buffer, "\r\n\r\n");
		vostok::debug::platform::on_error	(NULL, buffer, NULL, exception_information, vostok::debug::platform::error_type_unhandled_exception);
	}

	if ( debug_engine() && debug_engine()->terminate_on_error() )
	{
		vostok::debug::dump_call_stack	("", true, 0, 4, exception_information );
 		vostok::debug::terminate	( "" );
	}
 	else
...
void vostok::debug::platform::setup_unhandled_exception_handler	( error_mode const error_mode )
{
	s_error_mode				= error_mode;
	s_previous_handler			= SetUnhandledExceptionFilter( &unhandled_exception_handler );
}

void vostok::debug::platform::change_unhandled_exception_handler ( error_mode const error_mode )
{
	s_error_mode				= error_mode;
}

void vostok::debug::bugtrap::setup_unhandled_exception_handler	( )
{
