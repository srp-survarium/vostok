...
struct call_stack_size_calculator {
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS			( num_call_stack_lines, call_stack_id );
		static pcstr s_full_call_stack_line_format_local	= "%s(%d) : %s : %s : 0x%08x";
		static pcstr s_call_stack_line_format_local			= "%s       : %s : 0x%08x";
		char								buffer[8192];
		if ( line_number > 0 )
			vostok::sprintf					(buffer, sizeof(buffer), s_full_call_stack_line_format_local, file_name, 
											 line_number, function, module_name, address);
		else
			vostok::sprintf					(buffer, sizeof(buffer), s_call_stack_line_format_local, module_name, function, address);
