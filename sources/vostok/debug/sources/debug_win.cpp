...
void vostok::debug::platform::on_error	( bool* do_debug_break, char* const message, u32 const message_size, bool* ignore_always, _EXCEPTION_POINTERS* const exception_information, error_type_enum error_type )
{
	...
	// boost::function7::assign_to<bind_t<...>> machinery on these iterate calls -
	// our /Ob2 build inlines assign_to, target calls it out-of-line (per-instantiation
	// inline heuristic; dump_call_stack's identical idiom matches because its bind type
	// is not also instantiated elsewhere). Not source-steerable.
