...
static void save_minidump						( pcstr const output_file_name, _EXCEPTION_POINTERS* const exception_information )
{
	...
	vostok::debug::debug_engine()->create_folder_r	( output_file_name, false );
...
void vostok::debug::platform::save_minidump		( SYSTEMTIME const& date_time, _EXCEPTION_POINTERS* exception_information )
{
	string_path					temp;
...
void vostok::debug::platform::prologue_dump_call_stack	( _EXCEPTION_POINTERS* exception_information )
{
	__try {
		if ( !vostok::debug::platform::error_after_dialog( ) )
			vostok::debug::dump_call_stack	( "debug", true, 0, 0, exception_information );
