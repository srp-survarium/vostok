...
protected:
	template < typename T>
	inline	T*		clone_impl			( vostok::mutable_buffer& buffer ) const;

public:
	inline	bool	operator ==			( base_interpolator const& other ) const;
	inline	bool	operator !=			( base_interpolator const& other ) const;

	VOSTOK_DECLARE_PURE_VIRTUAL_DESTRUCTOR( base_interpolator )
}; // struct base_interpolator

	VOSTOK_ANIMATION_API comparison_result_enum compare	( base_interpolator const& left, base_interpolator const& right );
...
#include <vostok/animation/base_interpolator_inline.h>
