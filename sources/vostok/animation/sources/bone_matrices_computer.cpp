...
using vostok::animation::bone_transform;

bone_matrices_computer::bone_matrices_computer				(
		skeleton const* skeleton,
		vostok::animation::mixing::animation_state* const animations,
...
typedef vostok::buffer_vector< std::pair< vostok::float3, float > >	tranforms_type;
...
static inline vostok::math::quaternion mix_rotations	( tranforms_type& transforms )
{
	if ( transforms.empty() )
		return								vostok::math::quaternion( vostok::math::float4( 0.f, 0.f, 0.f, 1.f ) );
	...
	return									vostok::math::quaternion( mix_translations( transforms ) );
}
...
//	} while ( angle >= vostok::math::epsilon_3 );
//
//	return									result;
//}
...
inline vostok::animation::frame identity_frame( )
{
	vostok::animation::frame result;
