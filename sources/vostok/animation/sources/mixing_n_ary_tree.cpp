...
using vostok::animation::mixing::n_ary_tree_intrusive_base;
using vostok::animation::skeleton_animation_ptr;

#ifndef MASTER_GOLD
void n_ary_tree::fill_animation_states	( vostok::vectora<vostok::animation::editor_animation_state>& result )
{
	if ( !m_root ) {
		result.clear			( );
		return;
	...
	{
		vostok::animation::editor_animation_state new_state;
		R_ASSERT(i->user_data!=vostok::memory::uninitialized_value<u32>());
...
void n_ary_tree::dispatch_callbacks		(
		vostok::animation::callback_generator_info const* const callback_generators_head,
