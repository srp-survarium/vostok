...
VOSTOK_DECLARE_LINKAGE_ID(animation_anim_track)
...
EtCurve::EtCurve	( vostok::memory::base_allocator* a )
:keyList(a)
{
	isWeighted		= false;
	isStatic		= false;
...
{
	// find
	vostok::vectora<EtKey>::iterator it = std::lower_bound( keyList.begin(), keyList.end(), time );
	EtKey			k;
	k.time			= time;
	k.value			= value;
	k.inTanX		= -0.00001f;
...
void			EtCurve::calc_smooth_tangents		( )
{
	...
	float		inTanX = vostok::memory::uninitialized_value<float>(); 
	float		inTanY = vostok::memory::uninitialized_value<float>(); 
	float		outTanX = vostok::memory::uninitialized_value<float>(); 
	float		outTanY = vostok::memory::uninitialized_value<float>();
...
anm_track::anm_track	( vostok::memory::base_allocator* a )
:m_allocator( a )
{
	m_max_channels	= channel_max;
	m_channels		= VOSTOK_NEW_ARRAY_IMPL( m_allocator, EtCurve*, m_max_channels );
	memset			( m_channels, 0, m_max_channels*sizeof( m_channels[0] ) );
}

anm_track::anm_track	( vostok::memory::base_allocator* a, int max_channels )
:m_allocator( a )
{
...
anm_track::~anm_track	( )
{
	for( int i=0; i<m_max_channels; ++i )
	{
		if( m_channels[i] )
		{
			VOSTOK_DELETE_IMPL	( m_allocator, m_channels[i] );
			m_channels[i]		= 0;
		}
	}
	VOSTOK_DELETE_ARRAY_IMPL( m_allocator, m_channels );
}
...
void			anm_track::initialize_empty			( bool bposition, bool brotation, bool bscale )
{
	if(bposition)
	{
		m_channels[channel_translation_x]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
		m_channels[channel_translation_y]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
		m_channels[channel_translation_z]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
	}
	if(brotation)
	{
		m_channels[channel_rotation_x]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
		m_channels[channel_rotation_y]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
		m_channels[channel_rotation_z]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
	}
	if(bscale)
	{
		m_channels[channel_scale_x]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
		m_channels[channel_scale_y]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
		m_channels[channel_scale_z]		= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
	}

	for( int i = 9; i < m_max_channels; ++i )
		m_channels[i]	= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
}
...
EtCurve*		anm_track::get_channel				( enum_channel_id id, bool b_create )
{
	if(!m_channels[id] && b_create)
		m_channels[id] = VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
...
void			anm_track::remove					( u32 idx )
{
	...
	{
		EtCurve*	c =	get_channel	( (vostok::animation::enum_channel_id)i, false );
		if(c)
			c->remove	( idx );
	}
