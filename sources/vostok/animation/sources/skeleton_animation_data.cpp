...
void skeleton_animation_data::save( vostok::configs::lua_config_value &cfg )const
{
...
void skeleton_animation_data::load( vostok::configs::lua_config_value const &cfg )
{
	vostok::configs::lua_config_value const cfg_data  = cfg["data"];
...
bool skeleton_animation_data::compare( skeleton_animation_data const & data ) const
{
	...
	for( u32 i=0; i<size; ++i )
	{
		const vostok::animation::bone_animation_data&d = data.bone( i );
