...
namespace animation {
	...
	editable_animation_event_channels::~editable_animation_event_channels( )
	{
		...
		m_event_channels = 0;
		VOSTOK_FREE_IMPL	( debug::g_mt_allocator, m_event_channels_memory );
	}
	...
	void editable_animation_event_channels::create_channels_from_config()
	{
		...
		if ( m_event_channels )
			m_event_channels->~animation_event_channels	( );
		m_event_channels_memory = VOSTOK_REALLOC_IMPL( debug::g_mt_allocator, m_event_channels_memory, internal_mem_size + sizeof( animation_event_channels ) , "editable_animation_event_channels::event_channels" );
