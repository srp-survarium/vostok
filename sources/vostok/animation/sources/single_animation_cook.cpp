...
void single_animation_cook::translate_query				( resources::query_result_for_cook& parent )
{
	...
	vostok::resources::request arr[2] = {
		...
		{options_path.c_str( ), vostok::resources::binary_config_class}
	};
...
void single_animation_cook::on_sub_resources_loaded		( resources::queries_result& data )
{
	...
	vostok::animation::skeleton_animation_ptr anim = static_cast_resource_ptr<vostok::animation::skeleton_animation_ptr>( data[0].get_managed_resource( ) );
	...
	if( data[1].is_successful( ) )
	{
		...
		if( config->get_root( ).value_exists("interpolator") )
		{
			...
			case 0:
			{
				interpolator = VOSTOK_NEW_IMPL( resources::unmanaged_allocator( ), instant_interpolator )( );
				break;
			}

			case 1:
			{
				interpolator = VOSTOK_NEW_IMPL( resources::unmanaged_allocator( ), linear_interpolator )( interpolator_value["time"] );
				break;
			}
					
			case 2:
			{
				interpolator = VOSTOK_NEW_IMPL( resources::unmanaged_allocator( ), fermi_interpolator )( interpolator_value["time"], interpolator_value["epsilon"] );
				break;
			}
