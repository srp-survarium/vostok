...
b_loaded( false )
{
	...
	resources::request	resources[]	= {
		{ "resources/animations/controller_set.lua",	vostok::resources::config_lua_class },
		{ "resources/animations/skeletons/human",		vostok::resources::skeleton_class },
	};
...
{
	VOSTOK_UNREFERENCED_PARAMETER( next_action );
...
{
	VOSTOK_UNREFERENCED_PARAMETER( weights );
...
void	animation_controller_set::query_resources		( configs::lua_config_value const& config )
{
	...
	for( u32 i = 0; i < sz; ++i )
	{
		...
		resources[i].id		= vostok::resources::config_lua_class;
	}
	vostok::resources::query_resources(
			resources,
			sz,
...
void	animation_controller_set::on_config_loaded( vostok::resources::queries_result& resource )
{
	...
	vostok::configs::lua_config_ptr config	= static_cast_resource_ptr<vostok::configs::lua_config_ptr>(resource[0].get_unmanaged_resource());
	vostok::configs::lua_config_value const cfg= *config;
...
void animation_controller_set::on_resources_loaded( vostok::resources::queries_result& resource )
{
	...
	for(u32 i = 0; i < count; ++i  )
	{
		vostok::configs::lua_config_ptr config	= static_cast_resource_ptr<vostok::configs::lua_config_ptr>(resource[i].get_unmanaged_resource());
