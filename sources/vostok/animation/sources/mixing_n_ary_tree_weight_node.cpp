...
void n_ary_tree_weight_node::visit			( n_ary_tree_double_dispatcher& dispatcher, n_ary_tree_time_scale_node& node )
{
	dispatcher.dispatch			( node, *this );
}
...
void n_ary_tree_weight_node::visit			( n_ary_tree_double_dispatcher& dispatcher, n_ary_tree_weight_transition_node& node )
{
	dispatcher.dispatch			( node, *this );
}

void n_ary_tree_weight_node::visit			( n_ary_tree_double_dispatcher& dispatcher, n_ary_tree_time_scale_transition_node& node )
{
	dispatcher.dispatch			( node, *this );
}

bool vostok::animation::mixing::operator <	(
		n_ary_tree_weight_node const& left,
		n_ary_tree_weight_node const& right
	)
{
	interpolator_comparer		comparer;
	left.interpolator().accept	( comparer, right.interpolator() );
	switch ( comparer.result ) {
		case less : return		true;
		case more : return		false;
		case equal : return		left.weight() < right.weight();
