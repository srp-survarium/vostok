...
#ifndef	MASTER_GOLD
void bi_spline_skeleton_animation::save( vostok::configs::lua_config_value cfg )const
{
	vostok::configs::lua_config_value cfg_data  = cfg["data"];
...
///////////////////////////////////////////////////
	ASSERT( m_bone_names_memory );
	ASSERT( m_bone_names );
	vostok::configs::lua_config_value temp = cfg["bone_names"];
...
void bi_spline_skeleton_animation::create_event_channels	( vostok::configs::binary_config_value const &cfg )
{
...
void bi_spline_skeleton_animation::load						( vostok::configs::binary_config_value const &cfg )
{
	vostok::configs::binary_config_value const cfg_data  = cfg["data"];
...
bool bi_spline_skeleton_animation::compare					( bi_spline_skeleton_animation const & data ) const
{
	...
	for ( u32 i=0; i<size; ++i )
	{
		const vostok::animation::bi_spline_bone_animation&d = data.bone( i );
