...
using vostok::animation::mixing::n_ary_tree_n_ary_operation_node;
...
using vostok::animation::bone_mixer;
...
bool vostok::animation::mixing::animation_comparer_less_predicate::operator ( )	(
...
bool vostok::animation::mixing::animation_comparer_equal_predicate::operator ( )	( n_ary_tree_animation_node const& left, n_ary_tree_animation_node const& right ) const
{
	if ( left.synchronization_group_id() != right.synchronization_group_id() )
		return			false;
	...
	)
{
	...
	for ( n_ary_tree_animation_node** i = new_from_begin; j != from_end; j = j->m_next_animation ) {
		if (
				(compare_animation_intervals(*j, new_driving_animation) != vostok::animation::equal) ||
				(
