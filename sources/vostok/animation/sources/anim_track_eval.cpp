...
static void constrainInsideBounds (float *x1, float *x2)
{
	...
	if ((*x1 + sMachineTolerance) < kFourThirds) 
	{
		b = *x1 - 2.0f;
		c = *x1 - 1.0f;
		discr = vostok::math::sqrt (b * b - 4 * c * c);
		root = (-b + discr) * 0.5f;
...
void vostok::animation::engineHermiteCreate (EtCurve *animCurve, float x[4], float y[4])
{
...
static float evaluateInfinities (EtCurve *animCurve, float time, bool evalPre)
{
	...
	firstTime	= animCurve->keyList.front().time;
	lastTime	= animCurve->keyList.back().time;
	...
	if (timeRange == 0.0) 
	{
		...
		// of that key..
		*/
		return (animCurve->keyList.front().value);
	}
	if (time > lastTime) {
	...
	if (evalPre) {
		...
		if (animCurve->preInfinity == kInfinityLinear) 
		{
			...
			tanX = animCurve->keyList.front().inTanX;
			tanY = animCurve->keyList.front().inTanY;
			value = animCurve->keyList.front().value;
			if (tanX != 0.0) 
		...
		if (animCurve->postInfinity == kInfinityLinear) 
		{
			...
			tanX				= animCurve->keyList.back().outTanX;
			tanY				= animCurve->keyList.back().outTanY;
			value				= animCurve->keyList.back().value;
			if (tanX != 0.0) 
			{
				value += ((factoredTime * tanY) / tanX);
			}
	...
	{
		valueRange	= animCurve->keyList.back().value - animCurve->keyList.front().value;
...
float vostok::animation::engineAnimEvaluate (EtCurve *animCurve, float time)
{
	bool withinInterval = false;
	EtKey *nextKey		= 0;
	int index			= vostok::memory::uninitialized_value<int>();
	float value			= 0.0;
	float x[4];
	float y[4];
	...
	if (time < animCurve->keyList.front().time) 
	{
	...
	if (time > animCurve->keyList.back().time) 
	{
		if (animCurve->postInfinity == kInfinityConstant) 
			return (animCurve->keyList.back().value);
	...
	{
		if( (animCurve->lastIndex < ((int)animCurve->keyList.size() - 1)) && (time > animCurve->lastKey->time) )
		{
	...
	if(!withinInterval) 
	{
		...
		if (index == (int)animCurve->keyList.size()) 
		{
			/* Beyond range of this action return end keyframe value */
			animCurve->lastKey		= &(animCurve->keyList.front());
