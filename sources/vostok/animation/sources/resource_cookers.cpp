	...
	private boost::noncopyable
{
	...
	void	on_sub_resources_loaded( resources::queries_result & result )
	{
		...
		vostok::configs::lua_config_ptr config	= static_cast_resource_ptr<vostok::configs::lua_config_ptr>( result[0].get_unmanaged_resource() );
		skeleton_animation_data * out	=	NEW(skeleton_animation_data)(  );
		out->load						( (*config)["splines"] );

		if( check_animation_b_spline_approximation )
		{
			vostok::configs::lua_config_ptr config_check_animation	= static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[1].get_unmanaged_resource());
	...
	virtual void	translate_query( resources::query_result& parent )
	{
		...
		if( !check_animation_b_spline_approximation )
		{
			...
			resources::request arr[] = {	
				{ parent.get_requested_path(),		vostok::resources::config_lua_class },
				{ check_data_file_path,				vostok::resources::config_lua_class },
			};
