...
vostok::command_line::key	check_animation_b_spline_approximation( "check_animation_b_spline_approximation", "", "animation", "" );
...
void skeleton_animation_cook::on_bi_spline_data_arrived	( vostok::resources::queries_result& result )
{
	timing::timer					timer;
	timer.start						( );
	configs::binary_config_ptr config	= static_cast_resource_ptr< vostok::configs::binary_config_ptr >( result[0].get_unmanaged_resource() );
...
#pragma message( VOSTOK_TODO("Dima to all: we should create B-spline animations in-place and not from binary configs, but from a custom format") )
	...
	{
		vostok::configs::binary_config_ptr edit_data_config	= static_cast_resource_ptr< vostok::configs::binary_config_ptr >( result[1].get_unmanaged_resource() );
	...
	{
		vostok::configs::lua_config_ptr config_check_animation	= static_cast_resource_ptr<vostok::configs::lua_config_ptr>(result[2].get_unmanaged_resource());
...
void skeleton_animation_cook::on_cubic_spline_animation_cooked( vostok::resources::queries_result& data,
															   vostok::resources::unmanaged_resource_ptr check_data )
{
	...
	if ( check_animation_b_spline_approximation )
	{
		...
		vostok::configs::lua_config_ptr config_check_animation	= static_cast_resource_ptr<vostok::configs::lua_config_ptr>(check_data);
