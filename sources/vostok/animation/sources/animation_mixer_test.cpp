...
#include "pch.h"
#include <vostok/animation/animation_mixer.h>
#include <vostok/animation/bone_mixer.h>
...
#include <vostok/animation/mixing_animation_clip.h>
#include "mixing_n_ary_tree_base_node.h"

using vostok::animation::animation_mixer;
using vostok::animation::mixing::expression;
using vostok::animation::instant_interpolator;
using vostok::animation::mixing::animation_clip;
...
using vostok::animation::skeleton_animation_ptr;
	...
	vostok::animation::mixing::tree_stats volatile vostok::animation::mixing::g_binary_tree_stats;
	vostok::animation::mixing::tree_stats volatile vostok::animation::mixing::g_n_ary_tree_stats;
	...
	)
{
	...
	animation_lexeme			a( buffer, &animation_a, &linear2, animation_a.length() );
	animation_lexeme			b( buffer, &animation_b, &linear2, animation_b.length() );
	animation_lexeme			c( buffer, &animation_c, &linear2, animation_c.length() );
...
static void animation_test_mixer_impl	( skeleton_animation_ptr skeleton_animation, skeleton const& skeleton )
{
	...
	mutable_buffer buffer		( ALLOCA(animation_mixer::stack_buffer_size), animation_mixer::stack_buffer_size );
...
//	instant_interpolator		linear;
	...
	animation_clip				animation_a( "a", &interpolator, skeleton_animation, animation_type_full );
	animation_clip				animation_b( "b", &fermi,		 skeleton_animation, animation_type_full );
	animation_clip				animation_c( "c", &interpolator, skeleton_animation, animation_type_full );
	animation_clip				animation_d( "d", &interpolator, skeleton_animation, animation_type_full );
	animation_clip				animation_e( "e", &interpolator, skeleton_animation, animation_type_full );
	animation_clip				animation_f( "f", &interpolator, skeleton_animation, animation_type_full );
	animation_clip				animation_g( "g", &interpolator, skeleton_animation, animation_type_full );
	animation_clip				animation_h( "h", &interpolator, skeleton_animation, animation_type_full );

	expression tree				= test_animation_mixer_impl( buffer, interpolator, animation_a, animation_b, animation_c, animation_d );
	foo							( );
	...
	animation_lexeme			e( buffer, &animation_e, &linear2, animation_e.length() );
	animation_lexeme			f( buffer, &animation_f, &linear2, animation_f.length() );
	animation_lexeme			g( buffer, &animation_g, &linear2, animation_g.length() );
	animation_lexeme			h( buffer, &animation_h, &linear2, animation_h.length() );
...
//	animation_mixer mixer_instance		( bone_mixer, *g_allocator );
...
//	mixer_instance.set_target_and_tick	( (e*We + f*(1.f - We))*Wef + (g*Wg + h*(1.f - Wg))*Wgh + (a*Wa + b*(1.f - Wa))*Wab + (c*Wc + d*(1.f - Wc))*(1.f - Wab)*(1.f - Wef - Wgh), 400 );
	...
	for (u32 j=0; j<count; ++j) {
		...
		animation_mixer mixer_instance	( bone_mixer );
	...
	)
{
	...
	animation_clip				animation_a( "a", &instant, skeleton_animation0, animation_type_full );
	animation_clip				animation_b( "b", &instant,	skeleton_animation1, animation_type_full );
	animation_clip				animation_c( "c", &linear2, skeleton_animation0, animation_type_full );
	animation_clip				animation_d( "d", &linear2, skeleton_animation0, animation_type_full );
...
//		animation_lexeme			c( buffer, &animation_a, &linear2, animation_a.length(), 0.f, 1.f, 0 );
		animation_lexeme			b( buffer, &animation_b, &linear2, animation_b.length(), 0.f, 1.f, 0 );
//		animation_lexeme			a( buffer, &animation_a, &linear2, animation_a.length(), 0.f, 1.f, 0 );
		animation_lexeme			a( buffer, &animation_a, b );
		animation_lexeme			c( buffer, &animation_c, a );
...
//		weight_lexeme				w2( buffer, .7f, &linear2);
		...
		animation_lexeme			aa( buffer, &animation_a, &linear2, animation_a.length(), 0.f, 1.f, 0 );
		animation_lexeme			bb( buffer, &animation_b, aa );
		animation_lexeme			cc( buffer, &animation_c, aa );
	...
	)
{
	...
	animation_lexeme			a( buffer, &animation_a, &instant, animation_a.length(), 0.f, 1.f, 0 );
	animation_lexeme			b( buffer, &animation_b, &instant, animation_a.length(), 0.f, 0.5f, 0 );
...
void vostok::animation::test_animation_mixer		(
		skeleton_animation_ptr skeleton_animation0,
		skeleton_animation_ptr skeleton_animation1,
		skeleton const& skeleton
	)
