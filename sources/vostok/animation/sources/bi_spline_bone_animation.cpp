...
typedef vostok::animation::bi_spline_bone_animation::animation_channel_data animation_channel_data;
...
#ifndef MASTER_GOLD
void	bi_spline_bone_animation::save( vostok::configs::lua_config_value config  )const 
{
 ...
 void bi_spline_bone_animation::load		( vostok::configs::binary_config_value const& config  )
{
	for (u32 i = channel_translation_x ; i < channel_max; ++i )
...
{
	VOSTOK_UNREFERENCED_PARAMETER( d );
	for (u32 i = channel_translation_x ; i < channel_max; ++i )
	{
