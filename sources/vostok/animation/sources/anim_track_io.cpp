...
VOSTOK_DECLARE_LINKAGE_ID(animation_anim_track_io)
...
void anm_track::save( vostok::configs::lua_config_value t_root )
{
	t_root["version"]	= 1;
...
void anm_track::load( vostok::configs::lua_config_value const& t_root )
{
	...
	for( ;it!=it_e; ++it )
	{
		...
		//m_channels[ch_id]			= VOSTOK_NEW_IMPL( m_allocator, EtCurve )(m_allocator);
...
void anm_track::load( vostok::configs::binary_config_value const& t_root )
{
	int ver				= t_root["version"];
	if(ver!=1)
		return;
...
#ifndef MASTER_GOLD
void EtCurve::load( vostok::configs::lua_config_value const& t )
{
	keyList.clear	( );
	reset			( );
...
void EtCurve::load( vostok::configs::binary_config_value const& t )
{
	keyList.clear	( );
	reset			( );
...
#ifndef MASTER_GOLD
void EtCurve::save( vostok::configs::lua_config_value t )
{
...
void EtKey::load( vostok::configs::lua_config_value const& t )
{
	time					= t["time"];
...
void EtKey::load( vostok::configs::binary_config_value const& t )
{
	time					= t["time"];
	value					= t["value"];										/* key value (in internal units)*/
	math::float2 in_tan		= t["in_tan"];
...
#ifndef MASTER_GOLD
void EtKey::save( vostok::configs::lua_config_value t )
{
	t["time"]				= time;
	t["value"]				= value;
