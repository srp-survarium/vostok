...
struct VOSTOK_ANIMATION_API EtKey 
{
	...
	u8		type;			/* key type						*/
#ifndef MASTER_GOLD
	void	save			( vostok::configs::lua_config_value t_root );
	void	load			( vostok::configs::lua_config_value const& t_root );
#endif
	void	load			( vostok::configs::binary_config_value const& t_root );
...
struct VOSTOK_ANIMATION_API EtCurve 
{
...
struct VOSTOK_ANIMATION_API anm_track
{
							anm_track				( vostok::memory::base_allocator* a );
							anm_track				( vostok::memory::base_allocator* a, int max_channels );
							~anm_track				( );
