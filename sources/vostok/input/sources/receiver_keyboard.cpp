...
void keyboard::process		( handlers_type& handlers )
{
	for ( u32 i = 0; i < m_current_events_count; ++i ) {
		...
		if ( m_current_events[i].dwData & 0x80 ) {
			for ( ; it != it_e; ++it )
				if ( (*it)->on_keyboard_action( &m_world, enum_keyboard( m_current_events[i].dwOfs ), kb_key_down ) )
					break;
		...
		for ( ; it != it_e; ++it )
			if ( (*it)->on_keyboard_action( &m_world, enum_keyboard( m_current_events[i].dwOfs ), kb_key_up ) )
				break;
	}
