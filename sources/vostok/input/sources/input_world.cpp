...
input_world::input_world	( vostok::input::engine& engine, HWND const window_handle ) :
	m_engine		( engine ),
	m_direct_input	( 0 ),
	m_gamepad		( 0 ),
	m_keyboard		( 0 ),
...
{
	bool operator () (vostok::input::handler* handler, int prio)
	{
		return handler->input_priority() < prio;
	}
};

void input_world::add_handler( vostok::input::handler& handler )
{
	R_ASSERT			(
...
void input_world::remove_handler	( vostok::input::handler& handler )
{
