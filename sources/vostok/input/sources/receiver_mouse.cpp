...
// claude@NOTE: locals match the target set exactly (changed_buttons, it_e, 3x i -
// NO 'button' local: the convert call is passed inline, per locals-are-structure;
// a declared 'button' adds a phantom local). Residual is non-steerable codegen: the
// down loop's two forced on_mouse_key_action calls (the bitwise '|', down-then-hold)
// fuse to one 'or'-test here vs the target's two split 'test;jne' statements, and
// the inline convert is hoisted via loop-rotation (button spill) rather than the
// target's clean LICM - both are /Ox scheduling artifacts over identical structure.
