...
{
	m_motion_state->m_WorldTrans		= from_vostok( create_translation(m_render_model_offset) * transform );
...
{
	btVector3 rel_pos				= from_vostok(point_in_world) - m_bt_body->getCenterOfMassPosition();
...
{
	sync_motion_state* motion_state		= VOSTOK_NEW_IMPL( g_ph_allocator, sync_motion_state )();
	...
	btRigidBody* body							= VOSTOK_NEW_IMPL(g_ph_allocator, btRigidBody) ( info );
	bt_rigid_body* result						= VOSTOK_NEW_IMPL(g_ph_allocator, bt_rigid_body)( shape, body, motion_state, construction_info.m_render_model_offset );
	return result;
}
