...
void	space_item::space_insert( vostok::collision::space_partitioning_tree &space )
{
	ASSERT( !m_space );
	m_space = &space;
...
/*	
	...
	vostok::math::float4x4 s = vostok::math::create_scale( vostok::math::float3 (2,2,2 ) );
	vostok::math::float4x4 r = vostok::math::create_rotation( vostok::math::float3 ( 0,1,0 ) );
	vostok::math::float4x4 t = vostok::math::create_translation( vostok::math::float3 ( 0,1,0 ) );

	vostok::math::float4x4 m0 = t * s;
	vostok::math::float4x4 m1 = s * t;

	vostok::math::float4x4 m0i =  vostok::math::invert4x3( m0 );
	vostok::math::float4x4 m1i =  vostok::math::invert4x3( m1 );

	vostok::math::float4x4 m1ii =  vostok::math::invert4x3( t ) * vostok::math::invert4x3( s );

	vostok::math::float4x4 test0 = m0 * m0i;

	vostok::math::float4x4 test1 = m1 * m1i;
...
void	space_item::collide( island& island )
{
	...
	vostok::collision::objects_type &res = island.get_scheduler().world().get_temp_storage_holder().space_collision_results();
	res.clear();
	...
	vostok::collision::objects_type::iterator			i = res.begin();
	vostok::collision::objects_type::const_iterator	e = res.end();
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &origin, &direction, &max_distance, &distance );
	UNREACHABLE_CODE(return false);
}
...
bool space_item::aabb_query		( math::aabb const& aabb, vostok::collision::triangles_type& results ) const
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &aabb, &results );
	UNREACHABLE_CODE(return false);
}

bool space_item::cuboid_query	( math::cuboid const& cuboid, vostok::collision::triangles_type& results ) const
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &cuboid, &results );
	UNREACHABLE_CODE(return false);
}

bool space_item::ray_query		( float3 const& origin, float3 const& direction, float max_distance, float& distance, vostok::collision::ray_triangles_type& results, vostok::collision::triangles_predicate_type const& predicate ) const
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &origin, &direction, &max_distance, &distance, &predicate, &results );
	UNREACHABLE_CODE(return false);
}

void space_item::render			( vostok::render::scene_ptr const& scene, vostok::render::debug::renderer& renderer ) const 
{
