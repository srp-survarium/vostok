...
{
	VOSTOK_UNREFERENCED_PARAMETERS( &i, time_delta );
...
struct calculate_mass_tensor: public vostok::collision::enumerate_primitives_callback
{
	...
	virtual void enumerate( vostok::math::float4x4 const& transform_, vostok::collision::primitive const & p )
	{
		dMass add;
		vostok::math::float4x4 transform = transform_;
		vostok::math::remove_scale( transform );
		switch( p.type )
		{
...
void	body_ode::set_mass	( float density, const vostok::collision::geometry_instance &og,  float4x4 const &pose_in_body )
{
	
	calculate_mass_tensor v( density );

	//float4x4 pose_in_body = pose_in_body_;
	//vostok::math::remove_scale( pose_in_body );
	
	float4x4		body_transform	= get_pose( body_transform );
	float4x4 const	body_inverted	= vostok::math::invert4x3( body_transform );
	float4x4 const transf = pose_in_body * body_transform; 
	float4x4 const itransf =  vostok::math::invert4x3( transf ); 
