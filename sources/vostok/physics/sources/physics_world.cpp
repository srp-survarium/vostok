...
{
	return VOSTOK_MALLOC_IMPL( vostok::physics::g_ph_allocator, (u32)size, "ode" );
}

void* ode_realoc( void* p, size_t /*oldsize*/, size_t newsize )
{
	return VOSTOK_REALLOC_IMPL( vostok::physics::g_ph_allocator, p, (u32)newsize, "ode" );
}

void	ode_free( void* p, size_t /*size*/ )
{
	return VOSTOK_FREE_IMPL( vostok::physics::g_ph_allocator, p );
}
...
physics_world::physics_world( memory::base_allocator& allocator, engine& engine/*, vostok::render::scene_ptr const& scene,	 render::debug::renderer &renderer*/ ) :
	m_engine				( engine ),
	m_allocator				( allocator ),
	//m_space					( 0 ),
	m_space					( collision::new_space_partitioning_tree( &allocator, 2.f, 100 ) ),
	m_temp_storage_holder	( VOSTOK_NEW_IMPL( allocator, temp_storage_holder)() ),
	m_shell_static			( 0 ),
	m_test_box				( 0 ),
	m_test_sphere			( 0 ),
	m_test_cylinder			( 0 ),
	...
	m_b_ready				(false)
{
	...
	m_scheduler				= VOSTOK_NEW_IMPL( allocator, scheduler )( allocator, *this );
...
physics_world::~physics_world( )
{
	...
	VOSTOK_DELETE_IMPL( m_allocator, m_test_box );
	VOSTOK_DELETE_IMPL( m_allocator, m_test_sphere );
	VOSTOK_DELETE_IMPL( m_allocator, m_test_cylinder );

	VOSTOK_DELETE_IMPL( m_allocator, m_temp_storage_holder );
	if(m_shell_static)
	{
		vostok::collision::geometry_instance* temp_gi = &m_shell_static->get_element().get_geometry	( );
		//vostok::collision::geometry*			temp_g	= &(*temp_gi->get_geometry());
		//m_collision_world.destroy_geometry( temp_g );
		collision::delete_geometry_instance( &m_allocator, temp_gi );
		VOSTOK_DELETE_IMPL( m_allocator, m_shell_static );
	}
	
	VOSTOK_DELETE_IMPL( m_allocator, m_scheduler );
...
void physics_world::delete_shell( non_null<vostok::physics::shell>::ptr  sh )
{
	vostok::physics::shell* temp = &*sh;
	sh->deactivate( *this );
...
non_null<physics::shell>::ptr physics_world::new_physics_shell( non_null<collision::geometry_instance>::ptr g_instance )
{
	...
	body *body	= VOSTOK_NEW_IMPL( m_allocator, body_ode );
	float4x4 pose	= g_instance->get_matrix();
	body->set_pose( pose );
	non_null<element>::ptr  e = VOSTOK_NEW_IMPL( m_allocator, element )( *body, *g_instance );
	e->set_mass( 20.f );

	::shell* shell					= VOSTOK_NEW_IMPL( m_allocator, ::shell )(m_allocator);
	shell->add_element				( e );
...
//{
//	float4x4 pose = float4x4().identity();
//	collision::new_geometry_instance( vostok::physics::g_allocator, pose, geom  )
//}
...
void	physics_world::on_create_test_primitives_config_loaded( resources::queries_result& data )
{
	...
	vostok::configs::binary_config_ptr box		= static_cast_resource_ptr< configs::binary_config_ptr >( data[0].get_unmanaged_resource() );
	vostok::configs::binary_config_ptr sphere		= static_cast_resource_ptr< configs::binary_config_ptr >( data[1].get_unmanaged_resource() );
	vostok::configs::binary_config_ptr cylinder	= static_cast_resource_ptr< configs::binary_config_ptr >( data[2].get_unmanaged_resource() );
