...
struct s_generic_joint_proccess_result
{
	...
	void create_euler()
	{
		euler_hinge *j = VOSTOK_NEW_IMPL( vostok::physics::g_ph_allocator, euler_hinge )();
		j->connect( b0, b1 );
		axes_reset_to_order();
		set_axis( *j, x );
		set_axis( *j, y );
		...
		result(0), b0(_b0), b1(_b1),  x( _x ), y( _y ), z( _z )
	{
		...
		{
			joint_fixed *j = VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_fixed )();
			j->connect( b0, b1 );
			result = j;
			return;
		}
		...
		{
			joint_ball *j = VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_ball )();
			j->connect( b0, b1 );
			result = j;
			return;
		}
		...
		if( num_enabled == 1 )
		{
			...
			joint_hinge *j = VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_hinge )();
			j->connect( b0, b1 );
...
//static void	set_r_axis(const vostok::configs::lua_config_value &cfg, euler_hinge &joint, axis num )
//{
