...
{
	body* new_ode_body			= VOSTOK_NEW_IMPL( vostok::physics::g_ph_allocator, body_ode );
...
non_null< element >::ptr new_element( ConfigValueType const& table )
{
	...
	non_null< collision::geometry_instance >::ptr instance	= collision::new_element_collision( table["collision"], transform, vostok::physics::g_ph_allocator );
	non_null< element >::ptr element_ptr	= VOSTOK_NEW_IMPL( vostok::physics::g_ph_allocator, element )( *body, *instance );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &config, &transform );
	joint* result					= VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_fixed );
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &config, &transform );
	joint* result					= VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_ball );
	result->connect					( b0, b1 );
	return							result;
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &config, &transform );
	joint* result					= VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_wheel );
	result->connect					( b0, b1 );
	return							result;
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( &config, &transform );
	joint* result					=  VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, joint_slider_hinge );
	result->connect					( b0, b1 );
	return							result;
}
...
{
	::shell* shell						= VOSTOK_NEW_IMPL(vostok::physics::g_ph_allocator, ::shell )(*vostok::physics::g_ph_allocator);
