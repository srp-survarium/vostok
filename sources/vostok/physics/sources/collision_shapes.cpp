...
//	R_ASSERT				(&allocator == g_ph_allocator);
	...
	VOSTOK_DELETE_IMPL		( allocator, sh );
	VOSTOK_DELETE_IMPL		( allocator, shape );
}
	...
	case collision::primitive_sphere :
		{
			...
			result = VOSTOK_NEW_IMPL( allocator, btSphereShape)(radius);
		}break;
	case collision::primitive_box :
		{
			btVector3 half_extents(dim.x, dim.y, dim.z);
			result = VOSTOK_NEW_IMPL( allocator, btBoxShape)(half_extents);
		}break;
	case collision::primitive_cylinder :
		{
			btVector3 half_extents(dim.x, dim.y, dim.z);
			result = VOSTOK_NEW_IMPL( allocator, btCylinderShape)(half_extents);
		}break;
	}
	return result;
}
...
bt_collision_shape* create_primitive_shape( memory::base_allocator& allocator, collision::primitive_type type, float3 const& dim )
{
	...
	bt_collision_shape* result	= VOSTOK_NEW_IMPL( allocator, bt_collision_shape)( bt_shape );
	return						result;
}
...
bt_collision_shape* create_static_triangle_mesh_shape( memory::base_allocator& allocator, float3* vertices, u32* indices, u32 num_vertices, u32 num_indices )
{
	...
	btTriangleIndexVertexArray* mesh_interface	= VOSTOK_NEW_IMPL( allocator, btTriangleIndexVertexArray);
	mesh_interface->addIndexedMesh				( mesh );
	btBvhTriangleMeshShape* bt_shape			= VOSTOK_NEW_IMPL( allocator, btBvhTriangleMeshShape)(mesh_interface, true);
														...
														u32 num_indices )
{
	...
	btGImpactMeshShape* bt_shape	= VOSTOK_NEW_IMPL( allocator, btGImpactMeshShape)(mesh_interface);
	bt_shape->setLocalScaling		( btVector3(1,1,-1) );
...
{
//	R_ASSERT				(&allocator == g_ph_allocator);
	heightfield_terrain_shape* shape = VOSTOK_NEW_IMPL(allocator, heightfield_terrain_shape)(rowcol,
																				rowcol,
																				heighfield,
																				min_height, 
																				max_height,
	...
	bt_collision_shape* result		= VOSTOK_NEW_IMPL( allocator, bt_collision_shape)(shape/*, float3(32.0f, min_height+(max_height-min_height)/2.0f, -32.0f)*/ );
	return result;
}
