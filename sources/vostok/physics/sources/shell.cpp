...
shell::~shell( )
{
	...
	for( ;i!= e; ++i )
		VOSTOK_DELETE_IMPL			( m_allocator, *i );
	}
...
void shell::move( vostok::math::float3 const &delta )
{
	element_vec::iterator		i = m_elements.begin();
	element_vec::const_iterator	e = m_elements.end();
	for( ;i!= e; ++i )
...
void	shell::render( vostok::render::scene_ptr const& scene, vostok::render::debug::renderer& renderer )const
{
	{
...
void	shell::space_insert	( vostok::collision::space_partitioning_tree &space )
{ 
...
void shell::set_update_object( vostok::physics::update_object_interface *object )
{
...
void shell::on_step( scheduler &s )
{
	...
	vostok::math::float4x4* transforms = (vostok::math::float4x4*) ALLOCA( elements_count * sizeof( vostok::math::float4x4 ) );
...
void shell::deactivate			( vostok::physics::world& w )
{
	vostok::physics::physics_world * world = static_cast_checked<vostok::physics::physics_world*> ( &w );
