...
:m_contacts( vostok::physics::g_ph_allocator )
{}

contact& contact_storage::add( const contact_joint_info& c )
{
	m_contacts.push_back( VOSTOK_NEW_IMPL( vostok::physics::g_ph_allocator, contact)(c) );
...
void contact_storage::clear( )
{
	...
	for( ; i!=e; ++i )
		VOSTOK_DELETE_IMPL( vostok::physics::g_ph_allocator, *i);
...
void render( contact_storage &contacts, render::scene_ptr const& scene, render::debug::renderer& renderer )
{
		...
		private boost::noncopyable
	{
		...
		pred( vostok::render::scene_ptr const& scene, render::debug::renderer& renderer ): scene( scene ), renderer( renderer ) {}
