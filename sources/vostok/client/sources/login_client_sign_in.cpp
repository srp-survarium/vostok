	...
	)
{
	...
	pbyte buffer					= m_data;
	switch ( *buffer ) {
		case vostok::lobby_info_message_type : {
			++buffer;
			u8 host_name_length		= std::min( *buffer++, u8(max_host_name_length - 1) );

			vostok::connection_info connection_info;
...
void login_client::on_sign_in_handshaked( sign_in_callback_type const& callback, vostok::handshaking_error_types_enum const error )
{
...
void login_client::sign_in_on_connected	( vostok::connection_error_types_enum const connection_result, sign_in_callback_type const& callback )
{
	...
	*buffer++						= vostok::sign_in_message_type;
