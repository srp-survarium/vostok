...
#define VOSTOK_MANY_CONNECTIONS_TEST	1

#if !VOSTOK_MANY_CONNECTIONS_TEST
static void sign_in				( boost::asio::io_service& io_service, vostok::client& client )
{
	for (;;) {
		printf					( "login:      " );
		char account_name[ vostok::max_account_name_length ];
		scanf_s					( "%s", account_name, sizeof(account_name) - 1 );

		printf					( "password:   " );
		char password[ vostok::max_account_name_length + 1 ];
...
static void sign_up				( boost::asio::io_service& io_service, vostok::client& client )
{
	for (;;) {
		vostok::sign_up_info sign_up_info;
		printf					( "login:    " );
...
static bool login_screen		( boost::asio::io_service& io_service, vostok::client& client )
{
...
static bool lobby_screen		( boost::asio::io_service& io_service, vostok::client& client )
{
	...
	for (;;) {
		...
		switch ( choice ) {
			...
			case '2' : {
				client.set_status ( vostok::client::ready_for_battle );
				printf			( "-----------------------------------------\r\n" );
				return			true;
			}
			default : {
...
#else // #if !VOSTOK_MANY_CONNECTIONS_TEST
...
std::string generate_string		( random32& random, u32 const min, u32 const max )
{
	u32 const string_length		= min + random.random(max - min);
...
std::string generate_account_name	( random32& random, std::string const& thread_prefix, u32 const min, u32 const max )
{
	ASSERT						( max > thread_prefix.length() );
	return						thread_prefix + generate_string( random, min >= thread_prefix.length() ? min - thread_prefix.length() : 0, max - thread_prefix.length() );
}
	...
	)
{
	...
	std::string email;
	for (;;) {
		account_name	= generate_account_name( random, thread_prefix, 4, vostok::max_account_name_length );
		password		= generate_string( random, 4, vostok::max_password_length );
		email			= generate_string( random, 4, vostok::max_email_length );
		...
		offline_clients.push_back	(
			&*accounts.insert(
				std::make_pair(
					...
					boost::make_tuple(
						password,
						boost::posix_time::microsec_clock::local_time() - boost::posix_time::seconds( 2*vostok::login_seconds_to_next_sign_in_attempt ),
						false
					)
				)
			).first
...
u32 const total_sign_up_count	= 256;//8*1024;
...
typedef std::vector< std::pair<vostok::client*, accounts_type::value_type*> >	online_clients_type;
	...
	)
{
	...
	vostok::client& client			= *online_clients[ client_index ].first;
	client.sign_out					( );
	...
	)
{
	...
	for (;;) {
		...
		if ( now - found.second.get<1>() <= boost::posix_time::seconds(2*vostok::login_seconds_to_next_sign_in_attempt) )
			continue;

		found.second.get<1>()			= now;
		found.second.get<2>()			= true;

		vostok::client* const client		= new vostok::client( io_service, login_host );
...
static void test_server				( pcstr const login_host, std::string* thread_prefix )
{
	try {
		random32 random;
		accounts_type accounts;
		...
		boost::asio::io_service io_service;
		{
			vostok::client sign_up_client( io_service, login_host );
			bool signed_up_enough		= false;
			do {
				if ( accounts.size() <= sign_up_count ) {
					sign_up				( random, io_service, offline_clients, sign_up_client, accounts, *thread_prefix );
					continue;
				}
	...
	delete thread_prefix;
}
#endif // #if !VOSTOK_MANY_CONNECTIONS_TEST

int main							( int argc, char* argv[] )
{
	setlocale						( LC_ALL, "" );

	pcstr const login_host			= argc < 2 ? "localhost" : argv[1];
	if ( argc < 2 )
	...
	boost::asio::io_service io_service;
	{
		vostok::client client( io_service, login_host );
		for ( ; ; ) {
			if ( !client.is_signed_in() ) {
	...
	std::string computer_name_string	= computer_name;
	...
	for (u32 i = 0; i < thread_pool_size; ++i) {
		...
		threads.create_thread		(
			boost::bind(
				&test_server,
				login_host,
				new std::string(computer_name_string + std::string( temp ) + "_")
			)
		);
	}
