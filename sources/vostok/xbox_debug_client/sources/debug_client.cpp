...
void debug_client::make_error_descr(pcstr const user_descr, HRESULT const h_res)
{
	...
	{
		vostok::strings::append(m_error_descr.get_buffer(), 
...
{
	vostok::timing::timer	wait_timer;
	wait_timer.start();
...
DWORD __stdcall debug_client::notification_processor(LPCSTR str_notification)
{
	...
	if (!vostok::strings::compare(str_notification, NOTIF_HANDLER_PREFIX"!hello"))
	{
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	( dwNotification, dwParam );
