...
#define				VOSTOK_EXCEPTION_EXECUTE_HANDLER 1

VOSTOK_CORE_API vostok::assert_enum	 set_awaited_exception		(vostok::assert_enum);
VOSTOK_CORE_API void				 check_awaited_exception	(vostok::assert_enum);
VOSTOK_CORE_API void				 set_current_test			(pcstr suite_name, pcstr test_name);
VOSTOK_CORE_API bool				 run_tests_impl				(test_base* test, pcstr suite_name);
VOSTOK_CORE_API void				 on_new_suite				();
...
private:
	static  VOSTOK_DEFAULT_ALIGN	memory_buffer	s_buffer;
};
...
public:
	...
	virtual void	test				()
	{ 
		VOSTOK_CONSTRUCT_REFERENCE	(m_test, test_class); 
		timing::timer				timer;
		timer.start					();
		...
		VOSTOK_DESTROY_REFERENCE		(m_test);
	}
