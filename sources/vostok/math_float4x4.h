...
#ifndef VOSTOK_MATH_FLOAT4X4_H_INCLUDED
#define VOSTOK_MATH_FLOAT4X4_H_INCLUDED
...
class VOSTOK_CORE_API float4x4 {
public:
	typedef float		type;
...
// 4x4 operations
VOSTOK_CORE_API float4x4	mul4x4							( float4x4 const& left, float4x4 const& right );	/// this is 4x4 multiplication
VOSTOK_CORE_API float4x4	invert4x4						( float4x4 const& matrix_to_invert );
VOSTOK_CORE_API bool		try_invert4x4					( float4x4 const& matrix_to_invert, float4x4& out_result );
...
VOSTOK_CORE_API float4x4	create_perspective_projection	( float4x4::type vertical_field_of_view, float4x4::type aspect_ratio, float4x4::type near_plane, float4x4::type far_plane );
VOSTOK_CORE_API float4x4	create_orthographic_projection	( float4x4::type width, float4x4::type height, float4x4::type near_plane, float4x4::type far_plane );

VOSTOK_CORE_API float4x4	create_camera_at				( float3 const& from, float3 const& at, float3 const& up );
VOSTOK_CORE_API float4x4	create_camera_direction			( float3 const& from, float3 const& direction, float3 const& up );


VOSTOK_CORE_API bool		try_solve_linear_equations_system( float3 const& first, float3 const& second, float3 const& third, float3 const& b, float3& result );
VOSTOK_CORE_API bool		try_solve_linear_equations_system( float4 const& first,	float4 const& second, float4 const& third, float4 const& fourth, float4 const& b, float4& result );
...
#endif // #ifndef VOSTOK_MATH_FLOAT4X4_H_INCLUDED