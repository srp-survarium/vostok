...
struct animation_space_vertex_id {
	vostok::math::quaternion	rotation;
...
animation_space_vertex_id get_movement	(
		...
		vostok::animation::skeleton_animation_ptr const& left,
		pcstr const left_identifier,
		vostok::animation::skeleton_animation_ptr const& right,
...
public:
	typedef std::pair< vostok::animation::skeleton_animation_ptr, pcstr >		animation_pair_type;
	typedef vostok::debug::vector< animation_space_edge >						edges_type;
	typedef vostok::debug::vector< animation_pair_type >						animations_type;
...
public:
	...
	inline			animation_space_graph	(
			...
			vostok::animation::skeleton_animation_ptr const& forward_animation,
		...
		m_agent_radius			( agent_radius )
	{
		...
		m_step_time				= vostok::animation::cubic_spline_skeleton_animation_pinned( forward_animation )->length_in_frames() / vostok::animation::default_fps;
	...
	inline bool					is_passable	( animation_space_vertex_id const& from, animation_space_vertex_id const& to ) const
	{
		...
		VOSTOK_UNREFERENCED_PARAMETERS( &from, &to );
		return					true;
	}
...
private:
	...
	vostok::ai::navigation::world& m_navigation_world;
...
public:
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER	( vertex );
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER	( vertex_id );
...
public:
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER	( current_vertex_id_ptr );
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS( &current_vertex, &neighbour_vertex, &iterator );
		return						m_step_time;
	}
...
class animation_space_search_restrictor :
	public vostok::ai::search_restrictor::generic <
		vertex_id_type,
...
{
private:
	typedef vostok::ai::search_restrictor::generic <
		vertex_id_type,
		animation_space_graph_wrapper::const_edge_iterator,
		distance_type
	> super;
...
public:
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER	( edge_iterator );
...
public:
	...
	typedef vostok::ai::vertex_allocator::fixed_count			vertex_allocator_type;
	typedef vostok::ai::vertex_manager::fixed_count_hash		vertex_manager_type;
	typedef vostok::ai::priority_queue::binary_heap			priority_queue_type;
	typedef animation_space_graph_wrapper					graph_wrapper_type;
	typedef vostok::ai::path_constructor::edge				path_constructor_helper_type;
	typedef vostok::ai::a_star								algorithm_type;
	...
	typedef vostok::debug::vector< edge_id_type >					path_type;
	typedef animation_space_heuristics							path_heuristics_type;
	typedef vostok::ai::path_constructor::edge::impl<
				vertex_type,
				path_type
...
inline	u32	hash_value	( survarium::vertex_id_type const& vertex_id )
{
	boost::crc_32_type		processor;
	processor.process_block	( vostok::pointer_cast<pcbyte>( &vertex_id ), vostok::pointer_cast<pcbyte>( &vertex_id ) + sizeof(vertex_id.rotation) + sizeof(vertex_id.translation) );
