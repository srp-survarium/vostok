	...
	)
{
	...
	u32 const buffer_size		= vostok::animation::animation_player::stack_buffer_size;
	vostok::mutable_buffer buffer	( ALLOCA(buffer_size), buffer_size );

	vostok::animation::mixing::animation_lexeme left_lexeme(
		vostok::animation::mixing::animation_lexeme_parameters(
			buffer,
			left_identifier,
			left
		)
	...
	vostok::animation::mixing::weight_lexeme left_weight_lexeme( buffer, left_weight, vostok::animation::instant_interpolator() );

	vostok::animation::mixing::animation_lexeme right_lexeme(
		vostok::animation::mixing::animation_lexeme_parameters(
			buffer,
			right_identifier,
			right,
	...
	float const left_animation_length	= vostok::animation::cubic_spline_skeleton_animation_pinned( left )->length_in_frames() / vostok::animation::default_fps;
