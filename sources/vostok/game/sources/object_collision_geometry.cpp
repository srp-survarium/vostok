...
{ 
	...
	static vostok::console_commands::cc_bool		is_draw_collisions_cc		( "collision_geometry_draw_debug",		m_is_draw_collisions,	true, vostok::console_commands::command_type_user_specific );
...
void object_collision_geometry::load ( configs::binary_config_value const& config_value )
{
	...
	//init geometries buffer vector
	{
		...
		pvoid buffer		= VOSTOK_ALLOC_IMPL( g_allocator, collision::geometry*, geometries_count );
	...
	{
		pvoid buffer		= VOSTOK_ALLOC_IMPL( g_allocator, collision::geometry_instance*, count );
		...
		case collision::primitive_truncated_sphere: 
			{
				...
				float4* planes = VOSTOK_ALLOC_IMPL( g_allocator, float4, count );
...
bool object_collision_geometry::check_object_inside_containment_mode	( collision::geometry_instance const & testee )
{
	...
	for ( ; current != end; ++current )
	{
		...
		if( vostok::collision::contains( **current, testee, &renderer, &scene ) )
#else
		if( vostok::collision::contains( **current, testee ) )
#endif //#ifndef MASTER_GOLD
		{
			ret_value = true;
			break;
	...
	for ( ; current != end; ++current )
	{
		...
		if( vostok::collision::intersects( **current, testee, &renderer, &scene ) )
#else
		if( vostok::collision::intersects( **current, testee ) )
#endif //#ifndef MASTER_GOLD
		{
			return false;
		}
