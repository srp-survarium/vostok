...
void game::on_scene_created		( vostok::resources::queries_result& data )
{
	...
	m_render_output_window		= static_cast_resource_ptr< vostok::render::render_output_window_ptr >( data[2].get_unmanaged_resource() );
...
#ifndef MASTER_GOLD
	m_sound_stats				= NEW( vostok::sound::sound_debug_stats )( g_allocator, m_sound_world.get_logic_world_user(), m_sound_scene, *m_ui_world );
#endif //#ifndef MASTER_GOLD
...
{
	vostok::render::editor_renderer_configuration		render_configuration;
	...
	vostok::resources::user_data_variant const* data[] = { &temp_data, 0, &output_window_data };
...
void game::get_colliding_objects	( math::aabb const& query_aabb, vostok::ai::ai_objects_type& results )
{
	vostok::collision::objects_type	objects( g_allocator );
	m_spatial_tree->aabb_query		( collision_object_type_ai, query_aabb, objects );
	for ( vostok::collision::objects_type::const_iterator iter = objects.begin(); iter != objects.end(); ++iter )
	{
...
void game::get_visible_objects		( math::cuboid const& cuboid, vostok::ai::update_frustum_callback_type const& update_callback )
{
	vostok::collision::results_callback_type result_callback( boost::bind( &game::get_frustum_objects_callback, this, &update_callback, _1 ) );
...
void game::assign_behaviour				( )
{
	if ( m_selected_npc )
	{
		vostok::ai::behaviour_cook_params	behaviour_params;
