...
bool proximity_trigger_input_handler::on_keyboard_action ( input::world* input_world, input::enum_keyboard key, input::enum_keyboard_action action )
{
	...
	if( action == vostok::input::kb_key_up && key == vostok::input::key_lcontrol )
		m_is_control_pressed = false;

	if( action == vostok::input::kb_key_down && key == vostok::input::key_add )
	{
	...
	if( !( action == vostok::input::kb_key_down || action == vostok::input::kb_key_hold ) )
		return false;

	if( key == vostok::input::key_lcontrol )
	{
	...
	switch (key)
	{
		case vostok::input::key_uparrow:
			velocity.z = 10; accept = true; break;

		case vostok::input::key_downarrow:
			velocity.z = -10; accept = true; break;
		
		case vostok::input::key_leftarrow:
			velocity.x = -10; accept = true; break;

		case vostok::input::key_rightarrow:
			velocity.x = 10; accept = true; break;

		case vostok::input::key_pgup:
			velocity.y = 10; accept = true; break;

		case vostok::input::key_pgdn:
			velocity.y = -10; accept = true; break;
		...
		case vostok::input::key_numpad8:
			angles.x = .1f; accept = true; break;

		case vostok::input::key_numpad7:
			angles.z = .1f; accept = true; break;

		case vostok::input::key_numpad9:
			angles.z = -.1f; accept = true; break;
	}
