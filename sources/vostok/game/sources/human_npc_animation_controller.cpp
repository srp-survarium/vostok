...
void human_npc::setup_animations_controller	( )
{
	...
	m_model_instance->m_animation_player->subscribe	(
		vostok::animation::channel_id_on_animation_interval_end,
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( ended_animation, subscribed_channel, domain_data );
...
// 			LOG_INFO			( "next key point advanced successfully :)" );
	...
	vostok::animation::mixing::animation_lexeme left_animation(
	...
	vostok::animation::mixing::weight_lexeme left_weight(
		buffer,
		m_animation_space_graph->edges()[path.front()].first_animation_weight,
		animation::linear_interpolator( .25f )
	);

	vostok::animation::mixing::animation_lexeme right_animation(
		vostok::animation::mixing::animation_lexeme_parameters(
			buffer,
