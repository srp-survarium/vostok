...
void object_volumetric_sound::unload_contents			( )
{
	...
	if ( m_collision_geometry )
	{
		...
		VOSTOK_DELETE_IMPL									( g_allocator, m_collision_geometry );
	}
}
...
void object_volumetric_sound::on_collision_object_loaded( game_object_ptr_ const& object )
{
	...
	vostok::collision::geometry* geom	= &*vostok::collision::new_composite_geometry		( g_allocator, cont );
	m_collision_geometry			= &*vostok::collision::new_composite_geometry_instance( g_allocator, float4x4().identity(), geom );
}
...
{
	m_emitter							= static_cast_resource_ptr<vostok::sound::sound_emitter_ptr>( data[0].get_unmanaged_resource( ) );
	...
	m_proxy->play						( vostok::sound::looped );
}
