...
//console_commands::cc_token camera_cmd( "camera_type", s_type_camera, cameras, array_size( cameras ), true, vostok::console_commands::command_type_user_specific  );
//
...
free_fly_camera::free_fly_camera( survarium::game_world& w ) 
:super				( w ),
m_start_time_ms		( 0 ),
m_mouse_move		( 0,0,0 ),
										...
										vostok::input::enum_keyboard_action action )
{
	...
	if(action == vostok::input::kb_key_down && key == vostok::input::key_p)
		m_game_world.test_physics1();

	if(action == vostok::input::kb_key_down && key == vostok::input::key_o)
