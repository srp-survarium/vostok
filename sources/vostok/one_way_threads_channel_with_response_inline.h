...
#ifndef VOSTOK_ONE_WAY_THREADS_CHANNEL_WITH_RESPONSE_INLINE_H_INCLUDED
#define VOSTOK_ONE_WAY_THREADS_CHANNEL_WITH_RESPONSE_INLINE_H_INCLUDED
...
void   TEMPLATE_SPECIALIZATION::producer_finalize ()
{
	...
	VOSTOK_DELETE_IMPL						(null_node_allocator, null_node);
	VOSTOK_DELETE_IMPL						(queue_allocator, backward_queue);
}
...
typename TEMPLATE_SPECIALIZATION::pointer_type   TEMPLATE_SPECIALIZATION::producer_pop_processed ()
{
	...
	if ( to_delete )
	{
		...
		VOSTOK_DELETE_IMPL					(allocator, to_delete);
	}
...
typename TEMPLATE_SPECIALIZATION::backward_queue_type *   TEMPLATE_SPECIALIZATION::backward_queue_for_current_thread (memory::base_allocator * allocator)
{
	...
	backward_queue						=	VOSTOK_NEW_IMPL(allocator, backward_queue_type);
	backward_queue->set_pop_thread_id		();
	NullNodeType * const null_node		=	VOSTOK_NEW_IMPL(allocator, NullNodeType)(allocator);
...
ForwardQueueType *   TEMPLATE_SPECIALIZATION::forward_queue_construct_predicate::operator () (pvoid buffer)
{
	...
	NullNodeType * const null_node		=	VOSTOK_NEW_IMPL(m_allocator, NullNodeType)(m_allocator);
...
ForwardQueueType *   TEMPLATE_SPECIALIZATION::forward_queue_initialize_if_needed (memory::base_allocator * allocator, backward_queue_type * backward_queue)
{
	...
	VOSTOK_CONSTRUCT_REFERENCE_BY_PREDICATE_MT_SAFE	(m_forward_queue, forward_queue_construct_predicate(allocator, backward_queue));
...
#endif // #ifndef VOSTOK_ONE_WAY_THREADS_CHANNEL_WITH_RESPONSE_INLINE_H_INCLUDED
