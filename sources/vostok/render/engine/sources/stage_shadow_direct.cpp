...
static console_commands::cc_bool s_test_shadow_in_frustum_cc(
	"r_test_shadow_in_frustum",
	s_test_shadow_in_frustum,
	false,
	console_commands::command_type_user_specific
);

static bool s_test_shadow_culling0 = true;
static console_commands::cc_bool s_test_shadow_culling0_cc(
	"r_test_shadow_culling0",
	s_test_shadow_culling0,
	false,
	console_commands::command_type_user_specific
);

static bool s_test_shadow_culling1 = true;
static console_commands::cc_bool s_test_shadow_culling1_cc(
	"r_test_shadow_culling1",
	s_test_shadow_culling1,
	false,
	console_commands::command_type_user_specific
);

static bool s_draw_grass_shadows_value = true;
static console_commands::cc_bool s_draw_grass_shadows_value_cc(
	"r_draw_grass_shadows",
	s_draw_grass_shadows_value,
	false,
	console_commands::command_type_user_specific
);

static bool s_sun_moving_value = false;
static console_commands::cc_bool s_sun_moving(
	"sun_moving",
	s_sun_moving_value,
	false,
	console_commands::command_type_user_specific
...
static console_commands::cc_bool s_use_batched_shadow_geometry_cc(
	"use_batched_shadow_geometry",
	s_use_batched_shadow_geometry_value,
	false,
	console_commands::command_type_user_specific
);

static bool s_one_cascade_value = false;
static console_commands::cc_bool s_one_cascade(
	"one_cascade_value",
	s_one_cascade_value,
	false,
	console_commands::command_type_user_specific
...
static console_commands::cc_u32 s_shadows_in_cascade(
	"shadows_in_cascade",
	s_shadows_in_cascade_value,
	0,
	5,
	false,
	console_commands::command_type_user_specific
...
bool remove_inappropriate_models::operator()( render_surface_instance* in_model )
{
	...
	float dynamic_screen_factor =
		in_model->m_render_surface->get_material_effects( ).has_translucency ? 0.0001f : 0.0004f;
	...
	math::aabb bbox = in_model->m_parent->get_aabb( );
	bbox.modify( *in_model->m_transform );
	if ( m_frustum->test_inexact( bbox ) == math::intersection_outside )
		return true;
	return false;
}
		...
		opaque_models.erase(
			std::remove_if(
				...
				remove_inappropriate_models(
					...
					frust,
					sun->direction,
					m_context->m_sun_cascades[cascade_index].size
				)
			),
			opaque_models.end( )
		);
...
)
{
	...
	u32 const num_render = m_caster_model.size( ) / (refresh_rate - pass_index);
	if ( !num_render )
		return;
	...
	for ( ; it_d != end_d && render_index < num_render; ++it_d, ++render_index ) {
		...
		if ( effects.m_effects[forward_render_stage].c_ptr( ) )
			continue;

		if ( !effects.m_effects[gbuffer_render_stage].c_ptr( ) &&
			effects.m_effects[lighting_render_stage].c_ptr( ) )
			continue;

		if ( !effects.is_cast_shadow )
			continue;

		render_geometry& geometry = surface->m_render_geometry;
		if ( !geometry.geom.c_ptr( ) && !geometry.shadow_pass_geom.c_ptr( ) )
			continue;

		if ( effects.stage_enable[shadow_render_stage] &&
			effects.m_effects[shadow_render_stage].c_ptr( ) )
	...
	if ( !pass_index && s_draw_grass_shadows_value && m_context->scene( )->get_grass( ) )
		m_context->scene( )->get_grass( )->render(
			m_context,
			real_view_pos,
			shadow_render_stage,
			0,
...
void stage_shadow_direct::execute_cascade( u32 cascade_id, u32 cascade_index, u32 shadow_map_size )
{
	...
	float current_cascade_align_mult =
		1.f / powf( 2048.f / float(shadow_map_size), 2.f ) * cascade_align_mults[cascade_index];
	...
	float4x4 light_view_transform = math::create_camera_direction(
		sun->position,
		sun->direction,
		float3( 1.f, 0.f, 0.f )
	);
	float4x4 light_projection_transform = math::create_orthographic_projection(
		size,
		size,
		0.1f,
		sun_distance + 1.41421f * size
	);
	...
	if ( cascade_index == 0 ) {
		for ( u32 i = 0; i < 4; ++i ) {
			...
			eye_ray.normalize( );
			eye_ray = m_context->get_v_inverted( ).transform_direction( eye_ray );
			cascade_volume.view_frustum_rays.push_back(
				ray( view_pos + m_context->get_near( ) * eye_ray, eye_ray )
			);
		}
	}
	else
	...
	float3 adjastment = compute_aligment(
		light_shift_xz,
		light_view_transform * light_projection_transform,
		float(shadow_map_size),
		current_cascade_align_mult,
		gran
	);

	float4x4 light_view_transform_offset = math::create_camera_direction(
		sun->position + light_shift_xz + offset_to_viewer,
		sun->direction,
		float3( 1.f, 0.f, 0.f )
	);

	float3 gran0;
	float3 adjastment_of_offset = compute_aligment(
		light_shift_xz,
		light_view_transform_offset * light_projection_transform,
		float(shadow_map_size),
		1.f,
		gran0
	);

	light_view_transform = math::create_camera_direction(
		sun->position + light_shift_xz + adjastment + offset_to_viewer + adjastment_of_offset,
		sun->direction,
		float3( 1.f, 0.f, 0.f )
	);

	bool const need_refresh =
		options::ref( ).current.m_update_shadows_every_frame ||
		m_invalid_shadow ||
		!math::is_similar( adjastment.x, m_previous_adjastment[cascade_index].x, 0.01f ) ||
		!math::is_similar( adjastment.y, m_previous_adjastment[cascade_index].y, 0.01f );
	...
	if ( need_refresh ) {
		...
		float4x4 texture_space(
			float4( 0.5f, 0.f, 0.f, 0.f ),
			float4( 0.f, -0.5f, 0.f, 0.f ),
			float4( 0.f, 0.f, 1.f, 0.f ),
			float4( 0.5f, 0.5f, -m_context->m_sun_cascades[cascade_id].bias, 1.f )
		);
	...
	m_context->set_view2shadow(
		m_context->get_v_inverted( ) * m_view_to_shadow[cascade_index],
		cascade_index
	);
