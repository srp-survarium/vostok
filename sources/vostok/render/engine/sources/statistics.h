...
#ifndef VOSTOK_RENDER_STATISTICS_H_INCLUDED
#define VOSTOK_RENDER_STATISTICS_H_INCLUDED
...
struct statistics_group: public boost::noncopyable
{
					...
					~statistics_group	();
	u32				render				(vostok::ui::world& ui_world, u32 x, u32 y);
	void			start				();
...
private:
	...
	vostok::fixed_string<128>				m_name;
...
struct cpu_gpu_timer_scope: public boost::noncopyable
{
		...
		m_counter(counter)
	{
		...
		VOSTOK_UNREFERENCED_PARAMETER(flush_time);
	...
	~cpu_gpu_timer_scope()
	{
		m_counter.cpu_time.value += 1000.0 * double(m_cpu_tumer.get_elapsed_ticks()) / double(vostok::timing::g_qpc_per_second);
...
struct timer_scope: public boost::noncopyable
{
	...
	~timer_scope()
	{
		m_counter.value += 1000.0 * double(m_cpu_tumer.get_elapsed_ticks() / vostok::timing::g_qpc_per_second);
	}
...
public:
	...
	~statistics								();
	void start								();
	void render								(vostok::ui::world& ui_world, u32 x, u32 y);
	u32 get_num_text_lines					() const;
...
#endif // #ifndef VOSTOK_RENDER_STATISTICS_H_INCLUDED
