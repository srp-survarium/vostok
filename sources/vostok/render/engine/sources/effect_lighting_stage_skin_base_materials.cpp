...
void effect_lighting_stage_skin_base_materials::compile( effect_compiler& compiler, const custom_config_value& custom_config)
{
	{
	...
	//for (u32 light_type_index=0; light_type_index<num_light_types; light_type_index++)
	{
		...
		//compiler.begin_pass("blur_irradiance_texture", "skin_forward_lighting2d", configuration);
	...
	
	{
		...
		compiler.begin_technique();
			compiler.begin_pass("blur_irradiance_texture", NULL, "blur_skin_irradiance_texture", configuration, NULL);
				...
				compiler.set_texture("t_blurring_stretch", r2_rt_skin_scattering_stretch);
