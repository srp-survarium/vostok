...
stage_atmosphere::stage_atmosphere(
	...
	m_type				( type )
{
	effect_manager::ref( ).create_effect<effect_atmospheric_scattering>(
		&m_atmospheric_scattering_effect
	);

	m_to_sun_direction_parameter = backend::ref( ).register_constant_host(
		"to_sun_direction_parameter",
		rc_float
	);
	m_c_inverted_view_projection_matrix = backend::ref( ).register_constant_host(
		"inverted_view_projection_matrix",
		rc_float
	);
	m_c_atmosphere_parameters = backend::ref( ).register_constant_host(
		"atmosphere_parameters",
		rc_float
	);
	m_c_inscatter_parameters = backend::ref( ).register_constant_host(
		"inscatter_parameters",
		rc_float
	);
	m_c_eye_ray_corner = backend::ref( ).register_constant_host(
		"s_eye_ray_corner",
		rc_float
	);
	m_sky_clouds_parameters0 = backend::ref( ).register_constant_host(
		"sky_clouds_parameters0",
		rc_float
	);
	m_sky_clouds_parameters1 = backend::ref( ).register_constant_host(
		"sky_clouds_parameters1",
		rc_float
	);
	m_sky_clouds_parameters2 = backend::ref( ).register_constant_host(
		"sky_clouds_parameters2",
		rc_float
	);
	m_sun_moon_parameters = backend::ref( ).register_constant_host(
		"sun_moon_parameters",
		rc_float
	);
...
)
{
	...
	device::ref( ).d3d_context( )->RSGetViewports(
		&viewport_count,
		&view_port_saved
	);
	...
	u32 offset;
	screen_vertex* pv = reinterpret_cast<screen_vertex*>(
		backend::ref( ).vertex.lock( 4, sizeof(screen_vertex), offset )
	);
...
void stage_atmosphere::execute( )
{
	...
	post_process_parameters& pp_parameters =
	...
	if ( m_type == atmosphere_on_sky )
	{
		...
		if ( recalc_rayleigh_scattering )
		{
			...
			backend::ref( ).set_ps_constant(
				...
				float4( to_sun_direction, sun_int )
			);
			backend::ref( ).set_ps_constant(
				m_c_atmosphere_parameters,
				float4(
					pp_parameters.atmosphere_kresun_multiplier,
					pp_parameters.atmosphere_kmesun_multiplier,
					pp_parameters.atmosphere_kr4pi_multiplier,
					pp_parameters.atmosphere_km4pi_multiplier
				)
			);
			fill_surfaces(
				m_context->get_rt( rt_mie_scattering ),
				m_context->get_rt( rt_rayleigh_scattering ),
				false
			);
			m_context->get_scene_view( )->m_atmosphere_prev_targets_size =
				m_context->m_targets->get_id( );
		...
		{
			m_atmospheric_scattering_effect->apply(
				pp_parameters.sky_clouds_blend_mode ? 2 : 3,
				0
			);
			backend::ref( ).set_ps_texture(
				"sky_clouds_texture",
				pp_parameters.sky_clouds_texture.c_ptr( )
			);
			backend::ref( ).set_ps_constant(
				m_sky_clouds_parameters0,
				float4(
					pp_parameters.sky_clouds_color.xyz( ),
					pp_parameters.sky_clouds_u_tile
				)
			);
			backend::ref( ).set_ps_constant(
				m_sky_clouds_parameters1,
				float4(
					pp_parameters.sky_clouds_fog_power,
					pp_parameters.sky_clouds_fog_up_limit,
					0.0f,
					0.0f
				)
			);

			float const angle_in_rad =
				pp_parameters.sky_clouds_rotation / 180.0f * math::pi;
			backend::ref( ).set_vs_constant(
				m_sky_clouds_parameters2,
				float4(
					math::cos( angle_in_rad ),
		...
		if ( pp_parameters.sun_moon_texture && sun )
		{
			...
			float scale =
				( m_context->get_view_pos( ) - sun->position ).length( ) /
				384467000.0f * 6948400.0f *
				pp_parameters.sun_moon_billboard_scale;
			float4x4 rotation_X_translation(
				float4( L_up, 0.0f ),
				float4( L_right, 0.0f ),
				float4( L_dir, 0.0f ),
				float4( sun->position, 1.0f )
			);
			float4x4 world_transform =
				math::create_scale( float3( scale, scale, scale ) ) *
				math::create_rotation( rotation_X_translation.get_angles_xyz( ) ) *
				math::create_translation( sun->position + L_dir * 0.0f );
			m_context->set_w( world_transform );

			backend::ref( ).set_ps_texture(
				"sun_moon_texture",
				pp_parameters.sun_moon_texture.c_ptr( )
			);
			backend::ref( ).set_ps_constant(
				m_sun_moon_parameters,
				float4( pp_parameters.sun_moon_color, 0.0f )
			);
			backend::ref( ).render_indexed(
				D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST,
				6,
	...
	if ( m_type == atmosphere_on_geometry )
	{
		...
		if ( pp_parameters.use_atmosphere_inscattering_on_geometry )
		{
			...
			backend::ref( ).set_ps_constant(
				m_c_inscatter_parameters,
				float4(
					pp_parameters.atmosphere_inscatter_distance,
					pp_parameters.atmosphere_inscatter_power,
					0.0f,
					0.0f
				)
			);
