...
u32 get_stencil_mask( enum_vertex_input_type, bool )
{
	// STATE[STUB]
	// FUNCTION BODY[0x5e9c90]
	// claude@NOTE: no legacy ancestor - per-vertex-type stencil mask helper postdates the legacy corpus
	return 0;
}
...
)
{
	...
	configuration.use_anisotropic_direction_texture = custom_config.value_exists("use_anisotropic_direction_texture") ?
												  bool(custom_config["use_anisotropic_direction_texture"]) : false;
	...
	configuration.use_reflection_mask		= custom_config.value_exists("use_reflection_mask") ?
										  bool(custom_config["use_reflection_mask"]) : false;
	...
	if ( bool(custom_config["use_reflection"]))
	{
		...
		if ( custom_config.value_exists("reflection_power") )
			...
			compiler.set_constant			( "reflection_power", 1.f );

		if ( custom_config.value_exists("min_reflection_angle") )
			compiler.set_constant			( "min_reflection_angle", math::deg2rad(custom_config["min_reflection_angle"]) );
		else
			compiler.set_constant			( "min_reflection_angle", 0.f );
		...
		if ( custom_config.value_exists("specular_color_multiplier") )
			...
			compiler.set_constant			( "specular_color_multiplier", float4( 1.f, 1.f, 1.f, 1.f ) );
	}
	...
	{
			if (pass_index == 0)
				compile_begin("vertex_base", NULL, "geometry_pre_pass", compiler, &configuration, custom_config);
			else
			{
				configuration.use_anisotropic_direction_texture = false;
				compile_begin("vertex_base", NULL, "geometry_material_pass", compiler, &configuration, custom_config);
			}
...
// 			}
			...
			if (configuration.use_detail_texture)
				compiler.set_texture("t_detail", pcstr(custom_config["texture_ditail_diffuse"]), 0, true, 0);

			if (pass_index == 0)
			{
				if (configuration.use_anisotropic_direction_texture)
					compiler.set_texture("t_anisotropic_direction", pcstr(custom_config["anisotropic_direction_texture"]), 0, true, 0);
			}
			...
			{
				compiler.set_texture("t_cubemap", pcstr(custom_config["texture_cubemap"]), 0, true, 0);

				if (configuration.use_reflection_mask)
				{
					compiler.set_texture("t_cubemap_mask", pcstr(custom_config["texture_cubemap_mask"]), 0, true, 0);
				}
			...
			else
			{
				...
				{
					compiler.set_constant("sequence_parameters",
						float4(
			...
			if (configuration.use_detail_normal_texture)
			{
				...
				if (custom_config.value_exists("texture_detail_normal"))
					compiler.set_texture("t_detail_normal", pcstr(custom_config["texture_detail_normal"]), 0, true, 0);
			...
			if( configuration.use_parallax)
			{
				...
				compiler.set_texture("t_height_map", pcstr(custom_config["texture_bump"]), 0, true, 0);
			}
			...
			{
				compiler.set_texture("t_emission", pcstr(custom_config["texture_emissive"]), 0, true, 0);
			}
			...
			if( configuration.use_translucency_texture)
				compiler.set_texture("t_translucency", pcstr(custom_config["texture_translucency"]), 0, true, 0);
			//else
			//{
			...
			if (configuration.use_variation_mask)
			{
				...
				if (custom_config.value_exists("texture_variation_mask"))
					compiler.set_texture ( "t_variation_mask", pcstr(custom_config["texture_variation_mask"]), 0, true, 0);
				else
					compiler.set_texture ( "t_variation_mask", "", 0, true, 0);
			}
