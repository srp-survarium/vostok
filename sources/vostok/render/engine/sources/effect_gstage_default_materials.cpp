...
void effect_gstage_default_materials::compile( effect_compiler& compiler, const custom_config_value& custom_config)
{
...
// 			}
			...
			if (pass_index == 0)
			{
				if (configuration.use_anisotropic_direction_texture)
					compiler.set_texture("t_anisotropic_direction", pcstr(custom_config["anisotropic_direction_texture"]));
			}
			...
			if (configuration.use_reflection)
			{
				...
				{
					compiler.set_texture("t_cubemap_mask", pcstr(custom_config["texture_cubemap_mask"]));
				}
