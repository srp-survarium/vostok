...
#pragma message ( VOSTOK_TODO("is such order of members is intended? if doesn't matter make it perfect, please") )
	float	position[3];
	float	color[4];
	float	uv[2];
}; // struct particle_beamtrail_vertex
	...
	)
{
	...
	m_max_particles						= vostok::math::max<u32>(m_max_particles, in_num_max_particles);
	m_max_particles						= vostok::math::min<u32>(m_max_particles, 2000);
	
	switch(datatype)
	{
		case vostok::particle::particle_data_type_billboard:
		{
			if (use_subuv)
			{
				m_vertex_type = vostok::particle::particle_vertex_type_billboard_subuv;
			...
			{
				m_vertex_type = vostok::particle::particle_vertex_type_billboard;
				m_num_vertices = sizeof(subuv_particle_sprite_vertex) * m_max_particles * 4;
				m_num_indices  = sizeof(subuv_particle_sprite_vertex) * m_max_particles * 6;
				// we do not need to do this at all!
				// we should use single buffer for all the particles
		...
		case vostok::particle::particle_data_type_trail:
		case vostok::particle::particle_data_type_beam:
			{
				if (datatype==vostok::particle::particle_data_type_trail)
					m_vertex_type = vostok::particle::particle_vertex_type_trail;
				else
					m_vertex_type = vostok::particle::particle_vertex_type_beam;
				
				u32 num_sheets = vostok::math::max<u32>(beamtrail_parameters_num_sheets, 1);
				num_sheets = vostok::math::min<u32>(num_sheets, 10000);
		...
		case vostok::particle::particle_data_type_decal:
			{
				m_vertex_type = vostok::particle::particle_vertex_type_decal;
				break;
			}
		default:
			{
				m_vertex_type = vostok::particle::particle_vertex_type_unknown;
				break;
			}
	};
}
...
void render_particle_emitter_instance::render(vostok::math::float3 const& view_location, u32 const num_particles)
{
	switch (m_vertex_type)
	{
...
void render_particle_emitter_instance::render_beams(vostok::math::float3 const& view_location, u32 num_particles)
{
...
void render_particle_emitter_instance::render_trails(vostok::math::float3 const& view_location, vostok::particle::base_particle* start_particle, u32 num_particles)
{
...
vostok::render::enum_vertex_input_type render_particle_emitter_instance::get_vertex_input_type( )
{
	switch (m_vertex_type)
	{
		case particle::particle_vertex_type_billboard: 
...
void render_particle_emitter_instance::draw_debug(vostok::math::float4x4 const& view_matrix, vostok::particle::enum_particle_render_mode debug_mode)
{
