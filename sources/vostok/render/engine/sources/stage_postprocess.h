...
struct dof_shader_constants
{
	...
	void set(vostok::math::float3 const& blurriness_height_lights, float distance, float region, float power, float near_blur_amout, float far_blur_amout, float bokeh_dof_radius, float bokeh_dof_density);
...
struct scene_shader_constants
{
	...
	void set(vostok::math::float3 const& height_lights, vostok::math::float3 const& mid_tones, vostok::math::float3 const& shadows, vostok::math::float3 const& fade_color, float fade_amount, float gamma_correction_factor, float desaturation);
...
private:
	...
	void measure_per_pixel_luminance				(res_texture* scene_texture, vostok::math::float4& out_avrg_min_max);
	...
	vostok::math::float4					blur_offsets_weights[27];
	...
	vostok::render::material_ptr			m_test_material;
