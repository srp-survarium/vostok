...
cloud_key_parameters environment_temp::get_interp_key( float in_time )
{
	float const key_time = in_time / key_time_step;
	...
	m_prev_up_vector			( 0.0f, 0.0f, 0.0f )
{
	...
	m_clouds_scale = float3(
		static_cast<float>( m_clouds_size_x ),
		static_cast<float>( m_clouds_size_y ),
		static_cast<float>( m_clouds_size_z )
	) * m_clouds_scale_multiplier;

	for ( u32 i = 0; i < 2; ++i )
	{
		pcstr const texture_name = i == 0 ? "$user$cloud_density0" : "$user$cloud_density1";
		m_3d_clouds_density_texture[i] = resource_manager::ref( ).create_texture3d(
			texture_name,
			m_clouds_size_x,
			m_clouds_size_y,
			m_clouds_size_z,
...
void stage_clouds::fill_cloud_texture( u32 index )
{
	...
	if ( data )
	{
		memory::copy(
			...
			data_size,
			index == 0 ?
				m_context->scene( )->get_clouds( )->m_cloud_simulation_0->get_voxels( ) :
				m_context->scene( )->get_clouds( )->m_cloud_simulation_1->get_voxels( ),
			data_size
		);
	}
...
void stage_clouds::execute( )
{
	...
	float2 const view_dir_2d = math::normalize_safe(
	...
	float2 const offset_direction_ground = math::normalize_safe(
	...
	{
		m_camera_offset += offset_vector_ground.length( ) /
			m_clouds_scale_multiplier * math::abs( abs_ov_dot_dir_ground );
	}
	if ( abs_ov_dot_dir_ground < 0.0f )
	{
		m_camera_offset -= offset_vector_ground.length( ) /
	...
	if ( options::ref( ).current.m_clouds_allow_moving )
	{
		...
		{
			m_camera_offset += math::sign( m_wind_direction | view_dir_2d2 ) *
				( m_wind_direction * 0.0125f ).length( ) /
				m_wind_direction.length( ) *
				math::abs( m_wind_direction | view_dir_2d2 ) *
				m_clouds_scale_multiplier;
		}

		m_wind_offset += m_wind_direction *
			( m_clouds_scale_multiplier * 0.0125f ) * m_clouds_scale_multiplier;
	}

	if ( m_camera_offset >= 1.0f )
		m_camera_offset = frac( m_camera_offset );
	else if ( m_camera_offset < 0.0f && math::abs( m_camera_offset ) >= 1.0f )
	...
	for ( s32 i = options::ref( ).current.m_clouds_num_evaluate_slices; i > 0; --i )
	{
		...
		{
			float scale = ( static_cast<float>( i ) - m_camera_offset ) *
				m_clouds_scale_multiplier;
			float4x4 world_matrix = math::create_scale(
				float3( scale, scale, scale )
			) * math::create_translation( m_previous_view_position );

			m_clouds_effect->apply( 0, 0 );
			m_context->set_w( world_matrix );
			backend::ref( ).set_ps_constant(
				m_c_sphere_to_sky_matrix,
				math::transpose( sphere_to_clouds_matrix )
			);
			backend::ref( ).set_ps_constant(
				m_c_clouds_grid_size,
				float3(
					static_cast<float>( m_clouds_size_x ),
					static_cast<float>( m_clouds_size_y ),
					static_cast<float>( m_clouds_size_z )
			...
			backend::ref( ).set_ps_constant(
				m_c_clouds_offset,
				m_wind_offset + horizont_down_offset
			);
			backend::ref( ).set_ps_constant( m_c_interp_alpha, interp_alpha );
			backend::ref( ).set_ps_constant(
				m_c_layer_height,
				math::max( 0.5f, interp_key.layer_height )
			);
			backend::ref( ).set_ps_constant( m_c_cloud_base, cloud_base );
			backend::ref( ).set_ps_constant(
				m_c_light_multiplier_parameters,
				float4(
					interp_key.direct_light,
					interp_key.indirect_light,
					interp_key.ambient,
					0.0f
				)
			);
			...
			backend::ref( ).set_ps_texture(
				"t_clouds_0",
				m_3d_clouds_density_texture_left.c_ptr( )
			);
			backend::ref( ).set_ps_texture(
				"t_clouds_1",
				m_3d_clouds_density_texture_right.c_ptr( )
			);
		...
		if ( options::ref( ).current.m_use_god_rays )
		{
			...
			backend::ref( ).set_ps_constant(
				m_c_sphere_to_sky_matrix,
				math::transpose( world_to_god_rays_matrix )
			);
