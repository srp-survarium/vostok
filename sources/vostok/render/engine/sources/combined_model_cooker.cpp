...
void skeleton_combined_model_cook::translate_query( resources::query_result_for_cook& parent )
{
	...
	config_path.assignf(
		"resources/models/%s.combined_skin",
	...
	resources::query_resource(
		...
		boost::bind(
			&skeleton_combined_model_cook::on_config_loaded,
			this,
			_1,
			&parent
		),
...
)
{
	...
	cook_data->skeleton_name.assignf(
		"resources/animations/%s",
		(pcstr)root["skeleton"]
	);
	cook_data->bind_pose_name.assignf(
		"resources/models/%s.skinned_model/render/bind_pose",
		(pcstr)root["bind_pose"]
	);
...
)
{
	...
	resources::query_resources(
		...
		request_count,
		boost::bind(
			&skeleton_combined_model_cook::on_resources_loaded,
			this,
			_1,
			parent,
			cook_data
...
)
{
	...
	cook_data->skeleton = static_cast_resource_ptr< animation::skeleton_ptr >(
	...
	resources::user_data_variant* user_data_variants =
		ALLOC( resources::user_data_variant, parts_count );
	resources::user_data_variant** user_data_variants_ptrs =
		ALLOC( resources::user_data_variant*, parts_count );

	for ( u32 i = 0; i < cook_data->models_count; ++i )
	{
		cook_data->model_defs[i].converted_model = data[2 + i * 3].get_managed_resource( );
		cook_data->model_defs[i].owner_model_config =
			static_cast_resource_ptr< configs::binary_config_ptr >(
				data[3 + i * 3].get_unmanaged_resource( )
			);
		cook_data->model_defs[i].export_properties_config =
			static_cast_resource_ptr< configs::binary_config_ptr >(
				data[4 + i * 3].get_unmanaged_resource( )
			);

		configs::binary_config_value root =
			cook_data->model_defs[i].owner_model_config->get_root( );
		...
		mesh_type_enum model_type = (mesh_type_enum)(u16)
			cook_data->model_defs[i].export_properties_config->get_root( )["type"];

		requests[i].set(
			cook_data->model_defs[i].material_name.c_str( ),
			resources::material_effects_instance_class
		);
		user_data_variants_ptrs[i] = new ( &user_data_variants[i] )
	...
	resources::query_resources(
		...
		parts_count,
		boost::bind(
			&skeleton_combined_model_cook::on_material_effects_loaded,
			this,
			_1,
			parent,
			cook_data
...
)
{
	...
	for ( u32 i = 0; i < parts_count; ++i )
	{
		...
		resources::pinned_ptr_const< u8 > converted_model_ptr(
			cook_data->model_defs[i].converted_model
		);
		...
		surface->load(
			cook_data->model_defs[i].export_properties_config->get_root( ),
			model_reader
		);

		cook_data->model_defs[i].material_effects =
...
)
{
	...
	resources::query_resource(
		...
		resources::skeleton_combined_model_class,
		boost::bind(
			&skeleton_combined_render_model_instance_cook::on_resources_loaded,
			this,
			_1,
			&parent
		),
...
)
{
	...
	skeleton_render_model_ptr model =
		static_cast_resource_ptr< skeleton_render_model_ptr >(
			data[0].get_unmanaged_resource( )
		);
	skeleton_render_model_instance* created_resource =
		NEW( skeleton_render_model_instance );
	created_resource->assign_original( model );
	parent->set_unmanaged_resource(
		created_resource,
		resources::nocache_memory,
		sizeof( skeleton_render_model_instance )
	);
	parent->finish_query( result_success );
}
...
)
{
	...
	resources::query_resources(
		...
		2,
		boost::bind(
			&skeleton_combined_model_instance_cook::on_resources_loaded,
			this,
			_1,
			&parent
		),
...
)
{
	...
	created_resource->m_skeleton =
		static_cast_resource_ptr< animation::skeleton_ptr >(
			data[1].get_unmanaged_resource( )
		);

	skeleton_render_model_instance* skel_mesh =
		static_cast< skeleton_render_model_instance* >(
			created_resource->m_render_model.c_ptr( )
		);
	skel_mesh->m_bones_matrices.resize(
		skel_mesh->m_original->m_inverted_bones_matrices_in_bind_pose.size( )
	);
	...
	parent_query->set_unmanaged_resource(
		created_resource,
		resources::nocache_memory,
		sizeof( skeleton_model_instance )
	);
