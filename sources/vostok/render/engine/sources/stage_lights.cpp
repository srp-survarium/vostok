...
static vostok::console_commands::cc_float s_shadow_z_near("shadow_z_near", s_shadow_z_near_value, 0.0f, 1.0f, true, vostok::console_commands::command_type_engine_internal);
	...
	m_is_forward_lighting_pass	( is_forward_lighting_pass )
{
	...
	// claude@NOTE: only the legacy enable/shadow-target slice is recovered below; the shipped ctor
	// additionally creates the light accumulator effects/instancing buffers - matcher-phase
	...
	if (options::ref().current.m_enabled_local_light_shadows)
	{
		...
		m_shadow_depth_stencil_texture[0] = resource_manager::ref().create_texture(r2_rt_shadow_map_size_1024, 0, 0, false, true, true, 0);
		m_shadow_depth_stencil_texture[1] = resource_manager::ref().create_texture(r2_rt_shadow_map_size_512, 0, 0, false, true, true, 0);
		m_shadow_depth_stencil_texture[2] = resource_manager::ref().create_texture(r2_rt_shadow_map_size_256, 0, 0, false, true, true, 0);
	}
}
...
)
{
	...
	VOSTOK_UNREFERENCED_PARAMETERS(l, shadow_quality, marge);
	...
	// claude@NOTE: legacy kept the selection in the retired m_dynamic_visuals_to_shadow member;
	// canonical select_models grew view_pos/surface_flags/moved_only - local vector adaptation
	vector< render_surface_instance* > dynamic_visuals_to_shadow;
	m_context->scene()->select_models( m_context->get_culling_vp(), dynamic_visuals_to_shadow, view_position, cast_shadow_flag, false );

	vector< render_surface_instance* >::iterator it_d		= dynamic_visuals_to_shadow.begin();
	vector< render_surface_instance* >::const_iterator end_d	= dynamic_visuals_to_shadow.end();
	...
	for ( ; it_d != end_d; ++it_d)
	{
		...
		{
			m_shadow_effect->apply( 0, 0 );
		}
		else
		{
			me.m_effects[shadow_render_stage]->apply( 0, 0 );
		}
...
stage_lights::~stage_lights( )
{
	// claude@NOTE: no legacy ancestor - legacy dtor body is entirely commented out; the real teardown is new; matcher-phase work.
	// STATE[STUB]
...
{
	// claude@NOTE: no legacy ancestor - environment-probe lighting is new-in-target; matcher-phase work.
	// STATE[STUB]
...
)
{
	...
	VOSTOK_UNREFERENCED_PARAMETER(light_range);
...
void stage_lights::execute( )
{
	...
	// claude@NOTE: legacy kept the selection in the retired m_dynamic_visuals member;
	// canonical select_models grew view_pos/surface_flags/moved_only - local vector adaptation
	vector< render_surface_instance* > dynamic_visuals;
	m_context->scene()->select_models( m_context->get_culling_vp(), dynamic_visuals, m_context->get_v_inverted().c.xyz(), visible_flag, false );

	vector< render_surface_instance* >::iterator it_d		= dynamic_visuals.begin();
	vector< render_surface_instance* >::const_iterator end_d	= dynamic_visuals.end();
...
u32 stage_lights::index_to_shadow_size( u32 size_index ) const
{
	...
	// claude@NOTE: ported from the legacy stage_shadow_direct twin (stage_lights' own copy is not
	// in the legacy corpus; identical switch shape)
	switch (size_index)
	{
		case 0:
			return 2048;
...
void stage_lights::debug_render( )
{
	...
	for ( lights_type::const_iterator i = lights.begin(), e = lights.end(); i != e; ++i ) {
		...
		// claude@NOTE: legacy called draw_triangles with 4 arguments; the
		// canonical declaration carries a fifth covering_effect flag with no
		// default, so the non-covering (solid) debug path is spelled out.
