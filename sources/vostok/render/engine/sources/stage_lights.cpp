...
using vostok::render::stage_lights;
using vostok::render::renderer_context;
using vostok::render::render_model_instance;
using vostok::render::light;

struct screen_vertex
{
	vostok::math::float4 position;
	vostok::math::float2 tc;
	void set(vostok::math::float4 const& in_position, vostok::math::float2 const& in_tc)
	{
...
void stage_lights::render_to_cubemap_face( u32 face_index, vostok::math::float3 const& eye_position, float far_plane )
{
...
void stage_lights::render_to_hw_shadowmap(
		...
		vostok::math::float4x4 const& view_matrix,
		vostok::math::float4x4 const& projection_matrix,
		u32 const marge
	)
{
...
{
	VOSTOK_UNREFERENCED_PARAMETER(shadow_quality);
...
void stage_lights::render_particle_lighting(vostok::render::render_particle_emitter_instance* instance, light* l, u32 num_particles)
{
...
void stage_lights::fill_surface(vostok::render::ref_rt surf)
{
...
void stage_lights::make_skin_scattering_texture(vostok::render::render_surface_instance* instance, light* l)
{
...
*/	
	...
	if (draw)
	{
		...
		vostok::math::float4					kernel_offsets[8];
		vostok::math::float4					blur_offsets_weights[9];
...
void stage_lights::render_model_lighting(vostok::render::render_surface_instance* instance, light* l)
{
	float light_range				= l->range;
...
void stage_lights::render_speedtree_lighting (vostok::render::lod_entry const* lod,
...
// 		}
	...
	if (options::ref().m_enabled_draw_speedtree)
	{
		...
		for (speedtree_forest::tree_render_info_array_type::iterator it = visible_trees.begin(); it != visible_trees.end(); ++it)
		{
			m_context->set_w(vostok::math::float4x4().identity());
	...
	// Particles
	vostok::particle::world* part_world = m_context->scene()->particle_world();
	...
	for (particle::render_particle_emitter_instances_type::const_iterator it=emitters.begin(); it!=emitters.end(); ++it)
	{
		...
		vostok::particle::enum_particle_render_mode particle_render_mode = m_context->scene_view()->get_particles_render_mode();
