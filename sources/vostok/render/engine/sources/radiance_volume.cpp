...
static vostok::console_commands::cc_bool s_draw_radiance_debug_cc				("draw_radiance_debug",				s_draw_radiance_debug_value, false, vostok::console_commands::command_type_engine_internal);
static vostok::console_commands::cc_bool s_draw_propagated_radiance_debug_cc	("draw_propagated_radiance_debug",	s_draw_propagated_radiance_debug_value, false, vostok::console_commands::command_type_engine_internal);
static vostok::console_commands::cc_bool s_draw_radiance_occluders_debug_cc	("draw_radiance_occluders_debug",	s_draw_radiance_occluders_debug_value, false, vostok::console_commands::command_type_engine_internal);
...
void radiance_volume::set_origin(vostok::math::float3 const& in_origin)
{
	m_bbox.zero			();
	m_bbox.max			+= m_scale;
...
vostok::math::float3 const& radiance_volume::get_origin() const 
{ 
	return m_bbox.min;
}
...
void radiance_volume::inject_occluders(renderer_context* context, vostok::math::float3 const& light_position, vostok::math::float3 const& light_direction, u32 rsm_size)
{
...
void radiance_volume::inject_lighting(vostok::math::float3 const& light_position, vostok::math::float3 const& light_direction, float light_fov, u32 rsm_size)
{
	backend::ref().set_render_targets		(&*m_3d_rt_radiance_r, &*m_3d_rt_radiance_g, &*m_3d_rt_radiance_b, 0);
...
static void draw_line(float3 const& start, float3 const& end, vostok::math::color clr, bool use_depth)
{
	float3 line[2];
	line[0] = start;
	line[1] = end;
