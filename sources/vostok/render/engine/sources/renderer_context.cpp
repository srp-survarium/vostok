	...
	m_solid_material_parameters		( 10.f, 1.f, 0.f, 0.f)
{
	...
	for ( u32 i = 0; i < rt_num_render_targets; ++i )
	{
		...
		if ( name )
			m_family[i].texture = resource_manager::ref().create_texture( name );
	}

	m_t_null = resource_manager::ref().create_texture( "$user$null" );
}
...
void renderer_context::set_target_context(
...
)
{
	...
	for ( u32 i = 0; i < rt_num_render_targets; ++i )
	{
		ref_texture const& source = targets_context->m_family[i].texture;
		if ( source )
