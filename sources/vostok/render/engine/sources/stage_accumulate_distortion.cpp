...
void stage_accumulate_distortion::execute( )
{
	...
	backend::ref( ).set_render_targets(
		&*m_context->get_rt( rt_distortion ),
		&*m_context->get_rt( rt_distortion_mask ),
		0,
		0
	);
...

	{
		...
		if ( part_world )
		{
			...
			for ( particle::render_particle_emitter_instances_type::const_iterator it = emitters.begin( ); it != emitters.end( ); ++it )
			{
				...
				if (
					!num_particles
					|| !instance.get_material_effects( ).stage_enable[accumulate_distortion_render_stage]
					|| !instance.get_material_effects( ).m_effects[accumulate_distortion_render_stage]
				)
					continue;
