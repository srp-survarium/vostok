...
public:
	...
	void draw_screen_lines		( vostok::math::float3 const* points, u32 count, math::color const& color, float width, u32 pattern, bool use_depth, bool is_screen_space_coord);
	void draw_3D_point			( vostok::math::float3 const& position, float width, math::color const& color, bool use_depth);
	...
	void set_selection_parameters		( vostok::math::float4 selection_color, float selection_rate );
	
	
private:
	vostok::render::material_effects_instance_cook_data* m_cook_data_to_delete;
	bool is_effects_ready				() const;

	typedef vostok::render::map<render_model_instance*, material_effects> render_model_to_material_type;
	 render_model_to_material_type m_render_model_to_material;
	
	vostok::math::float4		m_selection_color;
	float					m_selection_rate;
	vostok::math::float4		m_current_selection_color;
	vostok::math::float4		m_ghost_model_color;
	
	void on_material_loaded (vostok::resources::queries_result& result);
