	...
	m_stride		( sizeof( vertex_type ) )
{
	...
	for ( float i = 0.f; i < 16.f; i += 1.f )
	{
		...
		for ( float j = 0.f; j < 8.f; j += 1.f )
		{
			...
			vertices[vertex_index].position.x = math::cos( move_y ) * math::sin( move_xz * math::deg2rad( 100.f ) );
			vertices[vertex_index].position.y = math::cos( move_xz * math::deg2rad( 100.f ) );
			vertices[vertex_index].position.z = math::sin( move_y ) * math::sin( move_xz * math::deg2rad( 100.f ) );
