...
#ifndef MASTER_GOLD
	void update_material_from_config	(fs_new::virtual_path_string const& material_name,  vostok::configs::lua_config_value const& config_value);
#endif // #ifndef MASTER_GOLD

private:
	
#ifndef MASTER_GOLD
	material_ptr create_material		(fs_new::virtual_path_string const& material_name, vostok::configs::lua_config_value const& config_value);
	void on_material_created			(vostok::resources::queries_result& data, material_ptr* out_material_ptr);
	material_ptr create_material		(vostok::configs::lua_config_value const& config_value);
#endif // #ifndef MASTER_GOLD
	
	pcstr get_materials_path			() const;
	pcstr get_material_instances_path	() const;
	
	void on_material_effects_instance_loaded (vostok::resources::queries_result& data, 
											  material_effects_instance_ptr* mtl_effects_instance_ptr);
	void on_material_loaded				(vostok::resources::queries_result& data, 
										 post_process_parameters* in_out_post_process_parameters);
#ifndef MASTER_GOLD
	void on_material_source_changed		(vostok::vfs::vfs_notification const & info);
