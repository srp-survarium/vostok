...
scene_cook::scene_cook():
	resources::unmanaged_cook(vostok::resources::scene_class, reuse_false, use_current_thread_id, use_current_thread_id)
{}
...
mutable_buffer scene_cook::allocate_resource(resources::query_result_for_cook& in_query, const_buffer raw_file_data, bool file_exist)
{
	...
	return mutable_buffer(ALLOC(u8, sizeof(vostok::render::scene)), sizeof(vostok::render::scene));
}
...
{	
	...
	vostok::render::editor_renderer_configuration renderer_configuration;
	
	if( in_out_query.user_data() )
		in_out_query.user_data()->try_get(renderer_configuration);
	
	scene* created_resource	= new(in_out_unmanaged_resource_buffer.c_ptr())vostok::render::scene(renderer_configuration);
