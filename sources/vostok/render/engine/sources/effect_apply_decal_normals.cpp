...
)
{
	...
	compiler.set_texture( "t_decals_smoothness", "$user$decals_smoothness", 0, false, u32( -1 ) );
	...
	compiler.set_depth( false, false );
	compiler.set_cull_mode( D3D_CULL_NONE );
	compiler.set_texture( "t_decals_blend_result", "$user$decals_blend_result", 0, false, u32( -1 ) );
	compiler.set_stencil( true, 0, 0xff, 0, D3D_COMPARISON_NOT_EQUAL );
	compiler.end_pass( );
	compiler.end_technique( );
	...
	compiler.set_depth( false, false );
	compiler.set_cull_mode( D3D_CULL_NONE );
	compiler.set_texture( "t_decals_diffuse", "$user$decals_diffuse", 0, false, u32( -1 ) );
	compiler.set_stencil( true, 0, 0xff, 0, D3D_COMPARISON_NOT_EQUAL );
