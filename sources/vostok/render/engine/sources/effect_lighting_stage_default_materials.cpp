...
) const
{
	...
	// claude@NOTE: no legacy ancestor - should_recompile_when_global_changes postdates the legacy corpus
	return false;
}
	...
	//for (u32 light_type_index=0; light_type_index<num_light_types; light_type_index++)
	{
		...
		compile_begin("vertex_base", "forward_lighting", compiler, &configuration, custom_config);
			...
			if (configuration.use_transparency_texture)
				compiler.set_texture("t_transparency", pcstr(custom_config["texture_transparency"]), 0, true, 0);
			//else
