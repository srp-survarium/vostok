...
SpeedTree::Mat4x4 vostok_to_speedtree(math::float4x4 const& m)
{
	...
	vostok::memory::copy(&result.m_afRowCol[0], sizeof(m.elements), &m.elements[0], sizeof(m.elements));
	return result;
}
...
math::float4x4 speedtree_to_vostok(SpeedTree::Mat4x4 const& m)
{
	...
	vostok::memory::copy(&result.elements[0], sizeof(m.m_afRowCol), &m.m_afRowCol[0], sizeof(m.m_afRowCol));
	return result;
}
