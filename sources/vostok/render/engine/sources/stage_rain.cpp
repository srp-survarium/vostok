	...
	m_previous_view_position( 0.0f, 0.0f, 0.0f )
{
	...
	m_t_rain_shadow_map = resource_manager::ref( ).create_texture( "$user$rain_shadow_map", 0, 0, false, true, true, -1 );
...
bool stage_rain::is_effects_ready( ) const
{
	...
	return m_rain_effect.c_ptr( ) &&
		m_effect_shadow_direct.c_ptr( );
}
stage_rain::~stage_rain( )
{
...
void stage_rain::execute( )
{
	...
	if ( !is_enabled( ) || !m_context->get_scene_view( )->post_process_parameters( ).environment_use_rain ||
	...
	else if ( m_camera_offset_view < 0.0f && math::abs( m_camera_offset_view ) >= 1.0f )
		m_camera_offset_view = 1.0f - frac( m_camera_offset_view );

	if ( abs_ov_dot_dir_ground2 > 0.0f )
		m_camera_offset_right += offset_vector_ground.length( ) * math::abs( abs_ov_dot_dir_ground2 ) * 0.5f;
	if ( abs_ov_dot_dir_ground2 < 0.0f )
		m_camera_offset_right -= offset_vector_ground.length( ) * math::abs( abs_ov_dot_dir_ground2 ) * 0.5f;
	if ( m_camera_offset_right >= 1.0f )
		m_camera_offset_right = frac( m_camera_offset_right );
	else if ( m_camera_offset_right < 0.0f && math::abs( m_camera_offset_right ) >= 1.0f )
	...
	if ( s_rain_debug2 ) {
		...
		for ( ; i < num_cones; ++i )
		{
			...
			float4x4 cone_transform =
				...
				math::create_rotation_z( rain_angle_y + math::sin( m_rain_rotation_y[i] ) * m_context->get_scene_view( )->post_process_parameters( ).environment_rain_random_rotaion_radius +
					m_rain_offsets[i].y * m_context->get_scene_view( )->post_process_parameters( ).environment_rain_random_base_offset ) *
				math::create_translation( m_context->get_view_pos( ) - float3( view_dir_2d.x, 0.0f, view_dir_2d.y ) * m_camera_offset_view -
					float3( right_dir_2d.x, 0.0f, right_dir_2d.y ) * m_camera_offset_right );
