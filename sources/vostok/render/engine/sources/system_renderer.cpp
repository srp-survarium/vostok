	...
	m_color_write		(true)
{
	...
	m_selection_color				= vostok::math::float4( 0.0f, 0.0f, 0.5f, 1.f );
	m_selection_rate				= 1.0f;
	m_current_selection_color		= vostok::math::float4( 0.f, 0.f, 0.f, 0.f );
	m_ghost_model_color				= vostok::math::float4( 0.2f, 0.2f, 0.2f, 0.2f );
...
static vostok::math::float2 clip_2_screen( vostok::math::float3 const& world_pixel, math::float4x4 const& wvpMatrix, u32 screen_width, u32 screen_height)
{
	vostok::math::float4 result = wvpMatrix.transform(float4(world_pixel,1.0f));
	...
	return vostok::math::float2( result.elements[0], result.elements[1]);
}
...
static u8 calc_pattern(vostok::math::float2 const& begin, vostok::math::float2 const& end)
{
	return static_cast_checked<u8>(frac( vostok::math::max(abs(end.elements[0]-begin.elements[0]), abs(end.elements[1]-begin.elements[1])) / (float)pattern_length ) * ((float)pattern_length));
}

void system_renderer::draw_screen_lines	( vostok::math::float3 const* points, u32 count, math::color const& color, float width, u32 pattern, bool use_depth, bool is_screen_space_coord)
{
	if (!is_effects_ready())
		return;
	...
	u32 screen_width = vostok::render::backend::ref().target_width();
	u32 screen_height = vostok::render::backend::ref().target_height();
	...
	{
		const vostok::math::float3& start_point	= points[ i-1 ],
		...
		vostok::math::float2 screen_start = clip_2_screen( start_point, wvpMatrix, screen_width, screen_height);
		vostok::math::float2 screen_end	= clip_2_screen( current_point, wvpMatrix, screen_width, screen_height);
...
void system_renderer::draw_3D_point	( vostok::math::float3 const& position, float width, math::color const& color, bool use_depth)
{
	if (!is_effects_ready())
		return;
...
void system_renderer::draw_render_models_selection	( render_surface_instances& render_models)
{
	...
	for (render_surface_instances::iterator it=render_models.begin(), it_end = render_models.end(); it != it_end; ++it)
	{
		...
		m_editor_selection_shader[instance.m_render_surface->get_vertex_input_type()]->apply();
		instance.set_constants();
		m_current_selection_color = m_selection_color * vostok::math::pow(vostok::math::abs(vostok::math::cos(m_renderer_context->m_current_time * m_selection_rate)), 0.5f);
...
void system_renderer::draw_particle_system_instance_selections ( render::vector<particle::particle_system_instance_ptr> const& instances)
{
	...
	m_current_selection_color							=	m_selection_color * 
															vostok::math::pow(vostok::math::abs(vostok::math::cos(m_renderer_context->m_current_time * m_selection_rate)), 0.5f);
		...
		for (particle::render_particle_emitter_instances_type::const_iterator it=emitters.begin(); it!=emitters.end(); ++it)
		{
			...
			*/
			...
			switch (instance->vertex_type()) {
				case vostok::particle::particle_vertex_type_billboard:
					m_sh_particle_selection->apply(effect_particle_selection::billboard);
					break;
				
				case vostok::particle::particle_vertex_type_billboard_subuv:
					m_sh_particle_selection->apply(effect_particle_selection::billboard_subuv);
					break;
				
				case vostok::particle::particle_vertex_type_trail:
				case vostok::particle::particle_vertex_type_beam:
