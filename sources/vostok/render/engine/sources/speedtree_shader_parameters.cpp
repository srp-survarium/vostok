...
void speedtree_wind_parameters::set(SpeedTree::CWind const& wind)
{
	...
	backend::ref().set_vs_constant		(m_wind_times_parameter,			speedtree_to_vostok(Vec4(shader_values + CWind::SH_TIME_PRIMARY)));
	backend::ref().set_vs_constant		(m_wind_distances_parameter,		speedtree_to_vostok(Vec4(shader_values + CWind::SH_DIST_PRIMARY)));
	backend::ref().set_vs_constant		(m_wind_leaves_parameter,			speedtree_to_vostok(Vec3(shader_values + CWind::SH_DIST_LEAVES)));
	backend::ref().set_vs_constant		(m_wind_gust_parameter,				speedtree_to_vostok(Vec3(shader_values + CWind::SH_STRENGTH_COMBINED)));
	backend::ref().set_vs_constant		(m_wind_gust_hints_parameter,		speedtree_to_vostok(Vec3(shader_values + CWind::SH_HEIGHT_OFFSET)));
	backend::ref().set_vs_constant		(m_wind_frond_ripple_parameter,		speedtree_to_vostok(Vec3(shader_values + CWind::SH_DIST_FROND_RIPPLE)));
	backend::ref().set_vs_constant		(m_wind_rolling_branches_parameter,	speedtree_to_vostok(Vec4(shader_values + CWind::SH_ROLLING_BRANCHES_MAX_SCALE)));
	backend::ref().set_vs_constant		(m_wind_rolling_leaves_parameter,	speedtree_to_vostok(Vec4(shader_values + CWind::SH_ROLLING_LEAVES_MAX_SCALE)));
	backend::ref().set_vs_constant		(m_wind_twitching_leaves_parameter,	speedtree_to_vostok(Vec4(shader_values + CWind::SH_LEAVES_TWITCH_AMOUNT)));
	backend::ref().set_vs_constant		(m_wind_tumbling_leaves_parameter,	speedtree_to_vostok(Vec4(shader_values + CWind::SH_LEAVES_TUMBLE_X)));
}
...
void speedtree_common_parameters::set(renderer_context* context, speedtree_tree_component* tree_component, vostok::math::float3 const& lod_reference_position)
{
	...
	backend::ref().set_vs_constant				(
		...
		transpose( speedtree_to_vostok(SpeedTree::Mat4x4(context->scene()->get_speedtree_forest()->get_speedtree_view().GetCameraFacingMatrix())) )
	);
