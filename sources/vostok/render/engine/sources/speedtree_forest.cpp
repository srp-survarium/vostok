...
)
{
	...
	for (s32 base_tree_index=0; base_tree_index<s32(m_visible_trees.m_pBaseTrees->size()); base_tree_index++)
	{
		...
		if (options::ref().current.m_enabled_draw_speedtree_billboards &&
		...
		if (options::ref().current.m_enabled_draw_speedtree_branches && base_tree->HasGeometryType(GEOMETRY_TYPE_BRANCHES))
		{
		...
		if (options::ref().current.m_enabled_draw_speedtree_fronds && base_tree->HasGeometryType(GEOMETRY_TYPE_FRONDS))
		{
		...
		if (options::ref().current.m_enabled_draw_speedtree_leafmeshes && base_tree->HasGeometryType(GEOMETRY_TYPE_LEAF_MESHES))
		{
			instance_lods = m_visible_trees.VisibleInstances(base_tree_index);
			if (instance_lods && !instance_lods->empty( ))
			{
		...
		if (options::ref().current.m_enabled_draw_speedtree_leafcards && base_tree->HasGeometryType(GEOMETRY_TYPE_LEAF_CARDS))
		{
			instance_lods = m_visible_trees.VisibleInstances(base_tree_index);
			if (instance_lods && !instance_lods->empty( ))
			{
...
void speedtree_forest::get_visible_tree_components( renderer_context* context, vector<speedtree_instance_ptr> const& instances, bool sort_result, tree_render_info_array_type& out_tree_render_info_array )
{
	...
	for (vector<speedtree_instance_ptr>::const_iterator instance_it = instances.begin(); instance_it != instances.end(); ++instance_it)
	{
...
void speedtree_forest::cull_and_compute_lod( renderer_context* context, float3 const& lod_reference_point, bool sort_result )
{
	...
	m_view.Set					(
		...
		vostok_to_speedtree		(context->get_p()),
		vostok_to_speedtree		(fix_view_matrix(context->get_culling_v())),
		context->get_near		(),
		context->get_far		()
	);

	m_view.SetLodRefPoint		(vostok_to_speedtree(lod_reference_point));
...
void speedtree_forest::add_instance( speedtree_instance_ptr st_instance_ptr, float4x4 const& transform )
{
	...
	if (!m_forest->TreeIsRegistered(base_tree))
	{
		...
		sLodProfile.Scale		(scale_speedtree_to_vostok );
