...
struct editor_visual
{
	...
	editor_visual(u32 id, vostok::render::visual_ptr visual, math::float4x4 transform, bool selected, bool system_object, bool beditor_chain):
	transform		(transform),
		visual			(visual),
		id				(id),
		selected		(selected),
	...
	float4x4		transform;
	vostok::render::visual_ptr	visual;
	u32				id;
	bool			selected;
	bool			system_object;
	bool			beditor_chain;
