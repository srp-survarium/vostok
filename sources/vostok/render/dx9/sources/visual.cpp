...
#include "visual.h"
//#include "dx9_world.h"
#include <vostok/render/common/sources/blender_notexture.h>
#include <vostok/render/dx9/sources/scene_context.h>
#include <vostok/render/dx9/sources/backend.h>
#include <vostok/render/common/sources/shader_manager.h>
...
void render_visual::load(memory::chunk_reader& chunk)
{
	...
	if(chunk.chunk_exists(ogf_chunk_collision_v))
	{
		...
		m_collision_geom		= &*collision::create_triangle_mesh_geometry( vostok::render::g_allocator, vertices, vertex_count, indices, index_count );
...
hierrarchy_visual::~hierrarchy_visual()
{
	...
	for( ; it != en; ++it )
		VOSTOK_DELETE_IMPL( *::vostok::render::g_allocator, *it );
}
