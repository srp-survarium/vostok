...
public:
		...
		ref_effect				create_effect( LPCSTR descriptor_name, vostok::configs::binary_config_value const& config, bool force_recompile);
		ref_effect				create_effect( LPCSTR descriptor_name, custom_config_value  const& config, bool force_recompile);
private:
		void					on_effect_created(ref_effect* out_effect_ptr, vostok::resources::queries_result& data);
...
private:
	...
	typedef vostok::vectora<shader_cache_info>	shader_cache_info_vector;
	...
	typedef vostok::vectora<effect_to_recompile_struct>	effects_to_recompile_type;
