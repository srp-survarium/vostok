...
inline void backend::set_vb_instance_data( untyped_buffer* vb_instance, u32 vb_stride, u32 offset /* in bytes*/)
{
	m_dirty_objects.vertex_buffer_instance_data |= ( m_vb_instance_data != vb_instance || vb_stride != m_vb_stride || m_vb_offset != offset);
	
	m_vb_instance_data			=	vb_instance;
	m_vb_stride_instance_data	=	vb_stride;
	m_vb_offset_instance_data	=	offset;
}

inline 	void set_vb_instance_data	( untyped_buffer * vb_instance, u32 vb_stride, u32 offset = 0 /*in bytes*/);
...
inline void	backend::render_indexed_instanced(D3D_PRIMITIVE_TOPOLOGY type, 
											  u32 index_count, 
											  u32 start_index, 
											  u32 base_vertex, 
											  u32 instance_count, 
											  u32 start_instance)
{
	...
	device::ref().d3d_context()->DrawIndexedInstanced	(index_count, instance_count, start_index, base_vertex, start_instance);
		
	// TODO
	switch (type)
	{
		case D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST: 
			num_total_rendered_triangles += instance_count * index_count / 3; 
			break;
			
		case D3D10_PRIMITIVE_TOPOLOGY_POINTLIST:
			num_total_rendered_points += instance_count * index_count;
			break;
			
		case D3D10_PRIMITIVE_TOPOLOGY_LINELIST: 
			num_total_rendered_triangles += instance_count * index_count / 2;
			break;
	}
}
