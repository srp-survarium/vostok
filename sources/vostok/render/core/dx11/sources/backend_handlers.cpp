...
// 				LOG_INFO( "The global shader constant binding for \"%s\"was ignored.", host.name.c_str());
			...
			{
				ASSERT( *((u32*)it->source().pointer()) != vostok::memory::debug::freed_memory, "The bound object was deleted !");
...
inline bool textures_handler<shader_type>::set_overwrite	( char const * name, res_texture* texture)
{
	...
	fixed_vector<texture_slot, 128> const & texture_slots = m_shader->data().textures;
...
void textures_handler<shader_type>::check_for_unset_textures( )
{
	...
	fixed_vector<texture_slot, 128> const & slots = m_shader->data().textures;
