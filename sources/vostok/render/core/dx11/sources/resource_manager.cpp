...
#include <vostok/render/core/state_descriptor.h>
...
// {
// 	VOSTOK_UNREFERENCED_PARAMETERS	(&name, &sources);
// 	
...
// 				m_compile_error_handler	( enum_shader_type_vertex, name, err_msg);
			...
			for (vostok::render::vector<pcstr>::iterator	inc_it = i->second.includes.begin(); 
	...
	else
	{
...
#endif // #if 0
		{
			...
			else
			{
				...
				//for (vostok::render::vector<pcstr>::iterator inc_it=i->second.includes.begin(); inc_it!=i->second.includes.end(); ++inc_it)
				//{
				//	if (m_previous_sources.find(*inc_it)!=m_previous_sources.end())
				//	{
				//		m_sources[*inc_it] = m_previous_sources[*inc_it];
...
// 
// 			VOSTOK_UNREFERENCED_PARAMETER	( err_msg );
...
static void change_substring(fs_new::virtual_path_string * src_and_dest, pcstr what, pcstr to)
{
 ...
 //static vostok::fs_new::virtual_path_string get_short_texture_name(pcstr name)
 //{
