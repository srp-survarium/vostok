...
)
{
	...
	effect_compiler	compiler				(*effect_resource, &in_out_query, true, NULL);
	
	compile_data->descriptor->compile		(compiler, compile_data->config->root());
	
	vectora<effect_compiler::shader_cache_info> const& cached_shaders_info = compiler.get_cached_shaders_info();
	...
	u32 request_index						= 0;
	for (vectora<effect_compiler::shader_cache_info>::const_iterator	it = cached_shaders_info.begin(); 
