...
void options::set_default_values()
{
	...
	m_num_test_lights					= 1024;
}

fs_new::virtual_path_string	options::get_current_configuration()
{
	cc_string* command					= static_cast_checked<cc_string*>(vostok::console_commands::find("r_current_render_configuration"));
...
{
	static vostok::console_commands::cc_string current_render_configuration_cc("r_current_render_configuration", s_current_render_configuration, 256, true, vostok::console_commands::command_type_engine_internal );
	...
	static render_cc_u32	num_test_lights_cc							("r_num_test_lights",							0, m_num_test_lights,		16, 1024 * 1024, true, command_type_engine_internal);
...
void options::load_impl(vostok::memory::reader& F)
{
	string4096				str;
	while (!F.eof())
	{
...
void options::load_from_config(vostok::configs::binary_config_value const& config)
{
...
void options::save_to_config(vostok::configs::lua_config_value& config)
{
	console_command* current = first_command;
