...
gpu_timer::gpu_timer()
{
	ID3DDevice* d3d_device = vostok::render::device::ref().d3d_device();
...
void gpu_timer::start()
{
	...
	vostok::render::device::ref().d3d_context()->Begin(m_freq_info);
	vostok::render::device::ref().d3d_context()->End(m_start_time);
...
void gpu_timer::stop()
{
	...
	vostok::render::device::ref().d3d_context()->End(m_stop_time);
	vostok::render::device::ref().d3d_context()->End(m_freq_info);
#endif
}
...
#else
	while ( vostok::render::device::ref().d3d_context()->GetData(m_freq_info, &timer_info, sizeof( timer_info ), 0) == S_FALSE);
	while ( vostok::render::device::ref().d3d_context()->GetData(m_start_time, &start_time, sizeof( start_time ), 0) == S_FALSE);
	while ( vostok::render::device::ref().d3d_context()->GetData(m_stop_time, &stop_time, sizeof( stop_time ), 0) == S_FALSE);
#endif
