...
vostok::render::texture_options::ETType get_texture_type_from_name( pcstr name )
{
	vostok::fs_new::virtual_path_string resource_path = name;
	vostok::fs_new::virtual_path_string resource_name;
	...
	if(resource_name.find("terrain") == 0)
	{
		...
		if( offset != -1 )
		{
			if( resource_name.find( "bump", offset ) != -1 )	return vostok::render::texture_options::terrain_bump;
			if( resource_name.find( "color", offset ) != -1 )	return vostok::render::texture_options::terrain_color;
		}
		return vostok::render::texture_options::terrain;
	}else
	{
		u32 offset = resource_name.rfind('_');
		if( offset != -1 )
		{
			if( resource_name.find( "sky", offset ) != -1 ) 			return vostok::render::texture_options::sky;
			if( resource_name.find( "bump", offset ) != -1 )			return vostok::render::texture_options::bump;
			if( resource_name.find( "nmap", offset ) != -1 )			return vostok::render::texture_options::normal_map;
		}
		return vostok::render::texture_options::type_2d;
	}
}
...
void texture_options_lua_cooker::create_default_options( resources::query_result_for_cook* parent )
{
	...
	options["ver"]						= "1.2";
	options["texture_dimension"]		= vostok::render::texture_options::texture_2D;
	options["has_mip_levels"]			= 1;
	options["bump_mode"]				= vostok::render::texture_options::paralax;
	
	vostok::render::texture_options::ETType t = get_texture_type_from_name(resource_name.c_str());

	options["srgb"]						= s32(t!=vostok::render::texture_options::normal_map);
	options["high_quality"]				= 0;
	
	if(t==vostok::render::texture_options::normal_map)
