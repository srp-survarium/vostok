...
#ifndef VOSTOK_RENDER_CORE_SHADER_DECLARED_MACROS_H_INCLUDED
#define VOSTOK_RENDER_CORE_SHADER_DECLARED_MACROS_H_INCLUDED
...
#pragma message (VOSTOK_TODO("Lain 2 IronNick: no strtok_s on PS3, please fix"))
	...
	if (defines_pos && cut_defines(shader_source_code, defines_pos, defines_end) && defines_end)
	{
		...
		vostok::memory::copy(defines_buffer, size, defines_pos, size);
		...
		while(token!=NULL)
		{
			if (vostok::strings::compare(token,"")!=0)
...
#endif // #ifndef VOSTOK_RENDER_CORE_SHADER_DECLARED_MACROS_H_INCLUDED
