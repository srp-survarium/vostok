...
#include <vostok/render/core/shader_defines.h>
#include <vostok/render/core/shader_declarated_macros.h>
...
static vostok::command_line::key s_no_cache_shaders_key( "no_cache_shaders", "", "", "disable binary shaders caching" );
...
#ifdef MASTER_GOLD
	vostok::vectora<shader_file_info>	loaded_includes_info(::vostok::memory::g_mt_allocator);
#else // #ifdef MASTER_GOLD
	vostok::vectora<shader_file_info>	loaded_includes_info(::vostok::debug::g_mt_allocator);
...
void shader_binary_source_cook::compile_shader_task(conveted_shader_loaded_data* data)
{
	...
	if (shader_code)
	{
		...
		vostok::memory::copy							(shader_data, sizeof(u32), (pcvoid)&num_size, sizeof(u32));
		vostok::memory::copy							(
