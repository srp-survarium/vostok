...
public:
	include_handler( /*resource_manager::map_shader_sources* source_map, */resource_manager::shader_source* compiled_source, shader_include_getter* include_getter = 0): 
		...
		includes_info(::vostok::debug::g_mt_allocator)
	...
	{
		VOSTOK_UNREFERENCED_PARAMETERS	( parent_data, include_type);
		...
		*size_ptr = shader_source.data.size();
		...
		vostok::strings::copy		(info.name, path.c_str());
	...
	{
		VOSTOK_UNREFERENCED_PARAMETER	( pData);
		return	S_OK;
	}
