...
using vostok::render::editor::renderer;
...
{
	m_debug				= NEW ( debug::renderer )	( world.editor_channel(), *vostok::render::editor::g_allocator, engine_world );
	m_ui				= NEW ( ui::renderer )		( world.editor_channel(), *vostok::render::editor::g_allocator, engine_world );
	m_scene				= NEW ( scene_renderer )	( world.editor_channel(), *vostok::render::editor::g_allocator, engine_world, &m_debug->frustum );




	static vostok::console_commands::cc_delegate apply_changes_cc("r_apply_changes", boost::bind( &renderer::apply_render_options_changes, this, _1 ), true );
}
...
void renderer::draw_screen_lines	( vostok::render::scene_ptr const& scene, float2 const* points, u32 count, vostok::math::color const& color, float width ,u32 pattern )
{
...
void renderer::draw_screen_rectangle	( vostok::render::scene_ptr const& scene, math::float2 const& left_top, math::float2 const& right_bottom, vostok::math::color const& color, u32 const pattern )
{
	VOSTOK_UNREFERENCED_PARAMETERS	(pattern);
	float2 points[5];
	points[0] = left_top;
...
void renderer::draw_3D_screen_lines	( vostok::render::scene_ptr const& scene, math::float3 const* points, u32 count, math::float4x4 const& tranform, vostok::math::color const& color, float width, u32 pattern, bool use_depth )
{
...
void	renderer::draw_3D_screen_line	( vostok::render::scene_ptr const& scene, math::float3 const& start, math::float3 const& end, vostok::math::color const& color, float const width, u32 const pattern, bool const use_depth )
{
...
void renderer::draw_3D_screen_ellipse	( vostok::render::scene_ptr const& scene, math::float4x4 const& matrix, vostok::math::color const& color, float width, u32 pattern, bool use_depth )
{
...
void renderer::draw_3D_screen_rectangle( vostok::render::scene_ptr const& scene, math::float4x4 const& matrix, vostok::math::color const& color, float width, u32 pattern, bool use_depth )
{
	float3 arr[5];
	vostok::buffer_vector<float3> verts(
		arr,
		sizeof(arr),
...
void	renderer::draw_3D_screen_point	( vostok::render::scene_ptr const& scene, math::float3 const& position, float width, vostok::math::color const& color, bool use_depth )
{
	m_world.editor_channel().owner_push_back(
		E_NEW( functor_with_big_buffer_to_copy_command< float3 > ) (
			boost::bind( &vostok::render::engine::world::draw_3D_screen_point, &m_render_engine_world, scene, _1, color, width, use_depth ),
			position,
...
void renderer::update_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps, vostok::configs::lua_config_value const& config_value )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
		E_NEW ( functor_command ) (
...
void	renderer::update_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps, math::float4x4 const& transform )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
...
void	renderer::add_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps, vostok::configs::lua_config_value const& init_values, math::float4x4 const& transform )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
		E_NEW( functor_with_big_buffer_to_copy_command< math::float4x4 > ) (
...
void	renderer::restart_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
		E_NEW( functor_command ) (
...
void	renderer::set_looping_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps, bool looping )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
		E_NEW( functor_command ) (
...
void	renderer::remove_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
		E_NEW( functor_command ) (
...
void	renderer::show_preview_particle_system	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps )
{
	R_ASSERT	( scene );
	m_world.editor_channel().owner_push_back  (
		E_NEW( functor_command ) (
...
u32 renderer::get_num_preview_particle_system_emitters( scene_ptr const& scene, vostok::particle::particle_system** preview_ps) const
{
	R_ASSERT	( scene );
	return		m_render_engine_world.get_num_preview_particle_system_emitters(preview_ps, scene);
}

void renderer::gather_preview_particle_system_statistics	( scene_ptr const& scene, vostok::particle::particle_system** preview_ps, vostok::particle::preview_particle_emitter_info* out_info )
{
	R_ASSERT	( scene );
...
void renderer::terrain_update_cell_buffer( scene_ptr const& scene, render::render_model_instance_ptr const& v, vostok::vectora<terrain_buffer_fragment> & fragments, float4x4 const& transform, on_command_finished_functor_type const& on_command_finished_functor)
{
	R_ASSERT	( scene );
...
void renderer::set_selection_parameters	( vostok::math::float4 const& selection_color, float selection_rate )
{
...
void renderer::apply_material_changes( fs_new::virtual_path_string const& material_name, vostok::configs::lua_config_value const& config_value )
{
	m_world.editor_channel().owner_push_back	(
		E_NEW( functor_with_big_buffer_to_copy_command< fs_new::virtual_path_string > ) (
			boost::bind(
