...
#ifndef VOSTOK_RENDER_BASE_PLATFORM_H_INCLUDED
#define VOSTOK_RENDER_BASE_PLATFORM_H_INCLUDED

#include "vostok/render/base/common_types.h"
#include <vostok/render/base/light_props.h>
#include <vostok/render/base/visual.h>
#include <vostok/render/base/common_types.h>
#include <vostok/render/base/particles.h>
...
struct VOSTOK_NOVTABLE platform {
	virtual	void	clear_resources					( ) = 0;

	virtual void	create_scene					( vostok::render::editor_renderer_configuration renderer_configuration, scene_base** scene_ptr /*out*/ ) = 0;
	...
	virtual	void	update_model					( scene_base* scene_base, render::render_model_instance_base_ptr v, vostok::math::float4x4 const& transform) = 0;
	virtual	void	remove_model					( scene_base* scene_base, render::render_model_instance_base_ptr v) = 0;

	virtual	void	update_system_model				( render::render_model_instance_base_ptr v, vostok::math::float4x4 const& transform) = 0;
	...
	virtual void	set_selection_parameters		( vostok::math::float4 selection_color, float selection_rate ) = 0;
...
//	virtual	void	change_model_shader				( render::render_model_instance_base_ptr v, char const* shader, char const* texture ) = 0;
	...
	virtual	void 	terrain_update_cell_buffer		( scene_base * scene_base, render::render_model_instance_base_ptr v, vostok::vectora<terrain_buffer_fragment> const& fragments, float4x4 const& transform) = 0;
	...
	virtual	void	setup_rotation_control_modes	(  bool color_write) {VOSTOK_UNREFERENCED_PARAMETERS( color_write);};
	
	virtual void	play_particle_system			( u32* instance_initialized, scene_base* scene_base, particle::particle_system_ptr ps, particle::particle_system_instance_ptr* out_instance, bool use_transform, bool always_looping, vostok::math::float4x4 const& transform ) = 0;
	...
	virtual void	update_particle_system_instance	( particle::particle_system_instance_ptr instance, scene_base* scene_base, vostok::math::float4x4 const& transform, bool visible = true, bool paused = false, bool selected = false  ) = 0;
	
#ifndef MASTER_GOLD
	virtual void	update_preview_particle_system	( vostok::particle::particle_system_base** preview_ps, scene_base* scene_base, vostok::configs::lua_config_value config_value ) = 0;
	virtual void	update_preview_particle_system_transform	( vostok::particle::particle_system_base** preview_ps, scene_base* scene_base, math::float4x4 const& transform ) = 0;
	virtual void	add_preview_particle_system		( vostok::particle::particle_system_base** preview_ps, scene_base* scene_base, vostok::configs::lua_config_value init_values, math::float4x4 const& transform = math::float4x4().identity() ) = 0;
	virtual void	remove_preview_particle_system	( vostok::particle::particle_system_base** preview_ps, scene_base* scene_base ) = 0;
	virtual void	show_preview_particle_system	( vostok::particle::particle_system_base** preview_ps, scene_base* scene_base ) = 0;
#endif // #ifndef MASTER_GOLD

//protected:
	VOSTOK_DECLARE_PURE_VIRTUAL_DESTRUCTOR( platform )
}; // class platform

} // namespace render
} // namespace vostok

#endif // #ifndef VOSTOK_RENDER_BASE_PLATFORM_H_INCLUDED
