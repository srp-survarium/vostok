...
#include <vostok/render/base/ui_renderer.h>
#include <vostok/render/base/game_renderer.h>
...
using vostok::render::engine::wrapper;
using vostok::render::visual_ptr;
...
{
	VOSTOK_UNREFERENCED_PARAMETER( wrapper);
	create_device					( );
...
void platform::set_view_matrix			( vostok::math::float4x4 const& view )
{
	m_view							= view;
	m_transform						= mul4x4( m_world, mul4x4( m_view, m_projection ) );
}

void platform::set_projection_matrix	( vostok::math::float4x4 const& projection )
{
	m_projection					= projection;
	m_transform						= mul4x4( m_world, mul4x4( m_view, m_projection ) );
}

void platform::set_world_matrix		( vostok::float4x4 const& world )
{
	m_world							= world;
	m_transform						= mul4x4( m_world, mul4x4( m_view, m_projection ) );
}

vostok::float4x4 const& platform::get_world_matrix( ) const
{
	return							m_world;
}

vostok::float4x4 const& platform::get_view_matrix			( ) const
{
	return							m_view;
}

vostok::float4x4 const& platform::get_projection_matrix		( ) const
{
	return							m_projection;
}

vostok::math::rectangle<vostok::math::int2_pod> platform::get_viewport	( ) const
{
...
void platform::draw_debug_lines		( vostok::render::vector< vostok::render::vertex_colored > const& vertices, vostok::render::vector< u16 > const& indices )
{
...
void platform::draw_debug_triangles	( vostok::render::vector< vostok::render::vertex_colored > const& vertices, vostok::render::vector< u16 > const& indices )
{
...
void platform::add_static				( vostok::resources::managed_resource_ptr const& collision, vostok::resources::managed_resource_ptr const& geometry, vostok::resources::managed_resource_ptr const& visuals )
{
	VOSTOK_UNREFERENCED_PARAMETERS	( collision, geometry, visuals );
}
...
void platform::add_visual				( u32 id, vostok::render::visual_ptr v, vostok::math::float4x4 const& transform, bool selected, bool system_object, bool beditor )
{
	VOSTOK_UNREFERENCED_PARAMETERS	( id, v, transform, selected, system_object, beditor );
}

void platform::update_visual			( u32 id, vostok::math::float4x4 const& transform, bool selected )
{
	VOSTOK_UNREFERENCED_PARAMETERS	( id, transform, selected );
}

void platform::remove_visual			( u32 id )
{
	VOSTOK_UNREFERENCED_PARAMETERS	( id );
}
...
void platform::update_visual_vertex_buffer	( vostok::render::visual_ptr v, vostok::vectora<vostok::render::buffer_fragment> const& fragments )
{
	VOSTOK_UNREFERENCED_PARAMETERS	( v, fragments);
}

void platform::update_visual_index_buffer	( vostok::render::visual_ptr v, vostok::vectora<vostok::render::buffer_fragment> const& fragments )
{
	VOSTOK_UNREFERENCED_PARAMETERS	( v, fragments );
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( points, count, clr, width, pattern, use_depth );
	UNREACHABLE_CODE();
}
...
void platform::draw_screen_lines		( vostok::math::float2 const* points, u32 count, vostok::math::color clr, float width, u32 pattern )
{
	VOSTOK_UNREFERENCED_PARAMETERS( points, count, clr, width, pattern );
	UNREACHABLE_CODE();
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS( step, ref_value );
	UNREACHABLE_CODE();
}
...
void platform::change_visual_shader		( vostok::render::visual_ptr v, char const* shader, char const* texture )
{
	VOSTOK_UNREFERENCED_PARAMETERS( v, shader, texture );
	UNREACHABLE_CODE();
}

void platform::add_light					( u32 id, vostok::render::light_props const& props, bool beditor )
{
	VOSTOK_UNREFERENCED_PARAMETERS	(id, props, beditor);
	UNREACHABLE_CODE();
}

void platform::update_light				( u32 id, vostok::render::light_props const& props, bool beditor )
{
	VOSTOK_UNREFERENCED_PARAMETERS	(id, props, beditor);
	UNREACHABLE_CODE();
}
...
{
	VOSTOK_UNREFERENCED_PARAMETERS	(id, beditor);
	UNREACHABLE_CODE();
}

void platform::terrain_add_cell			( visual_ptr v, bool beditor)
{
	VOSTOK_UNREFERENCED_PARAMETERS	(v, beditor);
	UNREACHABLE_CODE();
}
...
void platform::terrain_update_cell_buffer	( visual_ptr v, vostok::vectora<vostok::render::buffer_fragment_NEW> const& fragments, vostok::math::float4x4 const& transform)
{
	VOSTOK_UNREFERENCED_PARAMETERS	(v,fragments,transform);
	UNREACHABLE_CODE();
}

void platform::terrain_add_cell_texture	( visual_ptr v, vostok::render::texture_string const & texture, u32 user_tex_id)
{
	VOSTOK_UNREFERENCED_PARAMETERS	(v,texture,user_tex_id);
	UNREACHABLE_CODE();
}

void platform::terrain_remove_cell_texture	( visual_ptr v, u32 user_tex_id)
{
	VOSTOK_UNREFERENCED_PARAMETERS	(v,user_tex_id);
	UNREACHABLE_CODE();
}

void platform::terrain_exchange_texture	( vostok::render::texture_string const & old_texture, vostok::render::texture_string const & new_texture)
{
	VOSTOK_UNREFERENCED_PARAMETERS	(old_texture,new_texture);
	UNREACHABLE_CODE();
}

void platform::set_editor_render_mode		( bool draw_editor, bool draw_game )
{
	VOSTOK_UNREFERENCED_PARAMETERS	(draw_editor,draw_game);
	UNREACHABLE_CODE();
}
