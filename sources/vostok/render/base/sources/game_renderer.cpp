...
using vostok::render::game::game_renderer;
using vostok::render::base_world;
using vostok::render::buffer_fragment;
using vostok::resources::managed_resource_ptr;

game_renderer::game_renderer				( base_world& world, vostok::render::platform& platform ) :
	m_world								( world ),
	m_platform							( platform )
{
	m_debug				= NEW ( debug::debug_renderer )	( world.commands(), *vostok::render::logic::g_allocator, platform );
}
...
{
	m_world.commands().push_command	( L_NEW( command_delegate ) ( boost::bind( &vostok::render::base_world::draw_frame_logic, &m_world ) ) );
}
...
// void game_renderer::draw_visual		( vostok::render::visual_ptr v, vostok::math::float4x4 const& transform, bool selected )
// {
// 	m_world.commands( ).push_command	( L_NEW( command_draw_visual ) ( m_world, v, transform, selected ) );
// }

void game_renderer::add_visual				( u32 id, vostok::render::visual_ptr v, vostok::math::float4x4 const& transform, bool selected, bool system_object )
{
...
void game_renderer::update_visual			( u32 id, vostok::math::float4x4 const& transform, bool selected ) 
{
...
void game_renderer::update_visual_vertex_buffer	( vostok::render::visual_ptr const& object, vostok::vectora<buffer_fragment> const& fragments )
{
	m_world.commands( ).push_command	( L_NEW( command_update_visual_vertex_buffer ) ( m_world, object, fragments, *vostok::render::editor::g_allocator ) );
}

void game_renderer::change_visual_shader		( vostok::render::visual_ptr const& object, char const* shader, char const* texture )
{
	m_world.commands( ).push_command	( L_NEW( command_change_visual_shader ) ( m_world, object, shader, texture ) );
}

vostok::render::debug::renderer& game_renderer::debug	( )
{
	ASSERT	( m_debug );
	return	*m_debug;
}

void game_renderer::initialize_command_queue	( vostok::render::engine::command* null_command )
{
	m_world.commands().initialize_queue	( null_command );
}

void game_renderer::add_light				( u32 id, vostok::render::light_props const& props)
{
	m_world.commands( ).push_command	( L_NEW( command_add_light ) ( m_world, id, props, false) );
}

void game_renderer::update_light				( u32 id, vostok::render::light_props const& props)
{
...
void game_renderer::terrain_add_cell			( vostok::render::visual_ptr v )
{
	m_world.commands( ).push_command	( L_NEW( command_terrain_add_cell ) ( m_world, v, false) );
}

void game_renderer::terrain_remove_cell		( vostok::render::visual_ptr v)
{
