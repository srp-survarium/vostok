...
using vostok::render::debug::debug_renderer;
using vostok::render::platform;
using vostok::render::command_processor;
using vostok::float3;
...
debug_renderer::debug_renderer		( command_processor& processor, vostok::memory::base_allocator& allocator, platform& platform ) :
	m_platform					( platform ),
...
inline vertex_colored create_vertex	( vostok::float3 const& position, u32 const color )
{
...
void debug_renderer::draw_line		( float3 const& start_point, float3 const& end_point, u32 const color )
{
	...
	m_processor.push_command	( VOSTOK_NEW_IMPL( m_allocator, debug::draw_lines_command ) ( *this, vertices, indices ) );
}
...
void debug_renderer::draw_triangle	( vertex_colored const ( &vertices )[ 3 ] )
{
	...
	m_processor.push_command	( VOSTOK_NEW_IMPL( m_allocator, debug::draw_triangles_command ) ( *this, vertices, indices ) );
}
...
void debug_renderer::draw_lines		( vostok::vectora< vertex_colored > const& vertices, vostok::vectora< u16 > const& indices )
{
	m_processor.push_command	( VOSTOK_NEW_IMPL( m_allocator, debug::draw_lines_command ) ( *this, vertices, indices ) );
}
	...
	)
{
	...
	m_processor.push_command	( VOSTOK_NEW_IMPL( m_allocator, debug::draw_lines_command ) ( *this, temp_vertices, temp_indices ) );
}
...
void debug_renderer::draw_triangles	( debug_vertices_type const &vertices )
{
	...
	m_processor.push_command	( VOSTOK_NEW_IMPL( m_allocator, debug::draw_triangles_command ) ( *this, temp_vertices, temp_indices ) );
}
