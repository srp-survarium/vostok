...
namespace PAPI
{
    ...
    public:
        ...
        PDUnion(const PAPI::vector<pDomain *> &DomList)
        {
            TotalSize = 0.0f;
            for(PAPI::vector<pDomain *>::const_iterator it = DomList.begin(); it != DomList.end(); it++) {
        ...
        {
            TotalSize = 0.0f;
            for(PAPI::vector<pDomain *>::const_iterator it = P.Doms.begin(); it != P.Doms.end(); it++) {
                Doms.push_back((*it)->copy());
                TotalSize += (*it)->Size();
            }
        }
        ...
        ~PDUnion()
        {
            for(PAPI::vector<pDomain *>::iterator it = Doms.begin(); it != Doms.end(); it++) {
                DELETE( *it );
            }
        }
        ...
        {
            for(PAPI::vector<pDomain *>::const_iterator it = Doms.begin(); it != Doms.end(); it++)
