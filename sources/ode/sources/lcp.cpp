...
type* fake_lcp_new(int _n, int _nub, dReal *_Adata, dReal *_x, dReal *_b, dReal *_w,
	    dReal *_lo, dReal *_hi, dReal *_L, dReal *_d,
	    dReal *_Dell, dReal *_ell, dReal *_tmp,
	    int *_state, int *_findex, int *_p, int *_C, dReal **Arows)
{
	return new(dAlloc(sizeof(type))) type(_n,_nub,_Adata,_x,_b,_w,_lo,_hi,_L,_d,_Dell,_ell,_tmp,_state,_findex,_p,_C,Arows);
}
...
# endif
  ...
  // optimization relies on knowledge of how tmp is used, so be careful!
	dLCP local_lcp (n,nub,A,x,b,w,lo,hi,L,d,Dell,ell,delta_w,state,findex,p,C,Arows);
