...
//****************************************************************************
// utility
dVector3 dxWorld::gravity={0,0,0};		// gravity vector (m/s/s)
dReal dxWorld:: global_erp = 0;		// global error reduction parameter
dReal dxWorld::global_cfm = 0;		// global costraint force mixing parameter
...
void	dJointInitContact( dxJointContact *j, const dContact *c )
{
	 dAASSERT (c);
	 dAASSERT (j);
	// dJointInit ( 0, j, &__dcontact_vtable );
	 j->contact = *c;
}
...
int dxWorld::body_flags = 0;               // flags for new bodies
dxQuickStepParameters dxWorld::qs = {20,REAL(1.3)};
dxContactParameters dxWorld::contactp = {dInfinity,0};
dxDampingParameters dxWorld::dampingp = {0,0,REAL(0.01) * REAL(0.01),REAL(0.01) * REAL(0.01)}; // damping parameters
dReal dxWorld::max_angular_speed = dInfinity;      // limit the angular velocity to this magnitude
