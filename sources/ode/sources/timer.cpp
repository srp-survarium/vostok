...
{
  LARGE_INTEGER a;
  a.QuadPart = sys_time_get_system_time( );
  cc[0] = a.LowPart;
  cc[1] = a.HighPart;
}
...
double dTimerTicksPerSecond()
{
  ...
  if (!query) {
    LARGE_INTEGER a;
	a.QuadPart	= sys_time_get_timebase_frequency ();
    hz = double(a.QuadPart);
    query = 1;
  }
  return hz;
...
#if !defined(PENTIUM) && !defined(WIN32) && !defined(WIN64) && !defined(_XBOX) && !defined(SN_TARGET_PS3)
