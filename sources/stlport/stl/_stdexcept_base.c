...
_STLP_INLINE __Named_exception::__Named_exception(const string& __str) {
...
_STLP_INLINE __Named_exception::__Named_exception(const __Named_exception& __x) {
...
_STLP_INLINE __Named_exception& __Named_exception::operator = (const __Named_exception& __x) {
  size_t __size = strlen(__x._M_name) + 1;
...
_STLP_INLINE __Named_exception::~__Named_exception() _STLP_NOTHROW_INHERENTLY {
  if (_M_name != _M_static_name)
    free(_M_name);
}

_STLP_INLINE const char* __Named_exception::what() const _STLP_NOTHROW_INHERENTLY
{ return _M_name; }
