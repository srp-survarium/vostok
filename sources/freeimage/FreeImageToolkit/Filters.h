...
// Sinc function.
inline double sincf(const double x)
{
	if (fabs(x) < 0.0001f)
...
{
	const double EPSILON_RATIO = 1e-6;
...
/** Kaiser filter
*/
class CKaiserFilter : public CGenericFilter
{
public:
	CKaiserFilter(float w): CGenericFilter(w) { SetParameters(4.0, 1.5); }
	virtual ~CKaiserFilter() {}

	double Filter (double dVal)
	{
		static const double pi = 3.1415926535897932384626433833;
		const double sinc_value = sincf(float(pi * dVal * m_stretch));
		const double t = dVal / m_dWidth;
		if ((1 - t * t) >= 0) return sinc_value * bessel0(m_alpha * sqrtf(static_cast<float>(1 - t * t))) / bessel0(m_alpha);
		else return 0;
	}

	void SetParameters(float alpha, float stretch)
	{
		m_alpha = alpha;
		m_stretch = stretch;
	}
