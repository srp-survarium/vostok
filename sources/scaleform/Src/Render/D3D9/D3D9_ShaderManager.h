...
#define SF_RENDERER_VSHADER_PROFILE "vs_2_0"
...
// Eventually this class should be merged with FilterShaderManager.
...
class ShaderManager
{
    ...
    const MeshCacheParams&  MCParams;                   // Holds a reference to the mesh cache parameters. Renderer capabilities are derived from these.
    bool                    InstancingSupport;          // Set if device supports instancing (vs_3_0 HW only)
    unsigned                MaxTextureCoordinates;      // Maximum texture coordinates in a pixel shader.
    bool                    DynamicShaderFlowControl;   // Pixel shaders support dynamic flow control.
    ...
    struct                  PixelHandler
    {
        IDirect3DPixelShader9*  pPShader;    // Pixel shader.
        bool                    PShaderInit; // Used in delayed shader binding: 'true' if shader was created
        PixelHandler()
        {
            pPShader = 0;
            PShaderInit = false;
    ...
    PixelHandler PHandlerTable[PSTable_Size];
    ...
    IDirect3DPixelShader9*       pPrevPShader;
    ...
    static bool     createVertexShaderString(StringBuffer& sb, StdVertexShaderDesc& desc, unsigned maxInstances);
    static bool     createPixelShaderString(StringBuffer& sb, unsigned psBits);

    IDirect3DVertexShader9*      registerVertexShader(StdVertexShaderDesc& desc);
    IDirect3DPixelShader9*       registerPixelShader(unsigned psBits);
    IDirect3DVertexDeclaration9* registerVertexDeclaration(StdVertexShaderDesc& desc, bool posFloat = false);
    const VertexFormat*          registerVertexFormat(StdVertexShaderDesc& desc, bool posFloat = false);

    ShaderManager& operator=(const ShaderManager& sm) { SF_UNUSED(sm); };
...
public:
    ShaderManager(const MeshCacheParams& mcparams);
    ~ShaderManager();

    bool Initialize(IDirect3DDeviceX* pdevice, const ProfileViews* profiler, UInt32 vmcflags);
    void Reset();
    ...
    //   - If 'batch' format is reported at 0, batching will be disabled.
...
protected:
    ...
    bool applyVertexHandler( VertexHandler& vh, unsigned declIndex, 
                             unsigned fillType, PrimitiveBatch::BatchType batchType);
};
