...
bool DepthStencilSurface::Initialize()
{
    ...
    glRenderbufferStorageEXT(GL_RENDERBUFFER_EXT, GL_STENCIL_INDEX, Size.Width, Size.Height);    
    if ( glGetError() != 0 )
...
void TextureManager::Initialize(HAL* phal)
{
    ...
    // PowerVR texture support is different in 2.0-2.2 and 2.3
    // 2.2 doesn't support RGBA
    // 2.3 supports RGBA, but not BGRA, and still reports that BGRA is supported.
