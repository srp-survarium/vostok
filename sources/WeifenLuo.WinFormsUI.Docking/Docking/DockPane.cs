		...
		internal void SetContentBounds()
		{
			...
			foreach (IDockContent content in Contents)
                if (content.DockHandler.Pane == this)
                {
					...
					{
						if (rectContent.Width <= 0)
							rectContent.Inflate(new Size(-rectContent.Width + 2, 0));
						if (rectContent.Height <= 0)
							rectContent.Inflate(new Size(0, -rectContent.Height + 2));
						content.DockHandler.Form.Bounds = rectContent;
					}
					else
					{
						if (rectInactive.Width <= 0)
							rectInactive.Inflate(new Size(-rectInactive.Width + 2, 0));
						if (rectInactive.Height <= 0)
							rectInactive.Inflate(new Size(0, -rectInactive.Height + 2));
