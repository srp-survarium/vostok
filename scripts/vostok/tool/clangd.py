...
MSVC8 dialect: navigation is reliable, its diagnostics are not build truth.
"""
...
class Clangd:
    def __init__(self):
        self.proc = subprocess.Popen(
            ...
            cwd=VOSTOK_DIR, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
            stderr=subprocess.DEVNULL,
        )
        self._id = 0
        self._request("initialize", {
            "processId": os.getpid(),
            "rootUri": VOSTOK_DIR.as_uri(),
...
def rel(uri_or_path: str) -> str:
    ...
    try:
        return str(Path(p).relative_to(VOSTOK_DIR))
    except ValueError:
        return p
...
def main() -> None:
    ...
    try:
        ...
        path = (VOSTOK_DIR / args.file).resolve()
        if not path.is_file():
