...
Reads ``binaries/objdiff/report.json`` (regenerated by every ``vostok build`` base
...
Usage:
    ...
    python3 -m vostok ledger readme --max-code        # code matched vs best-ever
...
def regen_readme() -> str:
    ...
    rendered block (callable with no args). `vostok build` calls this at the end of every
