...
class weapon_sound_events_handler_state< weapon_core_fire_state > : public weapon_core_fire_state {
public:
	inline			weapon_sound_events_handler_state<survarium::weapon_core_fire_state>(
						weapon&			arg_0,
						const float		arg_1,
	...
	virtual			~weapon_sound_events_handler_state<survarium::weapon_core_fire_state>( ) { /* no source */ }

private:
	/* 0x0000 */	/* weapon_core_fire_state */
	/* 0x0168 */	weapon_sound_effect		m_sound_effect;
	/* 0x0190 */	pcvoid					m_buffer_for_sounds;
}; // class weapon_sound_events_handler_state< weapon_core_fire_state >

STATIC_SIZE_ASSERT(weapon_sound_events_handler_state< weapon_core_fire_state >, 0x198);
