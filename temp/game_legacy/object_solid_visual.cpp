...
object_visual::object_visual( game_scene& w )
:super( w )
{
}
...
object_solid_visual::object_solid_visual( game_scene& w )
:super					( w ),
//m_collision				( NULL ),
...
void object_dynamic_visual::set_transform( float4x4 const& m )
{
	...
	if(m_model)
	{
		m_game_scene.renderer().scene().update_model(	m_game_scene.get_render_scene(), 
														m_model->m_render_model, 
														m_transform );
	}
}
...
object_skeleton_visual::object_skeleton_visual( game_scene& w )
:super( w )
{}
...
void object_particle_visual::unload_contents( )
{
	if (m_particle_system_instance_ptr)
		m_game_scene.renderer().scene().remove_particle_system_instance( m_game_scene.get_render_scene(), m_particle_system_instance_ptr );
...
void object_particle_visual::load_contents( )
{
	...
	particle::world* p =  &m_game_scene.renderer().scene().particle_world( m_game_scene.get_render_scene() ) ;
	ud.set(p);
...
void object_particle_visual::on_visual_ready( resources::queries_result& data )
{
	...
	if (!m_playing)
	{
		m_particle_system_instance_ptr = static_cast_resource_ptr<vostok::particle::particle_system_instance_ptr>( data[0].get_unmanaged_resource());
		m_game_scene.renderer().scene().play_particle_system( m_game_scene.get_render_scene(), m_particle_system_instance_ptr, m_transform );
		m_playing = true;
	}
}

object_speedtree_visual::object_speedtree_visual( game_scene& w )
:super( w )
{}
...
void object_speedtree_visual::on_visual_ready( resources::queries_result& data )
{
	...
	m_speedtree_instance_ptr = static_cast_resource_ptr<vostok::render::speedtree_instance_ptr>( data[0].get_unmanaged_resource() );
