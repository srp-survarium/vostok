	...
	m_testee			( NULL )
{
	...
	if( vostok::strings::compare( testees_source, "HUD" ) == 0 )
	{
		m_testee = &owner->get_game_world( ).get_hud( )->get_caracter_capsule( );
	}

#ifndef MASTER_GOLD
	static vostok::console_commands::cc_bool		is_testee_volume_enabled_cc	( "proximity_trigger_testee_volume_enabled", m_is_testee_volume_enabled, true, console_commands::command_type_user_specific );

	if( !m_is_input_handler_enabled_catched )
	{
		m_input_handler			= NEW( proximity_trigger_input_handler )( *this );
		static vostok::console_commands::cc_bool		is_input_handler_enabled_cc	( "proximity_trigger_input_handler_enabled", m_is_input_handler_enabled, true, console_commands::command_type_user_specific );
