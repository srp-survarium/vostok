												...
												input::enum_keyboard_action action )
{
	...
	game_action_id game_action = survarium::key_binder::get_binded_action( key );
	...
	else if ( action == vostok::input::kb_key_down && game_action == kSPRINT_TOGGLE )
											...
											input::enum_mouse_key_action action )
{
	...
	game_action_id game_action = survarium::key_binder::get_binded_action( button );
