...
class samplers_handler< 0 > : public boost::noncopyable {
	inline	explicit	samplers_handler<0>		( res_xs_hw< vs_data > const* const& arg_0 ) { /* no source */ }
...
public:
	inline				~samplers_handler<0>	( ) { /* no source */ }
...
private:
	...
	/* 0x004c */	res_xs_hw< vs_data > const* const&		m_shader;
}; // class samplers_handler< 0 >

STATIC_SIZE_ASSERT(samplers_handler< 0 >, 0x50);
