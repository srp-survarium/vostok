...
class vostok::render::res_xs_hw<vostok::render::gs_data> const*;

namespace vostok {
namespace render {

class textures_handler< 2 > : public boost::noncopyable {
	inline	explicit	textures_handler<2>		( res_xs_hw< gs_data > const* const& arg_0 ) { /* no source */ }
...
public:
	inline				~textures_handler<2>	( ) { /* no source */ }
...
private:
	/* 0x020c */	res_xs_hw< gs_data > const* const&		m_shader;
	/* 0x0210 */	res_texture_list						m_custom_list;
}; // class textures_handler< 2 >

STATIC_SIZE_ASSERT(textures_handler< 2 >, 0x224);
