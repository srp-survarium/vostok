...
class samplers_handler< 2 > : public boost::noncopyable {
	inline	explicit	samplers_handler<2>		( res_xs_hw< gs_data > const* const& arg_0 ) { /* no source */ }
...
public:
	inline				~samplers_handler<2>	( ) { /* no source */ }
...
private:
	...
	/* 0x004c */	res_xs_hw< gs_data > const* const&		m_shader;
}; // class samplers_handler< 2 >

STATIC_SIZE_ASSERT(samplers_handler< 2 >, 0x50);
