...
#include "pch.h"
#include <vostok/render/base/sources/platform_dx11.h>
...
#include <vostok/render/base/sources/platform_api.h>

#include <vostok/render/core/backend.h>
#include <vostok/render/engine/scene_render.h>
#include <vostok/render/engine/visual.h>
#include <vostok/render/engine/terrain.h>
...
vostok::render::dx10::renderer& platform::extended	()
{
	return				( *this);
}

void platform::set_view_matrix						( vostok::math::float4x4 const& view_matrix)
{
	m_scene.set_v( view_matrix);
}

void platform::set_projection_matrix				( vostok::float4x4 const& projection)
{
...
void platform::add_static							( 
	vostok::resources::managed_resource_ptr const& collision,
	vostok::resources::managed_resource_ptr const& resource,
	vostok::resources::managed_resource_ptr const& visuals)
{
	VOSTOK_UNREFERENCED_PARAMETERS	( collision, resource, visuals);
	NOT_IMPLEMENTED();
}
...
void platform::draw_screen_lines	( vostok::math::float2 const* points, u32 count, color clr, float width, u32 pattern)
{
	vostok::math::float3* points_3d = NEW_ARRAY(vostok::math::float3,count);

	// normalize coordinates
	for ( u32 i = 0; i < count; ++i)
		points_3d[i].set(
		points[i].elements[0]/(float)vostok::render_dx10::device::ref().get_width()*2.0f-1.0f,
		-((points[i].elements[1]/(float)vostok::render_dx10::device::ref().get_height())*2.0f-1.0f),
		0);
...
void platform::draw_3D_screen_lines	( vostok::math::float3 const* points, u32 count, color clr, float width, u32 pattern, bool use_depth)
{
...
vostok::math::float4x4 const&	platform::get_view_matrix			() const
{
	return m_scene.get_v();
}

vostok::math::float4x4 const&	platform::get_projection_matrix	() const
{
...
#pragma warning(disable:4244)
	...
	return vostok::math::rectangle<vostok::math::int2_pod>( vostok::math::int2( viewport.TopLeftX, viewport.TopLeftY), vostok::math::int2( viewport.Width, viewport.Height));
...
void platform::terrain_update_cell_buffer	( visual_ptr v, vostok::vectora<vostok::render::buffer_fragment_NEW> const& fragments, float4x4 const& transform)
{
