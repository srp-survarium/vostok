...
base_world::base_world								( vostok::render::engine::wrapper& wrapper, HWND window_handle ) :
	m_wrapper	( wrapper )
{
...
vostok::render::engine::renderer& base_world::engine	( )
{
	ASSERT		( m_engine );
	return		( *m_engine );
}

vostok::render::game::renderer&	base_world::game		( )
{
	ASSERT		( m_game );
	return		( *m_game );
}

vostok::render::ui::renderer&	base_world::ui		( )
{
	ASSERT		( m_ui );
	return		( *m_ui );
}

vostok::render::editor::renderer& base_world::editor	( )
{
	ASSERT		( m_editor );
	return		( *m_editor );
}
...
void base_world::push_command							( vostok::render::engine::command* command )
{
	commands().push_command		( command );
}

void base_world::destroy_command							( vostok::render::engine::command* command )
{
	R_ASSERT					( command->remove_frame_id <= m_platform->frame_id() );
	L_DELETE					( command );
}

void base_world::push_command_editor							( vostok::render::engine::command* command )
{
	commands_editor().push_command		( command );
}

void base_world::destroy_command_editor							( vostok::render::engine::command* command )
{
	R_ASSERT					( command->remove_frame_id <= m_platform->frame_id() );
	E_DELETE					( command );
}
...
vostok::render::platform &	base_world::platform	( )
{
	return *m_platform;
}
