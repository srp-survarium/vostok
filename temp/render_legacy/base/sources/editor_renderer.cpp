...
using vostok::render::editor::editor_renderer;
using vostok::render::vertex_colored;
using vostok::render::platform;

using namespace vostok::geometry_utils;
...
editor_renderer::editor_renderer	( vostok::render::base_world& world, platform& platform ):
	m_world				( world ),
	m_platform			( platform )
{
	m_debug				= NEW ( debug::debug_renderer )	( world.commands_editor(), *vostok::render::editor::g_allocator, platform );
}
...
//	render_triangles();
	...
	for( ; it != en; ++it ) 
	{
//		vostok::buffer_vector< float2 > tmp_points( &m_screen_lines[cur_position], it->count );
...
{
	float3 arr[5];
	vostok::buffer_vector<float3> verts( arr, sizeof(arr), (float3*)rectangle::vertices, (float3*)rectangle::vertices + rectangle::vertex_count);
...
void editor_renderer::add_visual			( u32 id, vostok::render::visual_ptr v, vostok::math::float4x4 const& transform, bool selected, bool system_object )
{
...
void editor_renderer::update_visual			( u32 id, vostok::math::float4x4 const& transform, bool selected ) 
{
...
void editor_renderer::update_visual_vertex_buffer	( vostok::render::visual_ptr const& object, vectora<buffer_fragment> const& fragments )
{
	m_world.commands_editor( ).push_command	( E_NEW( command_update_visual_vertex_buffer ) ( m_world, object, fragments, *vostok::render::editor::g_allocator ) );
}

void editor_renderer::change_visual_shader		( vostok::render::visual_ptr const& object, char const* shader, char const* texture )
{
...
{
	m_world.commands_editor().push_command	( E_NEW( command_delegate ) ( boost::bind( &vostok::render::base_world::draw_frame_editor, &m_world  ) ) );
}
...
void editor_renderer::terrain_update_cell_buffer	( visual_ptr v, vostok::vectora<buffer_fragment_NEW> & fragments, float4x4 const& transform, command_finished_callback const& finish_callback, u32 finish_callback_argument)
{
...
{
	m_world.commands_editor( ).push_command	( E_NEW( command_delegate ) ( boost::bind(&vostok::render::base_world::set_editor_render_mode, &m_world, draw_editor, draw_game)) );
}
