...
{
	VOSTOK_UNREFERENCED_PARAMETER(force_recompile);
...
{
#ifndef MASTER_GOLD
	effects_to_recompile_type					effects_to_recompile(vostok::debug::g_mt_allocator);
	...
	for (effects_vector_type::iterator it = m_effects.begin(); it != m_effects.end(); ++it)
	{
		...
		{
			//for(vostok::vectora<fs_new::virtual_path_string>::iterator	used_it = it->second->used_shaders.begin(); used_it != it->second->used_shaders.end(); ++used_it)
			//LOG_INFO(used_it->c_str());
			vostok::vectora<fs_new::virtual_path_string>::iterator found = std::find(it->effect->used_shaders.begin(), it->effect->used_shaders.end(), *name_it);
