...
{
	VOSTOK_UNREFERENCED_PARAMETER	( vPN );
...
{
	collision::ray_triangles_type	results(vostok::render::g_allocator);
...
//**********************************************************************
#if 1
	VOSTOK_UNREFERENCED_PARAMETERS	( mat_vp, view_pos );
 	return;
#else // #if 1
...
void model_manager::update(const float3& new_view_point, const float4x4& wvp_mat)
{
	...
	if (m_collision_tree_portals) 
	{
		...
		collision::triangles_type	results(vostok::render::g_allocator);
		//Sectors_xrc.box_options	(CDB::OPT_FULL_TEST);
...
{
	VOSTOK_UNREFERENCED_PARAMETER	( collision );
...
{
	VOSTOK_DELETE_IMPL(	*::vostok::render::g_allocator, v );
}
...
void model_manager::update_vb(u32 buffer_id, vostok::vectora<buffer_fragment> const& fragments)
{
...
void model_manager::update_ib(u32 buffer_id, vostok::vectora<vostok::render::buffer_fragment> const& fragments)
{
...
#pragma message(VOSTOK_TODO("pass correct sizeof, not sizeof of a base class"))
...
void model_manager::add_editor_visual(u32 id, vostok::render::visual_ptr v, float4x4 trans, bool selected, bool system_object, bool beditor_chain )
{
