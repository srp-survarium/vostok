...
void flash_renderer::present( )
{
	if ( m_movies.empty() )
		return;
	vector<survarium::flash_movie*>::const_iterator it = m_movies.begin();
	vector<survarium::flash_movie*>::const_iterator it_e = m_movies.end();
	for ( ;it!=it_e;++it)
	{
		survarium::flash_movie* movie = *it;
...
void flash_renderer::show_movie( survarium::flash_movie* movie )
{
	m_movies.push_back(movie);
}

void flash_renderer::hide_movie( survarium::flash_movie* movie )
{ 
	vector<survarium::flash_movie*>::iterator it = std::find(m_movies.begin(), m_movies.end(), movie );
