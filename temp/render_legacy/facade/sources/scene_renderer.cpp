...
#ifndef MASTER_GOLD
void scene_renderer::draw_render_statistics( vostok::ui::world& ui_world, vostok::render::scene_view_ptr const& scene )
{
	m_channel.owner_push_back	(
...
// and lpv proved).
//
// add_light/update_light(light_props*): PORTED 2026-07-31 into the carcass
// (the carcass world/facade already take light_props*, so the cross-TU light-flow
// refactor this note used to describe is done). Ported as functor_command binding
// the raw pointer - the retail big-buffer specialization list has NO light_props
// entry, so the target does NOT copy the props into the command.
