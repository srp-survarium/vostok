...
void one_way_render_channel::process_next_frame_commands( )
{
	...
	base_command_list						new_next_frame_commands_queue;
...
void one_way_render_channel::render_on_draw_scene(
	vostok::render::base_scene_ptr const&		scene,
	vostok::render::base_scene_view_ptr const&	scene_view,
...
)
{
	...
	if ( !use_depth ) {
		for ( vostok::render::base_command* i = scene_view->first_command; i; i = i->deferred_next )
			i->execute						( );
	}
	...
	{
		bool found							= false;
		for ( base_scene_view_ptr i = m_scene_views; i; i = i->next_scene_view ) {
			if ( i == scene_view ) {
				found						= true;
				break;
			}
