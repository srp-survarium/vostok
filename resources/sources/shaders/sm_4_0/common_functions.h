...
{ 
    float rnd 	= frac(pow(9.213f, fmod(uv.x, 0.25f) + 3.0f) * pow(7.12345f, fmod(uv.y, 0.25f) + 3.0f)); 
		  rnd 	= (rnd - 0.5f); 
    return 		rnd; 
}

float beckmann_specular(float3 normal, float3 light_dir, float3 view_dir, float m)
{
	float3 half_vector = normalize(light_dir + view_dir);
	float a = acos(dot(normal, half_vector));
	
	float e = (1.0f - pow(cos(a),2)) / (pow(cos(a),2)*m*m);
	
	float specular = exp(e) / (3.14159f*m*m*pow(cos(a),4));
	
	return specular;
}

float get_color_luminance( float3 value )
{
    return  dot( value, float3( 0.2126f, 0.7152f, 0.0722f ) );
}

float squared_length	( float2 value )
{
	return	dot( value, value );
}

float squared_length	( float3 value )
{
	return	dot( value, value );
}

float squared_length	( float4 value )
{
	return	dot( value, value );
}
...
{
	return max(0,norm_w.y)*L_hemi_color.xyz;
}

//float3 calc_model_lq_lighting( float3 norm_w )    
//{
//	return L_material.x*calc_model_hemi_r1(norm_w) + L_ambient.xyz + L_material.y*calc_sun_r1(norm_w);
//}
...
//	float3	t_lmh = s_hemi.Sample( s_rtlinear, tc);
//	return	dot(t_lmh,1.h/4.h);
	float4	t_lmh = t_hemi.Sample( s_rtlinear, tc);
	return	t_lmh.a;
}
*/
...
float4 get_texcoord( float4 proj_position)
{
	return float4( proj_position.xy * float2( 0.5f, -0.5f) + float2( 0.5f, 0.5f)*proj_position.w, proj_position.zw);
}
...
float4 renormalize_color(float4 norm_value, float4 min_value, float4 max_value)
{
	return norm_value * (max_value - min_value) + min_value;
}

float4 pack_distortion(float2 distortion)
{
	//
	//distortion = distortion * 2 - 1;
	#if 1
		//return float4(max(distortion, 0.0f), abs(min(distortion, 0.0f)));
		return float4(distortion, 1.0f, 0.5f);
	#else
		distortion = clamp(distortion,-float2(255.0f, 255.0f), float2(255.0f, 255.0f)) / 255.f;
		return float4(distortion * 0.5f + 0.5f, 0.1f, 0.5f);
	#endif
}

float2 unpack_distortion(float4 accumulated_distortion)
{
	#if 1
		//return (accumulated_distortion.rg - accumulated_distortion.ba);
		return accumulated_distortion.xy;
	#else
		return accumulated_distortion.xy * 2.0f - 1.0f;
	#endif
}
