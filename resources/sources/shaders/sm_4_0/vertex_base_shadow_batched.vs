...
//////////////////////////////////////////////////////////////////////////
// static mesh, batched depth-pass layout
// (static_mesh_vertex_input_shadow_batched.h). The geometry is already
// baked into one buffer, so tc sits at TEXCOORD1 and TEXCOORD0 carries the
// local position the wind needs - there is no per-object m_W to read it
// from. No vertex-colour variant: types 1 and 2 ship the same blob.
//////////////////////////////////////////////////////////////////////////
...
void add_wind_motion(in float3 local_position, inout float3 position)
{
	...
	const float movement_mult 	=	0.07125f;

	// wind_scale first: the shipped dp2 spells cb1[31].yyyy before
	// cb2[17].wwww, and fxc preserves the multiply's operand order.
	float time					=	wind_scale * scene_time;

	dist						*=	wind_scale;
	dist						*=	movement_mult;

	sincos						(time + time + period, s, c);
...
void fill_output_vertex(in vertex_input_struct input, out vertex_output_struct output)
{
	...
	add_wind_motion				(input.local_position, position);

	output.homogeneous_position	= mul( m_WVP,  float4( position, 1.f));
	output.position				= mul( m_W,  float4( position, 1.f));
