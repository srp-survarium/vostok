...
{
	return	t_base.Sample( s_base, tc);
}
...
/*
		for( int i=0; i<nNumSteps; ++i )
		{
			...
			{	
				...
				fCurrHeight = s_bumpX.SampleLevel( s_base, vTexCurrentOffset.xy, 0 ).a; 
...
void UpdateTC( inout p_bumped I)
{
	float3	 eye = mul (float3x3(I.tbn_x.x, I.tbn_y.x, I.tbn_z.x,
								 I.tbn_x.y, I.tbn_y.y, I.tbn_z.y,
								 I.tbn_x.z, I.tbn_y.z, I.tbn_z.z), -I.position.xyz);
	
	float	height	= t_bumpX.Sample( s_bumpX, I.tcdh).w;	//
			//height  /= 2;
			//height  *= 0.8;
...
surface_bumped sload_i( p_bumped I)
{
	...
	float4 	Nu	= t_bump.Sample( s_bump, I.tcdh);		// IN:	normal.gloss
	float4 	NuE	= t_bumpX.Sample( s_bumpX, I.tcdh);	// IN:	normal_error.height
