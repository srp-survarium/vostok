...
half EdgeSamplesManualPCF(float4 ShadowPosition, float2 JitterTC)
{
	half ShadowCoverage = 0;

	half2 RandomCosSin = tex2D(jitter0, JitterTC).xy * 2 - 1;
	...
	for(int ChunkIndex = 0;ChunkIndex < NUM_EDGE_SAMPLE_CHUNKS;ChunkIndex++)
	{
		...
		Mask = ShadowPosition.z < ShadowDepths ? half2(1,1) : half2(0,0);
...
half RefiningSamplesManualPCF(float4 ShadowPosition)
{
	...
	for(int ChunkIndex = 0;ChunkIndex < NUM_REFINING_SAMPLE_CHUNKS;ChunkIndex++)
	{
		...
		ShadowDepths.x = tex2Dlod(s_smap, half4(FinalCoords.xy, 0, 0)).r;
		ShadowDepths.y = tex2Dlod(s_smap, half4(FinalCoords.zw, 0, 0)).r;
		
		FinalCoords = ShadowPosition.xyxy + RefiningSampleOffsets[ChunkIndex * 2 + 1].xyzw / SMAP_size.xxxx;
		
		ShadowDepths.z = tex2Dlod(s_smap, half4(FinalCoords.xy, 0, 0)).r;
		ShadowDepths.w = tex2Dlod(s_smap, half4(FinalCoords.zw, 0, 0)).r;
		
		//compare and accumulate all 4 results together
		Mask = ShadowPosition.z < ShadowDepths ? half4(1,1,1,1) : half4(0,0,0,0);
		RefinedShadowCoverage = RefinedShadowCoverage + dot(Mask, SampleWeight.xxxx);
	}
	return RefinedShadowCoverage;
}

half GetBranchingShadowCoverage(float4 ShadowPosition, float2 JitterTC)
{
	ShadowPosition.xyz /= ShadowPosition.w;
	half ShadowCoverage = EdgeSamplesManualPCF(ShadowPosition, JitterTC);
