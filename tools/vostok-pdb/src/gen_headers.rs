...
impl BaseClass {
    fn write_declaration(&self, f: &mut impl io::Write) -> io::Result<()> {
        if self.is_virtual {
            write!(f, "virtual ")?;
        }
        if let Some(access) = access_keyword(self.access) {
            write!(f, "{access} ")?;
        }
...
impl<'p> Class<'p> {
    ...
    fn add_method(
        ...
        match fn_t.name.as_str() {
            ...
            // Omit only compiler-generated copies without standalone definitions.
            fn_name
                // Constructor doesn't have definition
                if suppress_implicit_copy(
                    matches!(method, Method::FromHeaderFile { .. }),
                    data_attributes.is_compgenx(),
                    &fn_t.arg_types,
                    &self.orig_name,
                    fn_name == "operator="
...
impl Method {
    fn fmt(
        ...
        // A PDB can prove an inline declaration but cannot recover its source
        // body. Carcass output therefore remains a declaration; executable
        // placeholder bodies would falsely turn missing evidence into code.
        let (inline, body) = match attrs.contains(AttributeFlags::IS_INLINE) {
            true if attrs.contains(AttributeFlags::IS_VIRTUAL) => ("", ";"),
            true => ("inline\t", ";"),
            false => ("", ";"),
        };
    ...
    fn max_value_len(&self) -> u32 {
        ...
        for value in &self.values {
            max_value_len = max_value_len.max(match value.value {
                ...
                pdb::Variant::I8(v) => v.unsigned_abs() as u64,
                pdb::Variant::I16(v) => v.unsigned_abs() as u64,
                pdb::Variant::I32(v) => v.unsigned_abs() as u64,
                pdb::Variant::I64(v) => v.unsigned_abs(),
            });
        }
...
fn suppress_implicit_copy(
    header_only: bool,
    compiler_generated: bool,
    arg_types: &[String],
    class_name: &str,
    constructor_or_assignment: bool,
) -> bool {
    header_only
        && compiler_generated
        && constructor_or_assignment
        && matches!(arg_types, [arg] if
            arg.strip_suffix(" const&") == Some(class_name)
            || arg.strip_suffix('&') == Some(class_name))
}
...
mod copy_declaration_tests {
    ...
    fn inheritance_preserves_access_and_virtual_status() {
        for (access, expected) in [
            ...
            for is_virtual in [false, true] {
                let base = super::BaseClass {
                    type_name: crate::Type("base".into()),
                    offset: 0,
                    access,
                    is_virtual,
                };
                let mut output = Vec::new();
                base.write_declaration(&mut output).unwrap();
                let prefix = if is_virtual { "virtual " } else { "" };
                assert_eq!(
    ...
    #[test]
    fn enum_format_preserves_signed_minima_and_unsigned_maximum() {
        use super::{Enum, EnumValue, Name};
        for (value, expected) in [
            (pdb::Variant::I8(i8::MIN), "-0x80"),
            (pdb::Variant::I16(i16::MIN), "-0x8000"),
            (pdb::Variant::I32(i32::MIN), "-0x80000000"),
            (pdb::Variant::I64(i64::MIN), "-0x8000000000000000"),
            (pdb::Variant::U64(u64::MAX), "0xffffffffffffffff"),
        ] {
            let declaration = Enum {
                name: pdb::RawString::from("limits"),
                underlying_type_name: crate::Type("int".into()),
                values: vec![EnumValue {
                    name: pdb::RawString::from("limit"),
                    value,
                }],
                is_nested: false,
            };
            let mut output = Vec::new();
            declaration.fmt(Name::Full, &mut output).unwrap();
    ...
    #[test]
    fn retains_user_declared_copy_without_a_standalone_body() {
        let args = ["vostok::network_core::delayed_packets_predicate const&".into()];
        assert!(!suppress_implicit_copy(
            true,
            false,
            &args,
            "vostok::network_core::delayed_packets_predicate",
            true
        ));
    }

    #[test]
    fn suppresses_only_unemitted_compiler_generated_self_copies() {
        for arg in ["predicate const&", "predicate&"] {
            let args = [arg.into()];
            assert!(suppress_implicit_copy(true, true, &args, "predicate", true));
            assert!(!suppress_implicit_copy(
                false,
                true,
                &args,
            ...
            assert!(!suppress_implicit_copy(true, true, &args, "other", true));
        }
        assert!(!suppress_implicit_copy(true, true, &[], "predicate", true));
        assert!(!suppress_implicit_copy(
            true,
            true,
            &["predicate const&".into(), "int".into()],
            "predicate",
            true
        ));
    }
