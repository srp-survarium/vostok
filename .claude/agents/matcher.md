...
- **Preserve the original DEFINITION ORDER - never reorder or regroup.** Function and
  member definitions must appear in the SAME order as the original. **Derive that order
  from the TARGET STRUCTURE for the class**, which records it: the class declaration in
  `binaries/structure/target/sources/<path>/<class>.h` gives the member/method
  DECLARATION order, and `binaries/structure/target/sources/<path>/sources/<class>.cpp`
  gives the function DEFINITION order. We are REPLICATING the original code, not writing
  our own tidy version. So keep `private:` / `protected:` / `public:` exactly where that
  layout puts them - they may be REPEATED and INTERLEAVED (each appearing several times,
  out of the conventional grouped order) to preserve definition order, and that is
  CORRECT, not a mess to clean up. Do NOT move definitions around or collapse members
  under a single access specifier to look neat - that changes the layout being matched.
  (The structure-verifier checks that you obeyed this.)
